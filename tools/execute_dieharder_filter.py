...
def main() -> int:
    parser = argparse.ArgumentParser(description="Execute a Dieharder filter stage over candidate folders.")
    parser.add_argument("input_dir")
    parser.add_argument("output_dir")
    parser.add_argument("password_text")
    parser.add_argument("size_spec")
    parser.add_argument("--max-failed-tests", type=int, default=0)
    parser.add_argument("--max-weak-tests", type=int, default=999999)
    parser.add_argument("--dieharder-args", default="-a")
    ...
    if not summary_csv.exists() or not args.resume:
        summary_csv.write_text(
            "folder_name,candidate_id,function_name,status,passed_tests,weak_tests,failed_tests,total_tests,score_file,stream_file,report_file\n",
            encoding="utf-8",
        )
    ...
    for candidate_dir in candidate_dirs(input_dir):
        ...
        report_root = tmp_dir / "dieharder_reports"
        ...
        env = {
            ...
            "KEEP_STREAM": "1",
            "KEEP_REPORT": "1",
            "DIEHARDER_ARGS": args.dieharder_args,
        }
        try:
            output = subprocess.run(
                ["bash", "./run_registered_candidate_dieharder_once.sh"],
                cwd=ROOT,
                check=True,
                capture_output=True,
                text=True,
            ...
            print(f"ERROR {folder_name} candidate_id={candidate_id} dieharder runner failed")
            if stdout_text:
                print(stdout_text)
            if stderr_text:
                print(stderr_text)
        ...
        passed_tests = int(parsed.get("passed_tests", "0"))
        weak_tests = int(parsed.get("weak_tests", "0"))
        failed_tests = int(parsed.get("failed_tests", "0"))
        total_tests = int(parsed.get("total_tests", "0"))
        report_file = parsed.get("report_file", "")

        if total_tests <= 0:
            stage_status = "NO_REPORT"
            passes_stage = False
        elif failed_tests <= args.max_failed_tests and weak_tests <= args.max_weak_tests:
            stage_status = "PASS"
            passes_stage = True
        elif failed_tests > args.max_failed_tests:
            stage_status = "TOO_MANY_FAILED"
            passes_stage = False
        else:
            stage_status = "TOO_MANY_WEAK"
            passes_stage = False
        ...
        if passes_stage:
            ...
            report_source = report_root / run_label / "dieharder_output.txt"
            if report_source.exists():
                shutil.copy2(report_source, final_dir / f"dieharder_{args.password_text}_{stage_tag}.txt")
            final_score_file = final_dir / score_name
            final_stream_file = final_dir / stream_name
            final_report_file = final_dir / f"dieharder_{args.password_text}_{stage_tag}.txt"
            with passed_txt.open("a", encoding="utf-8") as handle:
                handle.write(f"{folder_name}\n")
            print(
                f"PASS {folder_name} candidate_id={candidate_id} "
                f"passed_tests={passed_tests} weak_tests={weak_tests} failed_tests={failed_tests} total_tests={total_tests} "
                f"max_failed_tests={args.max_failed_tests} max_weak_tests={args.max_weak_tests}"
            )
        else:
