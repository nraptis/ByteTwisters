...
void PasswordExpander::SeedRegisteredCandidate(
  ...
  if (pWorkerA != nullptr) {
    std::memset(pWorkerA, 0, kBlockLength);
  }
  if (pWorkerB != nullptr) {
    std::memset(pWorkerB, 0, kBlockLength);
  }
  if (pCandidate.key_seed != nullptr) {
    pCandidate.key_seed(pSource, pKeyBuffer, kBlockLength);
  }
  if (pCandidate.mask_seed_a != nullptr) {
    pCandidate.mask_seed_a(pSource, pWorkerA, pMaskBufferA, kBlockLength);
  }
  if (pCandidate.mask_seed_b != nullptr) {
    pCandidate.mask_seed_b(pSource, pWorkerB, pMaskBufferB, kBlockLength);
  }
