...
// Candidate 162762: TwistCandidate_162762
// family=ferocious_dual_worker op_budget=10 worker_shapes=2x1/2x4 worker_reverse=(False, True, False, True)/(True, True, True, True) twiddle1=mix/i_mul twiddle2=mix/none twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m2i1v231,aBreakerCD:4m0i0v120,aBreakerEF:4m1i1v123 lane_breaker=true braid_breaker=true jump_breaker=true swap_breaker=true mask_template=4 mask_seed_a=sbox_wave mask_seed_b=off lightning=false/3 typhoon=false hurricane=true final_reverse=(True, True, False, False)
// ferocious[wa=2x1; wb=2x4; tw1=mix/i_mul; tw2=mix/none; tw2src=B; saltbox=128:0010d7cd:daba402d; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m2i1v231,aBreakerCD:4m0i0v120,aBreakerEF:4m1i1v123; wa_rev=(False, True, False, True); wb_rev=(True, True, True, True); lane=on:3; braid=on:2; jump=on:3; swap=on:3; mask=workerbx4; lightning=off:3; typhoon=off; hurricane=on; final=5/(True, True, False, False); key_rot=30; maskA_seed=sbox_wave/False; maskB_seed=off; maskA_bias=228; maskB_bias=176]
static std::uint32_t TwistCandidate_162762_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0xB2801F0Eu;
  aState = AdvanceTwiddle32(
      ...
      0x27A1740Cu,
      6u);
  aState ^= RotateLeft32(pValue + pExtra + 0xF322C8E0u, 9U);
  return static_cast<std::uint32_t>(aState + 0x95216B02u);
}

static std::uint32_t TwistCandidate_162762_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xE404AA27u;
  aState = AdvanceTwiddle32(
      ...
      0x172662C7u,
      8u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xC9A681D5u, 7U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5DE2Bu);
}

static void TwistCandidate_162762_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_162762_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_162762_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_162762_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_162762_AdvanceTwiddle32B(
...
static void TwistCandidate_162762_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xF8U, 0x30U, 0x28U, 0x29U, 0x5BU, 0xC4U, 0xD1U, 0x5DU, 0x97U, 0xBBU, 0x39U, 0x07U, 0x4FU, 0x18U, 0xBFU, 0xE6U,
      0x81U, 0x9EU, 0x72U, 0xFEU, 0xCFU, 0x26U, 0x9DU, 0x01U, 0x62U, 0xC3U, 0x43U, 0x78U, 0xDDU, 0x9BU, 0x7BU, 0x94U,
      0xB2U, 0xA4U, 0x6CU, 0x48U, 0x58U, 0x38U, 0xB8U, 0xB5U, 0xE4U, 0xFFU, 0x64U, 0xB1U, 0xE1U, 0x2FU, 0x3CU, 0xA5U,
      0xBDU, 0xCEU, 0xC2U, 0x8FU, 0xC9U, 0xA7U, 0x6AU, 0x34U, 0x7EU, 0x57U, 0xC0U, 0xE8U, 0x36U, 0x1FU, 0x17U, 0x1DU,
      0x4CU, 0x00U, 0x3BU, 0xB7U, 0x2CU, 0xD0U, 0x40U, 0x27U, 0x56U, 0xECU, 0x51U, 0x04U, 0x37U, 0x02U, 0x06U, 0xE3U,
      0x4BU, 0xD6U, 0xEDU, 0x2AU, 0x80U, 0x77U, 0x4EU, 0x25U, 0x65U, 0x92U, 0xCAU, 0xFBU, 0xA9U, 0xA0U, 0x5FU, 0x3EU,
      0x03U, 0x61U, 0xD8U, 0x60U, 0x55U, 0xF6U, 0x83U, 0x8AU, 0xF2U, 0xA1U, 0x31U, 0x67U, 0xB6U, 0xABU, 0xBEU, 0xEFU,
      0x79U, 0xDCU, 0x0DU, 0x9CU, 0xF5U, 0x82U, 0xC8U, 0x50U, 0xF1U, 0x16U, 0xFDU, 0xAFU, 0x0FU, 0xFCU, 0x99U, 0xADU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 41u ^ 225u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 21u + (-4219)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 28u + (6074)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 21u + static_cast<unsigned int>(41u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 41u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(225u), 1u, true);
  ...
  TwistCandidate_162762_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(225u), 1u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(225u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 35u + (241)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 17u + (1614)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 35u + pSalt[(aSourceIndex + 236u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(236u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(236u), 6u, true);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_162762_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(236u), static_cast<std::uint32_t>(236u), 6u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(236u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_162762_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 7u + (-6493)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 17u + (-4086)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 41u) ^ ((b << 1u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_162762_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 203u ^ 228u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 11u + (-1309)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 23u + (4324)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (11u + 23u) + (6628)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 46u + (2261)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>(aMixBox[(aWorkerByte ^ a ^ static_cast<std::uint32_t>(85u)) & 0xFFu]);
    ...
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(85u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 11u * 8U + static_cast<std::size_t>(aReverseIndex) * 23u + 203u + static_cast<std::size_t>(aTurbulence * ((46u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(203u), 2u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 85u) * ((46u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 2u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 32u + (-1309)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 25u + (4324)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (32u + 25u) + (6628)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(85u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 32u * 8U + static_cast<std::size_t>(aReverseIndex) * 25u * 3U + 6u + static_cast<std::size_t>(aTurbulence * ((46u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 2u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(6u), 2u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 85u) * ((46u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 2u, true);
  }
}

static void TwistCandidate_162762_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 200u ^ 176u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-1650)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 15u + (-775)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (31u + 15u) + (2280)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 31u * 8U + static_cast<std::size_t>(aSourceIndex) * 15u + 200u + static_cast<std::size_t>(aTurbulence * ((40u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(200u), 2u, true);
  }
}

static void TwistCandidate_162762_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xCCU, 0x46U, 0x9EU, 0xF4U, 0xF0U, 0xE1U, 0xF2U, 0x34U, 0x6BU, 0x5AU, 0x15U, 0x88U, 0xE0U, 0xC2U, 0xF1U, 0xF8U,
      0x32U, 0xBBU, 0x9DU, 0xE2U, 0x17U, 0x54U, 0x01U, 0xD5U, 0x6EU, 0x08U, 0x3FU, 0x99U, 0xC8U, 0x6CU, 0x61U, 0x53U,
      0x44U, 0x55U, 0x7DU, 0x9FU, 0xB8U, 0x59U, 0xB1U, 0xBDU, 0x5DU, 0xD0U, 0xA9U, 0xA6U, 0x0BU, 0x45U, 0x39U, 0xADU,
      0x20U, 0x0CU, 0x7BU, 0x4AU, 0xE4U, 0x1FU, 0xF9U, 0xA7U, 0x8EU, 0x5BU, 0x80U, 0x0EU, 0xC9U, 0x02U, 0x76U, 0x3DU,
      0xA0U, 0x4CU, 0xE9U, 0xFDU, 0x36U, 0xD4U, 0x28U, 0x95U, 0x83U, 0x19U, 0xBCU, 0xC5U, 0x64U, 0x77U, 0x33U, 0x93U,
      0x50U, 0xC7U, 0x3CU, 0xEFU, 0x87U, 0x68U, 0x71U, 0xB0U, 0xC1U, 0x66U, 0x5EU, 0xD6U, 0x2FU, 0xA2U, 0x60U, 0xD3U,
      0xE3U, 0x04U, 0x92U, 0x72U, 0x9BU, 0x09U, 0xCAU, 0x9AU, 0x98U, 0x2AU, 0x4BU, 0x3AU, 0x6DU, 0xDBU, 0x8CU, 0x24U,
      0x73U, 0x18U, 0x11U, 0xA8U, 0x6FU, 0x96U, 0xABU, 0x05U, 0x12U, 0xC0U, 0xA3U, 0x94U, 0xE6U, 0xB2U, 0x5FU, 0x48U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0xD7C85DD2u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0xB751217Bu, 9u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x8566FEFEu ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0xA030E9AAu, 4u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-3756), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (3932), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-3770), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4599), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltA = static_cast<std::uint32_t>(pSalt[(i + 12u) & 31U]);
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0xB751217Bu, 9u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0xA030E9AAu, 4u);
...
static void TwistCandidate_162762_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x50U, 0xDBU, 0x65U, 0x60U, 0x5DU, 0x4AU, 0xDFU, 0x7DU, 0xD8U, 0x80U, 0xAEU, 0x0AU, 0x0CU, 0x57U, 0xFAU, 0x8FU,
      0x71U, 0xF1U, 0x2DU, 0x26U, 0x75U, 0xA9U, 0x9FU, 0x02U, 0xBAU, 0xEBU, 0x85U, 0x51U, 0x6DU, 0xA1U, 0xAAU, 0x38U,
      0xB2U, 0x68U, 0x16U, 0x67U, 0x63U, 0x03U, 0xB7U, 0x27U, 0xC0U, 0xF4U, 0x74U, 0x59U, 0x4BU, 0xE4U, 0xFDU, 0x0EU,
      0xF7U, 0x42U, 0x20U, 0x2FU, 0x53U, 0xABU, 0x5EU, 0x7AU, 0x40U, 0x72U, 0x45U, 0xBCU, 0x07U, 0x9CU, 0x0BU, 0xB3U,
      0xE7U, 0xE9U, 0x86U, 0x1CU, 0x8DU, 0x25U, 0x98U, 0x01U, 0x33U, 0xBBU, 0xAFU, 0x77U, 0xB0U, 0xC4U, 0xE0U, 0x4EU,
      0xCAU, 0xE2U, 0x96U, 0xC6U, 0x04U, 0x14U, 0xD4U, 0x62U, 0xD5U, 0x7FU, 0x55U, 0x9BU, 0x29U, 0x12U, 0x1FU, 0xC9U,
      0x0FU, 0xB5U, 0x99U, 0xB8U, 0x8EU, 0x82U, 0x6AU, 0xA4U, 0x3DU, 0xF6U, 0x58U, 0x69U, 0xB9U, 0x87U, 0x95U, 0x76U,
      0x19U, 0xDDU, 0xC8U, 0xF3U, 0x3CU, 0x9EU, 0x2AU, 0xFEU, 0x89U, 0x23U, 0xFFU, 0x54U, 0x6CU, 0x3BU, 0xF2U, 0x7CU
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x9DA327EBu, 0xC6D44731u, 3u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-509), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xC6D44731u, 3u);
...
static void TwistCandidate_162762_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_162762_AdvanceTwiddle32A(
      0xD1F2DF96u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_162762_AdvanceTwiddle32B(
      0x43241FCAu ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_162762_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_162762_AdvanceTwiddle32A(
      0xA583A2EAu ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[10U]),
      0x6732E4DDu ^ static_cast<std::uint32_t>(pSalt[9U]));
  std::uint32_t aPulseB = TwistCandidate_162762_AdvanceTwiddle32B(
      0xD4499E09u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[9U]),
      0xF39D7D49u ^ static_cast<std::uint32_t>(pSalt[10U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_162762_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_162762_AdvanceTwiddle32B(
...
static void TwistCandidate_162762_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0xF1706DD9u ^ (pRound * 57u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-6493), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0xF1706DD9u ^ (pRound * 57u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-4086), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-2996), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (-4257), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 76u + 12u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-4257 + 12), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(19u)) ^ static_cast<std::uint32_t>(88u)) & 0xFFu);
      const int aIndex2 = WrapRange(aReverseIndex + (3980), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA ^ static_cast<std::uint32_t>(pSalt[20u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(250u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 9u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (-5615), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (-7448), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 92u + 25u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-7448 + 25), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 1u + 3U) & 15U), static_cast<std::size_t>(i + 28u)));
      ...
      const int aIndex2 = WrapRange(aReverseIndex + (6496), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB ^ static_cast<std::uint32_t>(pSalt[1u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(12u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 4u);
    }
  }

  TwistCandidate_162762_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_162762_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_162762_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_162762_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (7145), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (4522), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (1938), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 28u + 1528u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-290), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (7162), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 1026u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 1026u + 29u);
  ...
  TwistCandidate_162762_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (2500), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (-5260), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (848), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (7127), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_162762_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_162762_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 15u + (5100)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 3u + (3850)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 17u + (-5908)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 15u + static_cast<unsigned int>(30u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 225u)));
...
static void TwistCandidate_162762_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 25u + (-3385)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 24u + (858)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 11u + (-4502)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(228u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(228u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_162762_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 25u + (-6443)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 8u + (-3617)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 29u + (3167)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(176u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(176u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_162762(
  ...
  TwistCandidate_162762_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_162762_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_162762_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_162762_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_162762_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_162762_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_162762_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_162762_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_162762",
    &TwistCandidate_162762,
    &TwistCandidate_162762_KeySeed,
    &TwistCandidate_162762_SaltSeed,
    &TwistCandidate_162762_MaskSeedA,
    &TwistCandidate_162762_MaskSeedB,
    &TwistCandidate_162762_TwistBlock,
    &TwistCandidate_162762_PushKeyRound,
    &TwistCandidate_162762_PushMaskRoundA,
    &TwistCandidate_162762_PushMaskRoundB,
  }
};
