...
// Candidate 155209: TwistCandidate_155209
// family=ferocious_dual_worker op_budget=8 worker_shapes=1x1/1x3 worker_reverse=(True, True, True, False)/(True, False, False, True) twiddle1=xor/i_mul twiddle2=xor/none twiddle2_source=A tsunami=custom breaker_shapes=aBreakerAB:4m3i0v130,aBreakerCD:4m1i3v003,aBreakerEF:4m0i3v332 lane_breaker=true braid_breaker=false jump_breaker=true swap_breaker=true mask_template=2 mask_seed_a=xor_wave mask_seed_b=sbox_wave lightning=false/0 typhoon=false hurricane=false final_reverse=(True, False, True, True)
// ferocious[wa=1x1; wb=1x3; tw1=xor/i_mul; tw2=xor/none; tw2src=A; saltbox=128:0010418d:0e10fd92; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m3i0v130,aBreakerCD:4m1i3v003,aBreakerEF:4m0i3v332; wa_rev=(True, True, True, False); wb_rev=(True, False, False, True); lane=on:3; braid=off:0; jump=on:3; swap=on:3; mask=workerax2; lightning=off:0; typhoon=off; hurricane=off; final=3/(True, False, True, True); key_rot=12; maskA_seed=xor_wave/False; maskB_seed=sbox_wave; maskA_bias=70; maskB_bias=187]
static std::uint32_t TwistCandidate_155209_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x5AF96512u;
  aState = AdvanceTwiddle32(
      ...
      0x98FBD1DDu,
      5u);
  aState ^= RotateLeft32(pValue + pExtra + 0xA402A53Fu, 12U);
  return static_cast<std::uint32_t>(aState + 0xC202B4CFu);
}

static std::uint32_t TwistCandidate_155209_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x5B5E7DFCu;
  aState = AdvanceTwiddle32(
      ...
      0xFF5CD8C3u,
      11u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x5A563DD5u, 6U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5AC21u);
}

static void TwistCandidate_155209_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_155209_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_155209_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_155209_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_155209_AdvanceTwiddle32B(
...
static void TwistCandidate_155209_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x03U, 0x8CU, 0x06U, 0x36U, 0xA1U, 0x0FU, 0x19U, 0xAEU, 0x93U, 0x9AU, 0x53U, 0x4EU, 0xB3U, 0x9EU, 0x0BU, 0x84U,
      0xDEU, 0xE6U, 0xA3U, 0x30U, 0x5BU, 0xCCU, 0x40U, 0xEBU, 0x7DU, 0xA0U, 0xBEU, 0x1AU, 0xF1U, 0x0AU, 0xBDU, 0xFEU,
      0xD9U, 0x95U, 0xBBU, 0x05U, 0x09U, 0x86U, 0x76U, 0xBCU, 0x1FU, 0xC6U, 0xE8U, 0xA8U, 0x41U, 0xD5U, 0x2CU, 0x14U,
      0x2BU, 0xD4U, 0x21U, 0x3CU, 0xAAU, 0x88U, 0x8BU, 0xD8U, 0xDDU, 0x1BU, 0x60U, 0xD3U, 0x37U, 0xC2U, 0x11U, 0x9FU,
      0x6FU, 0x61U, 0xDAU, 0x5AU, 0x3FU, 0x45U, 0xACU, 0x92U, 0x39U, 0x48U, 0x38U, 0x17U, 0xCDU, 0xEEU, 0xF0U, 0x1EU,
      0xC7U, 0x6EU, 0x24U, 0x25U, 0x72U, 0xF7U, 0xB9U, 0xABU, 0xE2U, 0x12U, 0x5EU, 0x02U, 0xC3U, 0x58U, 0x75U, 0x4DU,
      0x4BU, 0xA4U, 0x0EU, 0xF2U, 0xA2U, 0x20U, 0xE5U, 0x79U, 0xFAU, 0x63U, 0x4AU, 0x7CU, 0xD6U, 0xE3U, 0x04U, 0x27U,
      0x87U, 0xD1U, 0x32U, 0x98U, 0x81U, 0x1CU, 0xB1U, 0xF6U, 0xC1U, 0x2EU, 0x97U, 0xAFU, 0xB7U, 0x9DU, 0x56U, 0xB2U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 219u ^ 66u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 3u + (-1495)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 12u + (4424)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 3u + static_cast<unsigned int>(219u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 219u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(66u), 7u, true);
  ...
  TwistCandidate_155209_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(66u), 7u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(66u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
...
static void TwistCandidate_155209_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 21u + (3136)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 30u + (-4253)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 219u) ^ ((b << 7u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_155209_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 139u ^ 70u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 12u + (6844)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 13u + (-3814)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (12u + 13u) + (2488)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 48u + (-4120)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 2u)) ^ static_cast<std::uint32_t>(148u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 12u * 8U + static_cast<std::size_t>(aReverseIndex) * 13u + 139u + static_cast<std::size_t>(aTurbulence * ((48u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(139u), 3u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 148u) * ((48u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
  ...
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 63u + (6844)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 34u + (-3814)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (63u + 34u) + (2488)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 2u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 63u * 8U + static_cast<std::size_t>(aReverseIndex) * 34u * 3U + 130u + static_cast<std::size_t>(aTurbulence * ((48u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 6u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(130u), 6u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 148u) * ((48u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 6u, true);
  }
}

static void TwistCandidate_155209_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 196u ^ 187u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 4u + (-8)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 13u + (6071)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (4u + 13u) + (6796)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 42u + (-5607)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>(aMixBox[(aWorkerByte ^ a ^ static_cast<std::uint32_t>(30u)) & 0xFFu]);
    const std::uint32_t aSeedA = static_cast<std::uint32_t>((a ^ aTurbulence) & 0xFFu);
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(30u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 4u * 8U + static_cast<std::size_t>(aReverseIndex) * 13u + 196u + static_cast<std::size_t>(aTurbulence * ((42u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(196u), 1u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 30u) * ((42u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 6u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 50u + (-8)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 46u + (6071)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (50u + 46u) + (6796)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 42u + (-5607)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(30u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 50u * 8U + static_cast<std::size_t>(aSourceIndex) * 46u * 3U + 24u + static_cast<std::size_t>(aTurbulence * ((42u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 5u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(24u), 5u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 30u) * ((42u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 5u, true);
  }
}

static void TwistCandidate_155209_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xD5U, 0x06U, 0x18U, 0xE8U, 0xC9U, 0x30U, 0x4AU, 0x9AU, 0x38U, 0x80U, 0x98U, 0x58U, 0xFDU, 0x70U, 0x9CU, 0x25U,
      0x90U, 0xDFU, 0x97U, 0x39U, 0x59U, 0x36U, 0xD0U, 0xCCU, 0x3EU, 0xCEU, 0xF0U, 0x35U, 0x7AU, 0x4CU, 0x8AU, 0x0FU,
      0xF1U, 0x3AU, 0xD4U, 0x88U, 0xBAU, 0xF2U, 0x6EU, 0xA9U, 0x31U, 0x03U, 0x91U, 0x12U, 0x3DU, 0xA4U, 0xC3U, 0x34U,
      0xA2U, 0xA6U, 0xBEU, 0x2AU, 0x62U, 0xCFU, 0xC6U, 0x1FU, 0xC8U, 0x46U, 0x66U, 0xB9U, 0x6AU, 0x89U, 0x8CU, 0xCBU,
      0xCDU, 0x08U, 0x27U, 0x6DU, 0x41U, 0x5FU, 0xDCU, 0x64U, 0x9EU, 0xC7U, 0xD2U, 0xC0U, 0xDAU, 0xEDU, 0x0CU, 0xECU,
      0xD7U, 0x65U, 0x42U, 0xD3U, 0x07U, 0x5EU, 0x49U, 0x63U, 0xF7U, 0x26U, 0x77U, 0xABU, 0xC2U, 0x57U, 0x3CU, 0x1BU,
      0x4DU, 0xF6U, 0x99U, 0x1AU, 0x83U, 0x40U, 0xE6U, 0x2DU, 0x37U, 0x8DU, 0x7BU, 0x95U, 0x3FU, 0x5BU, 0x9FU, 0x4FU,
      0x78U, 0xB0U, 0x55U, 0x60U, 0x7EU, 0xEFU, 0x6BU, 0xC1U, 0xF9U, 0xB3U, 0xA5U, 0x84U, 0x2CU, 0x3BU, 0xE4U, 0x24U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x90E934B6u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x5BAAD540u, 8u);
  std::uint32_t aAccB = AdvanceTwiddle32(0xD31B2690u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x5DDE08E2u, 4u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-2412), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (6320), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-2391), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-6732), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x5BAAD540u, 8u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x5DDE08E2u, 4u);
...
static void TwistCandidate_155209_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xB4U, 0x16U, 0x69U, 0xC2U, 0xF0U, 0xE9U, 0x3DU, 0x9AU, 0xF2U, 0x95U, 0x4BU, 0x77U, 0x2BU, 0x09U, 0xB7U, 0x3BU,
      0x7BU, 0xFEU, 0xDFU, 0x43U, 0xEEU, 0xA9U, 0xF8U, 0x24U, 0x9EU, 0xCEU, 0x70U, 0x2AU, 0xCAU, 0x64U, 0xECU, 0xE7U,
      0x6DU, 0x9FU, 0x86U, 0xC0U, 0x40U, 0x9BU, 0xB9U, 0x10U, 0x5BU, 0x7EU, 0x44U, 0xB8U, 0x73U, 0x8DU, 0xCBU, 0x96U,
      0xD0U, 0x45U, 0x2CU, 0x48U, 0x81U, 0xA1U, 0x25U, 0xDEU, 0x7CU, 0x93U, 0x31U, 0xBAU, 0xE5U, 0x0CU, 0x92U, 0x20U,
      0xADU, 0xABU, 0x19U, 0xE1U, 0x8FU, 0xF3U, 0x54U, 0xB3U, 0x98U, 0x9DU, 0x1EU, 0x27U, 0x3FU, 0xA2U, 0x53U, 0x8AU,
      0x35U, 0xF1U, 0x6CU, 0x4DU, 0xAAU, 0x60U, 0x38U, 0xDCU, 0x28U, 0xF4U, 0xD2U, 0x72U, 0xEBU, 0x21U, 0x1AU, 0xE8U,
      0xFBU, 0xBEU, 0xB1U, 0x75U, 0x7DU, 0xDAU, 0xC7U, 0x56U, 0x1FU, 0x91U, 0xB6U, 0xE2U, 0xD5U, 0x36U, 0xB0U, 0xA4U,
      0x1CU, 0xF9U, 0xA8U, 0xFAU, 0x4CU, 0x97U, 0xD3U, 0x5CU, 0xF7U, 0x76U, 0x57U, 0xB5U, 0x1BU, 0x39U, 0xAFU, 0x7FU
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0xFC74EAEEu, 0xC36D92ABu, 5u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-3352), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xC36D92ABu, 5u);
...
static void TwistCandidate_155209_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_155209_AdvanceTwiddle32A(
      0xCCB492ADu ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_155209_AdvanceTwiddle32B(
      0x91673C27u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_155209_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_155209_AdvanceTwiddle32A(
      0xB16DAE78u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[24U]),
      0x73DF1449u ^ static_cast<std::uint32_t>(pSalt[16U]));
  std::uint32_t aPulseB = TwistCandidate_155209_AdvanceTwiddle32B(
      0x6398C050u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[16U]),
      0x444C63B7u ^ static_cast<std::uint32_t>(pSalt[24U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    for (std::size_t aLane = 0; aLane < kMatrixBlockBytes; ++aLane) {
      ...
      const unsigned char aWave = FixedMixBoxByte(static_cast<unsigned char>(
          ...
          pSalt[(aLane + 24U + (aPulseByteB & 3U)) & 31U] ^
          aPulseByteA ^
    ...
    aPulseA = TwistCandidate_155209_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_155209_AdvanceTwiddle32B(
...
static void TwistCandidate_155209_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0xACA0E020u ^ (pRound * 30u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (3136), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0xACA0E020u ^ (pRound * 30u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-4253), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(aReverseIndex + (-3058), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-3772), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 40u + 15u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-3772 + 15), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 5u + 3U) & 15U), static_cast<std::size_t>(i + 23u)));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(9u)) ^ static_cast<std::uint32_t>(141u)) & 0xFFu);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(71u)), 0x85EBCA6Bu, 10u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (-4289), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (2907), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 4u + 6u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (2907 + 6), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 10u + 3U) & 15U), static_cast<std::size_t>(i + 25u)));
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(4u)), 0x85EBCA6Bu, 1u);
    }
  }

  TwistCandidate_155209_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_155209_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_155209_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_155209_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (855), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-1660), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (3839), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 24u + 99u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatOther = WrapRange(static_cast<int>(aMaskFlat) + (3 + 2), 0, static_cast<int>(kMaskStackTotalBytes));
      const std::uint32_t aMaskByteA = static_cast<std::uint32_t>(pMaskStackA[(aMaskFlat / kMaskBytes) % kMaskStackDepth][aMaskFlat % kMaskBytes]);
  ...
  TwistCandidate_155209_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (-867), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (1742), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (-7536), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (1355), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_155209_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_155209_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 21u + (-2208)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 30u + (4655)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 10u + (6987)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 21u + static_cast<unsigned int>(12u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 66u)));
...
static void TwistCandidate_155209_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 1u + (5609)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 14u + (5088)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 18u + (-2645)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_155209_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 3u + (1980)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 13u + (3160)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 1u + (5098)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
void TwistCandidate_155209(
  ...
  TwistCandidate_155209_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_155209_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_155209_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_155209_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_155209_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_155209_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_155209_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_155209_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_155209",
    &TwistCandidate_155209,
    &TwistCandidate_155209_KeySeed,
    &TwistCandidate_155209_SaltSeed,
    &TwistCandidate_155209_MaskSeedA,
    &TwistCandidate_155209_MaskSeedB,
    &TwistCandidate_155209_TwistBlock,
    &TwistCandidate_155209_PushKeyRound,
    &TwistCandidate_155209_PushMaskRoundA,
    &TwistCandidate_155209_PushMaskRoundB,
  }
};
