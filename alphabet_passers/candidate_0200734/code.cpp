...
// Candidate 200735: TwistCandidate_200735
// family=ferocious_dual_worker op_budget=9 worker_shapes=1x2/3x4 worker_reverse=(False, True, False, True)/(True, True, True, False) twiddle1=xor/none twiddle2=xor/none twiddle2_source=A tsunami=custom breaker_shapes=aBreakerAB:4m2i2v330,aBreakerCD:4m2i0v102,aBreakerEF:4m1i1v120 lane_breaker=true braid_breaker=true jump_breaker=true swap_breaker=false mask_template=0 mask_seed_a=sbox_wave mask_seed_b=off lightning=false/0 typhoon=false hurricane=true final_reverse=(False, False, True, True)
// ferocious[wa=1x2; wb=3x4; tw1=xor/none; tw2=xor/none; tw2src=A; saltbox=128:000ee6e2:fbd39ca8; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m2i2v330,aBreakerCD:4m2i0v102,aBreakerEF:4m1i1v120; wa_rev=(False, True, False, True); wb_rev=(True, True, True, False); lane=on:0; braid=on:1; jump=on:1; swap=off:0; mask=workerbx0; lightning=off:0; typhoon=off; hurricane=on; final=2/(False, False, True, True); key_rot=25; maskA_seed=sbox_wave/True; maskB_seed=off; maskA_bias=61; maskB_bias=13]
static std::uint32_t TwistCandidate_200735_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x6EA2D368u;
  aState = AdvanceTwiddle32(
      ...
      0x4FBCA6D7u,
      5u);
  aState ^= RotateLeft32(pValue + pExtra + 0x471E8978u, 10U);
  return static_cast<std::uint32_t>(aState + 0x211E75BFu);
}

static std::uint32_t TwistCandidate_200735_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x8C194F9Eu;
  aState = AdvanceTwiddle32(
      ...
      0xCB07C6E6u,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x39AA9A4Eu, 6U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5E949u);
}

static void TwistCandidate_200735_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_200735_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_200735_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_200735_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_200735_AdvanceTwiddle32B(
...
static void TwistCandidate_200735_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xB5U, 0x24U, 0x90U, 0xBBU, 0x51U, 0x4DU, 0x3AU, 0x2FU, 0x5BU, 0x9DU, 0x30U, 0x07U, 0x56U, 0x20U, 0xE1U, 0x31U,
      0xDAU, 0x50U, 0xCDU, 0x80U, 0x95U, 0x41U, 0xE3U, 0x5CU, 0xCAU, 0xC6U, 0xBFU, 0x11U, 0x82U, 0xC5U, 0xDCU, 0xC8U,
      0x3FU, 0x03U, 0xFDU, 0xD7U, 0x77U, 0xA0U, 0x18U, 0xA4U, 0x86U, 0x5FU, 0x47U, 0x12U, 0x83U, 0xB0U, 0xEEU, 0x2AU,
      0x01U, 0xB4U, 0x88U, 0x6AU, 0x1DU, 0x34U, 0xA8U, 0x55U, 0x97U, 0x15U, 0xF9U, 0xEBU, 0xB1U, 0x57U, 0xDBU, 0x6CU,
      0x2BU, 0x7EU, 0x52U, 0x8EU, 0x3CU, 0x04U, 0x60U, 0xE4U, 0x39U, 0x70U, 0x98U, 0xB2U, 0x1AU, 0x7AU, 0xEAU, 0x4CU,
      0xA1U, 0x13U, 0x81U, 0x2CU, 0x19U, 0x00U, 0x7DU, 0x9BU, 0xACU, 0x02U, 0xC9U, 0x0EU, 0x7BU, 0xC3U, 0x17U, 0x3DU,
      0xA2U, 0xB9U, 0x8BU, 0x35U, 0x8DU, 0x1FU, 0x22U, 0xE7U, 0xF5U, 0xA5U, 0xD4U, 0xD1U, 0x45U, 0xCCU, 0x5EU, 0x0AU,
      0x0BU, 0xFEU, 0x28U, 0xADU, 0x4BU, 0x85U, 0xBDU, 0xEDU, 0x58U, 0x8CU, 0x4FU, 0x7CU, 0x94U, 0x25U, 0x54U, 0x48U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 130u ^ 63u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 10u + (7336)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 25u + (-7542)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 10u + static_cast<unsigned int>(130u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 130u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(63u), 7u, true);
  ...
  TwistCandidate_200735_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(63u), 7u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(63u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 62u + (2060)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 21u + (-2266)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 62u + pSalt[(aSourceIndex + 62u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(62u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(62u), 2u, true);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_200735_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(62u), static_cast<std::uint32_t>(62u), 2u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(62u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_200735_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 27u + (4068)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 1u + (6699)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 130u) ^ ((b << 7u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_200735_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 209u ^ 61u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 1u + (4664)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 30u + (-5523)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (1u + 30u) + (1893)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 56u + (4700)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>(aMixBox[(aWorkerByte ^ a ^ static_cast<std::uint32_t>(75u)) & 0xFFu]);
    ...
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(75u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 1u * 8U + static_cast<std::size_t>(aReverseIndex) * 30u + 209u + static_cast<std::size_t>(aTurbulence * ((56u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(209u), 5u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 75u) * ((56u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
  ...
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 60u + (4664)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 60u + (-5523)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (60u + 60u) + (1893)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(75u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 60u * 8U + static_cast<std::size_t>(aReverseIndex) * 60u * 3U + 138u + static_cast<std::size_t>(aTurbulence * ((56u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 5u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(138u), 5u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 75u) * ((56u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 5u, true);
  }
}

static void TwistCandidate_200735_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 8u ^ 13u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 13u + (3700)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 3u + (5129)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (13u + 3u) + (4950)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 13u * 8U + static_cast<std::size_t>(aSourceIndex) * 3u + 8u + static_cast<std::size_t>(aTurbulence * ((49u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(8u), 4u, true);
  }
}

static void TwistCandidate_200735_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xC5U, 0x40U, 0xAEU, 0x85U, 0x9BU, 0x26U, 0x3FU, 0x3AU, 0x09U, 0x01U, 0x4EU, 0xEBU, 0x92U, 0x05U, 0x35U, 0xB0U,
      0x75U, 0x4AU, 0xBAU, 0x94U, 0xC3U, 0x3EU, 0xD5U, 0x34U, 0x45U, 0xF2U, 0x70U, 0x10U, 0xE9U, 0xA6U, 0x29U, 0x44U,
      0x28U, 0xB3U, 0x49U, 0x32U, 0x7CU, 0x41U, 0x64U, 0x4CU, 0xC0U, 0xE7U, 0x82U, 0x78U, 0xFFU, 0xA9U, 0x36U, 0x8FU,
      0xA8U, 0xF5U, 0xC1U, 0x0AU, 0x2AU, 0x6CU, 0x17U, 0x1AU, 0xE5U, 0xA1U, 0x2EU, 0x9DU, 0x66U, 0x95U, 0xCBU, 0xD6U,
      0x93U, 0x2DU, 0x90U, 0x22U, 0x8CU, 0x98U, 0xFCU, 0x60U, 0x31U, 0x38U, 0xBDU, 0x7AU, 0xADU, 0x9FU, 0x2CU, 0x0CU,
      0x73U, 0x5AU, 0xF4U, 0x07U, 0xB7U, 0x33U, 0xB8U, 0x7EU, 0x72U, 0xABU, 0xCCU, 0x4DU, 0xDCU, 0x65U, 0x12U, 0xDEU,
      0x0DU, 0xCFU, 0x55U, 0x18U, 0xDDU, 0x50U, 0x83U, 0xF0U, 0xB9U, 0x00U, 0x91U, 0xB4U, 0x20U, 0x8BU, 0x56U, 0x59U,
      0xB6U, 0xEAU, 0x48U, 0xD7U, 0x16U, 0x37U, 0xA5U, 0xC2U, 0x52U, 0xE2U, 0xBCU, 0x88U, 0x8AU, 0x19U, 0x3BU, 0xD3U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0xC4919238u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x0644F3D5u, 8u);
  std::uint32_t aAccB = AdvanceTwiddle32(0xC3DAA184u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x5479F021u, 10u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (997), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (5038), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-1731), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-1277), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltA = static_cast<std::uint32_t>(pSalt[(i + 24u) & 31U]);
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x0644F3D5u, 8u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x5479F021u, 10u);
...
static void TwistCandidate_200735_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xC3U, 0xCFU, 0xB6U, 0xBAU, 0xF3U, 0x9BU, 0xCEU, 0x5EU, 0x54U, 0xCAU, 0x09U, 0x23U, 0xC0U, 0x53U, 0x72U, 0x37U,
      0x14U, 0xA4U, 0x38U, 0x75U, 0x7AU, 0x3AU, 0xE8U, 0xECU, 0x1CU, 0x80U, 0xA8U, 0x40U, 0x3EU, 0xEBU, 0xD2U, 0x57U,
      0x9FU, 0x99U, 0x89U, 0x81U, 0x1AU, 0x4DU, 0x1BU, 0x52U, 0xF7U, 0x48U, 0xB5U, 0x2EU, 0x66U, 0xC1U, 0xDCU, 0xC8U,
      0x32U, 0x77U, 0x49U, 0xE7U, 0x13U, 0x3DU, 0x4BU, 0xA9U, 0xACU, 0x07U, 0xD6U, 0xF1U, 0x1EU, 0xBDU, 0x8FU, 0x8DU,
      0x1FU, 0x0CU, 0xAEU, 0x64U, 0xD1U, 0xF0U, 0x6CU, 0x56U, 0xFAU, 0xFCU, 0x67U, 0x46U, 0xDAU, 0x6BU, 0x3BU, 0xF6U,
      0x18U, 0x85U, 0x6DU, 0xD5U, 0x12U, 0xFFU, 0x8EU, 0x21U, 0x6AU, 0x79U, 0x0DU, 0x2CU, 0x22U, 0xEDU, 0x15U, 0x76U,
      0x7EU, 0xD7U, 0x34U, 0x39U, 0x29U, 0x01U, 0x03U, 0xB9U, 0xA6U, 0x86U, 0x44U, 0xC4U, 0x2BU, 0x69U, 0x61U, 0x0EU,
      0x55U, 0x78U, 0xD8U, 0x04U, 0x90U, 0xB4U, 0xA2U, 0xD9U, 0xE5U, 0xAFU, 0x5DU, 0xBCU, 0x30U, 0x10U, 0xDBU, 0x9DU
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x1289C908u, 0x1E97528Bu, 7u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (406), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x1E97528Bu, 7u);
...
static void TwistCandidate_200735_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_200735_AdvanceTwiddle32A(
      0xAE3F7C0Bu ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_200735_AdvanceTwiddle32B(
      0x9B740C51u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_200735_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_200735_AdvanceTwiddle32A(
      0x0A2F51A2u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[2U]),
      0xC89E7BA1u ^ static_cast<std::uint32_t>(pSalt[28U]));
  std::uint32_t aPulseB = TwistCandidate_200735_AdvanceTwiddle32B(
      0xA303A45Cu + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[28U]),
      0x84D787B1u ^ static_cast<std::uint32_t>(pSalt[2U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_200735_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_200735_AdvanceTwiddle32B(
...
static void TwistCandidate_200735_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x54D65DF3u ^ (pRound * 9u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (4068), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x54D65DF3u ^ (pRound * 9u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (6699), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-1345), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (148), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 3u + 8u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (148 + 8), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 15u + 3U) & 15U), static_cast<std::size_t>(i + 25u)));
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(217u)), 0x85EBCA6Bu, 1u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (2783), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (2003), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 52u + 26u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (2003 + 26), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = 0U;
      const int aIndex2 = WrapRange(aReverseIndex + (4350), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(aReverseIndex + (1409), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(230u)), 0x85EBCA6Bu, 9u);
    }
  }

  TwistCandidate_200735_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_200735_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_200735_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_200735_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (2059), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (5053), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-6909), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 11u + 379u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-5066), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (310), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 1129u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 1129u + 29u);
  ...
  TwistCandidate_200735_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (3647), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-2103), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (-3632), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (2574), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_200735_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_200735_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 23u + (1962)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 31u + (1882)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 13u + (3849)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 23u + static_cast<unsigned int>(25u)) & 31U;
...
static void TwistCandidate_200735_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 4u + (-1976)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 19u + (4961)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 3u + (5437)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(61u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(61u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_200735_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 23u + (-3222)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 27u + (-7039)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 18u + (-1424)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(13u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(13u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_200735(
  ...
  TwistCandidate_200735_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_200735_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_200735_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_200735_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_200735_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_200735_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_200735_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_200735_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_200735",
    &TwistCandidate_200735,
    &TwistCandidate_200735_KeySeed,
    &TwistCandidate_200735_SaltSeed,
    &TwistCandidate_200735_MaskSeedA,
    &TwistCandidate_200735_MaskSeedB,
    &TwistCandidate_200735_TwistBlock,
    &TwistCandidate_200735_PushKeyRound,
    &TwistCandidate_200735_PushMaskRoundA,
    &TwistCandidate_200735_PushMaskRoundB,
  }
};
