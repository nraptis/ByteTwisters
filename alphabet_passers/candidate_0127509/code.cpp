...
// Candidate 127510: TwistCandidate_127510
// family=ferocious_dual_worker op_budget=7 worker_shapes=1x3/2x0 worker_reverse=(False, True, False, True)/(False, True, False, True) twiddle1=add/none twiddle2=add/none twiddle2_source=A tsunami=custom breaker_shapes=aBreakerAB:4m3i3v301,aBreakerCD:4m2i0v123 lane_breaker=true braid_breaker=false jump_breaker=true swap_breaker=false mask_template=1 mask_seed_a=off mask_seed_b=add_wave lightning=false/1 typhoon=false hurricane=false final_reverse=(False, True, False, True)
// ferocious[wa=1x3; wb=2x0; tw1=add/none; tw2=add/none; tw2src=A; saltbox=128:001100db:1b90480a; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m3i3v301,aBreakerCD:4m2i0v123; wa_rev=(False, True, False, True); wb_rev=(False, True, False, True); lane=on:1; braid=off:1; jump=on:3; swap=off:2; mask=workerax1; lightning=off:1; typhoon=off; hurricane=off; final=3/(False, True, False, True); key_rot=17; maskA_seed=off/False; maskB_seed=add_wave; maskA_bias=137; maskB_bias=73]
static std::uint32_t TwistCandidate_127510_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x4498B822u;
  aState = AdvanceTwiddle32(
      ...
      0x046B8C63u,
      11u);
  aState ^= RotateLeft32(pValue + pExtra + 0x26F339F5u, 8U);
  return static_cast<std::uint32_t>(aState + 0x40F33441u);
}

static std::uint32_t TwistCandidate_127510_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xC7CEB467u;
  aState = AdvanceTwiddle32(
      ...
      0xE13D8D92u,
      7u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x25D645B4u, 1U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A53804u);
}

static void TwistCandidate_127510_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_127510_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_127510_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_127510_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_127510_AdvanceTwiddle32B(
...
static void TwistCandidate_127510_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xC4U, 0x4FU, 0x5FU, 0x51U, 0xABU, 0x2EU, 0x96U, 0xD6U, 0x85U, 0x42U, 0xBDU, 0xADU, 0x05U, 0xE9U, 0x75U, 0x4EU,
      0x2FU, 0x99U, 0x0FU, 0x10U, 0xA3U, 0xF7U, 0xF4U, 0x58U, 0xFFU, 0x33U, 0x14U, 0x76U, 0xC9U, 0xD8U, 0x69U, 0x11U,
      0x74U, 0xAFU, 0xA9U, 0x72U, 0xB3U, 0x50U, 0xA8U, 0x9EU, 0x44U, 0x1EU, 0x55U, 0x32U, 0x90U, 0x91U, 0x07U, 0x00U,
      0x6FU, 0xF3U, 0xC3U, 0x81U, 0x73U, 0x5EU, 0xE7U, 0xB8U, 0xE3U, 0xCAU, 0xBCU, 0xA1U, 0xCFU, 0x3AU, 0xAEU, 0x18U,
      0x70U, 0x54U, 0x30U, 0xEAU, 0x57U, 0x84U, 0x71U, 0xBBU, 0x1FU, 0xE2U, 0xFCU, 0xF5U, 0x97U, 0x9DU, 0xDDU, 0x2DU,
      0x88U, 0x63U, 0x0EU, 0x34U, 0x5AU, 0x31U, 0xACU, 0x8BU, 0x9AU, 0x2CU, 0x09U, 0xD4U, 0x22U, 0xC2U, 0xD9U, 0x3CU,
      0xBAU, 0x7DU, 0x93U, 0x6CU, 0x6BU, 0x89U, 0xECU, 0xCEU, 0x7FU, 0xA6U, 0xF8U, 0xE0U, 0xBEU, 0xDAU, 0x8FU, 0x80U,
      0xD2U, 0xB1U, 0x4DU, 0x36U, 0xAAU, 0xFBU, 0x82U, 0xDFU, 0x60U, 0x26U, 0x20U, 0x61U, 0x7AU, 0x5CU, 0xD5U, 0x0AU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 96u ^ 80u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 14u + (-1601)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 11u + (4523)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 14u + static_cast<unsigned int>(96u)) & 31U;
    ...
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(80u), 6u, true);
  ...
  TwistCandidate_127510_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(80u), 6u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(80u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
...
static void TwistCandidate_127510_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 7u + (1174)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 11u + (-586)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 96u) ^ ((b << 6u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_127510_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 12u ^ 137u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 14u + (564)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 21u + (1501)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (14u + 21u) + (-6782)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 14u * 8U + static_cast<std::size_t>(aSourceIndex) * 21u + 12u + static_cast<std::size_t>(aTurbulence * ((16u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(12u), 4u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 41u + (564)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 24u + (1501)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (41u + 24u) + (-6782)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 41u * 8U + static_cast<std::size_t>(aReverseIndex) * 24u * 3U + 122u + static_cast<std::size_t>(aTurbulence * ((16u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 5u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(122u), 5u, false);
  }
}

static void TwistCandidate_127510_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 145u ^ 73u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 16u + (22)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 22u + (6267)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (16u + 22u) + (620)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 10u + (991)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte + b + static_cast<std::uint32_t>(157u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 16u * 8U + static_cast<std::size_t>(aReverseIndex) * 22u + 145u + static_cast<std::size_t>(aTurbulence * ((10u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 157u) * ((10u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 1u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 22u + (22)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 39u + (6267)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (22u + 39u) + (620)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 10u + (991)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aTurbulence & 0xFFu), 1u))) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c + static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 1u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 22u * 8U + static_cast<std::size_t>(aSourceIndex) * 39u * 3U + 210u + static_cast<std::size_t>(aTurbulence * ((10u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 157u) * ((10u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 7u, true);
  }
}

static void TwistCandidate_127510_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x87U, 0xE0U, 0x44U, 0x74U, 0x23U, 0x3AU, 0xCAU, 0x03U, 0x11U, 0x19U, 0x2DU, 0x30U, 0x73U, 0xE9U, 0x14U, 0x0FU,
      0x67U, 0x3FU, 0xB9U, 0x66U, 0x8BU, 0x04U, 0xCBU, 0xC9U, 0x1FU, 0xDBU, 0x3BU, 0x96U, 0xFBU, 0x6CU, 0x1AU, 0x2BU,
      0xA6U, 0xC3U, 0x17U, 0x8CU, 0xEFU, 0xEDU, 0xEEU, 0x58U, 0xFDU, 0xDDU, 0x56U, 0xF1U, 0x62U, 0x24U, 0x54U, 0xB7U,
      0xD5U, 0xAAU, 0xADU, 0x13U, 0xE2U, 0x0BU, 0x05U, 0xB5U, 0x84U, 0x46U, 0x47U, 0xF2U, 0xB1U, 0x40U, 0xEBU, 0x1CU,
      0xE7U, 0xD8U, 0xF4U, 0x76U, 0x01U, 0x36U, 0x70U, 0x0AU, 0x31U, 0x48U, 0xB3U, 0xE6U, 0xBAU, 0xFCU, 0xBBU, 0x63U,
      0x9DU, 0x02U, 0x9EU, 0x49U, 0xD9U, 0x3CU, 0x2EU, 0x38U, 0x22U, 0xC4U, 0x60U, 0xD0U, 0x50U, 0xB6U, 0x5CU, 0x00U,
      0x78U, 0xC2U, 0xE3U, 0x5BU, 0x2CU, 0x7CU, 0x08U, 0x57U, 0x29U, 0xFAU, 0x88U, 0xCDU, 0x43U, 0xF3U, 0x4CU, 0x4DU,
      0x0EU, 0xA4U, 0x81U, 0x92U, 0x5EU, 0x20U, 0xBFU, 0xB2U, 0x3EU, 0x42U, 0x21U, 0x32U, 0xA5U, 0xD1U, 0xB8U, 0x0DU
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x9B8CF87Fu ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x01D7479Du, 10u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x0C479600u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x951B0513u, 9u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-6529), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (1277), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-5221), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (2301), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x01D7479Du, 10u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x951B0513u, 9u);
...
static void TwistCandidate_127510_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x23U, 0x93U, 0x98U, 0x67U, 0x5AU, 0xADU, 0xA9U, 0x0BU, 0x35U, 0x45U, 0x7BU, 0x7DU, 0xDAU, 0x26U, 0x28U, 0x9BU,
      0x1FU, 0xA2U, 0x39U, 0x24U, 0xD4U, 0x4AU, 0xACU, 0xEEU, 0xF5U, 0x22U, 0xF8U, 0x77U, 0x2EU, 0x2FU, 0xF9U, 0xBCU,
      0xD9U, 0x49U, 0x41U, 0x53U, 0xF2U, 0xBBU, 0x44U, 0x5FU, 0x27U, 0xA7U, 0x91U, 0xD8U, 0x1AU, 0xCCU, 0x4EU, 0x84U,
      0xF6U, 0xB3U, 0x07U, 0x55U, 0xF4U, 0x9DU, 0xE1U, 0xA3U, 0x8EU, 0xC4U, 0xA0U, 0x7FU, 0xC6U, 0x81U, 0xAEU, 0x1BU,
      0x3DU, 0x3BU, 0x2BU, 0x03U, 0xF3U, 0xFAU, 0x95U, 0xF1U, 0x02U, 0x11U, 0x0CU, 0x16U, 0x33U, 0xB4U, 0xC8U, 0xB5U,
      0x30U, 0xCFU, 0x6EU, 0x75U, 0xA5U, 0xFFU, 0x17U, 0x05U, 0xA6U, 0xAAU, 0xA8U, 0x1CU, 0xABU, 0x3CU, 0xAFU, 0x42U,
      0x5CU, 0xFDU, 0xF0U, 0xE2U, 0x8FU, 0x1EU, 0x52U, 0x3EU, 0xC3U, 0x20U, 0x96U, 0x68U, 0x90U, 0xB9U, 0x82U, 0x86U,
      0x7EU, 0xDDU, 0x9FU, 0xE0U, 0xC2U, 0xB1U, 0x85U, 0x80U, 0x87U, 0x4DU, 0x88U, 0x18U, 0x3FU, 0x54U, 0x70U, 0xD0U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x71C5F458u, 0x46229B04u, 4u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-7266), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x46229B04u, 4u);
...
static void TwistCandidate_127510_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_127510_AdvanceTwiddle32A(
      0x20CF668Au ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_127510_AdvanceTwiddle32B(
      0xFA06B596u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_127510_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_127510_AdvanceTwiddle32A(
      0x55EE9E06u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[6U]),
      0x975E2A40u ^ static_cast<std::uint32_t>(pSalt[31U]));
  std::uint32_t aPulseB = TwistCandidate_127510_AdvanceTwiddle32B(
      0xFD0A6B5Fu + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[31U]),
      0xDADEB090u ^ static_cast<std::uint32_t>(pSalt[6U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_127510_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_127510_AdvanceTwiddle32B(
...
static void TwistCandidate_127510_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x86FFC3D6u ^ (pRound * 36u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (1174), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x86FFC3D6u ^ (pRound * 36u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-586), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (823), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (5139), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 176u + 7u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (5139 + 7), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 9u + 3U) & 15U), static_cast<std::size_t>(i + 30u)));
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(203u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[16u]), 0x27D4EB2Du, 10u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (-7454), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (2398), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 117u + 1u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (2398 + 1), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 15u + 3U) & 15U), static_cast<std::size_t>(i + 22u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 29u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = 0U;
      const int aIndex2 = WrapRange(aReverseIndex + (-2678), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(250u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[29u]), 0x27D4EB2Du, 4u);
    }
  }

  TwistCandidate_127510_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_127510_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_127510_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-4360), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-2455), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-4906), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 28u + 658u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  TwistCandidate_127510_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (4358), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (-6092), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-5805), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (4177), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_127510_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_127510_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 6u + (2117)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 4u + (6547)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 21u + (6644)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 6u + static_cast<unsigned int>(17u)) & 31U;
...
static void TwistCandidate_127510_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-417)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 3u + (382)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 8u + (-291)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(137u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(137u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_127510_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 26u + (6870)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 16u + (1434)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 13u + (-4584)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(73u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(73u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_127510(
  ...
  TwistCandidate_127510_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_127510_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_127510_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_127510_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_127510_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_127510_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_127510_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_127510_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_127510",
    &TwistCandidate_127510,
    &TwistCandidate_127510_KeySeed,
    &TwistCandidate_127510_SaltSeed,
    &TwistCandidate_127510_MaskSeedA,
    &TwistCandidate_127510_MaskSeedB,
    &TwistCandidate_127510_TwistBlock,
    &TwistCandidate_127510_PushKeyRound,
    &TwistCandidate_127510_PushMaskRoundA,
    &TwistCandidate_127510_PushMaskRoundB,
  }
};
