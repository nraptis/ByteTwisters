...
// Candidate 111554: TwistCandidate_111554
// family=ferocious_dual_worker op_budget=11 worker_shapes=2x0/2x0 worker_reverse=(False, True, False, False)/(True, False, True, False) twiddle1=add/i twiddle2=mix/none twiddle2_source=A tsunami=custom breaker_shapes=aBreakerAB:4m2i0v131,aBreakerCD:4m0i0v112,aBreakerEF:4m1i0v023 lane_breaker=true braid_breaker=true jump_breaker=true swap_breaker=true mask_template=2 mask_seed_a=off mask_seed_b=add_wave lightning=true/3 typhoon=false hurricane=true final_reverse=(True, True, False, False)
// ferocious[wa=2x0; wb=2x0; tw1=add/i; tw2=mix/none; tw2src=A; saltbox=128:000ffc8e:f19d570b; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m2i0v131,aBreakerCD:4m0i0v112,aBreakerEF:4m1i0v023; wa_rev=(False, True, False, False); wb_rev=(True, False, True, False); lane=on:0; braid=on:3; jump=on:1; swap=on:2; mask=workerbx2; lightning=on:3; typhoon=off; hurricane=on; final=1/(True, True, False, False); key_rot=20; maskA_seed=off/True; maskB_seed=add_wave; maskA_bias=142; maskB_bias=203]
static std::uint32_t TwistCandidate_111554_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0xF4839632u;
  aState = AdvanceTwiddle32(
      ...
      0xCF920B85u,
      10u);
  aState ^= RotateLeft32(pValue + pExtra + 0x5D12B605u, 12U);
  return static_cast<std::uint32_t>(aState + 0x3B119DB7u);
}

static std::uint32_t TwistCandidate_111554_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x0C3435B4u;
  aState = AdvanceTwiddle32(
      ...
      0x512683B1u,
      11u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x45AA19E3u, 11U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A63E31u);
}

static void TwistCandidate_111554_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_111554_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_111554_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_111554_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_111554_AdvanceTwiddle32B(
...
static void TwistCandidate_111554_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x85U, 0xD4U, 0xD9U, 0xBCU, 0x40U, 0x1DU, 0x19U, 0xF4U, 0x36U, 0x7EU, 0x14U, 0x39U, 0x77U, 0x9BU, 0x71U, 0x2DU,
      0xAAU, 0x95U, 0x53U, 0x4BU, 0xD3U, 0x31U, 0xF7U, 0xFFU, 0x5CU, 0x9FU, 0x6FU, 0x24U, 0xC7U, 0xF8U, 0x76U, 0xDBU,
      0xAEU, 0x99U, 0x69U, 0x91U, 0xD7U, 0xCBU, 0x27U, 0x60U, 0x28U, 0xC4U, 0x8AU, 0x88U, 0xD8U, 0x84U, 0x7DU, 0x25U,
      0x1BU, 0x55U, 0xA4U, 0x75U, 0x10U, 0xA7U, 0x01U, 0x9DU, 0x07U, 0xA1U, 0x94U, 0x9AU, 0x52U, 0xB6U, 0x1AU, 0xE9U,
      0x6BU, 0x4DU, 0x05U, 0xFDU, 0x62U, 0x3CU, 0x74U, 0xB1U, 0x0FU, 0xDCU, 0xFAU, 0x41U, 0xF2U, 0xF0U, 0x13U, 0xB4U,
      0xEEU, 0xE5U, 0x9CU, 0x6DU, 0xB3U, 0x17U, 0x79U, 0x67U, 0x09U, 0xE3U, 0xC5U, 0x6AU, 0xB5U, 0xFEU, 0xE6U, 0x4CU,
      0x3EU, 0x5DU, 0x5BU, 0xCEU, 0xE8U, 0x7AU, 0x0CU, 0xF3U, 0x3DU, 0x50U, 0x92U, 0x45U, 0xA6U, 0x57U, 0x33U, 0x80U,
      0xE7U, 0xCDU, 0x2FU, 0x64U, 0xD2U, 0x4EU, 0xF1U, 0x4FU, 0x96U, 0x56U, 0x8DU, 0x21U, 0x8FU, 0x35U, 0x2EU, 0x2BU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 233u ^ 165u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 10u + (1861)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 26u + (3386)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 10u + static_cast<unsigned int>(233u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 233u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(165u), 5u, true);
  ...
  TwistCandidate_111554_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(165u), 5u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(165u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 27u + (7368)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-2121)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 27u + pSalt[(aSourceIndex + 193u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(193u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(193u), 4u, true);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_111554_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(193u), static_cast<std::uint32_t>(193u), 4u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(193u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_111554_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 31u + (1585)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-1892)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 233u) ^ ((b << 5u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_111554_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 201u ^ 142u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 9u + (826)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 19u + (3440)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (9u + 19u) + (-7382)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 9u * 8U + static_cast<std::size_t>(aReverseIndex) * 19u + 201u + static_cast<std::size_t>(aTurbulence * ((39u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(201u), 1u, true);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 25u + (826)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 58u + (3440)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (25u + 58u) + (-7382)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 25u * 8U + static_cast<std::size_t>(aSourceIndex) * 58u * 3U + 19u + static_cast<std::size_t>(aTurbulence * ((39u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 7u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(19u), 7u, true);
  }
}

static void TwistCandidate_111554_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 245u ^ 203u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 4u + (4825)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 31u + (5216)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (4u + 31u) + (3771)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 9u + (-1569)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte + b + static_cast<std::uint32_t>(124u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 4u * 8U + static_cast<std::size_t>(aReverseIndex) * 31u + 245u + static_cast<std::size_t>(aTurbulence * ((9u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(245u), 7u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 124u) * ((9u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 7u, false);
  }
}

static void TwistCandidate_111554_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x4CU, 0x48U, 0xCAU, 0x8FU, 0x86U, 0x0FU, 0x1CU, 0x1BU, 0x57U, 0x66U, 0x1FU, 0xB9U, 0x7DU, 0x4DU, 0x60U, 0x2BU,
      0xD5U, 0xAFU, 0x35U, 0x3DU, 0x26U, 0x58U, 0x09U, 0x6DU, 0x94U, 0x77U, 0x21U, 0x39U, 0x1DU, 0x30U, 0x84U, 0xCEU,
      0xADU, 0xA9U, 0x61U, 0x53U, 0x63U, 0xCCU, 0x02U, 0xC2U, 0x67U, 0xAAU, 0x8EU, 0x85U, 0x7BU, 0xF1U, 0x08U, 0xC3U,
      0x0AU, 0x50U, 0xBBU, 0x76U, 0xF0U, 0xD1U, 0x64U, 0x5CU, 0xB7U, 0xF7U, 0xD9U, 0xA3U, 0x15U, 0x24U, 0x0CU, 0x82U,
      0x8BU, 0x6CU, 0xBCU, 0xB3U, 0x33U, 0xA4U, 0xFDU, 0xFFU, 0x98U, 0xDAU, 0xD3U, 0xDEU, 0x69U, 0x9EU, 0x40U, 0x34U,
      0xA2U, 0x28U, 0xDFU, 0x2FU, 0xE9U, 0x1AU, 0x06U, 0x93U, 0x45U, 0x4AU, 0xEEU, 0x47U, 0x6BU, 0xC7U, 0x92U, 0x72U,
      0x99U, 0x74U, 0xC0U, 0x07U, 0xFAU, 0xD2U, 0x3EU, 0x13U, 0xA6U, 0xFEU, 0xA8U, 0xE7U, 0xEFU, 0xEDU, 0x4EU, 0xC4U,
      0x3AU, 0x11U, 0x9DU, 0xB1U, 0x3CU, 0x41U, 0x18U, 0x0DU, 0xF5U, 0xE8U, 0xACU, 0x52U, 0x2CU, 0x7EU, 0xE5U, 0xE3U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0xBE0C7B9Fu ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x8690F4DCu, 7u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x63A3459Eu ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x311CE7D5u, 8u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-3470), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (2195), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (1611), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (651), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltA = static_cast<std::uint32_t>(pSalt[(i + 30u) & 31U]);
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x8690F4DCu, 7u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x311CE7D5u, 8u);
...
static void TwistCandidate_111554_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x0CU, 0x8BU, 0x4EU, 0x26U, 0x3DU, 0xACU, 0xAEU, 0xF1U, 0xC3U, 0xA0U, 0x96U, 0x74U, 0x45U, 0x1DU, 0x08U, 0x78U,
      0x5FU, 0x0EU, 0x46U, 0xD8U, 0x28U, 0x35U, 0x63U, 0x44U, 0x72U, 0x93U, 0x5AU, 0x9AU, 0x21U, 0xD1U, 0xCEU, 0x3CU,
      0xCBU, 0xC7U, 0x39U, 0x32U, 0x2BU, 0xA7U, 0x25U, 0x8AU, 0xD5U, 0xF2U, 0x77U, 0x04U, 0x9DU, 0xBAU, 0x4FU, 0xE2U,
      0x7EU, 0x94U, 0xD7U, 0xB1U, 0xA2U, 0x1AU, 0x5BU, 0x7BU, 0x15U, 0xF0U, 0x55U, 0xE4U, 0xF6U, 0xB9U, 0x7FU, 0xF8U,
      0x87U, 0x51U, 0xC9U, 0x53U, 0x1BU, 0xCFU, 0x5CU, 0x9FU, 0x1FU, 0x76U, 0x66U, 0x4CU, 0x0FU, 0xA8U, 0x3EU, 0xBFU,
      0x14U, 0x22U, 0xF5U, 0x01U, 0xE3U, 0x91U, 0xBEU, 0xB5U, 0xEDU, 0x60U, 0x90U, 0xE8U, 0x49U, 0xA6U, 0xBCU, 0x00U,
      0xEBU, 0x23U, 0xAAU, 0x7AU, 0x54U, 0xB4U, 0x2EU, 0x19U, 0xC8U, 0x58U, 0x6FU, 0x5EU, 0x8DU, 0x42U, 0x03U, 0x2CU,
      0x2DU, 0xC0U, 0x80U, 0x4BU, 0x59U, 0x0BU, 0x36U, 0x47U, 0x81U, 0x9EU, 0x9CU, 0x3BU, 0x41U, 0xB3U, 0x30U, 0xEFU
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x54A5F4ACu, 0x60012003u, 7u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (311), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltByte = static_cast<std::uint32_t>(pSalt[(i + 16u) & 31U]);
    const std::uint32_t aMix = static_cast<std::uint32_t>(aMixBox[(aDestByte ^ aSourceByte ^ aSaltByte ^ FoldWordToByte(aAcc)) & 127U]);
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x60012003u, 7u);
...
static void TwistCandidate_111554_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_111554_AdvanceTwiddle32A(
      0x8A9EA85Eu ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_111554_AdvanceTwiddle32B(
      0xEDA208B2u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_111554_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_111554_AdvanceTwiddle32A(
      0xBFD19EE6u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[6U]),
      0x7D64B2DFu ^ static_cast<std::uint32_t>(pSalt[30U]));
  std::uint32_t aPulseB = TwistCandidate_111554_AdvanceTwiddle32B(
      0xC34FA31Eu + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[30U]),
      0xE49B704Cu ^ static_cast<std::uint32_t>(pSalt[6U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    for (std::size_t aLane = 0; aLane < kMatrixBlockBytes; ++aLane) {
      ...
      const unsigned char aWave = FixedMixBoxByte(static_cast<unsigned char>(
          ...
          pSalt[(aLane + 6U + (aPulseByteB & 3U)) & 31U] ^
          aPulseByteA ^
    ...
    aPulseA = TwistCandidate_111554_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_111554_AdvanceTwiddle32B(
...
static void TwistCandidate_111554_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x894F0127u ^ (pRound * 44u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (1585), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x894F0127u ^ (pRound * 44u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-1892), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-6852), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (7479), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 121u + 29u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (7479 + 29), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 4u + 3U) & 15U), static_cast<std::size_t>(i + 12u)));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>((aLaneIndex ^ static_cast<std::uint32_t>(166u)) & 0xFFu);
      const int aIndex2 = WrapRange(aReverseIndex + (666), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(83u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[21u]), 0x27D4EB2Du, 8u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (-5418), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (2975), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 92u + 31u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (2975 + 31), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 13u + 3U) & 15U), static_cast<std::size_t>(i + 21u)));
      ...
      const int aIndex2 = WrapRange(i + (2396), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::uint32_t aRoute = ((aSourceCtrl >> 4u) ^ aWorkerCtrl ^ aKeyByte ^ aMaskByte) & 3U;
      std::uint32_t aValue = a;
      switch (aRoute) {
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA ^ static_cast<std::uint32_t>(pSalt[16u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(58u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 1u);
    }
  }

  TwistCandidate_111554_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_111554_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_111554_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_111554_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (1431), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (5391), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-6051), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 13u + 38u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatOther = WrapRange(static_cast<int>(aMaskFlat) + (4 + 2), 0, static_cast<int>(kMaskStackTotalBytes));
      const std::uint32_t aMaskByteA = static_cast<std::uint32_t>(pMaskStackB[(aMaskFlat / kMaskBytes) % kMaskStackDepth][aMaskFlat % kMaskBytes]);
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    auto storm_block = LoadBlock16Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (4784), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock16Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-490), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackA, chunk + 647u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackB, chunk + 647u + 17u);
    ...
    switch (static_cast<unsigned>(storm_control[1] ^ storm_mask_a[2] ^ storm_mask_b[3] ^ pSalt[0]) % 6u) {
      ...
      case 2u: storm.ApplyFastOp(LightningFastOp::kSwapColumns, static_cast<std::uint8_t>(storm_control[3] ^ storm_mask_a[4] ^ storm_mask_b[5]), static_cast<std::uint8_t>(storm_control[5] + storm_mask_a[6] + storm_mask_b[7])); break;
      ...
      case 5u: storm.ApplyFastOp(LightningFastOp::kWeaveRows, static_cast<std::uint8_t>(storm_control[3] ^ storm_mask_a[4] ^ storm_mask_b[5]), static_cast<std::uint8_t>(storm_control[5] + storm_mask_a[6] + storm_mask_b[7])); break;
    }
    ...
    ApplyLightningMixColumns(storm, pSalt, static_cast<std::uint8_t>((storm_control[11] + pRound + 28u) & 31U), static_cast<std::uint8_t>(storm_control[15] ^ storm_mask_a[0] ^ storm_mask_b[1] ^ pSalt[3]), static_cast<std::uint8_t>(3u));
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (696), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (7626), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 659u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 659u + 29u);
  ...
  TwistCandidate_111554_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (-2115), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (2657), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (6049), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (3387), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_111554_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_111554_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 13u + (356)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-1242)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 23u + (-7281)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 13u + static_cast<unsigned int>(20u)) & 31U;
...
static void TwistCandidate_111554_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 23u + (-6766)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 3u + (7154)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 22u + (2413)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(142u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(142u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_111554_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 4u + (-5238)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 6u + (484)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 25u + (-6051)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(203u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(203u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_111554(
  ...
  TwistCandidate_111554_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_111554_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_111554_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_111554_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_111554_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_111554_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_111554_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_111554_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_111554",
    &TwistCandidate_111554,
    &TwistCandidate_111554_KeySeed,
    &TwistCandidate_111554_SaltSeed,
    &TwistCandidate_111554_MaskSeedA,
    &TwistCandidate_111554_MaskSeedB,
    &TwistCandidate_111554_TwistBlock,
    &TwistCandidate_111554_PushKeyRound,
    &TwistCandidate_111554_PushMaskRoundA,
    &TwistCandidate_111554_PushMaskRoundB,
  }
};
