...
// Candidate 139284: TwistCandidate_139284
// family=ferocious_dual_worker op_budget=9 worker_shapes=2x0/2x3 worker_reverse=(False, False, True, True)/(False, False, False, True) twiddle1=mix/none twiddle2=add/i twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m3i3v131,aBreakerCD:4m1i1v020,aBreakerEF:4m2i3v333 lane_breaker=true braid_breaker=true jump_breaker=true swap_breaker=true mask_template=1 mask_seed_a=off mask_seed_b=sbox_wave lightning=false/3 typhoon=false hurricane=false final_reverse=(False, True, True, True)
// ferocious[wa=2x0; wb=2x3; tw1=mix/none; tw2=add/i; tw2src=B; saltbox=128:001047b5:6a94b85b; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m3i3v131,aBreakerCD:4m1i1v020,aBreakerEF:4m2i3v333; wa_rev=(False, False, True, True); wb_rev=(False, False, False, True); lane=on:2; braid=on:3; jump=on:0; swap=on:1; mask=workerax1; lightning=off:3; typhoon=off; hurricane=off; final=2/(False, True, True, True); key_rot=4; maskA_seed=off/False; maskB_seed=sbox_wave; maskA_bias=190; maskB_bias=10]
static std::uint32_t TwistCandidate_139284_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x0140418Bu;
  aState = AdvanceTwiddle32(
      ...
      0xC79ACB56u,
      6u);
  aState ^= RotateLeft32(pValue + pExtra + 0xA0D9AD73u, 11U);
  return static_cast<std::uint32_t>(aState + 0xC6DA8ADDu);
}

static std::uint32_t TwistCandidate_139284_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x043F7147u;
  aState = AdvanceTwiddle32(
      ...
      0xA4E6DC34u,
      10u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xA6271DBFu, 7U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5BA11u);
}

static void TwistCandidate_139284_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_139284_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_139284_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_139284_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_139284_AdvanceTwiddle32B(
...
static void TwistCandidate_139284_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xDDU, 0xCBU, 0x8DU, 0xE3U, 0xE5U, 0xA2U, 0x17U, 0x67U, 0x97U, 0x23U, 0x34U, 0xFFU, 0xD0U, 0x1CU, 0x8BU, 0x0BU,
      0xC6U, 0x9EU, 0xDBU, 0x89U, 0x55U, 0x51U, 0x72U, 0xC4U, 0x74U, 0xBAU, 0x77U, 0x2DU, 0xE0U, 0xFDU, 0xC5U, 0xB1U,
      0x30U, 0x86U, 0xC7U, 0xDCU, 0x6EU, 0x63U, 0xDAU, 0x7FU, 0x3CU, 0xCCU, 0x75U, 0xBBU, 0x06U, 0xEDU, 0xAEU, 0x27U,
      0x2CU, 0x38U, 0x5AU, 0x03U, 0x64U, 0xBDU, 0xDEU, 0xF2U, 0x24U, 0xFEU, 0x80U, 0x07U, 0x2FU, 0x9AU, 0xAFU, 0x31U,
      0x7BU, 0x54U, 0xC0U, 0xE9U, 0xF0U, 0xD5U, 0x79U, 0x4FU, 0x68U, 0xC1U, 0xD2U, 0x56U, 0x16U, 0xE2U, 0x95U, 0x53U,
      0x93U, 0x20U, 0xF8U, 0x1AU, 0xFBU, 0x2EU, 0x5BU, 0x15U, 0x62U, 0x09U, 0x43U, 0xF5U, 0xB0U, 0xA9U, 0xC8U, 0x7CU,
      0x0EU, 0x21U, 0x9CU, 0xE4U, 0x35U, 0x28U, 0xD3U, 0x29U, 0x6FU, 0x61U, 0x9DU, 0xF7U, 0x33U, 0xF6U, 0xDFU, 0xE7U,
      0xAAU, 0xF1U, 0x02U, 0x4BU, 0x19U, 0xF3U, 0x57U, 0x13U, 0x5DU, 0xB5U, 0x73U, 0xECU, 0x1FU, 0x88U, 0xD7U, 0x1BU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 118u ^ 209u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-741)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 6u + (-3275)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 31u + static_cast<unsigned int>(118u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 118u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(209u), 1u, false);
  ...
  TwistCandidate_139284_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(209u), 1u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(209u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 36u + (-3410)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 59u + (-606)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 36u + pSalt[(aSourceIndex + 198u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(198u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(198u), 3u, false);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_139284_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(198u), static_cast<std::uint32_t>(198u), 3u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(198u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_139284_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 16u + (-6424)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 11u + (-817)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 118u) ^ ((b << 1u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_139284_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 239u ^ 190u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 29u + (4143)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 4u + (348)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (29u + 4u) + (4027)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 29u * 8U + static_cast<std::size_t>(aReverseIndex) * 4u + 239u + static_cast<std::size_t>(aTurbulence * ((7u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(239u), 1u, true);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 17u + (4143)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 50u + (348)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (17u + 50u) + (4027)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 17u * 8U + static_cast<std::size_t>(aSourceIndex) * 50u * 3U + 64u + static_cast<std::size_t>(aTurbulence * ((7u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 7u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(64u), 7u, true);
  }
}

static void TwistCandidate_139284_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 247u ^ 10u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-6116)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 30u + (-5593)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (26u + 30u) + (-6638)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 59u + (-4394)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>(aMixBox[(aWorkerByte ^ a ^ static_cast<std::uint32_t>(236u)) & 0xFFu]);
    ...
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(236u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 26u * 8U + static_cast<std::size_t>(aSourceIndex) * 30u + 247u + static_cast<std::size_t>(aTurbulence * ((59u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(247u), 2u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 236u) * ((59u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 4u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 55u + (-6116)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 35u + (-5593)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (55u + 35u) + (-6638)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 59u + (-4394)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(236u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 55u * 8U + static_cast<std::size_t>(aSourceIndex) * 35u * 3U + 210u + static_cast<std::size_t>(aTurbulence * ((59u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 7u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(210u), 7u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 236u) * ((59u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 7u, true);
  }
}

static void TwistCandidate_139284_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xB1U, 0x0DU, 0xE4U, 0x77U, 0x89U, 0xF3U, 0x64U, 0x36U, 0xCAU, 0x5FU, 0x0BU, 0x52U, 0xB5U, 0xF8U, 0xD6U, 0x65U,
      0x19U, 0xFEU, 0x49U, 0x86U, 0x43U, 0xA1U, 0x44U, 0x6EU, 0x37U, 0x2FU, 0xAAU, 0x6DU, 0x07U, 0xA4U, 0xF5U, 0xF4U,
      0x4AU, 0xE3U, 0x32U, 0x1CU, 0xA6U, 0x38U, 0x2BU, 0x73U, 0x97U, 0x99U, 0x21U, 0xB6U, 0x91U, 0xC4U, 0x47U, 0xCEU,
      0x3FU, 0x94U, 0x5BU, 0x6BU, 0x42U, 0x3EU, 0xCDU, 0xB0U, 0x75U, 0x5EU, 0x69U, 0x1EU, 0x93U, 0x83U, 0xB9U, 0x2EU,

  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x8D7C94E3u, 0xD486042Cu, 5u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (7053), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xD486042Cu, 5u);
...
static void TwistCandidate_139284_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_139284_AdvanceTwiddle32A(
      0x9DBF9318u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_139284_AdvanceTwiddle32B(
      0x4C8E0EF8u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_139284_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_139284_AdvanceTwiddle32A(
      0xB68A2243u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[3U]),
      0x743A1881u ^ static_cast<std::uint32_t>(pSalt[23U]));
  std::uint32_t aPulseB = TwistCandidate_139284_AdvanceTwiddle32B(
      0xA857EED7u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[23U]),
      0x8F830E1Cu ^ static_cast<std::uint32_t>(pSalt[3U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_139284_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_139284_AdvanceTwiddle32B(
...
static void TwistCandidate_139284_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x0B83E8B5u ^ (pRound * 58u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-6424), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x0B83E8B5u ^ (pRound * 58u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-817), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-6736), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (-7034), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 97u + 15u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-7034 + 15), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 9u + 3U) & 15U), static_cast<std::size_t>(i + 0u)));
      ...
      const int aIndex2 = WrapRange(i + (-7649), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA ^ static_cast<std::uint32_t>(pSalt[2u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(251u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 10u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (4796), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (2615), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 250u + 24u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (2615 + 24), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 14u + 3U) & 15U), static_cast<std::size_t>(i + 15u)));
      ...
      const int aIndex2 = WrapRange(i + (2579), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::uint32_t aRoute = ((aSourceCtrl >> 2u) ^ aWorkerCtrl ^ aKeyByte ^ aMaskByte) & 3U;
      std::uint32_t aValue = a;
      switch (aRoute) {
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB + static_cast<std::uint32_t>(235u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[26u]), 0x27D4EB2Du, 8u);
    }
  }

  TwistCandidate_139284_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_139284_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_139284_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    const DualWorkerBreakerRecipe4 aBreakerEF{
        ...
        -2582,
        -4920,
        static_cast<std::size_t>(309u),
        static_cast<std::uint8_t>(9u),
        static_cast<std::uint8_t>(31u),
        static_cast<std::uint8_t>(0u),
        static_cast<std::uint8_t>(7u),
        static_cast<std::uint8_t>(0u),
        static_cast<std::uint8_t>(251u),
        static_cast<std::uint8_t>(142u),
        static_cast<std::uint8_t>(8u),
        static_cast<std::uint8_t>(0u),
        static_cast<std::uint8_t>(3u),
    ...
    TwistCandidate_139284_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-6414), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-1054), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (5691), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 5u + 332u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  TwistCandidate_139284_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (-1166), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (-2582), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (-2210), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (-1356), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_139284_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_139284_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 23u + (96)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 25u + (4720)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 24u + (1161)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 23u + static_cast<unsigned int>(4u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 209u)));
...
static void TwistCandidate_139284_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 1u + (-2676)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-1972)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 18u + (6869)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_139284_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 16u + (-1008)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 30u + (7452)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 8u + (-66)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(10u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(10u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_139284(
  ...
  TwistCandidate_139284_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_139284_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_139284_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_139284_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_139284_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_139284_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_139284_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_139284_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_139284",
    &TwistCandidate_139284,
    &TwistCandidate_139284_KeySeed,
    &TwistCandidate_139284_SaltSeed,
    &TwistCandidate_139284_MaskSeedA,
    &TwistCandidate_139284_MaskSeedB,
    &TwistCandidate_139284_TwistBlock,
    &TwistCandidate_139284_PushKeyRound,
    &TwistCandidate_139284_PushMaskRoundA,
    &TwistCandidate_139284_PushMaskRoundB,
  }
};
