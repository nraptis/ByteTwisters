...
// Candidate 83000: TwistCandidate_83000
// family=ferocious_dual_worker op_budget=10 worker_shapes=3x3/1x3 worker_reverse=(True, True, True, False)/(False, True, True, True) twiddle1=add/none twiddle2=xor/i twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m3i2v032,aBreakerCD:4m3i3v001,aBreakerEF:4m0i1v132 lane_breaker=true braid_breaker=true jump_breaker=false swap_breaker=true mask_template=3 mask_seed_a=add_wave mask_seed_b=xor_wave lightning=false/0 typhoon=true hurricane=true final_reverse=(False, False, True, False)
// ferocious[wa=3x3; wb=1x3; tw1=add/none; tw2=xor/i; tw2src=B; saltbox=128:001063e0:9d0fb59c; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m3i2v032,aBreakerCD:4m3i3v001,aBreakerEF:4m0i1v132; wa_rev=(True, True, True, False); wb_rev=(False, True, True, True); lane=on:3; braid=on:1; jump=off:3; swap=on:2; mask=workerax3; lightning=off:0; typhoon=on; hurricane=on; final=1/(False, False, True, False); key_rot=28; maskA_seed=add_wave/False; maskB_seed=xor_wave; maskA_bias=58; maskB_bias=255]
static std::uint32_t TwistCandidate_83000_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x63EDC8A0u;
  aState = AdvanceTwiddle32(
      ...
      0x351B5CB9u,
      9u);
  aState ^= RotateLeft32(pValue + pExtra + 0x30F782F3u, 12U);
  return static_cast<std::uint32_t>(aState + 0x56F69419u);
}

static std::uint32_t TwistCandidate_83000_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xF6BE87F4u;
  aState = AdvanceTwiddle32(
      ...
      0xC6490507u,
      11u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x2A36CDA7u, 10U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5DB4Du);
}

static void TwistCandidate_83000_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_83000_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_83000_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_83000_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_83000_AdvanceTwiddle32B(
...
static void TwistCandidate_83000_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xECU, 0x31U, 0x01U, 0xD9U, 0xC5U, 0x55U, 0x58U, 0xFEU, 0x49U, 0x7DU, 0xACU, 0x73U, 0xA8U, 0x79U, 0x45U, 0x16U,
      0x00U, 0x4AU, 0xADU, 0xF5U, 0x96U, 0xCFU, 0x2CU, 0x21U, 0xB0U, 0x20U, 0xD1U, 0x07U, 0xBEU, 0x6BU, 0x22U, 0x8AU,
      0x08U, 0xF8U, 0x3CU, 0xC6U, 0x2AU, 0xAFU, 0x40U, 0xD7U, 0x26U, 0xB8U, 0x6FU, 0x9FU, 0xEEU, 0x75U, 0x97U, 0x72U,
      0xEBU, 0x1BU, 0xDAU, 0x50U, 0x2EU, 0xF6U, 0x0EU, 0x13U, 0xB9U, 0x35U, 0x3BU, 0x85U, 0xD3U, 0x11U, 0xBBU, 0x1CU,
      0x95U, 0x03U, 0x64U, 0xA6U, 0xE3U, 0xC4U, 0x56U, 0xB5U, 0x62U, 0xA7U, 0x3EU, 0x8FU, 0x0CU, 0x92U, 0xD8U, 0x28U,
      0x52U, 0xF0U, 0xA5U, 0x14U, 0x8EU, 0x39U, 0x83U, 0xB3U, 0x10U, 0x0DU, 0xCDU, 0xBDU, 0x82U, 0xC3U, 0x15U, 0x65U,
      0xEAU, 0xB1U, 0x86U, 0x0AU, 0xD4U, 0xB7U, 0x42U, 0xE7U, 0x33U, 0x57U, 0xF2U, 0x5BU, 0xFFU, 0x8CU, 0x41U, 0xE5U,
      0x78U, 0x32U, 0x93U, 0x5DU, 0x1AU, 0xBAU, 0xC0U, 0x9EU, 0x2FU, 0xABU, 0xF9U, 0xD5U, 0x27U, 0xCAU, 0x76U, 0xBCU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 29u ^ 253u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 19u + (3401)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 2u + (4951)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 19u + static_cast<unsigned int>(29u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 29u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(253u), 4u, false);
  ...
  TwistCandidate_83000_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(253u), 4u);
...
static void TwistCandidate_83000_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 25u + (4109)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 21u + (-5764)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 29u) ^ ((b << 4u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_83000_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 24u ^ 58u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 13u + (2807)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 23u + (6408)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (13u + 23u) + (-203)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 7u + (-1269)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte + b + static_cast<std::uint32_t>(219u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 13u * 8U + static_cast<std::size_t>(aSourceIndex) * 23u + 24u + static_cast<std::size_t>(aTurbulence * ((7u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(24u), 4u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 219u) * ((7u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 3u, false);
  }
}

static void TwistCandidate_83000_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 246u ^ 255u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 22u + (-3073)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 12u + (7391)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (22u + 12u) + (3772)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 23u + (2006)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 1u)) ^ static_cast<std::uint32_t>(4u)) & 0xFFu);
    const std::uint32_t aSeedA = static_cast<std::uint32_t>((a ^ aTurbulence) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 22u * 8U + static_cast<std::size_t>(aReverseIndex) * 12u + 246u + static_cast<std::size_t>(aTurbulence * ((23u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(246u), 4u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 4u) * ((23u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 1u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 41u + (-3073)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 41u + (7391)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (41u + 41u) + (3772)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 1u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 41u * 8U + static_cast<std::size_t>(aSourceIndex) * 41u * 3U + 47u + static_cast<std::size_t>(aTurbulence * ((23u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 4u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(47u), 4u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 4u) * ((23u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 4u, true);
  }
}

static void TwistCandidate_83000_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x89U, 0x48U, 0xA7U, 0x8DU, 0x46U, 0x0AU, 0xF2U, 0xE6U, 0xEDU, 0xA2U, 0xAAU, 0x6BU, 0x84U, 0x20U, 0xF3U, 0xFBU,
      0xE8U, 0x08U, 0xC6U, 0x96U, 0x04U, 0x33U, 0x57U, 0x47U, 0xD7U, 0x27U, 0xD1U, 0x0FU, 0x01U, 0x14U, 0x4FU, 0x40U,
      0xF5U, 0xC4U, 0x82U, 0xCFU, 0x42U, 0x6CU, 0xA9U, 0xEFU, 0x72U, 0x8EU, 0xC9U, 0x10U, 0xD9U, 0x78U, 0x44U, 0x45U,
      0xD3U, 0x88U, 0x1BU, 0x81U, 0x67U, 0x4AU, 0xC3U, 0xE1U, 0xE3U, 0x77U, 0x2DU, 0xA4U, 0x19U, 0x5AU, 0x50U, 0xB1U,
      0xF4U, 0x11U, 0xB2U, 0x9CU, 0xC7U, 0xBCU, 0x65U, 0x5EU, 0x02U, 0xDCU, 0x49U, 0xECU, 0x31U, 0xDFU, 0xC0U, 0x53U,
      0x7DU, 0x85U, 0x76U, 0xEAU, 0xB6U, 0x7BU, 0x2AU, 0x35U, 0xDEU, 0x70U, 0x8BU, 0xA5U, 0x6EU, 0xADU, 0x59U, 0x94U,
      0x15U, 0xBBU, 0xCDU, 0x03U, 0x38U, 0x6AU, 0x66U, 0x52U, 0xAFU, 0x86U, 0x0EU, 0xB0U, 0x21U, 0x58U, 0x79U, 0x73U,
      0x56U, 0x18U, 0x87U, 0xDDU, 0xB5U, 0xACU, 0x1CU, 0x24U, 0x5FU, 0xB3U, 0x60U, 0x30U, 0x4BU, 0xD0U, 0x25U, 0x91U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x3E223FDCu ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0xBAC89BDCu, 8u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x506A8E50u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0xFD773801u, 8u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (2385), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (1905), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-2011), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4780), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltB = static_cast<std::uint32_t>(pSalt[(i + 5u) & 31U]);
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0xBAC89BDCu, 8u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0xFD773801u, 8u);
...
static void TwistCandidate_83000_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xDDU, 0x98U, 0x8AU, 0xB4U, 0x0FU, 0x74U, 0x7AU, 0xFCU, 0x2BU, 0x57U, 0xB9U, 0xCEU, 0x5FU, 0x0DU, 0x6FU, 0xEAU,
      0xC8U, 0x91U, 0x17U, 0x76U, 0x28U, 0xD0U, 0xE3U, 0x1FU, 0xBAU, 0x96U, 0xE9U, 0x3AU, 0xC1U, 0x09U, 0xBEU, 0x7EU,
      0x8EU, 0x8CU, 0x65U, 0xB0U, 0xA5U, 0x3DU, 0xB8U, 0xD4U, 0x6BU, 0xB7U, 0x4FU, 0x20U, 0x3BU, 0xB2U, 0x6DU, 0xACU,
      0x58U, 0xE8U, 0x80U, 0xEBU, 0xFDU, 0xF5U, 0xA7U, 0xDAU, 0x8DU, 0x01U, 0x24U, 0x25U, 0x4BU, 0x5EU, 0x1DU, 0xC9U,
      0xDBU, 0xCAU, 0x2EU, 0xD9U, 0x75U, 0xF0U, 0x62U, 0x1AU, 0xCCU, 0x86U, 0x59U, 0xFFU, 0x70U, 0xFAU, 0xD3U, 0x68U,
      0x46U, 0xA3U, 0xD1U, 0x48U, 0x64U, 0x95U, 0x0CU, 0x27U, 0x56U, 0x3FU, 0xF9U, 0x12U, 0x00U, 0xC4U, 0x6EU, 0x6CU,
      0x3EU, 0x06U, 0x50U, 0xCBU, 0x13U, 0x4AU, 0xE1U, 0x11U, 0x77U, 0x82U, 0x9EU, 0x21U, 0x87U, 0xD5U, 0xC7U, 0x2FU,
      0x03U, 0xA1U, 0x4EU, 0x36U, 0x63U, 0x92U, 0x71U, 0xF2U, 0x31U, 0x84U, 0x0BU, 0x5CU, 0x2CU, 0x29U, 0x5DU, 0xB3U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0xF4793231u, 0x5E4AC563u, 3u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-6286), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltByte = static_cast<std::uint32_t>(pSalt[(i + 29u) & 31U]);
    const std::uint32_t aMix = static_cast<std::uint32_t>(aMixBox[(aDestByte ^ aSourceByte ^ aSaltByte ^ FoldWordToByte(aAcc)) & 127U]);
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x5E4AC563u, 3u);
...
static void TwistCandidate_83000_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_83000_AdvanceTwiddle32A(
      0x7171F514u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_83000_AdvanceTwiddle32B(
      0x7FC28DCCu ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_83000_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_83000_AdvanceTwiddle32A(
      0xD26A7BB0u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[16U]),
      0x10D9C591u ^ static_cast<std::uint32_t>(pSalt[11U]));
  std::uint32_t aPulseB = TwistCandidate_83000_AdvanceTwiddle32B(
      0xA1134A6Bu + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[11U]),
      0x86C7C15Eu ^ static_cast<std::uint32_t>(pSalt[16U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    for (std::size_t aLane = 0; aLane < kMatrixBlockBytes; ++aLane) {
      ...
      const unsigned char aMix = FixedMixBoxByte(static_cast<unsigned char>(
          ...
          pSalt[(aLane + 11U + (aPulseByteA & 3U)) & 31U]));
    ...
    aPulseA = TwistCandidate_83000_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_83000_AdvanceTwiddle32B(
...
static void TwistCandidate_83000_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x6F973E28u ^ (pRound * 6u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (4109), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x6F973E28u ^ (pRound * 6u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-5764), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(aReverseIndex + (4851), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (4977), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 204u + 4u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (4977 + 4), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 15u + 3U) & 15U), static_cast<std::size_t>(i + 20u)));
      ...
      const int aIndex2 = WrapRange(aReverseIndex + (1005), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(aReverseIndex + (2141), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(129u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[16u]), 0x27D4EB2Du, 2u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (5561), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (705), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 197u + 16u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (705 + 16), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 12u + 3U) & 15U), static_cast<std::size_t>(i + 15u)));
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(57u)), 0x85EBCA6Bu, 9u);
    }
  }

  TwistCandidate_83000_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_83000_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_83000_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_83000_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (686), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-6681), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (7099), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 12u + 1017u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatOther = WrapRange(static_cast<int>(aMaskFlat) + (2 + 3), 0, static_cast<int>(kMaskStackTotalBytes));
      const std::uint32_t aMaskByteA = static_cast<std::uint32_t>(pMaskStackA[(aMaskFlat / kMaskBytes) % kMaskStackDepth][aMaskFlat % kMaskBytes]);
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kTyphoonBlockBytes) {
    auto storm_block = LoadBlock128Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (3118), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock128Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (3675), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackA, chunk + 1479u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackB, chunk + 1479u + 23u);
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-7601), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-3705), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 1236u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 1236u + 29u);
  ...
  TwistCandidate_83000_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (5033), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (5725), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (-169), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (-3076), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_83000_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_83000_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 6u + (-6939)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 18u + (1088)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 5u + (2517)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 6u + static_cast<unsigned int>(28u)) & 31U;
...
static void TwistCandidate_83000_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 1u + (2964)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 27u + (2176)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 24u + (4045)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(58u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(58u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_83000_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 10u + (2371)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 10u + (-7327)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 8u + (68)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(255u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(255u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_83000(
  ...
  TwistCandidate_83000_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_83000_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_83000_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_83000_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_83000_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_83000_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_83000_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_83000_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_83000",
    &TwistCandidate_83000,
    &TwistCandidate_83000_KeySeed,
    &TwistCandidate_83000_SaltSeed,
    &TwistCandidate_83000_MaskSeedA,
    &TwistCandidate_83000_MaskSeedB,
    &TwistCandidate_83000_TwistBlock,
    &TwistCandidate_83000_PushKeyRound,
    &TwistCandidate_83000_PushMaskRoundA,
    &TwistCandidate_83000_PushMaskRoundB,
  }
};
