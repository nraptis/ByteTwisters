...
// Candidate 123834: TwistCandidate_123834
// family=ferocious_dual_worker op_budget=8 worker_shapes=3x3/1x2 worker_reverse=(True, True, True, False)/(False, False, False, False) twiddle1=add/none twiddle2=add/none twiddle2_source=A tsunami=custom breaker_shapes=aBreakerAB:4m2i1v312,aBreakerCD:4m2i2v331 lane_breaker=true braid_breaker=false jump_breaker=false swap_breaker=true mask_template=3 mask_seed_a=off mask_seed_b=xor_wave lightning=true/3 typhoon=false hurricane=false final_reverse=(True, False, True, True)
// ferocious[wa=3x3; wb=1x2; tw1=add/none; tw2=add/none; tw2src=A; saltbox=128:000ff9fe:6393c916; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m2i1v312,aBreakerCD:4m2i2v331; wa_rev=(True, True, True, False); wb_rev=(False, False, False, False); lane=on:1; braid=off:1; jump=off:3; swap=on:1; mask=workerax3; lightning=on:3; typhoon=off; hurricane=off; final=4/(True, False, True, True); key_rot=26; maskA_seed=off/False; maskB_seed=xor_wave; maskA_bias=36; maskB_bias=28]
static std::uint32_t TwistCandidate_123834_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x9C1D4131u;
  aState = AdvanceTwiddle32(
      ...
      0xB0A114D4u,
      11u);
  aState ^= RotateLeft32(pValue + pExtra + 0x4ABD1ADAu, 10U);
  return static_cast<std::uint32_t>(aState + 0x2CBC55E5u);
}

static std::uint32_t TwistCandidate_123834_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x7304CE72u;
  aState = AdvanceTwiddle32(
      ...
      0x39B9D4A8u,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xD5D715D9u, 1U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5DAA6u);
}

static void TwistCandidate_123834_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_123834_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_123834_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_123834_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_123834_AdvanceTwiddle32B(
...
static void TwistCandidate_123834_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x62U, 0x3AU, 0x20U, 0xD3U, 0x22U, 0x41U, 0x89U, 0xDDU, 0xDAU, 0x94U, 0x52U, 0x5AU, 0xA8U, 0x05U, 0xC5U, 0xF7U,
      0xCCU, 0xBAU, 0xC3U, 0xFEU, 0x47U, 0xA1U, 0x78U, 0xF1U, 0x9BU, 0xB7U, 0x12U, 0x39U, 0xC9U, 0x07U, 0xF2U, 0xBBU,
      0xF3U, 0x6EU, 0xB4U, 0x9FU, 0x6CU, 0x01U, 0x02U, 0xE7U, 0x7BU, 0x72U, 0x5FU, 0xE3U, 0x5DU, 0xBFU, 0xD9U, 0xEEU,
      0x91U, 0x7DU, 0x85U, 0xBDU, 0x3EU, 0x34U, 0xDEU, 0x13U, 0x03U, 0x83U, 0x84U, 0xB5U, 0xC4U, 0xE4U, 0xACU, 0xDCU,
      0xD4U, 0xA7U, 0xC1U, 0x27U, 0xFCU, 0x54U, 0x99U, 0xEAU, 0xA2U, 0x65U, 0xC0U, 0x70U, 0xFFU, 0x0FU, 0x1CU, 0x97U,
      0xE6U, 0x17U, 0x67U, 0xCBU, 0x79U, 0xB2U, 0xCDU, 0x36U, 0xF6U, 0xD2U, 0x66U, 0x58U, 0x2DU, 0x5BU, 0x42U, 0x35U,
      0x5CU, 0xE0U, 0x75U, 0x76U, 0x19U, 0x15U, 0x93U, 0x25U, 0xA5U, 0xD6U, 0x14U, 0x81U, 0xB6U, 0x00U, 0xF9U, 0x95U,
      0xD7U, 0x63U, 0x18U, 0x7EU, 0x6AU, 0x31U, 0x8EU, 0x51U, 0x2FU, 0x56U, 0xD0U, 0xE2U, 0x59U, 0x16U, 0x11U, 0x96U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 135u ^ 32u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 30u + (-3735)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 20u + (-296)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 30u + static_cast<unsigned int>(135u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 135u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(32u), 6u, true);
  ...
  TwistCandidate_123834_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(32u), 6u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(32u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-7291)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 38u + (3260)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 29u + pSalt[(aSourceIndex + 210u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(210u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(210u), 2u, true);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_123834_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(210u), static_cast<std::uint32_t>(210u), 2u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(210u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_123834_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 18u + (-950)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 1u + (7085)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 135u) ^ ((b << 6u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_123834_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 131u ^ 36u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 26u + (-277)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 5u + (725)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (26u + 5u) + (-805)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 26u * 8U + static_cast<std::size_t>(aSourceIndex) * 5u + 131u + static_cast<std::size_t>(aTurbulence * ((49u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(131u), 4u, false);
  }
}

static void TwistCandidate_123834_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 164u ^ 28u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 20u + (7494)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 4u + (2835)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (20u + 4u) + (-3232)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 25u + (-6350)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 7u)) ^ static_cast<std::uint32_t>(230u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 20u * 8U + static_cast<std::size_t>(aReverseIndex) * 4u + 164u + static_cast<std::size_t>(aTurbulence * ((25u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(164u), 2u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 230u) * ((25u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 7u, true);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 20u + (7494)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 36u + (2835)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (20u + 36u) + (-3232)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 7u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 20u * 8U + static_cast<std::size_t>(aReverseIndex) * 36u * 3U + 214u + static_cast<std::size_t>(aTurbulence * ((25u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 6u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(214u), 6u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 230u) * ((25u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 6u, true);
  }
}

static void TwistCandidate_123834_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x9BU, 0x29U, 0x67U, 0xE0U, 0x52U, 0xC3U, 0xB2U, 0xDBU, 0x32U, 0x4CU, 0x3AU, 0x89U, 0xEDU, 0x2FU, 0x92U, 0xE2U,
      0xAAU, 0x43U, 0x68U, 0x4DU, 0xE1U, 0xE8U, 0x00U, 0x11U, 0xBEU, 0xE4U, 0x6BU, 0xF1U, 0x76U, 0x38U, 0x45U, 0xB9U,
      0xCCU, 0x36U, 0x85U, 0x6CU, 0x24U, 0x9DU, 0xA4U, 0xA1U, 0xACU, 0x58U, 0x47U, 0xD0U, 0x94U, 0xD3U, 0x49U, 0xA8U,
      0xF9U, 0xFAU, 0x40U, 0x04U, 0xE3U, 0xD2U, 0x8DU, 0x0EU, 0x88U, 0x37U, 0xAFU, 0x2EU, 0x0AU, 0x4EU, 0x7EU, 0xD1U,
      0x26U, 0x16U, 0x99U, 0xAEU, 0x0DU, 0x15U, 0x28U, 0xF4U, 0xABU, 0x1BU, 0x54U, 0x05U, 0x02U, 0xE7U, 0xC2U, 0x66U,
 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltByte = static_cast<std::uint32_t>(pSalt[(i + 26u) & 31U]);
    const std::uint32_t aMix = static_cast<std::uint32_t>(aMixBox[(aDestByte ^ aSourceByte ^ aSaltByte ^ FoldWordToByte(aAcc)) & 127U]);
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x2953C136u, 6u);
...
static void TwistCandidate_123834_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_123834_AdvanceTwiddle32A(
      0x3DB74A06u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_123834_AdvanceTwiddle32B(
      0xE439CC1Au ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_123834_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_123834_AdvanceTwiddle32A(
      0x2F2B7935u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[21U]),
      0xED9B7B0Au ^ static_cast<std::uint32_t>(pSalt[28U]));
  std::uint32_t aPulseB = TwistCandidate_123834_AdvanceTwiddle32B(
      0xD918511Cu + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[28U]),
      0xFECCEA50u ^ static_cast<std::uint32_t>(pSalt[21U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_123834_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_123834_AdvanceTwiddle32B(
...
static void TwistCandidate_123834_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0xD0E8495Au ^ (pRound * 15u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-950), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0xD0E8495Au ^ (pRound * 15u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (7085), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(aReverseIndex + (3107), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-809), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 50u + 19u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-809 + 19), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 9u + 3U) & 15U), static_cast<std::size_t>(i + 10u)));
      ...
      const int aIndex2 = WrapRange(aReverseIndex + (-2831), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(aReverseIndex + (-1503), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(51u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[13u]), 0x27D4EB2Du, 4u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (3753), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-5994), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 2u + 25u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-5994 + 25), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 3u + 3U) & 15U), static_cast<std::size_t>(i + 10u)));
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(253u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[18u]), 0x27D4EB2Du, 7u);
    }
  }

  TwistCandidate_123834_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_123834_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_123834_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-1250), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-2076), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (1862), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 10u + 630u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    auto storm_block = LoadBlock16Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (939), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock16Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-4200), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackA, chunk + 1168u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackB, chunk + 1168u + 17u);
    ...
    ApplyLightningMixColumns(storm, pSalt, static_cast<std::uint8_t>((storm_control[11] + pRound + 12u) & 31U), static_cast<std::uint8_t>(storm_control[15] ^ storm_mask_a[0] ^ storm_mask_b[1] ^ pSalt[3]), static_cast<std::uint8_t>(3u));
  ...
  TwistCandidate_123834_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (6175), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (4481), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (-1464), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (-3555), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_123834_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_123834_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 14u + (4661)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 10u + (-1519)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 21u + (3622)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 14u + static_cast<unsigned int>(26u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 32u)));
...
static void TwistCandidate_123834_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 21u + (7033)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-7200)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 4u + (1275)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(36u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(36u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_123834_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 8u + (2667)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 5u + (-4653)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 4u + (-7139)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(28u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(28u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_123834(
  ...
  TwistCandidate_123834_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_123834_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_123834_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_123834_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_123834_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_123834_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_123834_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_123834_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_123834",
    &TwistCandidate_123834,
    &TwistCandidate_123834_KeySeed,
    &TwistCandidate_123834_SaltSeed,
    &TwistCandidate_123834_MaskSeedA,
    &TwistCandidate_123834_MaskSeedB,
    &TwistCandidate_123834_TwistBlock,
    &TwistCandidate_123834_PushKeyRound,
    &TwistCandidate_123834_PushMaskRoundA,
    &TwistCandidate_123834_PushMaskRoundB,
  }
};
