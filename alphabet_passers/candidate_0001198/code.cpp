...
// Candidate 1199: TwistCandidate_1199
// family=ferocious_dual_worker op_budget=11 worker_shapes=2x1/3x0 worker_reverse=(True, False, False, True)/(False, False, True, False) twiddle1=mix/none twiddle2=xor/i twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m2i1v231,aBreakerCD:4m1i0v120,aBreakerEF:4m2i1v120 lane_breaker=true braid_breaker=true jump_breaker=true swap_breaker=true mask_template=1 mask_seed_a=sbox_wave mask_seed_b=off lightning=true/2 typhoon=false hurricane=true final_reverse=(True, True, True, True)
// ferocious[wa=2x1; wb=3x0; tw1=mix/none; tw2=xor/i; tw2src=B; saltbox=128:00100387:595f3bdb; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m2i1v231,aBreakerCD:4m1i0v120,aBreakerEF:4m2i1v120; wa_rev=(True, False, False, True); wb_rev=(False, False, True, False); lane=on:2; braid=on:2; jump=on:3; swap=on:1; mask=workerax1; lightning=on:2; typhoon=off; hurricane=on; final=5/(True, True, True, True); key_rot=21; maskA_seed=sbox_wave/False; maskB_seed=off; maskA_bias=219; maskB_bias=192]
static std::uint32_t TwistCandidate_1199_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0xE16B57CBu;
  aState = AdvanceTwiddle32(
      ...
      0x9C1CC00Bu,
      8u);
  aState ^= RotateLeft32(pValue + pExtra + 0x1B762C98u, 9U);
  return static_cast<std::uint32_t>(aState + 0x7D7797C0u);
}

static std::uint32_t TwistCandidate_1199_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x5FB96613u;
  aState = AdvanceTwiddle32(
      ...
      0x44CF4A8Bu,
      8u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x263AA256u, 9U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5A618u);
}

static void TwistCandidate_1199_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_1199_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_1199_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_1199_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_1199_AdvanceTwiddle32B(
...
static void TwistCandidate_1199_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x75U, 0xBCU, 0x50U, 0xD5U, 0xEFU, 0xB7U, 0x41U, 0xA6U, 0xAFU, 0xA2U, 0xA0U, 0xEAU, 0x3BU, 0xD8U, 0xC1U, 0xFCU,
      0x52U, 0x4CU, 0x5CU, 0x99U, 0xAAU, 0xF9U, 0xDDU, 0xDAU, 0x93U, 0x86U, 0xCAU, 0x57U, 0x2EU, 0xB4U, 0x1EU, 0x0BU,
      0x73U, 0xD0U, 0x14U, 0x39U, 0x3FU, 0x79U, 0x89U, 0xC8U, 0xE5U, 0x1CU, 0xB8U, 0x43U, 0x4AU, 0x29U, 0x3DU, 0x8EU,
      0xC3U, 0xBDU, 0x3EU, 0xE0U, 0xABU, 0xA4U, 0x6DU, 0x2FU, 0x7CU, 0x9FU, 0xE3U, 0xECU, 0x4FU, 0x98U, 0x81U, 0xBAU,
      0xFFU, 0x04U, 0xF7U, 0x8DU, 0x59U, 0xE4U, 0x95U, 0x94U, 0x7EU, 0x56U, 0xE8U, 0x05U, 0xE6U, 0xFEU, 0x2AU, 0xADU,
      0xC6U, 0x53U, 0xF0U, 0x63U, 0x25U, 0x00U, 0x32U, 0x4EU, 0x49U, 0x6CU, 0x0FU, 0x87U, 0x51U, 0x82U, 0x10U, 0xEDU,
      0x55U, 0x54U, 0x3AU, 0x65U, 0x12U, 0x38U, 0xDFU, 0x1DU, 0x5DU, 0x26U, 0x2BU, 0x91U, 0xA5U, 0xB3U, 0xC0U, 0xDEU,
      0xA3U, 0x33U, 0xF5U, 0x2DU, 0xD2U, 0x9CU, 0xA9U, 0x3CU, 0x9DU, 0xB2U, 0xC4U, 0x02U, 0xEEU, 0x8BU, 0x47U, 0x9BU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 238u ^ 224u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 11u + (5885)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-7552)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 11u + static_cast<unsigned int>(238u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 238u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(224u), 3u, false);
  ...
  TwistCandidate_1199_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(224u), 3u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(224u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
...
static void TwistCandidate_1199_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 3u + (-3908)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 2u + (1781)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 238u) ^ ((b << 3u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_1199_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 100u ^ 219u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 5u + (3450)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-2478)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (5u + 31u) + (-7285)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 20u + (3411)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>(aMixBox[(aWorkerByte ^ a ^ static_cast<std::uint32_t>(197u)) & 0xFFu]);
    ...
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(197u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 5u * 8U + static_cast<std::size_t>(aSourceIndex) * 31u + 100u + static_cast<std::size_t>(aTurbulence * ((20u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(100u), 7u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 197u) * ((20u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 5u, true);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 25u + (3450)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 23u + (-2478)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (25u + 23u) + (-7285)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(197u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 25u * 8U + static_cast<std::size_t>(aReverseIndex) * 23u * 3U + 219u + static_cast<std::size_t>(aTurbulence * ((20u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 5u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(219u), 5u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 197u) * ((20u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 5u, true);
  }
}

static void TwistCandidate_1199_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 177u ^ 192u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 2u + (-5970)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 15u + (4862)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (2u + 15u) + (-3304)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 2u * 8U + static_cast<std::size_t>(aSourceIndex) * 15u + 177u + static_cast<std::size_t>(aTurbulence * ((5u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(177u), 4u, true);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 25u + (-5970)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 49u + (4862)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (25u + 49u) + (-3304)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const std::uint32_t aWorkerByte = 0U;
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 25u * 8U + static_cast<std::size_t>(aSourceIndex) * 49u * 3U + 1u + static_cast<std::size_t>(aTurbulence * ((5u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 2u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(1u), 2u, true);
  }
}

static void TwistCandidate_1199_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xD1U, 0x70U, 0xBFU, 0x67U, 0x05U, 0x51U, 0xCEU, 0x4CU, 0xE4U, 0x0DU, 0x71U, 0x27U, 0x20U, 0x90U, 0xEFU, 0x50U,
      0x43U, 0x63U, 0x47U, 0xCFU, 0xE8U, 0x0AU, 0x38U, 0xD3U, 0x8CU, 0xD4U, 0xAEU, 0x7BU, 0x17U, 0x5FU, 0x73U, 0x91U,
      0x08U, 0x3FU, 0x7AU, 0xF6U, 0xDCU, 0x92U, 0x23U, 0x03U, 0xA5U, 0x16U, 0xF3U, 0xCBU, 0xDFU, 0x81U, 0x22U, 0xEEU,
      0xA1U, 0x72U, 0x36U, 0xC9U, 0x64U, 0x37U, 0x9DU, 0x4BU, 0x15U, 0x40U, 0x31U, 0xF1U, 0xE9U, 0xEAU, 0x1EU, 0x3EU,
      0xD5U, 0xC8U, 0xEBU, 0xB9U, 0x19U, 0xCCU, 0x09U, 0xD6U, 0x21U, 0x56U, 0xE5U, 0xBDU, 0x32U, 0xBBU, 0x54U, 0xF4U,
      0x58U, 0x6AU, 0xE7U, 0xA4U, 0x46U, 0xA7U, 0xACU, 0x0FU, 0xCAU, 0x9FU, 0x41U, 0xADU, 0xB2U, 0x0CU, 0x7EU, 0x1DU,
      0x9BU, 0xECU, 0x5EU, 0xC1U, 0xB0U, 0x84U, 0x3CU, 0x3BU, 0xB8U, 0xE6U, 0xC4U, 0xDEU, 0x2FU, 0xC0U, 0xDDU, 0xDBU,
      0x89U, 0x18U, 0xD9U, 0xABU, 0x26U, 0xD2U, 0x0BU, 0x88U, 0x78U, 0x93U, 0x8AU, 0x96U, 0x1FU, 0x24U, 0x66U, 0x04U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x1869E834u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0xDE3279A2u, 9u);
  std::uint32_t aAccB = AdvanceTwiddle32(0xB664927Bu ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x8C1A9607u, 3u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4169), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (2155), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (2895), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-959), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0xDE3279A2u, 9u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x8C1A9607u, 3u);
...
static void TwistCandidate_1199_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xFBU, 0x40U, 0x41U, 0x08U, 0x9AU, 0x42U, 0xF6U, 0x87U, 0x94U, 0xF8U, 0xABU, 0x3CU, 0xFDU, 0x34U, 0x78U, 0x6EU,
      0x53U, 0xF5U, 0x5CU, 0x91U, 0x89U, 0xBDU, 0x47U, 0x1EU, 0x46U, 0x09U, 0x80U, 0x0AU, 0x5AU, 0xA2U, 0x29U, 0x21U,
      0xD4U, 0x7EU, 0x33U, 0x4AU, 0x0EU, 0xFEU, 0xE0U, 0xBBU, 0xFCU, 0xBAU, 0x00U, 0xEEU, 0x88U, 0xEBU, 0x0FU, 0x49U,
      0x16U, 0x11U, 0xB9U, 0xA9U, 0x83U, 0xB8U, 0x95U, 0xDCU, 0xEFU, 0x1DU, 0xF4U, 0x70U, 0x1AU, 0xE1U, 0x3AU, 0xE4U,
      0x86U, 0xDFU, 0xB1U, 0xC3U, 0x6AU, 0x9CU, 0x2FU, 0xCFU, 0x60U, 0x24U, 0xBFU, 0x8AU, 0xB7U, 0xC6U, 0x3BU, 0xF3U,
      0xE2U, 0xC2U, 0x75U, 0xDAU, 0xC1U, 0xF0U, 0xE5U, 0x63U, 0x6DU, 0xDEU, 0x97U, 0xA8U, 0xE9U, 0x56U, 0x01U, 0x93U,
      0x73U, 0xBCU, 0x27U, 0x67U, 0xB5U, 0xA4U, 0xA7U, 0x5DU, 0x2BU, 0xD2U, 0x22U, 0xFFU, 0x9BU, 0x90U, 0x69U, 0xDBU,
      0x61U, 0x52U, 0x12U, 0xC7U, 0xA5U, 0x72U, 0x18U, 0xB0U, 0x77U, 0x8DU, 0x7CU, 0x54U, 0xAEU, 0x2CU, 0x48U, 0x7FU
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x82C29F5Eu, 0x1AF0797Fu, 3u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (1041), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x1AF0797Fu, 3u);
...
static void TwistCandidate_1199_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_1199_AdvanceTwiddle32A(
      0xA692281Bu ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_1199_AdvanceTwiddle32B(
      0xF3583581u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_1199_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_1199_AdvanceTwiddle32A(
      0x893C8CE1u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[1U]),
      0x4B8E94E2u ^ static_cast<std::uint32_t>(pSalt[31U]));
  std::uint32_t aPulseB = TwistCandidate_1199_AdvanceTwiddle32B(
      0xF1373CBFu + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[31U]),
      0xD6E3EFB3u ^ static_cast<std::uint32_t>(pSalt[1U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_1199_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_1199_AdvanceTwiddle32B(
...
static void TwistCandidate_1199_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x33B33294u ^ (pRound * 62u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-3908), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x33B33294u ^ (pRound * 62u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (1781), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(aReverseIndex + (-7487), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (-3520), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 239u + 16u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-3520 + 16), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 11u + 3U) & 15U), static_cast<std::size_t>(i + 11u)));
      ...
      const int aIndex2 = WrapRange(i + (-1010), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA ^ static_cast<std::uint32_t>(pSalt[10u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(198u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 9u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (803), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-5777), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 172u + 2u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-5777 + 2), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 14u + 3U) & 15U), static_cast<std::size_t>(i + 1u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 14u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>((aLaneIndex ^ static_cast<std::uint32_t>(28u)) & 0xFFu);
      const int aIndex2 = WrapRange(i + (-6094), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(aReverseIndex + (-6508), aStart, aEnd);
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex3]);
      const std::uint32_t aRoute = ((aSourceCtrl >> 7u) ^ aWorkerCtrl ^ aKeyByte ^ aMaskByte ^ static_cast<std::uint32_t>(0u)) % 5U;
      std::uint32_t aValue = a;
      switch (aRoute) {
        case 0U: aValue = a; break;
        case 1U: aValue = b; break;
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(250u)), 0x85EBCA6Bu, 11u);
    }
  }

  TwistCandidate_1199_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_1199_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_1199_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_1199_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (3202), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (6705), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-73), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 16u + 563u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatOther = WrapRange(static_cast<int>(aMaskFlat) + (7 + 1), 0, static_cast<int>(kMaskStackTotalBytes));
      const std::uint32_t aMaskByteA = static_cast<std::uint32_t>(pMaskStackA[(aMaskFlat / kMaskBytes) % kMaskStackDepth][aMaskFlat % kMaskBytes]);
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    auto storm_block = LoadBlock16Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (253), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock16Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (6739), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackA, chunk + 972u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackB, chunk + 972u + 17u);
    ...
    switch (static_cast<unsigned>(storm_control[7] ^ storm_mask_a[8] ^ storm_mask_b[9] ^ pSalt[1]) % 4u) {
      ...
      case 2u: storm.ApplySlowOp(LightningSlowOp::kTranspose, static_cast<std::uint8_t>(storm_control[9] + storm_mask_a[10] + storm_mask_b[11]), static_cast<std::uint8_t>(storm_block[11] ^ storm_mask_a[12] ^ storm_mask_b[13])); break;
    ...
    ApplyLightningMixColumns(storm, pSalt, static_cast<std::uint8_t>((storm_control[11] + pRound + 22u) & 31U), static_cast<std::uint8_t>(storm_control[15] ^ storm_mask_a[0] ^ storm_mask_b[1] ^ pSalt[3]), static_cast<std::uint8_t>(2u));
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (7614), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (3446), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 1319u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 1319u + 29u);
    ...
    switch (static_cast<unsigned>(storm_control[13] ^ storm_mask_a[17] ^ storm_mask_b[19] ^ pSalt[2]) % 5u) {
      ...
      case 4u: storm.ApplyFastOp(HurricaneFastOp::kSwapColumns, static_cast<std::uint8_t>(storm_control[29] ^ storm_mask_a[31] ^ storm_mask_b[37]), static_cast<std::uint8_t>(storm_control[43] + storm_mask_a[47] + storm_mask_b[53])); break;
    }
    }
  ...
  TwistCandidate_1199_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (3032), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (4493), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (-6644), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (6773), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_1199_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_1199_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 13u + (7156)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 28u + (3333)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 5u + (-6819)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 13u + static_cast<unsigned int>(21u)) & 31U;
...
static void TwistCandidate_1199_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 25u + (4085)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 25u + (-671)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 23u + (5745)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(219u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(219u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_1199_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 1u + (-318)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 30u + (7195)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 10u + (6405)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(192u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(192u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_1199(
  ...
  TwistCandidate_1199_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_1199_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_1199_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_1199_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_1199_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_1199_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_1199_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_1199_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_1199",
    &TwistCandidate_1199,
    &TwistCandidate_1199_KeySeed,
    &TwistCandidate_1199_SaltSeed,
    &TwistCandidate_1199_MaskSeedA,
    &TwistCandidate_1199_MaskSeedB,
    &TwistCandidate_1199_TwistBlock,
    &TwistCandidate_1199_PushKeyRound,
    &TwistCandidate_1199_PushMaskRoundA,
    &TwistCandidate_1199_PushMaskRoundB,
  }
};
