...
// Candidate 120870: TwistCandidate_120870
// family=ferocious_dual_worker op_budget=10 worker_shapes=2x3/3x3 worker_reverse=(True, True, False, False)/(False, True, False, False) twiddle1=xor/none twiddle2=add/none twiddle2_source=A tsunami=custom breaker_shapes=aBreakerAB:4m2i2v023,aBreakerCD:4m1i1v001,aBreakerEF:4m1i1v120 lane_breaker=true braid_breaker=true jump_breaker=false swap_breaker=true mask_template=2 mask_seed_a=add_wave mask_seed_b=off lightning=false/0 typhoon=true hurricane=true final_reverse=(True, True, False, False)
// ferocious[wa=2x3; wb=3x3; tw1=xor/none; tw2=add/none; tw2src=A; saltbox=128:000eb209:4e3625e6; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m2i2v023,aBreakerCD:4m1i1v001,aBreakerEF:4m1i1v120; wa_rev=(True, True, False, False); wb_rev=(False, True, False, False); lane=on:3; braid=on:3; jump=off:2; swap=on:2; mask=workerax2; lightning=off:0; typhoon=on; hurricane=on; final=5/(True, True, False, False); key_rot=28; maskA_seed=add_wave/True; maskB_seed=off; maskA_bias=225; maskB_bias=199]
static std::uint32_t TwistCandidate_120870_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0xD8C4F163u;
  aState = AdvanceTwiddle32(
      ...
      0x295389D6u,
      5u);
  aState ^= RotateLeft32(pValue + pExtra + 0x97948326u, 10U);
  return static_cast<std::uint32_t>(aState + 0xF19778B5u);
}

static std::uint32_t TwistCandidate_120870_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xEAF6BFE8u;
  aState = AdvanceTwiddle32(
      ...
      0x7D623CCEu,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x56272E31u, 6U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5363Eu);
}

static void TwistCandidate_120870_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_120870_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_120870_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_120870_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_120870_AdvanceTwiddle32B(
...
static void TwistCandidate_120870_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xB9U, 0xB1U, 0x54U, 0x60U, 0x6CU, 0x70U, 0x3FU, 0x45U, 0xE3U, 0x2BU, 0xADU, 0x63U, 0xA9U, 0xECU, 0x96U, 0x78U,
      0x7FU, 0x94U, 0x58U, 0xCBU, 0xC4U, 0xFFU, 0x41U, 0xCAU, 0x53U, 0x56U, 0x8CU, 0x4BU, 0x48U, 0x59U, 0xC3U, 0x1BU,
      0x49U, 0xD0U, 0xBCU, 0x73U, 0x13U, 0x8FU, 0x61U, 0x20U, 0xB7U, 0x9FU, 0xDEU, 0x4FU, 0x9AU, 0x12U, 0xF1U, 0xD1U,
      0xEBU, 0xBFU, 0x0FU, 0xF9U, 0x18U, 0x62U, 0xDAU, 0xB2U, 0xC0U, 0x72U, 0xD7U, 0xF7U, 0x40U, 0x35U, 0xCDU, 0xBBU,
      0xCFU, 0x95U, 0x33U, 0x0CU, 0x80U, 0xFCU, 0x27U, 0xD5U, 0xF8U, 0x38U, 0x14U, 0xD8U, 0x04U, 0x2DU, 0x86U, 0x24U,
      0x82U, 0xB8U, 0xC1U, 0x8EU, 0xC2U, 0xA0U, 0xDDU, 0x21U, 0x5DU, 0xDFU, 0x77U, 0x69U, 0x29U, 0x26U, 0xA8U, 0x25U,
      0x7BU, 0xF0U, 0xAFU, 0x0AU, 0x36U, 0x6EU, 0xC8U, 0x75U, 0xB6U, 0x2CU, 0x11U, 0x1DU, 0xB5U, 0x42U, 0x09U, 0x5FU,
      0x1CU, 0xD9U, 0x71U, 0x55U, 0x6FU, 0x08U, 0xE2U, 0x22U, 0x5AU, 0x4CU, 0x01U, 0xE6U, 0x19U, 0xAEU, 0x7CU, 0x06U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 252u ^ 189u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-3663)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 8u + (581)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 29u + static_cast<unsigned int>(252u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 252u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(189u), 7u, false);
  ...
  TwistCandidate_120870_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(189u), 7u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(189u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
...
static void TwistCandidate_120870_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-3881)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 14u + (-4298)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 252u) ^ ((b << 7u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_120870_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 248u ^ 225u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 14u + (6317)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 3u + (4935)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (14u + 3u) + (5777)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 42u + (3037)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte + b + static_cast<std::uint32_t>(207u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 14u * 8U + static_cast<std::size_t>(aReverseIndex) * 3u + 248u + static_cast<std::size_t>(aTurbulence * ((42u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(248u), 7u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 207u) * ((42u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
  ...
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 48u + (6317)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 21u + (4935)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (48u + 21u) + (5777)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aTurbulence & 0xFFu), 4u))) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c + static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 4u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 48u * 8U + static_cast<std::size_t>(aReverseIndex) * 21u * 3U + 9u + static_cast<std::size_t>(aTurbulence * ((42u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 6u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(9u), 6u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 207u) * ((42u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 6u, true);
  }
}

static void TwistCandidate_120870_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 231u ^ 199u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 11u + (-4553)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 15u + (6347)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (11u + 15u) + (-449)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 11u * 8U + static_cast<std::size_t>(aReverseIndex) * 15u + 231u + static_cast<std::size_t>(aTurbulence * ((6u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(231u), 2u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 18u + (-4553)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 54u + (6347)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (18u + 54u) + (-449)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const std::uint32_t aWorkerByte = 0U;
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 18u * 8U + static_cast<std::size_t>(aReverseIndex) * 54u * 3U + 198u + static_cast<std::size_t>(aTurbulence * ((6u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 1u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(198u), 1u, false);
  }
}

static void TwistCandidate_120870_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x8EU, 0xB1U, 0x0AU, 0x61U, 0x3DU, 0x64U, 0x76U, 0xDBU, 0x0DU, 0x87U, 0x13U, 0x08U, 0xABU, 0xBDU, 0x2DU, 0x3CU,
      0x1AU, 0x38U, 0xE1U, 0xCEU, 0x6AU, 0x63U, 0x4DU, 0x8FU, 0x93U, 0x7CU, 0x20U, 0xF9U, 0x8BU, 0x15U, 0x70U, 0xCBU,
      0x7AU, 0x0CU, 0x9CU, 0xC5U, 0xA3U, 0x89U, 0x35U, 0x7DU, 0xDAU, 0x11U, 0xE3U, 0x4FU, 0x33U, 0x92U, 0xB0U, 0x81U,
      0x8AU, 0x68U, 0x41U, 0x65U, 0x69U, 0x09U, 0x46U, 0xD3U, 0xC3U, 0x6BU, 0xA0U, 0x37U, 0x59U, 0xFDU, 0x7BU, 0x7FU,
      0x23U, 0x3AU, 0x2FU, 0x58U, 0x32U, 0xFFU, 0xD8U, 0xD7U, 0x3BU, 0x2AU, 0xE5U, 0x55U, 0x80U, 0xE9U, 0x2EU, 0x1EU,
      0x88U, 0xCAU, 0xACU, 0x4CU, 0x02U, 0xB5U, 0x34U, 0x1CU, 0x07U, 0xF7U, 0x50U, 0x82U, 0x97U, 0x43U, 0xFCU, 0xB3U,
      0x71U, 0xBBU, 0x3EU, 0x0BU, 0xDFU, 0xECU, 0x8CU, 0xEDU, 0xAEU, 0x52U, 0xFBU, 0x22U, 0x16U, 0x53U, 0xADU, 0xC9U,
      0x5DU, 0xEEU, 0xEBU, 0xDDU, 0xBAU, 0x84U, 0xB7U, 0xA6U, 0x30U, 0xD0U, 0x3FU, 0x21U, 0x18U, 0x86U, 0x78U, 0x44U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x0FAB6819u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0xB00ED58Eu, 6u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x0FE1A229u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0xE3D9F3BBu, 4u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (5016), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-5101), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-301), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (5966), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltA = static_cast<std::uint32_t>(pSalt[(i + 8u) & 31U]);
    const std::uint32_t aSaltB = static_cast<std::uint32_t>(pSalt[(i + 23u) & 31U]);
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0xB00ED58Eu, 6u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0xE3D9F3BBu, 4u);
...
static void TwistCandidate_120870_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xCBU, 0x0CU, 0xBDU, 0xB3U, 0x32U, 0xBFU, 0xB4U, 0xFAU, 0x44U, 0x0FU, 0x20U, 0x18U, 0x49U, 0xD7U, 0xC0U, 0xB5U,
      0xBEU, 0x96U, 0xF6U, 0x2AU, 0x27U, 0x93U, 0xDEU, 0x50U, 0x1BU, 0x97U, 0x91U, 0xF2U, 0xFBU, 0xE1U, 0x98U, 0x21U,
      0xCFU, 0x2DU, 0x65U, 0x56U, 0x0DU, 0xE0U, 0x04U, 0xAAU, 0x0AU, 0x5EU, 0x2CU, 0x90U, 0x12U, 0x62U, 0xB2U, 0x4BU,
      0x8CU, 0x51U, 0x9BU, 0xEAU, 0xC7U, 0xE8U, 0x24U, 0x53U, 0xABU, 0x23U, 0xDFU, 0x5AU, 0xEFU, 0x36U, 0x7CU, 0xAFU,
      0xD3U, 0xFCU, 0x11U, 0x73U, 0x2EU, 0xC2U, 0xACU, 0x41U, 0xA5U, 0x8EU, 0xA9U, 0xAEU, 0x29U, 0xDCU, 0x84U, 0x85U,
      0x55U, 0xF0U, 0xB9U, 0xF8U, 0xECU, 0x07U, 0xB8U, 0x9EU, 0x02U, 0x99U, 0x08U, 0xD4U, 0x67U, 0x64U, 0x40U, 0x01U,
      0x71U, 0x9FU, 0x79U, 0xF9U, 0xE5U, 0x5BU, 0xF4U, 0x6DU, 0xA4U, 0x80U, 0xDAU, 0x8FU, 0xBAU, 0x9DU, 0x2BU, 0x5FU,
      0x1DU, 0x26U, 0xE7U, 0xD5U, 0x72U, 0x6AU, 0x81U, 0x66U, 0x87U, 0x61U, 0xDDU, 0xFDU, 0x83U, 0x3DU, 0x9AU, 0xD8U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x3AB87F80u, 0x2494F008u, 1u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (6617), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x2494F008u, 1u);
...
static void TwistCandidate_120870_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_120870_AdvanceTwiddle32A(
      0x67E8181Au ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_120870_AdvanceTwiddle32B(
      0x56346D46u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_120870_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_120870_AdvanceTwiddle32A(
      0x1E9A6EE7u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[7U]),
      0xDC28C6E1u ^ static_cast<std::uint32_t>(pSalt[9U]));
  std::uint32_t aPulseB = TwistCandidate_120870_AdvanceTwiddle32B(
      0x4A0F55C9u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[9U]),
      0x6DDBF70Au ^ static_cast<std::uint32_t>(pSalt[7U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    for (std::size_t aLane = 0; aLane < kMatrixBlockBytes; ++aLane) {
      ...
      const unsigned char aMix = FixedMixBoxByte(static_cast<unsigned char>(
          ...
          pSalt[(aLane + 9U + (aPulseByteA & 3U)) & 31U]));
    ...
    aPulseA = TwistCandidate_120870_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_120870_AdvanceTwiddle32B(
...
static void TwistCandidate_120870_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x9D3E8091u ^ (pRound * 30u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-3881), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x9D3E8091u ^ (pRound * 30u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-4298), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(aReverseIndex + (-4146), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-3897), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 23u + 22u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-3897 + 22), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 7u + 3U) & 15U), static_cast<std::size_t>(i + 8u)));
      ...
      const int aIndex2 = WrapRange(aReverseIndex + (1727), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(111u)), 0x85EBCA6Bu, 6u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (2581), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (4296), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 109u + 25u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (4296 + 25), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 0u + 3U) & 15U), static_cast<std::size_t>(i + 24u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 22u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = 0U;
      const int aIndex2 = WrapRange(aReverseIndex + (2139), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(i + (3970), aStart, aEnd);
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex3]);
      const std::uint32_t aRoute = ((aSourceCtrl >> 7u) ^ aWorkerCtrl ^ aKeyByte ^ aMaskByte ^ static_cast<std::uint32_t>(3u)) % 5U;
      std::uint32_t aValue = a;
      switch (aRoute) {
        case 0U: aValue = a; break;
        case 1U: aValue = b; break;
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(18u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[22u]), 0x27D4EB2Du, 11u);
    }
  }

  TwistCandidate_120870_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_120870_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_120870_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    const DualWorkerBreakerRecipe4 aBreakerEF{
        ...
        2547,
        -5313,
        static_cast<std::size_t>(1077u),
        static_cast<std::uint8_t>(12u),
        static_cast<std::uint8_t>(29u),
        static_cast<std::uint8_t>(5u),
        static_cast<std::uint8_t>(10u),
        static_cast<std::uint8_t>(5u),
        static_cast<std::uint8_t>(108u),
        static_cast<std::uint8_t>(118u),
        static_cast<std::uint8_t>(8u),
        static_cast<std::uint8_t>(1u),
        static_cast<std::uint8_t>(1u),
        static_cast<std::uint8_t>(2u),
    ...
    TwistCandidate_120870_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-3313), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (100), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (3151), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 3u + 1189u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kTyphoonBlockBytes) {
    auto storm_block = LoadBlock128Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (5537), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock128Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-7229), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackA, chunk + 1341u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackB, chunk + 1341u + 23u);
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-4578), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (2135), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 713u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 713u + 29u);
  ...
  TwistCandidate_120870_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (-5915), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (2547), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-7079), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (-5796), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_120870_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_120870_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 28u + (2403)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 15u + (-499)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 31u + (5520)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 28u + static_cast<unsigned int>(28u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 189u)));
...
static void TwistCandidate_120870_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 20u + (-6123)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-214)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 16u + (-905)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(225u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(225u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_120870_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 14u + (-756)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 1u + (-2946)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-1346)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(199u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(199u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_120870(
  ...
  TwistCandidate_120870_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_120870_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_120870_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_120870_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_120870_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_120870_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_120870_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_120870_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_120870",
    &TwistCandidate_120870,
    &TwistCandidate_120870_KeySeed,
    &TwistCandidate_120870_SaltSeed,
    &TwistCandidate_120870_MaskSeedA,
    &TwistCandidate_120870_MaskSeedB,
    &TwistCandidate_120870_TwistBlock,
    &TwistCandidate_120870_PushKeyRound,
    &TwistCandidate_120870_PushMaskRoundA,
    &TwistCandidate_120870_PushMaskRoundB,
  }
};
