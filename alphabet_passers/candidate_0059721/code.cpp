...
// Candidate 59722: TwistCandidate_59722
// family=ferocious_dual_worker op_budget=7 worker_shapes=1x1/3x1 worker_reverse=(False, False, True, False)/(False, True, False, False) twiddle1=add/none twiddle2=xor/none twiddle2_source=A tsunami=custom breaker_shapes=aBreakerAB:4m3i2v112,aBreakerCD:4m3i1v200 lane_breaker=true braid_breaker=false jump_breaker=true swap_breaker=false mask_template=3 mask_seed_a=xor_wave mask_seed_b=off lightning=false/0 typhoon=false hurricane=false final_reverse=(False, True, True, False)
// ferocious[wa=1x1; wb=3x1; tw1=add/none; tw2=xor/none; tw2src=A; saltbox=128:001163e7:fc95368a; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m3i2v112,aBreakerCD:4m3i1v200; wa_rev=(False, False, True, False); wb_rev=(False, True, False, False); lane=on:3; braid=off:2; jump=on:0; swap=off:1; mask=workerbx3; lightning=off:0; typhoon=off; hurricane=off; final=3/(False, True, True, False); key_rot=12; maskA_seed=xor_wave/True; maskB_seed=off; maskA_bias=54; maskB_bias=199]
static std::uint32_t TwistCandidate_59722_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x60E6C989u;
  aState = AdvanceTwiddle32(
      ...
      0x453FAAF5u,
      8u);
  aState ^= RotateLeft32(pValue + pExtra + 0x43D990CBu, 10U);
  return static_cast<std::uint32_t>(aState + 0x25D9637Cu);
}

static std::uint32_t TwistCandidate_59722_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x869A5C8Du;
  aState = AdvanceTwiddle32(
      ...
      0xC543CC46u,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x262A95CFu, 9U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5F678u);
}

static void TwistCandidate_59722_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_59722_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_59722_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_59722_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_59722_AdvanceTwiddle32B(
...
static void TwistCandidate_59722_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x59U, 0x6BU, 0x16U, 0x83U, 0x02U, 0xC5U, 0x88U, 0x35U, 0x47U, 0xE1U, 0x57U, 0x12U, 0x32U, 0xE2U, 0xBFU, 0x2FU,
      0xE8U, 0xA3U, 0x28U, 0x95U, 0x76U, 0xE3U, 0x4DU, 0x62U, 0x6EU, 0x50U, 0x17U, 0x4AU, 0x0BU, 0x45U, 0x39U, 0x27U,
      0x8DU, 0x1EU, 0x7DU, 0x54U, 0x99U, 0xC1U, 0x5AU, 0x80U, 0xB0U, 0x51U, 0x3CU, 0xA2U, 0x75U, 0xC2U, 0x8BU, 0xE4U,
      0x81U, 0x91U, 0x44U, 0xA9U, 0xD5U, 0x1BU, 0x0EU, 0x33U, 0x43U, 0xBAU, 0x6DU, 0xC7U, 0x60U, 0x1CU, 0x2EU, 0x8CU,
      0xDDU, 0x7FU, 0x67U, 0xABU, 0xA6U, 0xFEU, 0x6CU, 0x52U, 0xCCU, 0x29U, 0xD4U, 0x2BU, 0xF5U, 0xE7U, 0x3BU, 0x0AU,
      0x98U, 0xA7U, 0x8FU, 0x34U, 0xCBU, 0xDBU, 0x96U, 0xCFU, 0x09U, 0x9DU, 0x78U, 0xF8U, 0xD0U, 0x84U, 0xEEU, 0xFCU,
      0xECU, 0xF3U, 0x4CU, 0xF2U, 0x37U, 0xF9U, 0xEAU, 0xF0U, 0x19U, 0x30U, 0x2DU, 0xD3U, 0xBBU, 0xF7U, 0xC4U, 0x55U,
      0x71U, 0xF6U, 0xD2U, 0x9CU, 0x5EU, 0x5CU, 0x3FU, 0x1DU, 0x4EU, 0xB7U, 0x73U, 0x07U, 0xBEU, 0x08U, 0xCEU, 0xAAU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 39u ^ 232u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 12u + (-3312)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-2529)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 12u + static_cast<unsigned int>(39u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 39u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(232u), 3u, true);
  ...
  TwistCandidate_59722_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(232u), 3u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(232u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 44u + (-7745)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 36u + (1904)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 44u + pSalt[(aSourceIndex + 164u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(164u), 7u, true);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_59722_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(164u), static_cast<std::uint32_t>(164u), 7u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
...
static void TwistCandidate_59722_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 9u + (-2849)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 13u + (4613)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 39u) ^ ((b << 3u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_59722_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 95u ^ 54u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 8u + (-5944)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 4u + (-7099)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (8u + 4u) + (3730)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 24u + (2604)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 3u)) ^ static_cast<std::uint32_t>(249u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 8u * 8U + static_cast<std::size_t>(aSourceIndex) * 4u + 95u + static_cast<std::size_t>(aTurbulence * ((24u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(95u), 7u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 249u) * ((24u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 3u, true);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 42u + (-5944)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 24u + (-7099)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (42u + 24u) + (3730)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 3u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 42u * 8U + static_cast<std::size_t>(aReverseIndex) * 24u * 3U + 144u + static_cast<std::size_t>(aTurbulence * ((24u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 4u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(144u), 4u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 249u) * ((24u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 4u, true);
  }
}

static void TwistCandidate_59722_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 95u ^ 199u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 11u + (143)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 29u + (1753)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (11u + 29u) + (2359)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 11u * 8U + static_cast<std::size_t>(aSourceIndex) * 29u + 95u + static_cast<std::size_t>(aTurbulence * ((16u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(95u), 4u, false);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 47u + (143)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 41u + (1753)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (47u + 41u) + (2359)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const std::uint32_t aWorkerByte = 0U;
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 47u * 8U + static_cast<std::size_t>(aSourceIndex) * 41u * 3U + 195u + static_cast<std::size_t>(aTurbulence * ((16u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 2u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(195u), 2u, false);
  }
}

static void TwistCandidate_59722_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xB6U, 0x39U, 0x50U, 0xA7U, 0x43U, 0x71U, 0x31U, 0x75U, 0x07U, 0xDFU, 0x94U, 0xCAU, 0xB2U, 0xA9U, 0xCBU, 0x57U,
      0x2CU, 0x86U, 0x21U, 0x13U, 0x10U, 0x4AU, 0x91U, 0xD6U, 0x4EU, 0xD0U, 0x24U, 0x3FU, 0xD7U, 0x34U, 0x80U, 0x7AU,
      0xECU, 0x55U, 0xD5U, 0x23U, 0xBBU, 0xB4U, 0x77U, 0x58U, 0x37U, 0x6FU, 0x78U, 0x5FU, 0x01U, 0x44U, 0x68U, 0xA2U,
      0x03U, 0x82U, 0x46U, 0xDDU, 0x9CU, 0x99U, 0x1CU, 0x02U, 0x6AU, 0xB0U, 0x4CU, 0xACU, 0xD1U, 0x45U, 0xF6U, 0x53U,
      0xF1U, 0xABU, 0xBEU, 0x52U, 0x1FU, 0x41U, 0xB7U, 0x04U, 0xFBU, 0x0BU, 0x67U, 0x8AU, 0x47U, 0x7DU, 0xE8U, 0x32U,
      0x20U, 0xD8U, 0xFEU, 0x00U, 0x1BU, 0x9FU, 0xE9U, 0xD4U, 0xCCU, 0x0EU, 0x49U, 0x26U, 0x63U, 0xC7U, 0x7EU, 0x2DU,
      0xA3U, 0xC9U, 0x0AU, 0x9BU, 0x93U, 0xE4U, 0x22U, 0x95U, 0xF0U, 0x2BU, 0x8BU, 0x90U, 0xB1U, 0x54U, 0x89U, 0x42U,
      0xF7U, 0x98U, 0xF2U, 0x3AU, 0x5AU, 0x1EU, 0x60U, 0x11U, 0x83U, 0x28U, 0x70U, 0xAFU, 0xF3U, 0x36U, 0x35U, 0x25U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x6EE44C4Fu ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x0F94BA34u, 9u);
  std::uint32_t aAccB = AdvanceTwiddle32(0xB8245760u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x077A3907u, 7u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-6844), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (2876), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-6367), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-5222), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltB = static_cast<std::uint32_t>(pSalt[(i + 13u) & 31U]);
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x0F94BA34u, 9u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x077A3907u, 7u);
...
static void TwistCandidate_59722_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x8BU, 0x3FU, 0x9DU, 0x88U, 0x34U, 0x2DU, 0x95U, 0xC8U, 0x7DU, 0x39U, 0x4DU, 0x3BU, 0xB7U, 0x4AU, 0xD3U, 0xD1U,
      0xF3U, 0xF1U, 0x47U, 0x52U, 0xF5U, 0x30U, 0x11U, 0x99U, 0x57U, 0x5CU, 0x25U, 0x98U, 0x6EU, 0xCBU, 0xCDU, 0x40U,
      0xB6U, 0xB5U, 0xCCU, 0x69U, 0xFDU, 0xCFU, 0xD7U, 0x15U, 0x02U, 0xDAU, 0xC9U, 0x85U, 0xE9U, 0x54U, 0xE3U, 0x24U,
      0x0CU, 0x70U, 0x4CU, 0xE0U, 0x96U, 0x43U, 0xEEU, 0xFAU, 0x82U, 0xC2U, 0x23U, 0x2AU, 0xE2U, 0xECU, 0xBEU, 0xEDU,
      0x3EU, 0x20U, 0x01U, 0x90U, 0xCEU, 0x48U, 0x86U, 0x29U, 0xE6U, 0x56U, 0xC7U, 0xC5U, 0xEFU, 0xDDU, 0x83U, 0x0DU,
      0x76U, 0xD4U, 0x22U, 0x67U, 0xBFU, 0x06U, 0x9AU, 0x9CU, 0x4FU, 0x87U, 0xACU, 0xF9U, 0x13U, 0xF0U, 0xC3U, 0x2BU,
      0x53U, 0x50U, 0x33U, 0x84U, 0x36U, 0xE8U, 0xCAU, 0xE1U, 0xF8U, 0x64U, 0x65U, 0x79U, 0x5FU, 0xEBU, 0x9FU, 0xB2U,
      0x12U, 0x38U, 0x74U, 0x81U, 0xA8U, 0xF2U, 0xE5U, 0xAAU, 0xA5U, 0xAEU, 0x8FU, 0xFEU, 0xBCU, 0x4EU, 0xEAU, 0xFBU
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0xA2134603u, 0xD489E41Au, 7u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-1689), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xD489E41Au, 7u);
...
static void TwistCandidate_59722_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_59722_AdvanceTwiddle32A(
      0x9A93A116u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_59722_AdvanceTwiddle32B(
      0x9962DB4Au ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_59722_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_59722_AdvanceTwiddle32A(
      0x032999EDu ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[13U]),
      0xC19C7FD7u ^ static_cast<std::uint32_t>(pSalt[12U]));
  std::uint32_t aPulseB = TwistCandidate_59722_AdvanceTwiddle32B(
      0xD9FCCD4Cu + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[12U]),
      0xFE283E8Fu ^ static_cast<std::uint32_t>(pSalt[13U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    for (std::size_t aLane = 0; aLane < kMatrixBlockBytes; ++aLane) {
      ...
      const unsigned char aMix = FixedMixBoxByte(static_cast<unsigned char>(
          ...
          pSalt[(aLane + 12U + (aPulseByteA & 3U)) & 31U]));
    ...
    aPulseA = TwistCandidate_59722_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_59722_AdvanceTwiddle32B(
...
static void TwistCandidate_59722_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0xE5692247u ^ (pRound * 30u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-2849), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0xE5692247u ^ (pRound * 30u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (4613), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-5291), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-7135), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 75u + 24u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-7135 + 24), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(159u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[25u]), 0x27D4EB2Du, 8u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (-4611), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (2878), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 65u + 1u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (2878 + 1), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 8u + 3U) & 15U), static_cast<std::size_t>(i + 21u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 3u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = 0U;
      const int aIndex2 = WrapRange(aReverseIndex + (4216), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(i + (-3663), aStart, aEnd);
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex3]);
      const std::uint32_t aRoute = ((aSourceCtrl >> 6u) ^ aWorkerCtrl ^ aKeyByte ^ aMaskByte ^ static_cast<std::uint32_t>(1u)) % 5U;
      std::uint32_t aValue = a;
      switch (aRoute) {
        case 0U: aValue = a; break;
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(181u)), 0x85EBCA6Bu, 8u);
    }
  }

  TwistCandidate_59722_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_59722_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_59722_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-1551), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (5932), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (5016), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 11u + 332u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  TwistCandidate_59722_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (3405), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (-2129), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (4730), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (3430), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_59722_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_59722_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 30u + (-5559)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 3u + (-2287)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 8u + (-7179)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 30u + static_cast<unsigned int>(12u)) & 31U;
...
static void TwistCandidate_59722_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 16u + (5573)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 6u + (4042)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 23u + (-1628)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_59722_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 23u + (4602)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 28u + (2978)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 30u + (1915)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
void TwistCandidate_59722(
  ...
  TwistCandidate_59722_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_59722_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_59722_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_59722_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_59722_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_59722_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_59722_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_59722_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_59722",
    &TwistCandidate_59722,
    &TwistCandidate_59722_KeySeed,
    &TwistCandidate_59722_SaltSeed,
    &TwistCandidate_59722_MaskSeedA,
    &TwistCandidate_59722_MaskSeedB,
    &TwistCandidate_59722_TwistBlock,
    &TwistCandidate_59722_PushKeyRound,
    &TwistCandidate_59722_PushMaskRoundA,
    &TwistCandidate_59722_PushMaskRoundB,
  }
};
