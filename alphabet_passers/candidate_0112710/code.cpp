...
// Candidate 112711: TwistCandidate_112711
// family=ferocious_dual_worker op_budget=9 worker_shapes=2x0/2x2 worker_reverse=(False, True, False, False)/(False, False, True, False) twiddle1=add/i_mul twiddle2=add/none twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m3i2v021,aBreakerCD:4m1i2v320 lane_breaker=true braid_breaker=false jump_breaker=true swap_breaker=false mask_template=3 mask_seed_a=off mask_seed_b=add_wave lightning=false/1 typhoon=true hurricane=true final_reverse=(False, False, True, True)
// ferocious[wa=2x0; wb=2x2; tw1=add/i_mul; tw2=add/none; tw2src=B; saltbox=128:0012421c:90055f55; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m3i2v021,aBreakerCD:4m1i2v320; wa_rev=(False, True, False, False); wb_rev=(False, False, True, False); lane=on:0; braid=off:1; jump=on:2; swap=off:2; mask=workerbx3; lightning=off:1; typhoon=on; hurricane=on; final=2/(False, False, True, True); key_rot=30; maskA_seed=off/False; maskB_seed=add_wave; maskA_bias=20; maskB_bias=220]
static std::uint32_t TwistCandidate_112711_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x2B2CFA43u;
  aState = AdvanceTwiddle32(
      ...
      0x69B7CCCFu,
      7u);
  aState ^= RotateLeft32(pValue + pExtra + 0x249B109Du, 11U);
  return static_cast<std::uint32_t>(aState + 0x429B368Cu);
}

static std::uint32_t TwistCandidate_112711_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xAA127F61u;
  aState = AdvanceTwiddle32(
      ...
      0x8E896FFCu,
      10u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xB9B66A3Fu, 8U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5B3AEu);
}

static void TwistCandidate_112711_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_112711_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_112711_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_112711_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_112711_AdvanceTwiddle32B(
...
static void TwistCandidate_112711_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xA0U, 0xBEU, 0x3DU, 0x1DU, 0x40U, 0x7AU, 0xDAU, 0x43U, 0x3BU, 0x97U, 0x44U, 0x91U, 0xBCU, 0x94U, 0xEEU, 0x6EU,
      0xC8U, 0x59U, 0xCEU, 0x8EU, 0xD7U, 0x86U, 0x45U, 0x88U, 0x38U, 0x85U, 0x87U, 0xA3U, 0x3AU, 0x28U, 0x9AU, 0xE4U,
      0x60U, 0xAFU, 0x00U, 0x35U, 0x64U, 0xB0U, 0xF0U, 0x90U, 0x14U, 0xF8U, 0x5FU, 0x80U, 0xCBU, 0x7CU, 0x84U, 0x0DU,
      0x10U, 0xC2U, 0x18U, 0xA1U, 0x50U, 0xCDU, 0x20U, 0x4DU, 0x9EU, 0xF3U, 0xD3U, 0x79U, 0x48U, 0xC5U, 0x5EU, 0xD8U,
      0x63U, 0x8CU, 0x53U, 0x61U, 0xA8U, 0xB8U, 0x34U, 0x12U, 0xB1U, 0xE8U, 0xABU, 0x6FU, 0xFBU, 0xC0U, 0x6AU, 0x62U,
      0x2AU, 0xD2U, 0x8DU, 0x17U, 0x4EU, 0x5CU, 0xC7U, 0xD5U, 0x9CU, 0xEAU, 0xDEU, 0xFFU, 0x2EU, 0x13U, 0xF2U, 0x9DU,
      0xAAU, 0x7BU, 0xECU, 0x9BU, 0xD9U, 0x8BU, 0x69U, 0x06U, 0x4BU, 0xD0U, 0x46U, 0xA6U, 0xE1U, 0xA2U, 0x11U, 0x99U,
      0x76U, 0x33U, 0x1FU, 0xD1U, 0xE2U, 0xC6U, 0xA7U, 0xDCU, 0xB2U, 0x7EU, 0xE3U, 0x4CU, 0xF5U, 0xFEU, 0xC4U, 0xB5U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 85u ^ 51u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 2u + (-5222)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 22u + (-2617)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 2u + static_cast<unsigned int>(85u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 85u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(51u), 2u, false);
  ...
  TwistCandidate_112711_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(51u), 2u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(51u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
...
static void TwistCandidate_112711_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 9u + (-3879)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 6u + (7100)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 85u) ^ ((b << 2u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_112711_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 148u ^ 20u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 13u + (-2543)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 24u + (-4487)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (13u + 24u) + (1025)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 13u * 8U + static_cast<std::size_t>(aSourceIndex) * 24u + 148u + static_cast<std::size_t>(aTurbulence * ((14u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(148u), 1u, true);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 24u + (-2543)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 60u + (-4487)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (24u + 60u) + (1025)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 24u * 8U + static_cast<std::size_t>(aReverseIndex) * 60u * 3U + 140u + static_cast<std::size_t>(aTurbulence * ((14u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 4u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(140u), 4u, true);
  }
}

static void TwistCandidate_112711_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 96u ^ 220u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 2u + (1161)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-74)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (2u + 29u) + (7406)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 54u + (5362)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte + b + static_cast<std::uint32_t>(206u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 2u * 8U + static_cast<std::size_t>(aSourceIndex) * 29u + 96u + static_cast<std::size_t>(aTurbulence * ((54u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(96u), 3u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 206u) * ((54u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 2u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 49u + (1161)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 28u + (-74)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (49u + 28u) + (7406)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 54u + (5362)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aTurbulence & 0xFFu), 2u))) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c + static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 2u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 49u * 8U + static_cast<std::size_t>(aReverseIndex) * 28u * 3U + 172u + static_cast<std::size_t>(aTurbulence * ((54u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 206u) * ((54u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 5u, true);
  }
}

static void TwistCandidate_112711_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xEFU, 0xDBU, 0x20U, 0x21U, 0xA2U, 0x89U, 0x35U, 0xBAU, 0xFCU, 0x3AU, 0xFBU, 0xBCU, 0x86U, 0x39U, 0x54U, 0xF9U,
      0x14U, 0x02U, 0x4EU, 0x0CU, 0x22U, 0x2BU, 0x19U, 0xCEU, 0xC8U, 0xE2U, 0xE9U, 0x1DU, 0xE7U, 0xDCU, 0xD0U, 0x15U,
      0x9EU, 0x78U, 0x18U, 0xC4U, 0xCDU, 0x64U, 0x71U, 0xCCU, 0x90U, 0x34U, 0x7FU, 0xBFU, 0xD8U, 0x85U, 0x81U, 0x94U,
      0x3CU, 0x4CU, 0x5AU, 0x5CU, 0x7CU, 0xE3U, 0x00U, 0xD2U, 0x0DU, 0xAAU, 0xE6U, 0xEDU, 0x62U, 0x36U, 0x25U, 0x57U,
      0x1EU, 0x42U, 0x55U, 0xB5U, 0x60U, 0x46U, 0x8AU, 0x6DU, 0x73U, 0x01U, 0x88U, 0x8FU, 0xB3U, 0x50U, 0x0EU, 0xA4U,
      0x3EU, 0x4FU, 0x03U, 0xB4U, 0x0BU, 0x45U, 0x5DU, 0xC7U, 0x28U, 0x5EU, 0xDFU, 0x12U, 0x83U, 0x7BU, 0x6FU, 0x92U,
      0x8BU, 0x33U, 0x3DU, 0xD6U, 0x77U, 0x6BU, 0x53U, 0xC2U, 0xC3U, 0xB8U, 0x61U, 0x0FU, 0x13U, 0x38U, 0x9AU, 0x9CU,
      0x8CU, 0x56U, 0xE1U, 0x04U, 0x4AU, 0xA5U, 0x51U, 0xFAU, 0x9FU, 0xE5U, 0x44U, 0xB2U, 0xC9U, 0x65U, 0x98U, 0x17U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0xB33EF128u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x8BEC4938u, 8u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x7DBB605Du ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x3983E4A8u, 11u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (5461), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (5639), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (1501), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-6917), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltA = static_cast<std::uint32_t>(pSalt[(i + 15u) & 31U]);
    const std::uint32_t aSaltB = static_cast<std::uint32_t>(pSalt[(i + 6u) & 31U]);
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x8BEC4938u, 8u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x3983E4A8u, 11u);
...
static void TwistCandidate_112711_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x51U, 0xFDU, 0xB6U, 0xB7U, 0x4FU, 0x46U, 0xD5U, 0x26U, 0x0CU, 0xD0U, 0x4DU, 0x69U, 0xD8U, 0x6CU, 0x0BU, 0x08U,
      0x8EU, 0x85U, 0x17U, 0x27U, 0xF9U, 0x8BU, 0xC3U, 0xAAU, 0x1AU, 0x72U, 0xB3U, 0x48U, 0x61U, 0x33U, 0xD7U, 0xE3U,
      0x5CU, 0x56U, 0xD4U, 0xE4U, 0xDDU, 0x55U, 0xE0U, 0x02U, 0x6DU, 0xBEU, 0x9AU, 0xE9U, 0xC5U, 0xF4U, 0x3BU, 0xF7U,
      0x4AU, 0x3AU, 0x2CU, 0x9DU, 0x54U, 0xCAU, 0x7EU, 0xA1U, 0x0EU, 0x03U, 0x7FU, 0x7CU, 0x70U, 0x90U, 0x57U, 0x92U,
      0xB9U, 0x38U, 0xCDU, 0xD2U, 0x49U, 0x1EU, 0x5FU, 0xF8U, 0xE5U, 0x4EU, 0x20U, 0x44U, 0x73U, 0xBBU, 0xA2U, 0xC1U,
      0x58U, 0x59U, 0x87U, 0x1FU, 0x06U, 0x95U, 0xECU, 0xBCU, 0xB5U, 0x24U, 0x96U, 0x81U, 0xEEU, 0x83U, 0xA8U, 0x77U,
      0xB4U, 0x0DU, 0x25U, 0x5BU, 0xBFU, 0x7DU, 0x60U, 0x3DU, 0x31U, 0x65U, 0x42U, 0xD3U, 0xB1U, 0x07U, 0xEBU, 0x88U,
      0x22U, 0x50U, 0xEAU, 0xF0U, 0xF6U, 0x6AU, 0x10U, 0xA7U, 0x23U, 0x8DU, 0xAFU, 0xCBU, 0x79U, 0x66U, 0x3EU, 0x7AU
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0xAC5675DDu, 0xFB7AB7AAu, 4u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (57), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xFB7AB7AAu, 4u);
...
static void TwistCandidate_112711_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_112711_AdvanceTwiddle32A(
      0x99D98B23u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_112711_AdvanceTwiddle32B(
      0xA04C3509u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_112711_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_112711_AdvanceTwiddle32A(
      0x3C459049u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[9U]),
      0xFEF7CE77u ^ static_cast<std::uint32_t>(pSalt[19U]));
  std::uint32_t aPulseB = TwistCandidate_112711_AdvanceTwiddle32B(
      0xCB053873u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[19U]),
      0xECD1E387u ^ static_cast<std::uint32_t>(pSalt[9U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_112711_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_112711_AdvanceTwiddle32B(
...
static void TwistCandidate_112711_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x2E784726u ^ (pRound * 37u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-3879), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x2E784726u ^ (pRound * 37u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (7100), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-6808), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-7501), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 142u + 15u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-7501 + 15), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 7u + 3U) & 15U), static_cast<std::size_t>(i + 15u)));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(5u)) ^ static_cast<std::uint32_t>(109u)) & 0xFFu);
      const int aIndex2 = WrapRange(aReverseIndex + (-7438), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(136u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[27u]), 0x27D4EB2Du, 11u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (-3002), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-3666), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 227u + 5u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-3666 + 5), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 10u + 3U) & 15U), static_cast<std::size_t>(i + 23u)));
      ...
      const int aIndex2 = WrapRange(i + (4629), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB + static_cast<std::uint32_t>(30u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[10u]), 0x27D4EB2Du, 5u);
    }
  }

  TwistCandidate_112711_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_112711_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_112711_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-6861), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-6801), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-2795), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 19u + 360u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatOther = WrapRange(static_cast<int>(aMaskFlat) + (7 + 3), 0, static_cast<int>(kMaskStackTotalBytes));
      const std::uint32_t aMaskByteA = static_cast<std::uint32_t>(pMaskStackB[(aMaskFlat / kMaskBytes) % kMaskStackDepth][aMaskFlat % kMaskBytes]);
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kTyphoonBlockBytes) {
    auto storm_block = LoadBlock128Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-2579), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock128Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (3129), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackA, chunk + 577u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackB, chunk + 577u + 23u);
    ...
    switch (static_cast<unsigned>(storm_control[3] ^ storm_mask_a[11] ^ storm_mask_b[13] ^ pSalt[0]) % 6u) {
      case 0u: storm.ApplyFastOp(TyphoonFastOp::kXorColumnIntoColumn, static_cast<std::uint8_t>(storm_control[17] ^ storm_mask_a[23] ^ storm_mask_b[29]), static_cast<std::uint8_t>(storm_control[29] + storm_mask_a[31] + storm_mask_b[37])); break;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-6990), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-2568), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 288u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 288u + 29u);
  ...
  TwistCandidate_112711_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (-5139), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-3267), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (-4843), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (-4818), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_112711_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_112711_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 3u + (-3130)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 1u + (2133)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 17u + (-6597)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 3u + static_cast<unsigned int>(30u)) & 31U;
...
static void TwistCandidate_112711_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 24u + (-6425)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 2u + (-2181)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 22u + (3259)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_112711_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 19u + (913)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 2u + (-6619)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 15u + (6146)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(220u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(220u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_112711(
  ...
  TwistCandidate_112711_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_112711_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_112711_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_112711_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_112711_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_112711_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_112711_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_112711_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_112711",
    &TwistCandidate_112711,
    &TwistCandidate_112711_KeySeed,
    &TwistCandidate_112711_SaltSeed,
    &TwistCandidate_112711_MaskSeedA,
    &TwistCandidate_112711_MaskSeedB,
    &TwistCandidate_112711_TwistBlock,
    &TwistCandidate_112711_PushKeyRound,
    &TwistCandidate_112711_PushMaskRoundA,
    &TwistCandidate_112711_PushMaskRoundB,
  }
};
