...
// Candidate 135967: TwistCandidate_135967
// family=ferocious_dual_worker op_budget=7 worker_shapes=1x4/3x1 worker_reverse=(True, False, False, False)/(False, True, True, False) twiddle1=mix/i_mul twiddle2=add/none twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m1i1v130,aBreakerCD:4m1i1v212 lane_breaker=false braid_breaker=true jump_breaker=true swap_breaker=false mask_template=3 mask_seed_a=off mask_seed_b=xor_wave lightning=false/3 typhoon=false hurricane=false final_reverse=(False, False, True, True)
// ferocious[wa=1x4; wb=3x1; tw1=mix/i_mul; tw2=add/none; tw2src=B; saltbox=128:000e7ee6:f4ac02ee; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m1i1v130,aBreakerCD:4m1i1v212; wa_rev=(True, False, False, False); wb_rev=(False, True, True, False); lane=off:2; braid=on:0; jump=on:3; swap=off:1; mask=workerax3; lightning=off:3; typhoon=off; hurricane=off; final=5/(False, False, True, True); key_rot=5; maskA_seed=off/True; maskB_seed=xor_wave; maskA_bias=114; maskB_bias=160]
static std::uint32_t TwistCandidate_135967_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0xA1CB4BF9u;
  aState = AdvanceTwiddle32(
      ...
      0xF76507EDu,
      6u);
  aState ^= RotateLeft32(pValue + pExtra + 0x30AE5D19u, 11U);
  return static_cast<std::uint32_t>(aState + 0x56AE4C14u);
}

static std::uint32_t TwistCandidate_135967_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x34C13361u;
  aState = AdvanceTwiddle32(
      ...
      0x046F6E78u,
      10u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xA63ABA71u, 7U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A4348Cu);
}

static void TwistCandidate_135967_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_135967_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_135967_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_135967_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_135967_AdvanceTwiddle32B(
...
static void TwistCandidate_135967_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x7BU, 0x41U, 0x68U, 0x4CU, 0x51U, 0x73U, 0x8FU, 0x72U, 0x2CU, 0xF3U, 0xD4U, 0x36U, 0xABU, 0xF1U, 0xEAU, 0xE7U,
      0x6CU, 0x80U, 0x8DU, 0xCEU, 0x32U, 0xE3U, 0xC4U, 0x4DU, 0xFCU, 0xBCU, 0xB0U, 0x3BU, 0x33U, 0x4AU, 0xB3U, 0x8EU,
      0x71U, 0x7CU, 0xEDU, 0xD1U, 0x29U, 0x9CU, 0xD6U, 0x25U, 0x00U, 0xADU, 0x7EU, 0x4FU, 0x06U, 0x8CU, 0x01U, 0xD0U,
      0x75U, 0xB7U, 0x28U, 0xFDU, 0xCDU, 0x14U, 0x18U, 0xBDU, 0x53U, 0x9AU, 0xB2U, 0x95U, 0x77U, 0x6AU, 0x6DU, 0x52U,
      0x9BU, 0xD3U, 0xF4U, 0x57U, 0x91U, 0x61U, 0x3AU, 0x0EU, 0xB6U, 0x4EU, 0x89U, 0xEEU, 0x92U, 0x6EU, 0x98U, 0x66U,
      0x0AU, 0x67U, 0xA1U, 0x3CU, 0x50U, 0xF8U, 0x76U, 0xB5U, 0x49U, 0x46U, 0xDFU, 0x0CU, 0x10U, 0x7FU, 0xA3U, 0x37U,
      0xC0U, 0x60U, 0x19U, 0x08U, 0x1DU, 0xA4U, 0x05U, 0xF2U, 0x20U, 0x39U, 0x65U, 0x64U, 0x63U, 0xA6U, 0x2AU, 0x8AU,
      0x09U, 0xFFU, 0xE1U, 0x0DU, 0x3FU, 0x23U, 0xC1U, 0x55U, 0x27U, 0xC6U, 0x0FU, 0x04U, 0xC3U, 0xE6U, 0x6FU, 0xAEU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 41u ^ 189u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 16u + (-3453)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 28u + (-7329)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 16u + static_cast<unsigned int>(41u)) & 31U;
    ...
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(189u), 1u, true);
  ...
  TwistCandidate_135967_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(189u), 1u);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 25u + (-1456)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 48u + (-9326)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 25u + pSalt[(aSourceIndex + 249u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(249u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(249u), 3u, true);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_135967_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(249u), static_cast<std::uint32_t>(249u), 3u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(249u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_135967_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 23u + (-391)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 30u + (4937)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_135967_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 4u ^ 114u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 12u + (6149)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 27u + (3802)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (12u + 27u) + (2772)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 12u * 8U + static_cast<std::size_t>(aReverseIndex) * 27u + 4u + static_cast<std::size_t>(aTurbulence * ((9u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(4u), 7u, true);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 28u + (6149)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 60u + (3802)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (28u + 60u) + (2772)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 28u * 8U + static_cast<std::size_t>(aReverseIndex) * 60u * 3U + 166u + static_cast<std::size_t>(aTurbulence * ((9u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 6u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(166u), 6u, true);
  }
}

static void TwistCandidate_135967_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 125u ^ 160u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 21u + (-4059)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 11u + (-2084)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (21u + 11u) + (-3885)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 14u + (5856)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 6u)) ^ static_cast<std::uint32_t>(213u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 21u * 8U + static_cast<std::size_t>(aSourceIndex) * 11u + 125u + static_cast<std::size_t>(aTurbulence * ((14u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(125u), 4u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 213u) * ((14u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 6u, false);
  }
}

static void TwistCandidate_135967_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x96U, 0x7CU, 0xBFU, 0xE5U, 0x1FU, 0xF7U, 0x8AU, 0x58U, 0xC9U, 0xB2U, 0x8CU, 0xE7U, 0xACU, 0x5FU, 0xDDU, 0xF9U,
      0x89U, 0x46U, 0xB9U, 0x5EU, 0x04U, 0xEAU, 0x3DU, 0x64U, 0x33U, 0xC6U, 0xC5U, 0xC4U, 0x52U, 0xD9U, 0x9FU, 0x37U,
      0x03U, 0x25U, 0x82U, 0xF2U, 0xECU, 0x1CU, 0xBDU, 0xC2U, 0xEBU, 0x9BU, 0xCAU, 0x74U, 0x95U, 0xB6U, 0x61U, 0x60U,
      0xCDU, 0xE9U, 0x98U, 0x8FU, 0xF8U, 0x2DU, 0x47U, 0x4EU, 0x2AU, 0xA1U, 0x17U, 0xDEU, 0xA8U, 0xB4U, 0x35U, 0x10U,
      0xA5U, 0xE0U, 0x65U, 0xCFU, 0x32U, 0x1EU, 0x69U, 0x3EU, 0x09U, 0xC1U, 0x19U, 0xD4U, 0xC7U, 0xBCU, 0x07U, 0xA0U,
      0xF3U, 0x0AU, 0x55U, 0x4DU, 0x6FU, 0x92U, 0x3BU, 0x00U, 0x20U, 0x3FU, 0xBAU, 0x7BU, 0xB0U, 0x2CU, 0x84U, 0x27U,
      0x86U, 0x21U, 0x62U, 0x4FU, 0x01U, 0x68U, 0x02U, 0xDAU, 0x2FU, 0xAAU, 0x0FU, 0x28U, 0xDBU, 0x34U, 0x36U, 0x94U,
      0x88U, 0x14U, 0x26U, 0x50U, 0x90U, 0x6EU, 0x39U, 0xB8U, 0xE6U, 0xF5U, 0x06U, 0x6DU, 0xE1U, 0x3CU, 0x91U, 0x4CU
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x162D0F8Du ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x3A778D0Fu, 7u);
  std::uint32_t aAccB = AdvanceTwiddle32(0xD5696815u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0xCC46A8DDu, 5u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-7570), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-6198), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-744), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-5172), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x3A778D0Fu, 7u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0xCC46A8DDu, 5u);
...
static void TwistCandidate_135967_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xC9U, 0xACU, 0x9CU, 0x6BU, 0xD5U, 0x43U, 0xBBU, 0x77U, 0xC3U, 0xCEU, 0x5DU, 0xABU, 0xADU, 0x86U, 0x1EU, 0x75U,
      0xDBU, 0x97U, 0x31U, 0x62U, 0xFCU, 0x7BU, 0xC7U, 0x10U, 0x32U, 0x0FU, 0x02U, 0x54U, 0xE8U, 0xD2U, 0x71U, 0x35U,
      0x76U, 0x0CU, 0x18U, 0x70U, 0xEAU, 0x67U, 0xF4U, 0xD6U, 0x2BU, 0xA7U, 0x38U, 0x25U, 0xB8U, 0x89U, 0xF1U, 0x91U,
      0x9EU, 0x6AU, 0xF9U, 0xA8U, 0x83U, 0x22U, 0x29U, 0x27U, 0x6EU, 0xF0U, 0x68U, 0x8BU, 0xE4U, 0x87U, 0xAEU, 0x9AU,
      0x28U, 0x13U, 0x69U, 0x93U, 0x4EU, 0x6DU, 0xF7U, 0x90U, 0x5CU, 0x9BU, 0x46U, 0x99U, 0x56U, 0xCFU, 0x84U, 0x17U,
      0x2AU, 0xD0U, 0x51U, 0xA1U, 0x6FU, 0x33U, 0x82U, 0xE5U, 0x44U, 0x88U, 0x0AU, 0x09U, 0xA0U, 0x45U, 0x80U, 0x55U,
      0xFDU, 0x8CU, 0xF3U, 0xECU, 0xB3U, 0x94U, 0x1DU, 0x4FU, 0xB5U, 0xEFU, 0x7FU, 0x5BU, 0x3AU, 0x7AU, 0x00U, 0x48U,
      0xF2U, 0x37U, 0x03U, 0xBDU, 0x4DU, 0xF6U, 0xBAU, 0x42U, 0xE0U, 0xDEU, 0xD9U, 0x74U, 0xDFU, 0x16U, 0x8AU, 0x24U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x4E44BAD0u, 0x2C988A80u, 3u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-3257), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x2C988A80u, 3u);
...
static void TwistCandidate_135967_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_135967_AdvanceTwiddle32A(
      0x9973490Bu ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_135967_AdvanceTwiddle32B(
      0x84684D51u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_135967_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_135967_AdvanceTwiddle32A(
      0xBB68CF73u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[19U]),
      0x79DC5351u ^ static_cast<std::uint32_t>(pSalt[7U]));
  std::uint32_t aPulseB = TwistCandidate_135967_AdvanceTwiddle32B(
      0x33634DC7u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[7U]),
      0x14B7AF01u ^ static_cast<std::uint32_t>(pSalt[19U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_135967_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_135967_AdvanceTwiddle32B(
...
static void TwistCandidate_135967_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x5A1CEB6Cu ^ (pRound * 23u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-391), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x5A1CEB6Cu ^ (pRound * 23u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (4937), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(aReverseIndex + (-5617), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-479), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 173u + 1u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-479 + 1), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 11u + 3U) & 15U), static_cast<std::size_t>(i + 25u)));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(3u)) ^ static_cast<std::uint32_t>(168u)) & 0xFFu);
      const std::uint32_t aRoute = ((aSourceCtrl >> 3u) ^ aWorkerCtrl ^ aKeyByte) & 3U;
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA ^ static_cast<std::uint32_t>(pSalt[0u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(133u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 6u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (2857), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-693), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 143u + 18u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-693 + 18), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 3u + 3U) & 15U), static_cast<std::size_t>(i + 21u)));
      ...
      const int aIndex2 = WrapRange(aReverseIndex + (-5629), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(aReverseIndex + (-443), aStart, aEnd);
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB + static_cast<std::uint32_t>(199u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[19u]), 0x27D4EB2Du, 2u);
    }
  }

  TwistCandidate_135967_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_135967_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_135967_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (3984), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-1539), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (4329), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 23u + 66u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  TwistCandidate_135967_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (-5248), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (3924), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (5958), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (6160), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_135967_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_135967_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 3u + (6807)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 19u + (5254)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 28u + (-4668)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 3u + static_cast<unsigned int>(5u)) & 31U;
...
static void TwistCandidate_135967_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 13u + (3320)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 10u + (-3433)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 4u + (-5293)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_135967_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 19u + (4070)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 24u + (3792)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 10u + (-811)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(160u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(160u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_135967(
  ...
  TwistCandidate_135967_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_135967_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_135967_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_135967_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_135967_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_135967_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_135967_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_135967_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_135967",
    &TwistCandidate_135967,
    &TwistCandidate_135967_KeySeed,
    &TwistCandidate_135967_SaltSeed,
    &TwistCandidate_135967_MaskSeedA,
    &TwistCandidate_135967_MaskSeedB,
    &TwistCandidate_135967_TwistBlock,
    &TwistCandidate_135967_PushKeyRound,
    &TwistCandidate_135967_PushMaskRoundA,
    &TwistCandidate_135967_PushMaskRoundB,
  }
};
