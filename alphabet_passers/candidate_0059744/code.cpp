...
// Candidate 59745: TwistCandidate_59745
// family=ferocious_dual_worker op_budget=5 worker_shapes=2x2/1x4 worker_reverse=(True, False, True, True)/(True, True, False, False) twiddle1=xor/i twiddle2=add/none twiddle2_source=A tsunami=custom breaker_shapes=aBreakerAB:4m0i2v210 lane_breaker=false braid_breaker=false jump_breaker=false swap_breaker=false mask_template=2 mask_seed_a=sbox_wave mask_seed_b=xor_wave lightning=false/0 typhoon=false hurricane=false final_reverse=(False, True, True, True)
// ferocious[wa=2x2; wb=1x4; tw1=xor/i; tw2=add/none; tw2src=A; saltbox=128:000fa5fb:72ebb5da; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m0i2v210; wa_rev=(True, False, True, True); wb_rev=(True, True, False, False); lane=off:3; braid=off:0; jump=off:3; swap=off:0; mask=workerax2; lightning=off:0; typhoon=off; hurricane=off; final=3/(False, True, True, True); key_rot=15; maskA_seed=sbox_wave/True; maskB_seed=xor_wave; maskA_bias=9; maskB_bias=36]
static std::uint32_t TwistCandidate_59745_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x8CF2929Bu;
  aState = AdvanceTwiddle32(
      ...
      0x9E70EB36u,
      7u);
  aState ^= RotateLeft32(pValue + pExtra + 0x748269EFu, 10U);
  return static_cast<std::uint32_t>(aState + 0x128279ADu);
}

static std::uint32_t TwistCandidate_59745_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x5DD506A7u;
  aState = AdvanceTwiddle32(
      ...
      0x29576F48u,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xB64A01E3u, 8U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5ED91u);
}

static void TwistCandidate_59745_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_59745_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_59745_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_59745_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_59745_AdvanceTwiddle32B(
...
static void TwistCandidate_59745_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x91U, 0xD5U, 0xF3U, 0x2AU, 0x9FU, 0x96U, 0x02U, 0xECU, 0x31U, 0x2DU, 0x8BU, 0x5BU, 0x87U, 0xACU, 0x3BU, 0xDCU,
      0x1DU, 0x03U, 0x7BU, 0xCBU, 0xDBU, 0x28U, 0xB1U, 0xBDU, 0x29U, 0xCAU, 0xF0U, 0x08U, 0x5DU, 0x3AU, 0x85U, 0xDDU,
      0xB4U, 0xBFU, 0x4CU, 0xE8U, 0x04U, 0x00U, 0x1AU, 0x11U, 0x8CU, 0xADU, 0xE7U, 0x1EU, 0x88U, 0x37U, 0x36U, 0x75U,
      0xD2U, 0x15U, 0xE1U, 0xA0U, 0xE5U, 0x86U, 0xD6U, 0x3DU, 0x6DU, 0x56U, 0x33U, 0xF6U, 0x84U, 0xA1U, 0xA2U, 0xE9U,
      0x1CU, 0xFBU, 0x4DU, 0x7FU, 0xFFU, 0x68U, 0xEBU, 0x44U, 0xB6U, 0xC3U, 0x9BU, 0xA5U, 0x64U, 0x9CU, 0xD0U, 0x48U,
      0xD3U, 0x46U, 0x19U, 0xEDU, 0xD8U, 0x6AU, 0xF1U, 0x81U, 0x5FU, 0xCFU, 0xA7U, 0x8EU, 0x4BU, 0x35U, 0x82U, 0x26U,
      0x6EU, 0x92U, 0xC8U, 0x2BU, 0x14U, 0x71U, 0xA6U, 0x42U, 0x55U, 0x78U, 0xAEU, 0x45U, 0x20U, 0x16U, 0x38U, 0x52U,
      0xC5U, 0x7AU, 0x3CU, 0x21U, 0x70U, 0x2CU, 0x8DU, 0x61U, 0xC4U, 0xC6U, 0xD1U, 0xB2U, 0x06U, 0x6BU, 0x58U, 0xBAU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 132u ^ 164u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 7u + (-311)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 10u + (-7649)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 7u + static_cast<unsigned int>(132u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 132u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(164u), 2u, false);
  ...
  TwistCandidate_59745_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(164u), 2u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(164u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 56u + (394)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 59u + (-8354)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 56u + pSalt[(aSourceIndex + 248u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(248u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(248u), 2u, false);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_59745_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(248u), static_cast<std::uint32_t>(248u), 2u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(248u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_59745_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 26u + (1134)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 24u + (-3665)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 132u) ^ ((b << 2u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_59745_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 93u ^ 9u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 25u + (-1826)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 4u + (1589)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (25u + 4u) + (-7673)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 27u + (-4998)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>(aMixBox[(aWorkerByte ^ a ^ static_cast<std::uint32_t>(2u)) & 0xFFu]);
    ...
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(2u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 25u * 8U + static_cast<std::size_t>(aReverseIndex) * 4u + 93u + static_cast<std::size_t>(aTurbulence * ((27u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(93u), 7u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 2u) * ((27u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
  ...
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 23u + (-1826)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 40u + (1589)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (23u + 40u) + (-7673)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(2u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 23u * 8U + static_cast<std::size_t>(aSourceIndex) * 40u * 3U + 43u + static_cast<std::size_t>(aTurbulence * ((27u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 4u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(43u), 4u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 2u) * ((27u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 4u, true);
  }
}

static void TwistCandidate_59745_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 45u ^ 36u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 30u + (-207)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 30u + (5018)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (30u + 30u) + (2705)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 28u + (-6244)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 2u)) ^ static_cast<std::uint32_t>(197u)) & 0xFFu);
    const std::uint32_t aSeedA = static_cast<std::uint32_t>((a ^ aTurbulence) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 30u * 8U + static_cast<std::size_t>(aSourceIndex) * 30u + 45u + static_cast<std::size_t>(aTurbulence * ((28u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(45u), 6u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 197u) * ((28u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 2u, true);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 53u + (-207)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 33u + (5018)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (53u + 33u) + (2705)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 28u + (-6244)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 2u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 53u * 8U + static_cast<std::size_t>(aSourceIndex) * 33u * 3U + 139u + static_cast<std::size_t>(aTurbulence * ((28u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 7u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(139u), 7u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 197u) * ((28u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 7u, true);
  }
}

static void TwistCandidate_59745_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x68U, 0x88U, 0xF0U, 0xDDU, 0x09U, 0x78U, 0x5AU, 0xC7U, 0x2DU, 0x18U, 0x53U, 0x1BU, 0xADU, 0x69U, 0xE4U, 0x37U,
      0x36U, 0xD0U, 0x61U, 0x57U, 0xD3U, 0xEFU, 0x17U, 0x46U, 0x97U, 0x03U, 0xA3U, 0x1DU, 0xD1U, 0xB1U, 0x1FU, 0x27U,
      0xBFU, 0xD8U, 0xF6U, 0x0EU, 0x45U, 0x93U, 0xC9U, 0xFAU, 0x1EU, 0x2FU, 0xF2U, 0x7CU, 0x75U, 0xDCU, 0x48U, 0x3BU,
      0x8FU, 0xB5U, 0x14U, 0x0FU, 0x7FU, 0x1AU, 0x47U, 0x4AU, 0x50U, 0xF3U, 0xA2U, 0x64U, 0xCCU, 0x00U, 0xFDU, 0x41U,
      0x3AU, 0xA9U, 0xF7U, 0x29U, 0x58U, 0xA4U, 0x7AU, 0x72U, 0xA7U, 0x7BU, 0x07U, 0x40U, 0x6DU, 0xDEU, 0x8DU, 0xBBU,
      0xAEU, 0x4CU, 0xD2U, 0x9FU, 0x02U, 0x62U, 0xB2U, 0x8EU, 0x4EU, 0x13U, 0xD9U, 0xC0U, 0x7DU, 0x71U, 0x99U, 0xA0U,
      0x39U, 0x52U, 0x51U, 0x59U, 0xE5U, 0xBCU, 0xDFU, 0x8AU, 0x5DU, 0xE9U, 0x96U, 0x6EU, 0x1CU, 0x84U, 0x33U, 0x12U,
      0xF1U, 0xE2U, 0x76U, 0x9BU, 0x3CU, 0xC5U, 0xBDU, 0xD7U, 0xB7U, 0x20U, 0x55U, 0xCEU, 0x8CU, 0x04U, 0x81U, 0x85U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0xFD79DB0Du ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x711697D7u, 10u);
  std::uint32_t aAccB = AdvanceTwiddle32(0xA82357BCu ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x0FE09719u, 11u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-2353), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (2406), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (3817), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (5468), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x711697D7u, 10u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x0FE09719u, 11u);
...
static void TwistCandidate_59745_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x20U, 0xF1U, 0x9BU, 0xA3U, 0xB5U, 0x3FU, 0xDFU, 0x99U, 0x84U, 0xC0U, 0x17U, 0x51U, 0x1BU, 0x4AU, 0xBDU, 0x37U,
      0x0BU, 0x24U, 0x34U, 0x27U, 0x08U, 0x21U, 0xCAU, 0xCBU, 0x81U, 0xD0U, 0x6BU, 0xA4U, 0x91U, 0x14U, 0xC7U, 0x9DU,
      0xBBU, 0xDAU, 0xB3U, 0x19U, 0x55U, 0x3AU, 0xEAU, 0xB4U, 0xC8U, 0x2EU, 0x0AU, 0x1CU, 0xE0U, 0x5BU, 0xD9U, 0x10U,
      0x6DU, 0x7BU, 0x77U, 0x79U, 0xADU, 0x92U, 0x42U, 0x4FU, 0x38U, 0xE8U, 0x15U, 0x9AU, 0xB1U, 0x4DU, 0x18U, 0xE5U,
      0x8EU, 0x16U, 0xBAU, 0x0DU, 0x94U, 0xD2U, 0x23U, 0xD7U, 0xEDU, 0x90U, 0xCFU, 0x1EU, 0x41U, 0x61U, 0xB7U, 0x1FU,
      0x05U, 0xBFU, 0x9EU, 0x76U, 0x03U, 0x5EU, 0x22U, 0x6EU, 0x13U, 0x68U, 0x5CU, 0xE9U, 0x3DU, 0xE6U, 0x52U, 0x7DU,
      0xC9U, 0x4EU, 0xD6U, 0xEEU, 0x95U, 0xB6U, 0xC4U, 0x60U, 0xD4U, 0x49U, 0x88U, 0xF6U, 0x58U, 0xB8U, 0x75U, 0x06U,
      0x39U, 0xC5U, 0xFBU, 0xC2U, 0x97U, 0xEFU, 0x32U, 0xB9U, 0xFCU, 0x1AU, 0xBCU, 0x8CU, 0x80U, 0x54U, 0x0EU, 0x2CU
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0xA1D0D77Du, 0x9CBF42C9u, 6u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-3435), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x9CBF42C9u, 6u);
...
static void TwistCandidate_59745_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_59745_AdvanceTwiddle32A(
      0xD38FB175u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_59745_AdvanceTwiddle32B(
      0x91922B2Fu ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_59745_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_59745_AdvanceTwiddle32A(
      0x8F87A0D1u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[17U]),
      0x4D36FEEEu ^ static_cast<std::uint32_t>(pSalt[18U]));
  std::uint32_t aPulseB = TwistCandidate_59745_AdvanceTwiddle32B(
      0xDF49D3F2u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[18U]),
      0xF89D4915u ^ static_cast<std::uint32_t>(pSalt[17U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    for (std::size_t aLane = 0; aLane < kMatrixBlockBytes; ++aLane) {
      ...
      const unsigned char aWave = FixedMixBoxByte(static_cast<unsigned char>(
          ...
          pSalt[(aLane + 17U + (aPulseByteB & 3U)) & 31U] ^
          aPulseByteA ^
    ...
    aPulseA = TwistCandidate_59745_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_59745_AdvanceTwiddle32B(
...
static void TwistCandidate_59745_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x5CB86D90u ^ (pRound * 28u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (1134), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x5CB86D90u ^ (pRound * 28u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-3665), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(aReverseIndex + (-2989), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (2235), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 71u + 28u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (2235 + 28), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 6u + 3U) & 15U), static_cast<std::size_t>(i + 16u)));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>((aLaneIndex ^ static_cast<std::uint32_t>(18u)) & 0xFFu);
      const int aIndex2 = WrapRange(i + (-4186), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(247u)), 0x85EBCA6Bu, 4u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (-5953), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (6126), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 90u + 29u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (6126 + 29), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(148u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[0u]), 0x27D4EB2Du, 11u);
    }
  }

  TwistCandidate_59745_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_59745_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-5595), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (512), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-1135), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 21u + 1372u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  TwistCandidate_59745_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (6129), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (6915), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (-4563), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (2219), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_59745_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_59745_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 16u + (-1158)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 1u + (-6282)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-1295)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_59745_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 10u + (6820)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 16u + (-3863)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 23u + (2935)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(9u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(9u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_59745_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 27u + (6879)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 10u + (4702)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 16u + (6611)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
void TwistCandidate_59745(
  ...
  TwistCandidate_59745_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_59745_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_59745_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_59745_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_59745_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_59745_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_59745_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_59745_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_59745",
    &TwistCandidate_59745,
    &TwistCandidate_59745_KeySeed,
    &TwistCandidate_59745_SaltSeed,
    &TwistCandidate_59745_MaskSeedA,
    &TwistCandidate_59745_MaskSeedB,
    &TwistCandidate_59745_TwistBlock,
    &TwistCandidate_59745_PushKeyRound,
    &TwistCandidate_59745_PushMaskRoundA,
    &TwistCandidate_59745_PushMaskRoundB,
  }
};
