...
// Candidate 2540: TwistCandidate_2540
// family=ferocious_dual_worker op_budget=9 worker_shapes=1x3/1x4 worker_reverse=(True, True, False, False)/(False, False, False, True) twiddle1=xor/none twiddle2=xor/none twiddle2_source=A tsunami=custom breaker_shapes=aBreakerAB:4m0i3v312,aBreakerCD:4m2i0v133,aBreakerEF:4m1i2v202 lane_breaker=true braid_breaker=false jump_breaker=true swap_breaker=true mask_template=1 mask_seed_a=off mask_seed_b=xor_wave lightning=false/1 typhoon=true hurricane=false final_reverse=(True, True, False, True)
// ferocious[wa=1x3; wb=1x4; tw1=xor/none; tw2=xor/none; tw2src=A; saltbox=128:00117106:dd5c6801; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m0i3v312,aBreakerCD:4m2i0v133,aBreakerEF:4m1i2v202; wa_rev=(True, True, False, False); wb_rev=(False, False, False, True); lane=on:1; braid=off:3; jump=on:2; swap=on:0; mask=workerbx1; lightning=off:1; typhoon=on; hurricane=off; final=0/(True, True, False, True); key_rot=3; maskA_seed=off/False; maskB_seed=xor_wave; maskA_bias=46; maskB_bias=54]
static std::uint32_t TwistCandidate_2540_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x31EFD847u;
  aState = AdvanceTwiddle32(
      ...
      0x468928D4u,
      7u);
  aState ^= RotateLeft32(pValue + pExtra + 0x11665717u, 12U);
  return static_cast<std::uint32_t>(aState + 0x7766F093u);
}

static std::uint32_t TwistCandidate_2540_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x852CDAF4u;
  aState = AdvanceTwiddle32(
      ...
      0x944A8DE3u,
      11u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xC63A662Au, 8U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5F220u);
}

static void TwistCandidate_2540_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_2540_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_2540_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_2540_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_2540_AdvanceTwiddle32B(
...
static void TwistCandidate_2540_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x84U, 0xF5U, 0x6BU, 0xCBU, 0xB9U, 0x28U, 0xE9U, 0x64U, 0x9EU, 0x3FU, 0x93U, 0xA7U, 0x00U, 0x91U, 0x7DU, 0xCEU,
      0x8EU, 0xAAU, 0xC6U, 0x44U, 0x82U, 0x39U, 0xD3U, 0xCFU, 0x99U, 0x2CU, 0xE0U, 0xC4U, 0x15U, 0xD5U, 0x5AU, 0xA3U,
      0x2DU, 0xE8U, 0x77U, 0x7BU, 0xB3U, 0xADU, 0x07U, 0x16U, 0x5CU, 0xA4U, 0xB1U, 0x70U, 0x47U, 0xCDU, 0x43U, 0xEFU,
      0x1FU, 0x53U, 0x76U, 0x5BU, 0xE1U, 0xA5U, 0xFCU, 0xEAU, 0xBEU, 0x46U, 0x87U, 0x52U, 0x22U, 0x3BU, 0xD2U, 0x0EU,
      0x85U, 0xC5U, 0x10U, 0x94U, 0x9DU, 0x62U, 0xD4U, 0x9CU, 0xECU, 0xBAU, 0xFFU, 0xD7U, 0xD0U, 0x2AU, 0xF2U, 0x88U,
      0x6CU, 0x1CU, 0xF3U, 0x98U, 0xB4U, 0xB8U, 0x0FU, 0xDCU, 0x20U, 0x0DU, 0x67U, 0x6EU, 0x95U, 0x7FU, 0xB5U, 0x7CU,
      0xA0U, 0x6AU, 0x32U, 0x6FU, 0xBDU, 0x4CU, 0x09U, 0x06U, 0xABU, 0xD6U, 0xAEU, 0xC0U, 0x5DU, 0xEBU, 0xDEU, 0xD9U,
      0x97U, 0x24U, 0xA6U, 0x2EU, 0xCAU, 0xF6U, 0xBBU, 0xFAU, 0xEDU, 0x7AU, 0xFEU, 0x75U, 0x8CU, 0x1EU, 0x17U, 0x41U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 157u ^ 22u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 2u + (-1960)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 1u + (-5497)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 2u + static_cast<unsigned int>(157u)) & 31U;
    ...
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(22u), 2u, true);
  ...
  TwistCandidate_2540_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(22u), 2u);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 44u + (-4187)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 44u + (-3270)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 44u + pSalt[(aSourceIndex + 30u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(30u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(30u), 4u, true);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_2540_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(30u), static_cast<std::uint32_t>(30u), 4u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(30u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_2540_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 13u + (-5514)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 20u + (-3711)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 157u) ^ ((b << 2u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_2540_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 148u ^ 46u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 30u + (931)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 7u + (-6061)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (30u + 7u) + (911)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 30u * 8U + static_cast<std::size_t>(aSourceIndex) * 7u + 148u + static_cast<std::size_t>(aTurbulence * ((58u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(148u), 7u, true);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 45u + (931)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 30u + (-6061)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (45u + 30u) + (911)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 45u * 8U + static_cast<std::size_t>(aReverseIndex) * 30u * 3U + 77u + static_cast<std::size_t>(aTurbulence * ((58u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 1u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(77u), 1u, true);
  }
}

static void TwistCandidate_2540_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 115u ^ 54u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 27u + (-5249)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 23u + (-1232)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (27u + 23u) + (3052)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 57u + (-3045)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 1u)) ^ static_cast<std::uint32_t>(237u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 27u * 8U + static_cast<std::size_t>(aSourceIndex) * 23u + 115u + static_cast<std::size_t>(aTurbulence * ((57u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(115u), 7u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 237u) * ((57u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 1u, false);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 32u + (-5249)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 37u + (-1232)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (32u + 37u) + (3052)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 57u + (-3045)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 1u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 32u * 8U + static_cast<std::size_t>(aReverseIndex) * 37u * 3U + 159u + static_cast<std::size_t>(aTurbulence * ((57u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 6u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(159u), 6u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 237u) * ((57u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 6u, true);
  }
}

static void TwistCandidate_2540_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x5EU, 0xA9U, 0xC4U, 0x94U, 0xECU, 0x4DU, 0xD3U, 0xD2U, 0xDFU, 0x89U, 0x11U, 0xC2U, 0x1EU, 0x59U, 0x3BU, 0x6BU,
      0x9DU, 0xAAU, 0x85U, 0xC1U, 0x74U, 0xA0U, 0x51U, 0x49U, 0x47U, 0x6DU, 0x5BU, 0x32U, 0x2AU, 0x69U, 0x4CU, 0xA7U,
      0x2DU, 0xEDU, 0x86U, 0x42U, 0x0EU, 0xC7U, 0x1FU, 0xE9U, 0x4AU, 0xABU, 0xCDU, 0xFDU, 0xC3U, 0xCBU, 0x44U, 0x04U,

  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x481EEE97u, 0xA3AEB4C2u, 2u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-6060), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xA3AEB4C2u, 2u);
...
static void TwistCandidate_2540_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_2540_AdvanceTwiddle32A(
      0x6D2A86E0u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_2540_AdvanceTwiddle32B(
      0x777D6300u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_2540_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_2540_AdvanceTwiddle32A(
      0xA949764Fu ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[15U]),
      0x6BFC1476u ^ static_cast<std::uint32_t>(pSalt[11U]));
  std::uint32_t aPulseB = TwistCandidate_2540_AdvanceTwiddle32B(
      0xB96BD10Bu + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[11U]),
      0x9EBF4A50u ^ static_cast<std::uint32_t>(pSalt[15U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    for (std::size_t aLane = 0; aLane < kMatrixBlockBytes; ++aLane) {
      ...
      const unsigned char aWave = FixedMixBoxByte(static_cast<unsigned char>(
          ...
          pSalt[(aLane + 15U + (aPulseByteB & 3U)) & 31U] ^
          aPulseByteA ^
    ...
    aPulseA = TwistCandidate_2540_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_2540_AdvanceTwiddle32B(
...
static void TwistCandidate_2540_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x2CB2153Bu ^ (pRound * 6u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-5514), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x2CB2153Bu ^ (pRound * 6u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-3711), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(aReverseIndex + (-7544), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (4274), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 142u + 7u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (4274 + 7), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 8u + 3U) & 15U), static_cast<std::size_t>(i + 10u)));
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(108u)), 0x85EBCA6Bu, 5u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (-834), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (6692), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 70u + 11u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (6692 + 11), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 3u + 3U) & 15U), static_cast<std::size_t>(i + 16u)));
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(198u)), 0x85EBCA6Bu, 4u);
    }
  }

  TwistCandidate_2540_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_2540_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_2540_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    const DualWorkerBreakerRecipe4 aBreakerEF{
        ...
        -333,
        -3091,
        static_cast<std::size_t>(1352u),
        static_cast<std::uint8_t>(11u),
        static_cast<std::uint8_t>(8u),
        static_cast<std::uint8_t>(0u),
        static_cast<std::uint8_t>(9u),
        static_cast<std::uint8_t>(14u),
        static_cast<std::uint8_t>(242u),
    ...
    TwistCandidate_2540_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-2927), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (3829), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-4416), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 15u + 1034u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kTyphoonBlockBytes) {
    auto storm_block = LoadBlock128Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (6035), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock128Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (3381), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackA, chunk + 472u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackB, chunk + 472u + 23u);
  ...
  TwistCandidate_2540_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (4927), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (-333), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (6445), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (4110), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_2540_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_2540_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 18u + (6)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-1574)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 21u + (2452)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 18u + static_cast<unsigned int>(3u)) & 31U;
...
static void TwistCandidate_2540_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 3u + (1708)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 2u + (7644)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 6u + (-2581)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_2540_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 5u + (1588)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 12u + (3781)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-2673)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
void TwistCandidate_2540(
  ...
  TwistCandidate_2540_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_2540_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_2540_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_2540_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_2540_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_2540_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_2540_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_2540_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_2540",
    &TwistCandidate_2540,
    &TwistCandidate_2540_KeySeed,
    &TwistCandidate_2540_SaltSeed,
    &TwistCandidate_2540_MaskSeedA,
    &TwistCandidate_2540_MaskSeedB,
    &TwistCandidate_2540_TwistBlock,
    &TwistCandidate_2540_PushKeyRound,
    &TwistCandidate_2540_PushMaskRoundA,
    &TwistCandidate_2540_PushMaskRoundB,
  }
};
