...
// Candidate 53273: TwistCandidate_53273
// family=ferocious_dual_worker op_budget=8 worker_shapes=1x2/1x3 worker_reverse=(False, True, True, True)/(True, True, True, False) twiddle1=xor/i_mul twiddle2=xor/i twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m0i1v303,aBreakerCD:4m2i3v022,aBreakerEF:4m1i0v002 lane_breaker=true braid_breaker=true jump_breaker=false swap_breaker=true mask_template=1 mask_seed_a=off mask_seed_b=xor_wave lightning=false/3 typhoon=false hurricane=false final_reverse=(True, True, True, False)
// ferocious[wa=1x2; wb=1x3; tw1=xor/i_mul; tw2=xor/i; tw2src=B; saltbox=128:000ee46b:2cd8e5ea; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m0i1v303,aBreakerCD:4m2i3v022,aBreakerEF:4m1i0v002; wa_rev=(False, True, True, True); wb_rev=(True, True, True, False); lane=on:1; braid=on:1; jump=off:3; swap=on:3; mask=workerax1; lightning=off:3; typhoon=off; hurricane=off; final=0/(True, True, True, False); key_rot=15; maskA_seed=off/False; maskB_seed=xor_wave; maskA_bias=6; maskB_bias=205]
static std::uint32_t TwistCandidate_53273_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x3E60BCC3u;
  aState = AdvanceTwiddle32(
      ...
      0xEB5525F7u,
      11u);
  aState ^= RotateLeft32(pValue + pExtra + 0xB335497Bu, 10U);
  return static_cast<std::uint32_t>(aState + 0xD5359934u);
}

static std::uint32_t TwistCandidate_53273_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x28F0C002u;
  aState = AdvanceTwiddle32(
      ...
      0x9BC58979u,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xDA26463Cu, 1U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5E5F5u);
}

static void TwistCandidate_53273_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_53273_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_53273_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_53273_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_53273_AdvanceTwiddle32B(
...
static void TwistCandidate_53273_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x99U, 0x67U, 0xAAU, 0x1FU, 0xFBU, 0x01U, 0xEEU, 0x38U, 0xFAU, 0xBDU, 0x53U, 0x20U, 0xD5U, 0x00U, 0xCAU, 0x9DU,
      0x76U, 0xF0U, 0xE8U, 0x9CU, 0x5EU, 0x55U, 0x30U, 0x75U, 0xB3U, 0xC0U, 0x29U, 0xA7U, 0xF4U, 0x15U, 0xCDU, 0xD0U,
      0xF3U, 0xBBU, 0x11U, 0xEFU, 0x6FU, 0x91U, 0x3BU, 0x98U, 0x51U, 0x22U, 0xDCU, 0x77U, 0xFCU, 0x33U, 0x45U, 0xE1U,
      0x73U, 0x8BU, 0xB7U, 0xA2U, 0x54U, 0x2EU, 0xBCU, 0x7AU, 0x06U, 0xA3U, 0x6EU, 0xB8U, 0x62U, 0xE7U, 0x8EU, 0x1AU,
      0x36U, 0x60U, 0xD1U, 0x7DU, 0x1CU, 0x05U, 0x0FU, 0xE4U, 0x12U, 0x17U, 0x32U, 0xE0U, 0x7CU, 0x84U, 0x79U, 0x3CU,
      0x19U, 0xF1U, 0xC6U, 0xBAU, 0x86U, 0xF6U, 0x2BU, 0x47U, 0x8AU, 0x89U, 0xE9U, 0x95U, 0xCEU, 0x02U, 0x74U, 0x07U,
      0x6DU, 0xC3U, 0x1BU, 0x42U, 0x81U, 0xFEU, 0x48U, 0x97U, 0x50U, 0xB9U, 0xDDU, 0x6CU, 0xABU, 0x03U, 0xCFU, 0x3DU,
      0x40U, 0x04U, 0x82U, 0x94U, 0x57U, 0x26U, 0x71U, 0x4CU, 0x25U, 0x18U, 0x66U, 0x6AU, 0xECU, 0xFFU, 0x5BU, 0x2AU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 105u ^ 161u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 14u + (-3635)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 19u + (3507)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 14u + static_cast<unsigned int>(105u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 105u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(161u), 6u, false);
  ...
  TwistCandidate_53273_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(161u), 6u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(161u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
...
static void TwistCandidate_53273_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 16u + (-1701)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 20u + (7378)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 105u) ^ ((b << 6u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_53273_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 134u ^ 6u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 28u + (6193)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 24u + (6345)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (28u + 24u) + (-563)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 28u * 8U + static_cast<std::size_t>(aReverseIndex) * 24u + 134u + static_cast<std::size_t>(aTurbulence * ((8u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(134u), 1u, true);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 22u + (6193)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 48u + (6345)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (22u + 48u) + (-563)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 22u * 8U + static_cast<std::size_t>(aReverseIndex) * 48u * 3U + 191u + static_cast<std::size_t>(aTurbulence * ((8u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 5u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(191u), 5u, true);
  }
}

static void TwistCandidate_53273_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 226u ^ 205u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 21u + (-3459)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 3u + (7387)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (21u + 3u) + (5635)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 19u + (7494)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 4u)) ^ static_cast<std::uint32_t>(30u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 21u * 8U + static_cast<std::size_t>(aSourceIndex) * 3u + 226u + static_cast<std::size_t>(aTurbulence * ((19u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(226u), 7u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 30u) * ((19u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 4u, true);
  }
}

static void TwistCandidate_53273_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xD8U, 0x7BU, 0x2EU, 0x6CU, 0xB5U, 0xEBU, 0x92U, 0xD7U, 0x2FU, 0x52U, 0x3EU, 0x54U, 0x17U, 0x55U, 0xF8U, 0xE5U,
      0x3CU, 0x9DU, 0x43U, 0xD9U, 0xE9U, 0x41U, 0x4BU, 0x97U, 0xC5U, 0xB3U, 0x51U, 0xF5U, 0x93U, 0x61U, 0x1FU, 0xF1U,
      0x4EU, 0xF4U, 0x0AU, 0x28U, 0x83U, 0x25U, 0x49U, 0xF0U, 0x7AU, 0x76U, 0x4AU, 0x6EU, 0x1CU, 0x47U, 0xC2U, 0x46U,
      0x59U, 0x44U, 0xBCU, 0x3DU, 0x0DU, 0x00U, 0xA9U, 0x12U, 0x24U, 0x65U, 0x8EU, 0x77U, 0x6DU, 0xC3U, 0x30U, 0x2AU,
      0xC0U, 0xA2U, 0xE8U, 0x66U, 0x06U, 0x7EU, 0xF9U, 0xF2U, 0xBDU, 0xD3U, 0x4CU, 0x86U, 0x87U, 0xA6U, 0x5EU, 0xE0U,
      0x73U, 0x94U, 0xA0U, 0x04U, 0xFCU, 0x16U, 0x0FU, 0xCFU, 0x2DU, 0xF6U, 0x35U, 0x6BU, 0xC1U, 0x8BU, 0xD6U, 0x2BU,
      0x34U, 0x37U, 0xFAU, 0x7CU, 0x1AU, 0x15U, 0x98U, 0x78U, 0x7DU, 0xEEU, 0xB6U, 0x68U, 0x95U, 0xFFU, 0xE3U, 0x88U,
      0x90U, 0xACU, 0x19U, 0x33U, 0x3BU, 0x75U, 0x0CU, 0x4FU, 0xCDU, 0xAFU, 0x89U, 0x03U, 0xE7U, 0xD5U, 0x32U, 0x48U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x4F09CEC1u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x78AC79BFu, 7u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x36E8E373u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x46C716C1u, 6u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4113), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4192), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-4494), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-665), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltB = static_cast<std::uint32_t>(pSalt[(i + 12u) & 31U]);
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x78AC79BFu, 7u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x46C716C1u, 6u);
...
static void TwistCandidate_53273_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x2EU, 0x20U, 0xC2U, 0x7DU, 0xCCU, 0xF2U, 0xECU, 0x64U, 0x42U, 0x58U, 0x26U, 0x96U, 0xB4U, 0x45U, 0x9BU, 0x4AU,
      0x8AU, 0x1AU, 0x23U, 0xF9U, 0xDDU, 0x6EU, 0xC9U, 0xA1U, 0xE1U, 0xC8U, 0x33U, 0x48U, 0x72U, 0x1DU, 0x0EU, 0xFAU,
      0xFEU, 0x8FU, 0xAFU, 0xDCU, 0xBBU, 0xD5U, 0x06U, 0xB1U, 0x9CU, 0x77U, 0x08U, 0x3FU, 0x74U, 0xA5U, 0x21U, 0x73U,
      0xDEU, 0x29U, 0x84U, 0x4EU, 0x0AU, 0xD3U, 0x63U, 0x53U, 0x12U, 0x3AU, 0xA0U, 0x05U, 0x34U, 0x9AU, 0x9FU, 0xCEU,
      0x1EU, 0x27U, 0x19U, 0x92U, 0xA7U, 0x9DU, 0x0FU, 0x90U, 0xD9U, 0xA4U, 0x13U, 0x3BU, 0x1CU, 0x6DU, 0xF8U, 0x0BU,
      0xF5U, 0x85U, 0xCFU, 0xF6U, 0x40U, 0xDFU, 0x07U, 0x38U, 0x86U, 0xF4U, 0x0DU, 0x70U, 0x0CU, 0x5EU, 0x76U, 0x81U,
      0x50U, 0x01U, 0xC5U, 0x68U, 0x2BU, 0x4CU, 0xE0U, 0x91U, 0x10U, 0x65U, 0xFFU, 0x71U, 0xD6U, 0x59U, 0x51U, 0x56U,
      0xB9U, 0xB2U, 0x7BU, 0x49U, 0xACU, 0x2DU, 0xC7U, 0x4FU, 0xD2U, 0xABU, 0x57U, 0xEFU, 0x44U, 0x88U, 0x09U, 0x78U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x01DCBDCAu, 0xF06AE1C6u, 6u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-6153), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xF06AE1C6u, 6u);
...
static void TwistCandidate_53273_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_53273_AdvanceTwiddle32A(
      0x2510027Du ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_53273_AdvanceTwiddle32B(
      0x607C8AD7u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_53273_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_53273_AdvanceTwiddle32A(
      0x866F1C09u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[9U]),
      0x44DE7836u ^ static_cast<std::uint32_t>(pSalt[4U]));
  std::uint32_t aPulseB = TwistCandidate_53273_AdvanceTwiddle32B(
      0x941A5EE4u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[4U]),
      0xB3CEE606u ^ static_cast<std::uint32_t>(pSalt[9U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    for (std::size_t aLane = 0; aLane < kMatrixBlockBytes; ++aLane) {
      ...
      const unsigned char aMix = FixedMixBoxByte(static_cast<unsigned char>(
          ...
          pSalt[(aLane + 4U + (aPulseByteA & 3U)) & 31U]));
    ...
    aPulseA = TwistCandidate_53273_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_53273_AdvanceTwiddle32B(
...
static void TwistCandidate_53273_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x08C6B976u ^ (pRound * 5u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-1701), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x08C6B976u ^ (pRound * 5u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (7378), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-836), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (7274), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 38u + 17u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (7274 + 17), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 5u + 3U) & 15U), static_cast<std::size_t>(i + 25u)));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(3u)) ^ static_cast<std::uint32_t>(70u)) & 0xFFu);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(69u)), 0x85EBCA6Bu, 3u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (1257), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-5362), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 124u + 3u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-5362 + 3), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 13u + 3U) & 15U), static_cast<std::size_t>(i + 18u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 21u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>((aLaneIndex ^ static_cast<std::uint32_t>(102u)) & 0xFFu);
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(196u)), 0x85EBCA6Bu, 2u);
    }
  }

  TwistCandidate_53273_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_53273_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_53273_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_53273_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-899), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-1928), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (1635), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 14u + 1082u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatOther = WrapRange(static_cast<int>(aMaskFlat) + (4 + 1), 0, static_cast<int>(kMaskStackTotalBytes));
      const std::uint32_t aMaskByteA = static_cast<std::uint32_t>(pMaskStackA[(aMaskFlat / kMaskBytes) % kMaskStackDepth][aMaskFlat % kMaskBytes]);
  ...
  TwistCandidate_53273_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (-2978), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (-648), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (6046), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (-6506), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_53273_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_53273_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 24u + (7509)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 23u + (-4941)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 19u + (7212)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 24u + static_cast<unsigned int>(15u)) & 31U;
...
static void TwistCandidate_53273_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 21u + (-2774)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 14u + (-5024)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 11u + (-7170)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_53273_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 4u + (3385)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 15u + (-3951)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 28u + (3884)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
void TwistCandidate_53273(
  ...
  TwistCandidate_53273_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_53273_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_53273_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_53273_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_53273_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_53273_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_53273_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_53273_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_53273",
    &TwistCandidate_53273,
    &TwistCandidate_53273_KeySeed,
    &TwistCandidate_53273_SaltSeed,
    &TwistCandidate_53273_MaskSeedA,
    &TwistCandidate_53273_MaskSeedB,
    &TwistCandidate_53273_TwistBlock,
    &TwistCandidate_53273_PushKeyRound,
    &TwistCandidate_53273_PushMaskRoundA,
    &TwistCandidate_53273_PushMaskRoundB,
  }
};
