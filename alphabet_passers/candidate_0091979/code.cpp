...
// Candidate 91980: TwistCandidate_91980
// family=ferocious_dual_worker op_budget=11 worker_shapes=2x1/3x3 worker_reverse=(True, False, True, False)/(True, True, False, True) twiddle1=add/none twiddle2=mix/i_mul twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m3i0v233,aBreakerCD:4m2i3v032,aBreakerEF:4m2i3v330 lane_breaker=true braid_breaker=true jump_breaker=false swap_breaker=true mask_template=0 mask_seed_a=add_wave mask_seed_b=xor_wave lightning=true/3 typhoon=true hurricane=true final_reverse=(True, False, True, False)
// ferocious[wa=2x1; wb=3x3; tw1=add/none; tw2=mix/i_mul; tw2src=B; saltbox=128:000e5e1e:b745f534; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m3i0v233,aBreakerCD:4m2i3v032,aBreakerEF:4m2i3v330; wa_rev=(True, False, True, False); wb_rev=(True, True, False, True); lane=on:2; braid=on:1; jump=off:3; swap=on:2; mask=workerbx0; lightning=on:3; typhoon=on; hurricane=on; final=2/(True, False, True, False); key_rot=7; maskA_seed=add_wave/True; maskB_seed=xor_wave; maskA_bias=24; maskB_bias=130]
static std::uint32_t TwistCandidate_91980_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0xFAD1213Du;
  aState = AdvanceTwiddle32(
      ...
      0x3241995Au,
      11u);
  aState ^= RotateLeft32(pValue + pExtra + 0xAE91E1CDu, 11U);
  return static_cast<std::uint32_t>(aState + 0xC890B867u);
}

static std::uint32_t TwistCandidate_91980_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xF1E4B57Cu;
  aState = AdvanceTwiddle32(
      ...
      0x5F7554B1u,
      10u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x5A4675EEu, 1U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A52C26u);
}

static void TwistCandidate_91980_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_91980_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_91980_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_91980_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_91980_AdvanceTwiddle32B(
...
static void TwistCandidate_91980_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x95U, 0xC2U, 0xF8U, 0xD1U, 0x0FU, 0xC9U, 0xC7U, 0xD4U, 0x03U, 0x84U, 0xFCU, 0x7AU, 0x92U, 0x30U, 0xC4U, 0x55U,
      0x40U, 0x60U, 0x8BU, 0xFDU, 0x98U, 0xC0U, 0xE0U, 0x29U, 0xB8U, 0x6EU, 0x3CU, 0x14U, 0xE9U, 0x0BU, 0xCAU, 0x71U,
      0x2AU, 0x18U, 0xEAU, 0xDAU, 0x9CU, 0x21U, 0x67U, 0x1BU, 0x78U, 0xA1U, 0xEBU, 0xAEU, 0x9FU, 0xEDU, 0x0EU, 0x0DU,
      0xA2U, 0xDCU, 0x11U, 0x48U, 0xBBU, 0x4EU, 0x7BU, 0x25U, 0xEFU, 0x5DU, 0x37U, 0x96U, 0xCFU, 0xB5U, 0x09U, 0x4FU,
      0x97U, 0x5BU, 0xD0U, 0x24U, 0x8CU, 0x3DU, 0x1AU, 0xBEU, 0xA3U, 0xAAU, 0x56U, 0x2DU, 0x65U, 0x2CU, 0x35U, 0x22U,
      0x99U, 0x49U, 0x4BU, 0xDFU, 0x08U, 0x62U, 0xBAU, 0xA8U, 0x70U, 0xF3U, 0x5CU, 0x06U, 0xEEU, 0xB9U, 0x74U, 0x26U,
      0x04U, 0x39U, 0x8AU, 0xFFU, 0xABU, 0xA0U, 0x16U, 0x10U, 0x6FU, 0x3BU, 0x1CU, 0x79U, 0xF9U, 0x1EU, 0x6CU, 0x0AU,
      0x4AU, 0xBDU, 0x58U, 0x5EU, 0xA9U, 0x50U, 0xDBU, 0x59U, 0x28U, 0xE7U, 0x93U, 0xB7U, 0x61U, 0x9DU, 0x1DU, 0x34U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 176u ^ 137u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-478)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 25u + (1631)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 26u + static_cast<unsigned int>(176u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 176u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(137u), 6u, true);
  ...
  TwistCandidate_91980_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(137u), 6u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(137u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 56u + (-6543)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 58u + (7696)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 56u + pSalt[(aSourceIndex + 242u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(242u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(242u), 3u, true);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_91980_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(242u), static_cast<std::uint32_t>(242u), 3u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(242u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_91980_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 29u + (441)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 19u + (6140)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 176u) ^ ((b << 6u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_91980_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 154u ^ 24u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 28u + (4940)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 15u + (-5115)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (28u + 15u) + (4070)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 18u + (-4719)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte + b + static_cast<std::uint32_t>(195u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 28u * 8U + static_cast<std::size_t>(aSourceIndex) * 15u + 154u + static_cast<std::size_t>(aTurbulence * ((18u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(154u), 5u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 195u) * ((18u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 1u, false);
  }
}

static void TwistCandidate_91980_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 29u ^ 130u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 3u + (7656)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 12u + (-66)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (3u + 12u) + (-5754)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 61u + (-1728)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 3u)) ^ static_cast<std::uint32_t>(241u)) & 0xFFu);
    const std::uint32_t aSeedA = static_cast<std::uint32_t>((a ^ aTurbulence) & 0xFFu);
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 3u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 3u * 8U + static_cast<std::size_t>(aSourceIndex) * 12u + 29u + static_cast<std::size_t>(aTurbulence * ((61u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(29u), 4u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 241u) * ((61u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 3u, true);
  }
}

static void TwistCandidate_91980_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x96U, 0xCBU, 0x19U, 0xDDU, 0xA7U, 0xFAU, 0x81U, 0xBAU, 0x4BU, 0x36U, 0xAFU, 0x3AU, 0x76U, 0x6FU, 0xE0U, 0x69U,
      0x73U, 0x65U, 0xD8U, 0x3CU, 0x1BU, 0x12U, 0x01U, 0x43U, 0x6AU, 0xD0U, 0x99U, 0xA8U, 0xF7U, 0xEEU, 0x7DU, 0x08U,
      0xF4U, 0xB4U, 0xC5U, 0xBBU, 0xCCU, 0x80U, 0x16U, 0x83U, 0x58U, 0xB1U, 0xE3U, 0x64U, 0x45U, 0xA9U, 0xF3U, 0x88U,
      0xF5U, 0x77U, 0xC9U, 0x07U, 0x82U, 0x23U, 0x78U, 0x9DU, 0x0BU, 0xA6U, 0x9FU, 0x7FU, 0x9AU, 0xC0U, 0xE8U, 0xDBU,
      0xCDU, 0x79U, 0x40U, 0x49U, 0x8FU, 0xD6U, 0x09U, 0xD4U, 0xEDU, 0x7BU, 0x44U, 0x95U, 0xA2U, 0x13U, 0x86U, 0x5AU,
      0x46U, 0xBEU, 0x04U, 0x3FU, 0xF8U, 0xEBU, 0x8EU, 0xD7U, 0x4CU, 0x61U, 0xC6U, 0xACU, 0x5DU, 0xE6U, 0xE4U, 0x0DU,
      0x62U, 0x93U, 0xDAU, 0xFCU, 0x34U, 0x8AU, 0x11U, 0x90U, 0xB7U, 0xF6U, 0x2BU, 0x57U, 0xE9U, 0x41U, 0x22U, 0xECU,
      0x1DU, 0x15U, 0xC1U, 0x52U, 0x4EU, 0x8BU, 0x31U, 0x32U, 0x5CU, 0xE7U, 0x35U, 0xA3U, 0xDFU, 0x6EU, 0x68U, 0xAEU
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x9657AD08u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x379779ACu, 8u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x2538362Eu ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0xD8A93CDBu, 7u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (5000), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4487), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-5573), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (5404), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltA = static_cast<std::uint32_t>(pSalt[(i + 10u) & 31U]);
    const std::uint32_t aSaltB = static_cast<std::uint32_t>(pSalt[(i + 28u) & 31U]);
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x379779ACu, 8u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0xD8A93CDBu, 7u);
...
static void TwistCandidate_91980_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xCEU, 0x85U, 0xF8U, 0xC8U, 0x8BU, 0x3CU, 0xEAU, 0x06U, 0x2EU, 0xD2U, 0xEFU, 0xE5U, 0x44U, 0x94U, 0x4EU, 0xA0U,
      0x80U, 0x88U, 0x48U, 0x22U, 0x28U, 0xF4U, 0x04U, 0x1DU, 0x27U, 0x54U, 0x0CU, 0xA4U, 0x97U, 0xB4U, 0x59U, 0x79U,
      0xEBU, 0x99U, 0xF5U, 0x9BU, 0x1CU, 0x9CU, 0x9DU, 0x8EU, 0x5DU, 0xC7U, 0x5AU, 0xEDU, 0x4CU, 0x7AU, 0x5CU, 0xE3U,
      0x07U, 0x0FU, 0x68U, 0x33U, 0x6AU, 0x23U, 0x47U, 0x2FU, 0x51U, 0xC1U, 0x8CU, 0xD0U, 0xABU, 0x32U, 0xE1U, 0x35U,
      0xCCU, 0xB5U, 0xEEU, 0x89U, 0x0EU, 0x11U, 0x58U, 0xD5U, 0x91U, 0x76U, 0x70U, 0x37U, 0xDAU, 0x4AU, 0xB7U, 0x2BU,
      0xC3U, 0x77U, 0x7FU, 0x55U, 0xBAU, 0xDBU, 0x25U, 0x5BU, 0x96U, 0xCDU, 0x78U, 0xA2U, 0xCBU, 0x5EU, 0x74U, 0x95U,
      0x2CU, 0x6EU, 0xA9U, 0xE0U, 0x56U, 0x71U, 0xF3U, 0xE2U, 0x3DU, 0x53U, 0x5FU, 0x02U, 0x4DU, 0x0AU, 0xA1U, 0x7CU,
      0x50U, 0x41U, 0x62U, 0x86U, 0x36U, 0x93U, 0x16U, 0x7DU, 0x60U, 0xFBU, 0xB6U, 0x05U, 0xFFU, 0x30U, 0x09U, 0xAFU
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x949E8E41u, 0x4384CF1Du, 7u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4574), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltByte = static_cast<std::uint32_t>(pSalt[(i + 8u) & 31U]);
    const std::uint32_t aMix = static_cast<std::uint32_t>(aMixBox[(aDestByte ^ aSourceByte ^ aSaltByte ^ FoldWordToByte(aAcc)) & 127U]);
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x4384CF1Du, 7u);
...
static void TwistCandidate_91980_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_91980_AdvanceTwiddle32A(
      0x676BA080u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_91980_AdvanceTwiddle32B(
      0xA2DD1A60u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_91980_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_91980_AdvanceTwiddle32A(
      0x0B2C5035u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[21U]),
      0xC9A0E20Bu ^ static_cast<std::uint32_t>(pSalt[27U]));
  std::uint32_t aPulseB = TwistCandidate_91980_AdvanceTwiddle32B(
      0xE510D47Bu + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[27U]),
      0xC2C46F8Cu ^ static_cast<std::uint32_t>(pSalt[21U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    for (std::size_t aLane = 0; aLane < kMatrixBlockBytes; ++aLane) {
      ...
      const unsigned char aWave = FixedMixBoxByte(static_cast<unsigned char>(
          ...
          pSalt[(aLane + 21U + (aPulseByteB & 3U)) & 31U] ^
          aPulseByteA ^
    ...
    aPulseA = TwistCandidate_91980_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_91980_AdvanceTwiddle32B(
...
static void TwistCandidate_91980_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0xFC7699E4u ^ (pRound * 56u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (441), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0xFC7699E4u ^ (pRound * 56u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (6140), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(aReverseIndex + (-494), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (4724), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 139u + 21u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (4724 + 21), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const int aIndex2 = WrapRange(i + (-2036), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(7u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[25u]), 0x27D4EB2Du, 1u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (3194), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (-6524), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 48u + 15u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-6524 + 15), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 3u + 3U) & 15U), static_cast<std::size_t>(i + 14u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 6u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(37u)) ^ static_cast<std::uint32_t>(173u)) & 0xFFu);
      const int aIndex2 = WrapRange(aReverseIndex + (-7351), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(i + (-4103), aStart, aEnd);
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex3]);
      const std::uint32_t aRoute = ((aSourceCtrl >> 1u) ^ aWorkerCtrl ^ aKeyByte ^ aMaskByte ^ static_cast<std::uint32_t>(3u)) % 5U;
      std::uint32_t aValue = a;
      switch (aRoute) {
        case 0U: aValue = a; break;
        case 1U: aValue = b; break;
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB ^ static_cast<std::uint32_t>(pSalt[6u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(198u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 8u);
    }
  }

  TwistCandidate_91980_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_91980_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_91980_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_91980_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-5486), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (6773), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (2511), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 20u + 145u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    auto storm_block = LoadBlock16Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (545), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock16Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (7046), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackA, chunk + 1300u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackB, chunk + 1300u + 17u);
    ...
    switch (static_cast<unsigned>(storm_control[1] ^ storm_mask_a[2] ^ storm_mask_b[3] ^ pSalt[0]) % 5u) {
      case 0u: storm.ApplyFastOp(LightningFastOp::kRotateRowLeft, static_cast<std::uint8_t>(storm_control[3] ^ storm_mask_a[4] ^ storm_mask_b[5]), static_cast<std::uint8_t>(storm_control[5] + storm_mask_a[6] + storm_mask_b[7])); break;
      case 1u: storm.ApplyFastOp(LightningFastOp::kRotateRowRight, static_cast<std::uint8_t>(storm_control[3] ^ storm_mask_a[4] ^ storm_mask_b[5]), static_cast<std::uint8_t>(storm_control[5] + storm_mask_a[6] + storm_mask_b[7])); break;
      case 2u: storm.ApplyFastOp(LightningFastOp::kAddRowIntoRow, static_cast<std::uint8_t>(storm_control[3] ^ storm_mask_a[4] ^ storm_mask_b[5]), static_cast<std::uint8_t>(storm_control[5] + storm_mask_a[6] + storm_mask_b[7])); break;
      case 3u: storm.ApplyFastOp(LightningFastOp::kRotateColumnDown, static_cast<std::uint8_t>(storm_control[3] ^ storm_mask_a[4] ^ storm_mask_b[5]), static_cast<std::uint8_t>(storm_control[5] + storm_mask_a[6] + storm_mask_b[7])); break;
      case 4u: storm.ApplyFastOp(LightningFastOp::kXorRowIntoRow, static_cast<std::uint8_t>(storm_control[3] ^ storm_mask_a[4] ^ storm_mask_b[5]), static_cast<std::uint8_t>(storm_control[5] + storm_mask_a[6] + storm_mask_b[7])); break;
    }
    }
    switch (static_cast<unsigned>(storm_control[7] ^ storm_mask_a[8] ^ storm_mask_b[9] ^ pSalt[1]) % 4u) {
      ...
      case 1u: storm.ApplySlowOp(LightningSlowOp::kRotateRight, static_cast<std::uint8_t>(storm_control[9] + storm_mask_a[10] + storm_mask_b[11]), static_cast<std::uint8_t>(storm_block[11] ^ storm_mask_a[12] ^ storm_mask_b[13])); break;
    ...
    ApplyLightningMixColumns(storm, pSalt, static_cast<std::uint8_t>((storm_control[11] + pRound + 3u) & 31U), static_cast<std::uint8_t>(storm_control[15] ^ storm_mask_a[0] ^ storm_mask_b[1] ^ pSalt[3]), static_cast<std::uint8_t>(3u));
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kTyphoonBlockBytes) {
    auto storm_block = LoadBlock128Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-5888), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock128Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (11), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackA, chunk + 442u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackB, chunk + 442u + 23u);
    ...
    switch (static_cast<unsigned>(storm_control[3] ^ storm_mask_a[11] ^ storm_mask_b[13] ^ pSalt[0]) % 4u) {
      ...
      case 2u: storm.ApplyFastOp(TyphoonFastOp::kRotateColumnDown, static_cast<std::uint8_t>(storm_control[17] ^ storm_mask_a[23] ^ storm_mask_b[29]), static_cast<std::uint8_t>(storm_control[29] + storm_mask_a[31] + storm_mask_b[37])); break;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (3072), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-5431), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 228u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 228u + 29u);
    ...
    switch (static_cast<unsigned>(storm_control[13] ^ storm_mask_a[17] ^ storm_mask_b[19] ^ pSalt[2]) % 6u) {
      ...
      case 2u: storm.ApplyFastOp(HurricaneFastOp::kXorRowIntoRow, static_cast<std::uint8_t>(storm_control[29] ^ storm_mask_a[31] ^ storm_mask_b[37]), static_cast<std::uint8_t>(storm_control[43] + storm_mask_a[47] + storm_mask_b[53])); break;
      ...
      case 5u: storm.ApplyFastOp(HurricaneFastOp::kRotateRowRight, static_cast<std::uint8_t>(storm_control[29] ^ storm_mask_a[31] ^ storm_mask_b[37]), static_cast<std::uint8_t>(storm_control[43] + storm_mask_a[47] + storm_mask_b[53])); break;
    }
    }
    switch (static_cast<unsigned>(storm_control[53] ^ storm_mask_a[59] ^ storm_mask_b[61] ^ pSalt[3]) % 5u) {
      case 0u: storm.ApplySlowOp(HurricaneSlowOp::kFlipHorizontal, static_cast<std::uint8_t>(storm_control[61] + storm_mask_a[67] + storm_mask_b[71]), static_cast<std::uint8_t>(storm_block[71] ^ storm_mask_a[73] ^ storm_mask_b[79])); break;
      case 1u: storm.ApplySlowOp(HurricaneSlowOp::kRotateRight, static_cast<std::uint8_t>(storm_control[61] + storm_mask_a[67] + storm_mask_b[71]), static_cast<std::uint8_t>(storm_block[71] ^ storm_mask_a[73] ^ storm_mask_b[79])); break;
      case 2u: storm.ApplySlowOp(HurricaneSlowOp::kRotateRing, static_cast<std::uint8_t>(storm_control[61] + storm_mask_a[67] + storm_mask_b[71]), static_cast<std::uint8_t>(storm_block[71] ^ storm_mask_a[73] ^ storm_mask_b[79])); break;
      case 3u: storm.ApplySlowOp(HurricaneSlowOp::kFlipDiagB, static_cast<std::uint8_t>(storm_control[61] + storm_mask_a[67] + storm_mask_b[71]), static_cast<std::uint8_t>(storm_block[71] ^ storm_mask_a[73] ^ storm_mask_b[79])); break;
  ...
  TwistCandidate_91980_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (-2807), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-2040), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (4205), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (3090), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_91980_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_91980_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 17u + (-926)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 27u + (-3757)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 21u + (-5159)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 17u + static_cast<unsigned int>(7u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 137u)));
...
static void TwistCandidate_91980_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 7u + (-4222)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 24u + (5980)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 3u + (-5890)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(24u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(24u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_91980_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 22u + (7658)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 15u + (-112)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-4318)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(130u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(130u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_91980(
  ...
  TwistCandidate_91980_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_91980_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_91980_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_91980_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_91980_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_91980_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_91980_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_91980_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_91980",
    &TwistCandidate_91980,
    &TwistCandidate_91980_KeySeed,
    &TwistCandidate_91980_SaltSeed,
    &TwistCandidate_91980_MaskSeedA,
    &TwistCandidate_91980_MaskSeedB,
    &TwistCandidate_91980_TwistBlock,
    &TwistCandidate_91980_PushKeyRound,
    &TwistCandidate_91980_PushMaskRoundA,
    &TwistCandidate_91980_PushMaskRoundB,
  }
};
