...
// Candidate 170475: TwistCandidate_170475
// family=ferocious_dual_worker op_budget=9 worker_shapes=2x4/1x3 worker_reverse=(True, False, True, True)/(False, False, True, True) twiddle1=mix/i twiddle2=mix/i_mul twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m0i3v100,aBreakerCD:4m1i3v033,aBreakerEF:4m1i2v202 lane_breaker=true braid_breaker=true jump_breaker=false swap_breaker=true mask_template=3 mask_seed_a=add_wave mask_seed_b=sbox_wave lightning=false/3 typhoon=true hurricane=false final_reverse=(False, False, True, False)
// ferocious[wa=2x4; wb=1x3; tw1=mix/i; tw2=mix/i_mul; tw2src=B; saltbox=128:000f08d5:2388bcc0; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m0i3v100,aBreakerCD:4m1i3v033,aBreakerEF:4m1i2v202; wa_rev=(True, False, True, True); wb_rev=(False, False, True, True); lane=on:3; braid=on:0; jump=off:2; swap=on:0; mask=workerbx3; lightning=off:3; typhoon=on; hurricane=off; final=3/(False, False, True, False); key_rot=16; maskA_seed=add_wave/True; maskB_seed=sbox_wave; maskA_bias=205; maskB_bias=26]
static std::uint32_t TwistCandidate_170475_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x4ECF89BAu;
  aState = AdvanceTwiddle32(
      ...
      0x3B651763u,
      7u);
  aState ^= RotateLeft32(pValue + pExtra + 0x13AAB8BFu, 10U);
  return static_cast<std::uint32_t>(aState + 0x75AA9ED9u);
}

static std::uint32_t TwistCandidate_170475_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xF8C0CC96u;
  aState = AdvanceTwiddle32(
      ...
      0xEB6A7429u,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x3A39FDE3u, 8U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5DBF5u);
}

static void TwistCandidate_170475_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_170475_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_170475_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_170475_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_170475_AdvanceTwiddle32B(
...
static void TwistCandidate_170475_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x83U, 0xE9U, 0xF6U, 0xB7U, 0xD7U, 0x20U, 0x2DU, 0x40U, 0x10U, 0xA6U, 0xA3U, 0xE1U, 0x63U, 0xEFU, 0x19U, 0x4FU,
      0xA8U, 0xF9U, 0x23U, 0x57U, 0x86U, 0x39U, 0x01U, 0x52U, 0x1AU, 0x61U, 0x84U, 0xF2U, 0xAAU, 0xB8U, 0x90U, 0x5EU,
      0x44U, 0xFDU, 0xCFU, 0x78U, 0xBFU, 0x00U, 0x8DU, 0x9DU, 0x3AU, 0xCAU, 0x99U, 0xE0U, 0x38U, 0xC5U, 0x5FU, 0xB3U,
      0x79U, 0x5AU, 0x46U, 0x95U, 0xC3U, 0xCDU, 0xFAU, 0xDAU, 0x55U, 0x74U, 0xA0U, 0x18U, 0xC9U, 0xE2U, 0x15U, 0xC4U,
      0x16U, 0xFEU, 0x6AU, 0xBDU, 0x3CU, 0x94U, 0x88U, 0x9BU, 0xADU, 0x6CU, 0xBBU, 0x60U, 0xA5U, 0x96U, 0xCEU, 0x1BU,
      0xB2U, 0xD5U, 0x59U, 0x7EU, 0x31U, 0xBCU, 0x4DU, 0x08U, 0xDDU, 0x06U, 0x2FU, 0xE4U, 0xA7U, 0xF8U, 0x04U, 0x4BU,
      0x9CU, 0x8CU, 0x2CU, 0xE6U, 0x9EU, 0x28U, 0x68U, 0x49U, 0xD8U, 0x07U, 0x71U, 0xB9U, 0x37U, 0x7CU, 0x82U, 0x27U,
      0x2AU, 0x35U, 0x02U, 0x81U, 0x24U, 0x1FU, 0x67U, 0x8EU, 0x26U, 0xD3U, 0x4CU, 0x56U, 0xC8U, 0x6BU, 0x1DU, 0xC0U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 198u ^ 5u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 9u + (6129)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 10u + (1928)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 9u + static_cast<unsigned int>(198u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 198u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(5u), 2u, false);
  ...
  TwistCandidate_170475_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(5u), 2u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(5u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 37u + (10190)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 25u + (-2133)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 37u + pSalt[(aSourceIndex + 81u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(81u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(81u), 7u, false);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_170475_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(81u), static_cast<std::uint32_t>(81u), 7u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(81u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_170475_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 7u + (2631)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 28u + (-4816)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 198u) ^ ((b << 2u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_170475_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 215u ^ 205u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 8u + (-1779)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 18u + (-595)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (8u + 18u) + (3729)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 25u + (-6215)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 8u * 8U + static_cast<std::size_t>(aReverseIndex) * 18u + 215u + static_cast<std::size_t>(aTurbulence * ((25u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(215u), 3u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 183u) * ((25u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
  ...
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 28u + (-1779)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-595)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (28u + 31u) + (3729)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aTurbulence & 0xFFu), 6u))) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c + static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 6u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 28u * 8U + static_cast<std::size_t>(aSourceIndex) * 31u * 3U + 220u + static_cast<std::size_t>(aTurbulence * ((25u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 4u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(220u), 4u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 183u) * ((25u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 4u, true);
  }
}

static void TwistCandidate_170475_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 167u ^ 26u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 15u + (-2581)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 10u + (6704)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (15u + 10u) + (-1734)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 57u + (-3456)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>(aMixBox[(aWorkerByte ^ a ^ static_cast<std::uint32_t>(195u)) & 0xFFu]);
    const std::uint32_t aSeedA = static_cast<std::uint32_t>((a ^ aTurbulence) & 0xFFu);
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(195u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 15u * 8U + static_cast<std::size_t>(aSourceIndex) * 10u + 167u + static_cast<std::size_t>(aTurbulence * ((57u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(167u), 3u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 195u) * ((57u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 4u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 43u + (-2581)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 31u + (6704)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (43u + 31u) + (-1734)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 57u + (-3456)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(195u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 43u * 8U + static_cast<std::size_t>(aSourceIndex) * 31u * 3U + 138u + static_cast<std::size_t>(aTurbulence * ((57u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 6u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(138u), 6u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 195u) * ((57u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 6u, true);
  }
}

static void TwistCandidate_170475_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xF3U, 0x85U, 0xB2U, 0x20U, 0xD2U, 0x9AU, 0x7BU, 0x10U, 0x71U, 0x31U, 0xEBU, 0x26U, 0x1DU, 0x08U, 0x3FU, 0x3AU,
      0x74U, 0x03U, 0x68U, 0x98U, 0x50U, 0x5BU, 0xB5U, 0xEDU, 0x2CU, 0x1AU, 0x36U, 0x01U, 0xC2U, 0x48U, 0xC5U, 0xB8U,
      0xC9U, 0x2AU, 0x3BU, 0xECU, 0x0EU, 0xFFU, 0xB1U, 0x5FU, 0xC7U, 0x69U, 0x81U, 0xB9U, 0x75U, 0x5DU, 0x88U, 0x7EU,

  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x49A10186u, 0xB733459Fu, 1u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-5768), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltByte = static_cast<std::uint32_t>(pSalt[(i + 30u) & 31U]);
    const std::uint32_t aMix = static_cast<std::uint32_t>(aMixBox[(aDestByte ^ aSourceByte ^ aSaltByte ^ FoldWordToByte(aAcc)) & 127U]);
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xB733459Fu, 1u);
...
static void TwistCandidate_170475_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_170475_AdvanceTwiddle32A(
      0xFB5F7CBFu ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_170475_AdvanceTwiddle32B(
      0xB843FD9Du ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_170475_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_170475_AdvanceTwiddle32A(
      0xAA43DD68u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[8U]),
      0x68F2155Cu ^ static_cast<std::uint32_t>(pSalt[14U]));
  std::uint32_t aPulseB = TwistCandidate_170475_AdvanceTwiddle32B(
      0xDA64D66Eu + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[14U]),
      0xFDB04D66u ^ static_cast<std::uint32_t>(pSalt[8U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_170475_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_170475_AdvanceTwiddle32B(
...
static void TwistCandidate_170475_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x345C2876u ^ (pRound * 27u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (2631), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x345C2876u ^ (pRound * 27u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-4816), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(aReverseIndex + (5352), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (6147), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 160u + 11u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (6147 + 11), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 4u + 3U) & 15U), static_cast<std::size_t>(i + 24u)));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>((aLaneIndex ^ static_cast<std::uint32_t>(120u)) & 0xFFu);
      const int aIndex2 = WrapRange(i + (-4446), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA ^ static_cast<std::uint32_t>(pSalt[24u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(200u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 2u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (-6000), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (-2366), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 90u + 14u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-2366 + 14), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 1u + 3U) & 15U), static_cast<std::size_t>(i + 15u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 27u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(3u)) ^ static_cast<std::uint32_t>(254u)) & 0xFFu);
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB ^ static_cast<std::uint32_t>(pSalt[27u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(50u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 7u);
    }
  }

  TwistCandidate_170475_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_170475_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_170475_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_170475_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (2008), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (1086), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (5171), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 27u + 856u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kTyphoonBlockBytes) {
    auto storm_block = LoadBlock128Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-6514), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock128Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (1054), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackA, chunk + 358u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackB, chunk + 358u + 23u);
    ...
    switch (static_cast<unsigned>(storm_control[3] ^ storm_mask_a[11] ^ storm_mask_b[13] ^ pSalt[0]) % 4u) {
      ...
      case 2u: storm.ApplyFastOp(TyphoonFastOp::kRotateColumnUp, static_cast<std::uint8_t>(storm_control[17] ^ storm_mask_a[23] ^ storm_mask_b[29]), static_cast<std::uint8_t>(storm_control[29] + storm_mask_a[31] + storm_mask_b[37])); break;
  ...
  TwistCandidate_170475_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (1577), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (7245), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (3461), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (-4018), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_170475_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_170475_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 1u + (-6165)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-683)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 19u + (4849)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 1u + static_cast<unsigned int>(16u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 5u)));
...
static void TwistCandidate_170475_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 18u + (7447)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 5u + (-1084)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-1552)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(205u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(205u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_170475_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 14u + (4115)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 9u + (-6598)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 12u + (2573)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(26u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(26u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_170475(
  ...
  TwistCandidate_170475_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_170475_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_170475_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_170475_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_170475_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_170475_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_170475_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_170475_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_170475",
    &TwistCandidate_170475,
    &TwistCandidate_170475_KeySeed,
    &TwistCandidate_170475_SaltSeed,
    &TwistCandidate_170475_MaskSeedA,
    &TwistCandidate_170475_MaskSeedB,
    &TwistCandidate_170475_TwistBlock,
    &TwistCandidate_170475_PushKeyRound,
    &TwistCandidate_170475_PushMaskRoundA,
    &TwistCandidate_170475_PushMaskRoundB,
  }
};
