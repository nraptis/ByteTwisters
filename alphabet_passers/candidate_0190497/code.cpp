...
// Candidate 190498: TwistCandidate_190498
// family=ferocious_dual_worker op_budget=8 worker_shapes=3x2/2x2 worker_reverse=(True, False, True, True)/(False, True, True, True) twiddle1=mix/none twiddle2=xor/none twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m2i0v310,aBreakerCD:4m3i2v321,aBreakerEF:4m0i3v323 lane_breaker=true braid_breaker=true jump_breaker=true swap_breaker=false mask_template=1 mask_seed_a=off mask_seed_b=off lightning=false/3 typhoon=false hurricane=false final_reverse=(True, False, True, True)
// ferocious[wa=3x2; wb=2x2; tw1=mix/none; tw2=xor/none; tw2src=B; saltbox=128:00109018:34db1750; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m2i0v310,aBreakerCD:4m3i2v321,aBreakerEF:4m0i3v323; wa_rev=(True, False, True, True); wb_rev=(False, True, True, True); lane=on:3; braid=on:1; jump=on:0; swap=off:3; mask=workerbx1; lightning=off:3; typhoon=off; hurricane=off; final=4/(True, False, True, True); key_rot=31; maskA_seed=off/True; maskB_seed=off; maskA_bias=140; maskB_bias=50]
static std::uint32_t TwistCandidate_190498_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0xF3CDA9B1u;
  aState = AdvanceTwiddle32(
      ...
      0x7D48AF19u,
      8u);
  aState ^= RotateLeft32(pValue + pExtra + 0xE8858CA0u, 10U);
  return static_cast<std::uint32_t>(aState + 0x8E8506A8u);
}

static std::uint32_t TwistCandidate_190498_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xBEED50BAu;
  aState = AdvanceTwiddle32(
      ...
      0x5668DC1Au,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x4A3685CBu, 9U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5FFA3u);
}

static void TwistCandidate_190498_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_190498_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_190498_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_190498_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_190498_AdvanceTwiddle32B(
...
static void TwistCandidate_190498_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x94U, 0xA6U, 0x9BU, 0x6EU, 0x14U, 0x58U, 0xFBU, 0xEDU, 0x6BU, 0x1FU, 0xC8U, 0xC6U, 0x56U, 0x03U, 0xE9U, 0xCFU,
      0x1CU, 0xC5U, 0x59U, 0x83U, 0x5AU, 0x33U, 0x20U, 0xC0U, 0x93U, 0x10U, 0xD2U, 0x15U, 0xB6U, 0xFCU, 0x25U, 0x12U,
      0xA0U, 0x73U, 0x8AU, 0x0CU, 0x71U, 0x09U, 0x4CU, 0x07U, 0xF5U, 0xACU, 0x9EU, 0x4BU, 0x41U, 0x05U, 0x82U, 0x8BU,
      0x4DU, 0x18U, 0x00U, 0x5EU, 0x1AU, 0xC2U, 0x61U, 0xEFU, 0x75U, 0x8EU, 0xE6U, 0x29U, 0xF7U, 0x64U, 0x88U, 0xF6U,
      0x99U, 0xADU, 0xD6U, 0xFFU, 0xB0U, 0x57U, 0x23U, 0xF9U, 0x3BU, 0x06U, 0x37U, 0x7DU, 0xE2U, 0x4AU, 0x49U, 0xF4U,
      0x1BU, 0x7CU, 0x9CU, 0xF8U, 0x24U, 0x9AU, 0xF2U, 0x91U, 0x28U, 0x63U, 0x4FU, 0xB7U, 0x27U, 0xA9U, 0x19U, 0xC4U,
      0x98U, 0xAEU, 0xCAU, 0x1DU, 0xFAU, 0xABU, 0x54U, 0xF1U, 0x2CU, 0xB9U, 0xEAU, 0x0EU, 0x5DU, 0x8FU, 0x65U, 0x0DU,
      0xDEU, 0x7FU, 0x69U, 0xBAU, 0xF3U, 0xB1U, 0x76U, 0x7EU, 0x51U, 0xBCU, 0x70U, 0x2DU, 0x90U, 0xC3U, 0x62U, 0xF0U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 144u ^ 119u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-3925)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 6u + (-2849)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 26u + static_cast<unsigned int>(144u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 144u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(119u), 3u, false);
  ...
  TwistCandidate_190498_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(119u), 3u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(119u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 44u + (-5290)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 51u + (-1484)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 44u + pSalt[(aSourceIndex + 137u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(137u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(137u), 2u, false);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_190498_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(137u), static_cast<std::uint32_t>(137u), 2u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(137u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_190498_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 18u + (-3618)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 22u + (7191)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 144u) ^ ((b << 3u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_190498_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 108u ^ 140u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-2770)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 11u + (7139)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (26u + 11u) + (3997)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 26u * 8U + static_cast<std::size_t>(aSourceIndex) * 11u + 108u + static_cast<std::size_t>(aTurbulence * ((28u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(108u), 1u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 19u + (-2770)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 27u + (7139)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (19u + 27u) + (3997)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 19u * 8U + static_cast<std::size_t>(aReverseIndex) * 27u * 3U + 51u + static_cast<std::size_t>(aTurbulence * ((28u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 7u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(51u), 7u, false);
  }
}

static void TwistCandidate_190498_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 77u ^ 50u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 3u + (324)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 5u + (-1763)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (3u + 5u) + (-7134)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const std::uint32_t aWorkerByte = 0U;
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 3u * 8U + static_cast<std::size_t>(aSourceIndex) * 5u + 77u + static_cast<std::size_t>(aTurbulence * ((6u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(77u), 2u, true);
  }
}

static void TwistCandidate_190498_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x27U, 0x6AU, 0x7FU, 0x28U, 0x02U, 0xACU, 0x79U, 0x04U, 0xE6U, 0x59U, 0x9AU, 0xB4U, 0x67U, 0x88U, 0xD3U, 0x8CU,
      0x12U, 0x81U, 0xF2U, 0x5FU, 0x89U, 0x76U, 0xD1U, 0xB3U, 0x42U, 0x93U, 0x3EU, 0xEFU, 0x4DU, 0x3DU, 0x5CU, 0x56U,
      0xA2U, 0x29U, 0xE4U, 0xC5U, 0xA8U, 0xF6U, 0x48U, 0x3FU, 0xA5U, 0x40U, 0x98U, 0xA9U, 0xE0U, 0x0CU, 0x91U, 0xFBU,
      0x23U, 0x7AU, 0xBBU, 0x34U, 0x73U, 0xB1U, 0xF0U, 0xD0U, 0x47U, 0x09U, 0x97U, 0xF5U, 0x7EU, 0xE8U, 0xD8U, 0x0AU,
      0x77U, 0x19U, 0x4EU, 0xFDU, 0x49U, 0xFAU, 0xB5U, 0x61U, 0xE3U, 0x1CU, 0x9DU, 0x11U, 0xDBU, 0x71U, 0x6DU, 0x45U,
      0xC4U, 0x1BU, 0x58U, 0xA1U, 0xCEU, 0x2DU, 0x92U, 0x3BU, 0x74U, 0x75U, 0x2EU, 0x6EU, 0x2AU, 0xB9U, 0xE5U, 0x54U,
      0xA7U, 0xD2U, 0x36U, 0xD4U, 0xEDU, 0x21U, 0x4BU, 0x16U, 0x0DU, 0xF9U, 0x2FU, 0x84U, 0xF1U, 0x0FU, 0x1FU, 0x46U,
      0x44U, 0x9BU, 0x57U, 0x95U, 0x7CU, 0x07U, 0xD6U, 0x8FU, 0xC6U, 0x2BU, 0x66U, 0x6FU, 0xF8U, 0xF3U, 0x37U, 0xEEU
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0xEEFA2D3Au ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0xB5ECBD7Au, 8u);
  std::uint32_t aAccB = AdvanceTwiddle32(0xF0DD4540u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0xD72A9533u, 5u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (7612), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (5968), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-3436), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (3683), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltB = static_cast<std::uint32_t>(pSalt[(i + 21u) & 31U]);
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0xB5ECBD7Au, 8u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0xD72A9533u, 5u);
...
static void TwistCandidate_190498_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xDCU, 0xB3U, 0x76U, 0x30U, 0x23U, 0x68U, 0xFBU, 0xBCU, 0x97U, 0x4BU, 0xB0U, 0xEFU, 0x22U, 0x10U, 0x7DU, 0xF3U,
      0x63U, 0x82U, 0x0AU, 0x7AU, 0x46U, 0x9AU, 0xD9U, 0x7BU, 0x7CU, 0x0BU, 0xE7U, 0x1AU, 0x2BU, 0x90U, 0x3EU, 0xDDU,
      0xB1U, 0x7EU, 0xC7U, 0x64U, 0x4DU, 0x91U, 0xDAU, 0x02U, 0xA6U, 0x6DU, 0xCBU, 0x2CU, 0xE0U, 0x5DU, 0x15U, 0x94U,
      0x0DU, 0x0EU, 0x52U, 0x72U, 0x6AU, 0x1DU, 0xADU, 0x28U, 0xAAU, 0x9DU, 0x37U, 0x48U, 0x05U, 0x98U, 0x83U, 0xBBU,
      0x0FU, 0x89U, 0x35U, 0xBFU, 0xDEU, 0x69U, 0xF8U, 0x18U, 0x5EU, 0xD1U, 0x8EU, 0x1FU, 0xB2U, 0x5CU, 0xA0U, 0xC0U,
      0x73U, 0xE5U, 0x17U, 0x61U, 0xFCU, 0x16U, 0xF5U, 0xE4U, 0xF4U, 0x8FU, 0x86U, 0x9FU, 0xF2U, 0x54U, 0x3DU, 0xEEU,
      0x99U, 0x8BU, 0x1EU, 0x6EU, 0x47U, 0x65U, 0x58U, 0xCEU, 0xD5U, 0xD2U, 0x51U, 0xB9U, 0xECU, 0xB5U, 0xC6U, 0xE9U,
      0x1CU, 0x2EU, 0xA5U, 0x39U, 0x33U, 0x5BU, 0x66U, 0xD7U, 0xD0U, 0x44U, 0x88U, 0x81U, 0x74U, 0xD4U, 0x3BU, 0x93U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0xAC783A65u, 0xF4FBAC60u, 1u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4784), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xF4FBAC60u, 1u);
...
static void TwistCandidate_190498_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_190498_AdvanceTwiddle32A(
      0x9E5EE3FEu ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_190498_AdvanceTwiddle32B(
      0xC596E492u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_190498_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_190498_AdvanceTwiddle32A(
      0xEBE055E5u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[5U]),
      0x29540DDAu ^ static_cast<std::uint32_t>(pSalt[18U]));
  std::uint32_t aPulseB = TwistCandidate_190498_AdvanceTwiddle32B(
      0x204F6DD2u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[18U]),
      0x079B9F29u ^ static_cast<std::uint32_t>(pSalt[5U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_190498_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_190498_AdvanceTwiddle32B(
...
static void TwistCandidate_190498_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0xF8EFF6A0u ^ (pRound * 14u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-3618), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0xF8EFF6A0u ^ (pRound * 14u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (7191), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(aReverseIndex + (2011), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (1376), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 248u + 17u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (1376 + 17), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 15u + 3U) & 15U), static_cast<std::size_t>(i + 14u)));
      ...
      const int aIndex2 = WrapRange(i + (-4530), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(aReverseIndex + (-1302), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA ^ static_cast<std::uint32_t>(pSalt[12u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(50u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 5u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (7356), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (4836), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 19u + 1u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (4836 + 1), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 5u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = 0U;
      const int aIndex2 = WrapRange(aReverseIndex + (-7064), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(12u)), 0x85EBCA6Bu, 7u);
    }
  }

  TwistCandidate_190498_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_190498_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_190498_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_190498_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (5342), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-4382), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (574), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 7u + 792u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  TwistCandidate_190498_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (631), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-6516), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (815), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (2370), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_190498_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_190498_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 8u + (-2996)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 31u + (3752)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 6u + (6967)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 8u + static_cast<unsigned int>(31u)) & 31U;
...
static void TwistCandidate_190498_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 9u + (6487)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 11u + (-6869)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 1u + (5240)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_190498_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 23u + (-2521)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 23u + (-2458)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 13u + (-4578)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(50u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(50u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_190498(
  ...
  TwistCandidate_190498_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_190498_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_190498_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_190498_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_190498_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_190498_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_190498_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_190498_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_190498",
    &TwistCandidate_190498,
    &TwistCandidate_190498_KeySeed,
    &TwistCandidate_190498_SaltSeed,
    &TwistCandidate_190498_MaskSeedA,
    &TwistCandidate_190498_MaskSeedB,
    &TwistCandidate_190498_TwistBlock,
    &TwistCandidate_190498_PushKeyRound,
    &TwistCandidate_190498_PushMaskRoundA,
    &TwistCandidate_190498_PushMaskRoundB,
  }
};
