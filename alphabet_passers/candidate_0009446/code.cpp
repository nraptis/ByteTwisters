...
// Candidate 9447: TwistCandidate_9447
// family=ferocious_dual_worker op_budget=11 worker_shapes=1x2/1x3 worker_reverse=(False, False, True, True)/(False, True, False, False) twiddle1=add/i_mul twiddle2=mix/i twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m3i1v330,aBreakerCD:4m1i3v011,aBreakerEF:4m1i1v132 lane_breaker=true braid_breaker=true jump_breaker=true swap_breaker=true mask_template=1 mask_seed_a=xor_wave mask_seed_b=xor_wave lightning=true/1 typhoon=false hurricane=true final_reverse=(False, False, True, False)
// ferocious[wa=1x2; wb=1x3; tw1=add/i_mul; tw2=mix/i; tw2src=B; saltbox=128:001097e4:8d381b4a; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m3i1v330,aBreakerCD:4m1i3v011,aBreakerEF:4m1i1v132; wa_rev=(False, False, True, True); wb_rev=(False, True, False, False); lane=on:2; braid=on:2; jump=on:0; swap=on:0; mask=workerax1; lightning=on:1; typhoon=off; hurricane=on; final=3/(False, False, True, False); key_rot=14; maskA_seed=xor_wave/False; maskB_seed=xor_wave; maskA_bias=214; maskB_bias=62]
static std::uint32_t TwistCandidate_9447_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x3F6DCDCEu;
  aState = AdvanceTwiddle32(
      ...
      0x5F0DD7D9u,
      11u);
  aState ^= RotateLeft32(pValue + pExtra + 0x06611616u, 10U);
  return static_cast<std::uint32_t>(aState + 0x60601A17u);
}

static std::uint32_t TwistCandidate_9447_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x9CA87664u;
  aState = AdvanceTwiddle32(
      ...
      0x9AC96072u,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xDA372E40u, 1U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5A1BDu);
}

static void TwistCandidate_9447_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_9447_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_9447_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_9447_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_9447_AdvanceTwiddle32B(
...
static void TwistCandidate_9447_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xA5U, 0xCFU, 0x35U, 0x74U, 0xC8U, 0xCAU, 0x85U, 0x6EU, 0x77U, 0x45U, 0x33U, 0xC1U, 0x7AU, 0x16U, 0xC3U, 0xF1U,
      0x03U, 0xAAU, 0xFAU, 0x79U, 0x21U, 0xFEU, 0xA4U, 0xD9U, 0x1BU, 0x84U, 0xA1U, 0x25U, 0xF3U, 0x46U, 0xAFU, 0x24U,
      0x4FU, 0x7FU, 0xF6U, 0xAEU, 0x28U, 0x02U, 0xBEU, 0x72U, 0x87U, 0xECU, 0xC0U, 0x76U, 0x31U, 0x2DU, 0x62U, 0xCCU,
      0xE4U, 0x3AU, 0x8EU, 0xC2U, 0x49U, 0x05U, 0xE8U, 0xC5U, 0x09U, 0x4EU, 0x0DU, 0x88U, 0x95U, 0xF7U, 0x4CU, 0x2CU,
      0x83U, 0xDFU, 0xD4U, 0x81U, 0xBDU, 0x91U, 0x63U, 0xB4U, 0x57U, 0x97U, 0xA9U, 0x64U, 0x53U, 0x51U, 0xA3U, 0x50U,
      0xBBU, 0x44U, 0x9DU, 0x8FU, 0xD5U, 0xEDU, 0xEBU, 0xA0U, 0x3EU, 0x6AU, 0x15U, 0xF5U, 0x55U, 0x17U, 0x0EU, 0xE1U,
      0x27U, 0x90U, 0xBAU, 0x59U, 0x19U, 0x1FU, 0xC6U, 0xF9U, 0x30U, 0x82U, 0xE3U, 0x67U, 0x8AU, 0x65U, 0xADU, 0x6FU,
      0x22U, 0x40U, 0x9CU, 0x61U, 0xE5U, 0xF8U, 0xB7U, 0x11U, 0xA6U, 0x8BU, 0xFCU, 0x3CU, 0xE7U, 0x70U, 0xEFU, 0x0AU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 61u ^ 38u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 31u + (3148)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 7u + (515)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 31u + static_cast<unsigned int>(61u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 61u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(38u), 6u, false);
  ...
  TwistCandidate_9447_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(38u), 6u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(38u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 51u + (7270)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 33u + (-3607)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 51u + pSalt[(aSourceIndex + 43u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(43u), 7u, false);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_9447_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(43u), static_cast<std::uint32_t>(43u), 7u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
...
static void TwistCandidate_9447_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 3u + (291)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 5u + (-1435)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 61u) ^ ((b << 6u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_9447_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 30u ^ 214u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 29u + (2143)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 18u + (7502)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (29u + 18u) + (3111)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 16u + (-392)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 29u * 8U + static_cast<std::size_t>(aReverseIndex) * 18u + 30u + static_cast<std::size_t>(aTurbulence * ((16u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(30u), 3u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 221u) * ((16u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
  ...
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 41u + (2143)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 20u + (7502)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (41u + 20u) + (3111)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 2u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 41u * 8U + static_cast<std::size_t>(aReverseIndex) * 20u * 3U + 120u + static_cast<std::size_t>(aTurbulence * ((16u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 6u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(120u), 6u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 221u) * ((16u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 6u, true);
  }
}

static void TwistCandidate_9447_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 140u ^ 62u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 29u + (-6416)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 20u + (5294)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (29u + 20u) + (-6643)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 22u + (2364)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 5u)) ^ static_cast<std::uint32_t>(60u)) & 0xFFu);
    const std::uint32_t aSeedA = static_cast<std::uint32_t>((a ^ aTurbulence) & 0xFFu);
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 29u * 8U + static_cast<std::size_t>(aReverseIndex) * 20u + 140u + static_cast<std::size_t>(aTurbulence * ((22u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(140u), 3u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 60u) * ((22u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 5u, true);
  }
}

static void TwistCandidate_9447_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x38U, 0x55U, 0x0EU, 0x9DU, 0xCCU, 0x9CU, 0x78U, 0xFCU, 0x6BU, 0x80U, 0xAEU, 0x18U, 0x8AU, 0x02U, 0xFAU, 0xC3U,
      0xE2U, 0x2BU, 0xB6U, 0x5FU, 0xA8U, 0xCEU, 0xEDU, 0x30U, 0x69U, 0xDFU, 0xAAU, 0x14U, 0xD0U, 0xF1U, 0x48U, 0x06U,
      0x57U, 0x17U, 0xA7U, 0xFBU, 0xACU, 0x07U, 0x7DU, 0x00U, 0xF4U, 0x1DU, 0x36U, 0x66U, 0x7FU, 0x6FU, 0x45U, 0x22U,
      0x67U, 0x3BU, 0xADU, 0x6EU, 0xE1U, 0xBEU, 0x34U, 0x41U, 0x39U, 0x4CU, 0x88U, 0x23U, 0x43U, 0xDDU, 0x89U, 0xC4U,
      0xD8U, 0xBAU, 0x03U, 0x6AU, 0x5AU, 0x13U, 0x5BU, 0x11U, 0x8FU, 0x1CU, 0xE3U, 0xBCU, 0x47U, 0xFDU, 0x71U, 0x9EU,
      0x76U, 0xD5U, 0xA9U, 0x20U, 0xA4U, 0xB4U, 0xDCU, 0x3EU, 0x85U, 0x2AU, 0x2CU, 0x27U, 0xE5U, 0xCDU, 0x6DU, 0xB0U,
      0xC7U, 0xD3U, 0x15U, 0xC6U, 0x92U, 0xECU, 0x56U, 0xE4U, 0x10U, 0xCBU, 0x70U, 0x29U, 0x96U, 0x8DU, 0xF8U, 0x9BU,
      0x12U, 0x63U, 0x64U, 0xE7U, 0x65U, 0x4AU, 0x82U, 0x8CU, 0xC8U, 0x81U, 0x28U, 0xEAU, 0x31U, 0xF6U, 0x8EU, 0xBDU
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0xFBB6DF86u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x744870D4u, 5u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x8D47F07Du ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0xAC7B5589u, 3u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (950), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-1690), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-1458), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (1060), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x744870D4u, 5u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0xAC7B5589u, 3u);
...
static void TwistCandidate_9447_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x44U, 0x9DU, 0xE5U, 0xB9U, 0x22U, 0x2DU, 0x21U, 0xA2U, 0x6FU, 0x29U, 0xB1U, 0xF0U, 0xB5U, 0xDCU, 0x3EU, 0x8BU,
      0xF1U, 0x6DU, 0xF9U, 0xBCU, 0x68U, 0x62U, 0x1BU, 0xBAU, 0x17U, 0xAEU, 0xFAU, 0x73U, 0x02U, 0xA4U, 0x37U, 0x84U,
      0xD5U, 0x60U, 0xE9U, 0xA0U, 0x92U, 0x10U, 0xC3U, 0xE8U, 0x1CU, 0x30U, 0xFCU, 0x90U, 0x35U, 0xC5U, 0x2EU, 0xF3U,
      0xA8U, 0x8DU, 0xCEU, 0x38U, 0x78U, 0x57U, 0x65U, 0x9AU, 0xFEU, 0xB4U, 0x4AU, 0x7AU, 0x58U, 0x1FU, 0xF2U, 0x94U,
      0x3DU, 0xB2U, 0xD8U, 0x71U, 0x7BU, 0x53U, 0x95U, 0xEAU, 0x70U, 0x4CU, 0xA5U, 0xE1U, 0x6EU, 0x2BU, 0x36U, 0xAFU,
      0xEBU, 0x64U, 0x75U, 0xE3U, 0xD9U, 0xDEU, 0xA1U, 0xCFU, 0x1DU, 0x86U, 0xADU, 0x39U, 0xCDU, 0x8AU, 0x06U, 0xC6U,
      0x9EU, 0x76U, 0x98U, 0xC0U, 0x2AU, 0x97U, 0x05U, 0xDBU, 0xF5U, 0xC4U, 0x0CU, 0x19U, 0xFFU, 0x03U, 0x24U, 0xECU,
      0x8FU, 0x74U, 0x5AU, 0xCBU, 0x0FU, 0xB7U, 0x82U, 0x87U, 0xC7U, 0x11U, 0x3CU, 0xC2U, 0x93U, 0x96U, 0x99U, 0x59U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x2D1EF953u, 0x8AD060ADu, 4u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (3342), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltByte = static_cast<std::uint32_t>(pSalt[(i + 7u) & 31U]);
    const std::uint32_t aMix = static_cast<std::uint32_t>(aMixBox[(aDestByte ^ aSourceByte ^ aSaltByte ^ FoldWordToByte(aAcc)) & 127U]);
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x8AD060ADu, 4u);
...
static void TwistCandidate_9447_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_9447_AdvanceTwiddle32A(
      0x326DE883u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_9447_AdvanceTwiddle32B(
      0xC83B8C29u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_9447_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_9447_AdvanceTwiddle32A(
      0xFDB47584u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[4U]),
      0x3F0731C0u ^ static_cast<std::uint32_t>(pSalt[1U]));
  std::uint32_t aPulseB = TwistCandidate_9447_AdvanceTwiddle32B(
      0x8E88EB21u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[1U]),
      0xA95C3041u ^ static_cast<std::uint32_t>(pSalt[4U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_9447_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_9447_AdvanceTwiddle32B(
...
static void TwistCandidate_9447_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0xF4837581u ^ (pRound * 2u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (291), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0xF4837581u ^ (pRound * 2u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-1435), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-3977), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (-577), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 153u + 18u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-577 + 18), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 4u + 3U) & 15U), static_cast<std::size_t>(i + 22u)));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(17u)) ^ static_cast<std::uint32_t>(106u)) & 0xFFu);
      const std::uint32_t aRoute = ((aSourceCtrl >> 4u) ^ aWorkerCtrl ^ aKeyByte) & 3U;
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(238u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[19u]), 0x27D4EB2Du, 1u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (1465), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-1455), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 226u + 21u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-1455 + 21), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 7u + 3U) & 15U), static_cast<std::size_t>(i + 7u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 1u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>((aLaneIndex ^ static_cast<std::uint32_t>(90u)) & 0xFFu);
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB ^ static_cast<std::uint32_t>(pSalt[1u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(206u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 1u);
    }
  }

  TwistCandidate_9447_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_9447_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_9447_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    const DualWorkerBreakerRecipe4 aBreakerEF{
        ...
        6283,
        7308,
        static_cast<std::size_t>(1232u),
        static_cast<std::uint8_t>(14u),
        static_cast<std::uint8_t>(4u),
        static_cast<std::uint8_t>(29u),
        static_cast<std::uint8_t>(10u),
        static_cast<std::uint8_t>(15u),
        static_cast<std::uint8_t>(62u),
        static_cast<std::uint8_t>(168u),
    ...
    TwistCandidate_9447_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-3218), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (4455), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (6414), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 6u + 619u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatOther = WrapRange(static_cast<int>(aMaskFlat) + (5 + 1), 0, static_cast<int>(kMaskStackTotalBytes));
      const std::uint32_t aMaskByteA = static_cast<std::uint32_t>(pMaskStackA[(aMaskFlat / kMaskBytes) % kMaskStackDepth][aMaskFlat % kMaskBytes]);
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    auto storm_block = LoadBlock16Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-4710), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock16Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (2862), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackA, chunk + 1079u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackB, chunk + 1079u + 17u);
    ...
    storm.InjectAdd(pSalt, kSaltBytes, 26u);
    ...
    switch (static_cast<unsigned>(storm_control[1] ^ storm_mask_a[2] ^ storm_mask_b[3] ^ pSalt[0]) % 4u) {
      case 0u: storm.ApplyFastOp(LightningFastOp::kRotateRowRight, static_cast<std::uint8_t>(storm_control[3] ^ storm_mask_a[4] ^ storm_mask_b[5]), static_cast<std::uint8_t>(storm_control[5] + storm_mask_a[6] + storm_mask_b[7])); break;
    ...
    switch (static_cast<unsigned>(storm_control[7] ^ storm_mask_a[8] ^ storm_mask_b[9] ^ pSalt[1]) % 2u) {
      ...
      case 1u: storm.ApplySlowOp(LightningSlowOp::kRotateLeft, static_cast<std::uint8_t>(storm_control[9] + storm_mask_a[10] + storm_mask_b[11]), static_cast<std::uint8_t>(storm_block[11] ^ storm_mask_a[12] ^ storm_mask_b[13])); break;
    }
    ...
    ApplyLightningMixColumns(storm, pSalt, static_cast<std::uint8_t>((storm_control[11] + pRound + 26u) & 31U), static_cast<std::uint8_t>(storm_control[15] ^ storm_mask_a[0] ^ storm_mask_b[1] ^ pSalt[3]), static_cast<std::uint8_t>(1u));
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-6441), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-3476), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 788u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 788u + 29u);
  ...
  TwistCandidate_9447_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (-2389), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (6283), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (-6600), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (3278), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_9447_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_9447_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 13u + (-456)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 7u + (-921)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 25u + (-3747)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 13u + static_cast<unsigned int>(14u)) & 31U;
...
static void TwistCandidate_9447_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 16u + (4959)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 22u + (238)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 7u + (-7528)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(214u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(214u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_9447_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 4u + (993)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-697)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 19u + (5767)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
void TwistCandidate_9447(
  ...
  TwistCandidate_9447_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_9447_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_9447_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_9447_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_9447_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_9447_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_9447_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_9447_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_9447",
    &TwistCandidate_9447,
    &TwistCandidate_9447_KeySeed,
    &TwistCandidate_9447_SaltSeed,
    &TwistCandidate_9447_MaskSeedA,
    &TwistCandidate_9447_MaskSeedB,
    &TwistCandidate_9447_TwistBlock,
    &TwistCandidate_9447_PushKeyRound,
    &TwistCandidate_9447_PushMaskRoundA,
    &TwistCandidate_9447_PushMaskRoundB,
  }
};
