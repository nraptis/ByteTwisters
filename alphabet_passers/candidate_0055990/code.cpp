...
// Candidate 55991: TwistCandidate_55991
// family=ferocious_dual_worker op_budget=8 worker_shapes=2x3/3x2 worker_reverse=(False, True, True, True)/(True, True, False, False) twiddle1=mix/none twiddle2=xor/i_mul twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m2i1v010,aBreakerCD:4m1i2v303 lane_breaker=false braid_breaker=true jump_breaker=false swap_breaker=true mask_template=3 mask_seed_a=xor_wave mask_seed_b=xor_wave lightning=true/1 typhoon=false hurricane=false final_reverse=(True, True, True, False)
// ferocious[wa=2x3; wb=3x2; tw1=mix/none; tw2=xor/i_mul; tw2src=B; saltbox=128:0010911c:ce4403c5; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m2i1v010,aBreakerCD:4m1i2v303; wa_rev=(False, True, True, True); wb_rev=(True, True, False, False); lane=off:2; braid=on:3; jump=off:2; swap=on:1; mask=workerbx3; lightning=on:1; typhoon=off; hurricane=off; final=5/(True, True, True, False); key_rot=21; maskA_seed=xor_wave/False; maskB_seed=xor_wave; maskA_bias=182; maskB_bias=40]
static std::uint32_t TwistCandidate_55991_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0xAC4BE51Du;
  aState = AdvanceTwiddle32(
      ...
      0xF68A5D4Fu,
      10u);
  aState ^= RotateLeft32(pValue + pExtra + 0x3CC1653Bu, 10U);
  return static_cast<std::uint32_t>(aState + 0x5AC1B852u);
}

static std::uint32_t TwistCandidate_55991_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x352FF5E9u;
  aState = AdvanceTwiddle32(
      ...
      0x09EE90D2u,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xB63A75EFu, 11U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5A8A6u);
}

static void TwistCandidate_55991_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_55991_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_55991_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_55991_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_55991_AdvanceTwiddle32B(
...
static void TwistCandidate_55991_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x4EU, 0x90U, 0xAEU, 0x82U, 0xF5U, 0x66U, 0xA7U, 0xC4U, 0xB8U, 0x8FU, 0x0DU, 0x21U, 0xA2U, 0x38U, 0x8EU, 0x44U,
      0xA0U, 0x65U, 0xCCU, 0x7FU, 0x75U, 0x3CU, 0x77U, 0xC2U, 0xD6U, 0xC7U, 0x11U, 0x17U, 0x67U, 0xB2U, 0xA1U, 0x0AU,
      0x1CU, 0xD7U, 0xFEU, 0x2BU, 0xEEU, 0xE1U, 0xECU, 0x4AU, 0x54U, 0x43U, 0x4DU, 0xDBU, 0x53U, 0x93U, 0x81U, 0x51U,
      0x91U, 0x74U, 0xA4U, 0xCAU, 0xBAU, 0x42U, 0x6BU, 0xD8U, 0x0BU, 0xACU, 0xDFU, 0xF7U, 0x08U, 0x1FU, 0xBCU, 0x16U,
      0xB3U, 0x1AU, 0x58U, 0xE7U, 0xF9U, 0xE3U, 0x19U, 0xF1U, 0x1DU, 0xD9U, 0x0EU, 0x02U, 0xFDU, 0x37U, 0x9DU, 0x5EU,
      0xD1U, 0xE8U, 0x06U, 0x9CU, 0xBDU, 0x85U, 0xD4U, 0x2CU, 0xFFU, 0x33U, 0xCDU, 0x6FU, 0x3BU, 0x8DU, 0xD2U, 0xF0U,
      0x86U, 0x7EU, 0x5AU, 0x88U, 0x8AU, 0x10U, 0x7AU, 0x24U, 0xB4U, 0xF6U, 0xF8U, 0x9EU, 0x7BU, 0x84U, 0x89U, 0x1BU,
      0x34U, 0xE0U, 0x2FU, 0xAAU, 0x28U, 0x2EU, 0xC3U, 0xE6U, 0x18U, 0x9BU, 0x55U, 0x73U, 0xE2U, 0x49U, 0x45U, 0xC5U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 189u ^ 101u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 10u + (6637)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-6343)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 10u + static_cast<unsigned int>(189u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 189u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(101u), 5u, true);
  ...
  TwistCandidate_55991_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(101u), 5u);
...
static void TwistCandidate_55991_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 25u + (-2)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 1u + (-664)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 189u) ^ ((b << 5u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_55991_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 133u ^ 182u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 19u + (5123)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 25u + (7243)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (19u + 25u) + (-7327)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 1u + (-657)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 6u)) ^ static_cast<std::uint32_t>(122u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 19u * 8U + static_cast<std::size_t>(aSourceIndex) * 25u + 133u + static_cast<std::size_t>(aTurbulence * ((1u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(133u), 7u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 122u) * ((1u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
  ...
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 40u + (5123)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 48u + (7243)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (40u + 48u) + (-7327)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 6u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 40u * 8U + static_cast<std::size_t>(aReverseIndex) * 48u * 3U + 35u + static_cast<std::size_t>(aTurbulence * ((1u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 1u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(35u), 1u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 122u) * ((1u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 1u, true);
  }
}

static void TwistCandidate_55991_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 241u ^ 40u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 18u + (174)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 10u + (-2321)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (18u + 10u) + (-2050)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 36u + (-6013)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 3u)) ^ static_cast<std::uint32_t>(186u)) & 0xFFu);
    const std::uint32_t aSeedA = static_cast<std::uint32_t>((a ^ aTurbulence) & 0xFFu);
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 18u * 8U + static_cast<std::size_t>(aReverseIndex) * 10u + 241u + static_cast<std::size_t>(aTurbulence * ((36u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(241u), 3u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 186u) * ((36u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 3u, false);
  }
}

static void TwistCandidate_55991_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x46U, 0x0EU, 0x47U, 0x66U, 0x9FU, 0x70U, 0x65U, 0xCCU, 0x86U, 0xCAU, 0x61U, 0x1FU, 0x4BU, 0xD3U, 0x73U, 0x55U,
      0x75U, 0xD9U, 0x09U, 0xD4U, 0x34U, 0x93U, 0x26U, 0x58U, 0x7DU, 0x24U, 0xF7U, 0x7BU, 0xB8U, 0xBBU, 0xF5U, 0xBFU,
      0xBDU, 0x8FU, 0xACU, 0xD8U, 0x6FU, 0x6DU, 0x79U, 0xF2U, 0xB3U, 0xDEU, 0x96U, 0xDDU, 0x8DU, 0xF6U, 0x30U, 0x7AU,
      0xE4U, 0x8EU, 0x18U, 0x19U, 0x60U, 0x20U, 0x10U, 0xB0U, 0x92U, 0x03U, 0x45U, 0x4FU, 0x6AU, 0x54U, 0x8CU, 0xECU,
      0x5AU, 0x05U, 0x36U, 0xEFU, 0x32U, 0x95U, 0x04U, 0x90U, 0xCEU, 0x53U, 0x1CU, 0xFAU, 0x25U, 0xFCU, 0xE6U, 0xE8U,
      0x62U, 0xC6U, 0xB9U, 0x88U, 0xAAU, 0x3FU, 0xB5U, 0x9BU, 0xEBU, 0x4DU, 0x0DU, 0x2DU, 0x5EU, 0x2CU, 0x78U, 0x07U,
      0x17U, 0xC0U, 0xE7U, 0xA0U, 0x63U, 0x44U, 0xA2U, 0x9EU, 0x9CU, 0x40U, 0x1AU, 0x5CU, 0x83U, 0x85U, 0xFFU, 0x11U,
      0x57U, 0xE5U, 0xBEU, 0x82U, 0x0FU, 0xE2U, 0x38U, 0xC5U, 0x5FU, 0x21U, 0xF3U, 0x51U, 0x15U, 0x7CU, 0xC7U, 0xD7U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0xC89F0AF2u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x7CA3B679u, 10u);
  std::uint32_t aAccB = AdvanceTwiddle32(0xDAF2148Fu ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0xF0D6B420u, 8u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (6935), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-1775), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4831), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-2868), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x7CA3B679u, 10u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0xF0D6B420u, 8u);
...
static void TwistCandidate_55991_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x8EU, 0x4BU, 0x95U, 0xE2U, 0xA6U, 0xC7U, 0xE9U, 0x66U, 0xAFU, 0x4CU, 0x73U, 0x6EU, 0x86U, 0x21U, 0xB7U, 0x84U,
      0x37U, 0xACU, 0x17U, 0xBAU, 0xFCU, 0xFBU, 0xA9U, 0x13U, 0x7BU, 0x06U, 0x03U, 0x38U, 0x3FU, 0xADU, 0x29U, 0xF2U,
      0xE6U, 0x97U, 0xDFU, 0x35U, 0xDAU, 0xDEU, 0x91U, 0xCDU, 0x6AU, 0x63U, 0xAAU, 0xF3U, 0x61U, 0x87U, 0x99U, 0x77U,
      0xB0U, 0x4AU, 0x3AU, 0x05U, 0x09U, 0xF7U, 0x94U, 0xE0U, 0x54U, 0x0AU, 0x2BU, 0xC1U, 0x8FU, 0x04U, 0x9DU, 0x22U,
      0x0FU, 0xB3U, 0xD4U, 0x10U, 0xF8U, 0xCFU, 0xC8U, 0xA0U, 0x50U, 0x49U, 0x5DU, 0x58U, 0x2DU, 0xC3U, 0xD6U, 0x3EU,
      0x2EU, 0xD9U, 0x2AU, 0x7DU, 0x80U, 0x3CU, 0xA8U, 0x57U, 0x71U, 0xABU, 0x36U, 0x75U, 0x78U, 0x89U, 0xC6U, 0x19U,
      0x02U, 0x55U, 0x32U, 0x7CU, 0xBBU, 0x27U, 0x25U, 0xE4U, 0x1EU, 0x8BU, 0x62U, 0x0DU, 0x98U, 0xB6U, 0x90U, 0xBFU,
      0x85U, 0x64U, 0xB2U, 0x9CU, 0x0EU, 0x41U, 0x40U, 0x01U, 0xCCU, 0xA1U, 0x46U, 0x1BU, 0x45U, 0x6CU, 0x08U, 0xD8U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x928826F7u, 0x9F319A93u, 1u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-3230), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x9F319A93u, 1u);
...
static void TwistCandidate_55991_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_55991_AdvanceTwiddle32A(
      0xF40EBC53u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_55991_AdvanceTwiddle32B(
      0x4DEDEED9u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_55991_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_55991_AdvanceTwiddle32A(
      0x42E40477u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[23U]),
      0x8056D44Cu ^ static_cast<std::uint32_t>(pSalt[23U]));
  std::uint32_t aPulseB = TwistCandidate_55991_AdvanceTwiddle32B(
      0x72FCC5D7u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[23U]),
      0x55285737u ^ static_cast<std::uint32_t>(pSalt[23U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_55991_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_55991_AdvanceTwiddle32B(
...
static void TwistCandidate_55991_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x13973EEBu ^ (pRound * 44u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-2), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x13973EEBu ^ (pRound * 44u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-664), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (5825), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (3392), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 17u + 9u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (3392 + 9), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const int aIndex2 = WrapRange(aReverseIndex + (-2121), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA ^ static_cast<std::uint32_t>(pSalt[26u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(209u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 3u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (5819), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-18), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 57u + 21u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-18 + 21), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 12u + 3U) & 15U), static_cast<std::size_t>(i + 3u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 2u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(29u)) ^ static_cast<std::uint32_t>(193u)) & 0xFFu);
      const int aIndex2 = WrapRange(aReverseIndex + (3175), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(i + (-5441), aStart, aEnd);
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex3]);
      const std::uint32_t aRoute = ((aSourceCtrl >> 2u) ^ aWorkerCtrl ^ aKeyByte ^ aMaskByte ^ static_cast<std::uint32_t>(2u)) % 5U;
      std::uint32_t aValue = a;
      switch (aRoute) {
        case 0U: aValue = a; break;
        case 1U: aValue = b; break;
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(109u)), 0x85EBCA6Bu, 10u);
    }
  }

  TwistCandidate_55991_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_55991_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_55991_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-3331), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-3969), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-5384), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 16u + 428u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatOther = WrapRange(static_cast<int>(aMaskFlat) + (4 + 3), 0, static_cast<int>(kMaskStackTotalBytes));
      const std::uint32_t aMaskByteA = static_cast<std::uint32_t>(pMaskStackB[(aMaskFlat / kMaskBytes) % kMaskStackDepth][aMaskFlat % kMaskBytes]);
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    auto storm_block = LoadBlock16Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (1529), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock16Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (5068), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackA, chunk + 318u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackB, chunk + 318u + 17u);
    ...
    storm.InjectAdd(pSalt, kSaltBytes, 1u);
    ...
    ApplyLightningMixColumns(storm, pSalt, static_cast<std::uint8_t>((storm_control[11] + pRound + 1u) & 31U), static_cast<std::uint8_t>(storm_control[15] ^ storm_mask_a[0] ^ storm_mask_b[1] ^ pSalt[3]), static_cast<std::uint8_t>(1u));
  ...
  TwistCandidate_55991_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (1744), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (-5619), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (6246), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (6378), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_55991_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_55991_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-1108)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 19u + (-6783)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 4u + (-1540)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 26u + static_cast<unsigned int>(21u)) & 31U;
...
static void TwistCandidate_55991_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 20u + (-3767)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 27u + (-1800)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 7u + (6378)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(182u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(182u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_55991_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-5170)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 21u + (4391)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 30u + (6174)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
void TwistCandidate_55991(
  ...
  TwistCandidate_55991_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_55991_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_55991_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_55991_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_55991_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_55991_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_55991_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_55991_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_55991",
    &TwistCandidate_55991,
    &TwistCandidate_55991_KeySeed,
    &TwistCandidate_55991_SaltSeed,
    &TwistCandidate_55991_MaskSeedA,
    &TwistCandidate_55991_MaskSeedB,
    &TwistCandidate_55991_TwistBlock,
    &TwistCandidate_55991_PushKeyRound,
    &TwistCandidate_55991_PushMaskRoundA,
    &TwistCandidate_55991_PushMaskRoundB,
  }
};
