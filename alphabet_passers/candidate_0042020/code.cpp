...
// Candidate 42021: TwistCandidate_42021
// family=ferocious_dual_worker op_budget=8 worker_shapes=2x3/3x0 worker_reverse=(False, False, True, False)/(True, True, True, False) twiddle1=add/none twiddle2=add/i_mul twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m1i3v322,aBreakerCD:4m0i0v132 lane_breaker=false braid_breaker=false jump_breaker=false swap_breaker=true mask_template=0 mask_seed_a=off mask_seed_b=off lightning=true/0 typhoon=true hurricane=false final_reverse=(True, True, True, True)
// ferocious[wa=2x3; wb=3x0; tw1=add/none; tw2=add/i_mul; tw2src=B; saltbox=128:000fa944:775dc67e; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m1i3v322,aBreakerCD:4m0i0v132; wa_rev=(False, False, True, False); wb_rev=(True, True, True, False); lane=off:3; braid=off:1; jump=off:0; swap=on:2; mask=workerax0; lightning=on:0; typhoon=on; hurricane=off; final=4/(True, True, True, True); key_rot=2; maskA_seed=off/False; maskB_seed=off; maskA_bias=215; maskB_bias=171]
static std::uint32_t TwistCandidate_42021_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x11F946EEu;
  aState = AdvanceTwiddle32(
      ...
      0x08503CD9u,
      6u);
  aState ^= RotateLeft32(pValue + pExtra + 0x7FA89985u, 9U);
  return static_cast<std::uint32_t>(aState + 0x19A97A37u);
}

static std::uint32_t TwistCandidate_42021_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xCBF5EAE3u;
  aState = AdvanceTwiddle32(
      ...
      0xB45D7366u,
      8u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xC656BA54u, 7U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5D63Au);
}

static void TwistCandidate_42021_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_42021_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_42021_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_42021_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_42021_AdvanceTwiddle32B(
...
static void TwistCandidate_42021_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xB7U, 0x75U, 0x3DU, 0x7CU, 0x1EU, 0x39U, 0x85U, 0xC7U, 0x94U, 0xE7U, 0x49U, 0x5CU, 0xDCU, 0xF4U, 0x65U, 0xD4U,
      0x6AU, 0xDFU, 0x4AU, 0x5DU, 0xADU, 0x72U, 0x23U, 0xF2U, 0xA6U, 0xF5U, 0x3CU, 0xD9U, 0x14U, 0x29U, 0x34U, 0xD0U,
      0x19U, 0xE5U, 0x42U, 0xB6U, 0xC2U, 0xEFU, 0x16U, 0x26U, 0x20U, 0xDBU, 0x52U, 0x10U, 0x57U, 0x00U, 0x46U, 0x61U,
      0xE0U, 0x7DU, 0xC9U, 0xB0U, 0x15U, 0xAFU, 0x54U, 0x04U, 0xDAU, 0x64U, 0x2CU, 0x9BU, 0xA8U, 0x58U, 0x9AU, 0xAEU,
      0x12U, 0x06U, 0xB3U, 0x5FU, 0xC4U, 0x8CU, 0x7AU, 0x0AU, 0x1BU, 0x82U, 0xFAU, 0x51U, 0xD1U, 0xA9U, 0x28U, 0x9FU,
      0xE1U, 0xFDU, 0x22U, 0x67U, 0x31U, 0x7FU, 0xD8U, 0x40U, 0x62U, 0xF9U, 0xD2U, 0xD7U, 0x9EU, 0x41U, 0xCCU, 0x56U,
      0x2BU, 0x4BU, 0x3AU, 0x7EU, 0x93U, 0xF7U, 0xB4U, 0xCAU, 0xBFU, 0x74U, 0xCDU, 0x77U, 0x05U, 0x5EU, 0x53U, 0x1AU,
      0xA7U, 0x36U, 0xFBU, 0x6DU, 0xC0U, 0xAAU, 0xBAU, 0x44U, 0xC6U, 0x47U, 0x32U, 0x59U, 0x0DU, 0x17U, 0x6BU, 0xBEU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 224u ^ 137u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 18u + (-5277)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 27u + (5857)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 18u + static_cast<unsigned int>(224u)) & 31U;
    ...
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(137u), 1u, true);
  ...
  TwistCandidate_42021_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(137u), 1u);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 52u + (-11995)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 26u + (12575)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 52u + pSalt[(aSourceIndex + 233u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(233u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(233u), 1u, true);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_42021_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(233u), static_cast<std::uint32_t>(233u), 1u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(233u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_42021_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 16u + (2267)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 22u + (-3462)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 224u) ^ ((b << 1u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_42021_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 49u ^ 215u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 17u + (-5770)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 16u + (-912)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (17u + 16u) + (-2717)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 17u * 8U + static_cast<std::size_t>(aSourceIndex) * 16u + 49u + static_cast<std::size_t>(aTurbulence * ((23u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(49u), 2u, false);
  }
}

static void TwistCandidate_42021_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 156u ^ 171u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 8u + (7575)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 17u + (5746)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (8u + 17u) + (4461)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const std::uint32_t aWorkerByte = 0U;
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 8u * 8U + static_cast<std::size_t>(aSourceIndex) * 17u + 156u + static_cast<std::size_t>(aTurbulence * ((42u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(156u), 5u, true);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 20u + (7575)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 56u + (5746)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (20u + 56u) + (4461)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 20u * 8U + static_cast<std::size_t>(aReverseIndex) * 56u * 3U + 135u + static_cast<std::size_t>(aTurbulence * ((42u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 1u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(135u), 1u, true);
  }
}

static void TwistCandidate_42021_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xB4U, 0x2EU, 0xABU, 0xDDU, 0x3BU, 0x54U, 0x0AU, 0x7AU, 0x71U, 0x28U, 0xFBU, 0x2AU, 0x38U, 0x03U, 0xCDU, 0x0FU,
      0xECU, 0xF7U, 0xB0U, 0x4DU, 0x79U, 0xD2U, 0x48U, 0x31U, 0x26U, 0xE4U, 0xF2U, 0x64U, 0xC6U, 0xA6U, 0x3FU, 0x18U,
      0x73U, 0xE0U, 0xF1U, 0xB7U, 0x8CU, 0x15U, 0xFEU, 0x83U, 0x6EU, 0x94U, 0x33U, 0x97U, 0xBAU, 0xA1U, 0x4CU, 0x2BU,
      0x68U, 0xBFU, 0x62U, 0x91U, 0xCEU, 0x5AU, 0xE9U, 0x3CU, 0xBCU, 0x17U, 0x0DU, 0x59U, 0x92U, 0xE2U, 0xF4U, 0x45U,
      0x3EU, 0xD0U, 0xAEU, 0xEEU, 0x5FU, 0xF8U, 0x56U, 0x88U, 0x47U, 0xBDU, 0xEBU, 0x19U, 0x3DU, 0x1DU, 0x36U, 0x1AU,
      0x9CU, 0x44U, 0xA4U, 0xD1U, 0x60U, 0x74U, 0x06U, 0xFAU, 0xC4U, 0x13U, 0x76U, 0xADU, 0x7FU, 0x95U, 0x7DU, 0xE7U,
      0xB3U, 0xEFU, 0xE8U, 0xC1U, 0xA2U, 0x09U, 0xDCU, 0xD9U, 0xF0U, 0x1BU, 0x80U, 0x96U, 0x01U, 0x0CU, 0x07U, 0xCAU,
      0x35U, 0xC2U, 0x77U, 0x27U, 0xD3U, 0x5EU, 0x0EU, 0x6BU, 0x10U, 0xF6U, 0x39U, 0x9FU, 0x57U, 0x5BU, 0x37U, 0x0BU
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0xEDAE8DF4u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0xA0A864B0u, 6u);
  std::uint32_t aAccB = AdvanceTwiddle32(0xAB98A94Fu ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0xA60C862Fu, 7u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-6878), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-1632), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-4800), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (3728), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0xA0A864B0u, 6u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0xA60C862Fu, 7u);
...
static void TwistCandidate_42021_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xB4U, 0x18U, 0xA9U, 0xB1U, 0x25U, 0x61U, 0x53U, 0x91U, 0x79U, 0xFBU, 0xEBU, 0x01U, 0x99U, 0x5EU, 0xB3U, 0x6CU,
      0x13U, 0xA7U, 0xC9U, 0xD0U, 0x1EU, 0xCDU, 0x4BU, 0x65U, 0x5DU, 0x5BU, 0xB0U, 0x1CU, 0xE4U, 0x2BU, 0x3AU, 0x21U,
      0xA5U, 0xD1U, 0xB6U, 0x6FU, 0x82U, 0x07U, 0xDFU, 0x50U, 0x24U, 0x6EU, 0xCCU, 0xAFU, 0x3FU, 0xE8U, 0x9DU, 0xDAU,
      0x31U, 0xD7U, 0x14U, 0xEDU, 0xE7U, 0xFCU, 0xA0U, 0x10U, 0x4AU, 0x04U, 0x46U, 0x78U, 0xC3U, 0x0EU, 0x51U, 0x73U,
      0xF5U, 0xFDU, 0xAAU, 0xCAU, 0x58U, 0x44U, 0x43U, 0x2CU, 0x59U, 0xC5U, 0x1BU, 0xF4U, 0x39U, 0x9EU, 0xCEU, 0x86U,
      0xDBU, 0xACU, 0x02U, 0xD9U, 0xC6U, 0x75U, 0x40U, 0xF7U, 0x7CU, 0x77U, 0x88U, 0x29U, 0xF2U, 0xF3U, 0xD6U, 0x17U,
      0x32U, 0xF6U, 0xC0U, 0x2EU, 0x2DU, 0x0AU, 0xF9U, 0x98U, 0x3BU, 0x3EU, 0xA3U, 0xFAU, 0x22U, 0xB7U, 0x60U, 0x57U,
      0x09U, 0xFEU, 0x45U, 0x1FU, 0x34U, 0x26U, 0x76U, 0x05U, 0x42U, 0x28U, 0x3CU, 0x8CU, 0x7AU, 0xADU, 0xC2U, 0x54U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x37B4B653u, 0x032B5745u, 2u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (3106), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltByte = static_cast<std::uint32_t>(pSalt[(i + 2u) & 31U]);
    const std::uint32_t aMix = static_cast<std::uint32_t>(aMixBox[(aDestByte ^ aSourceByte ^ aSaltByte ^ FoldWordToByte(aAcc)) & 127U]);
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x032B5745u, 2u);
...
static void TwistCandidate_42021_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_42021_AdvanceTwiddle32A(
      0xC4BE2AD1u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_42021_AdvanceTwiddle32B(
      0xA0C1F3D3u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_42021_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_42021_AdvanceTwiddle32A(
      0xE460E1FCu ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[28U]),
      0x26D421C2u ^ static_cast<std::uint32_t>(pSalt[20U]));
  std::uint32_t aPulseB = TwistCandidate_42021_AdvanceTwiddle32B(
      0x18299634u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[20U]),
      0x3FFD854Fu ^ static_cast<std::uint32_t>(pSalt[28U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_42021_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_42021_AdvanceTwiddle32B(
...
static void TwistCandidate_42021_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x337CAA45u ^ (pRound * 43u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (2267), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x337CAA45u ^ (pRound * 43u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-3462), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-3159), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (5150), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 245u + 12u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (5150 + 12), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 15u + 3U) & 15U), static_cast<std::size_t>(i + 23u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 8u) & 31U]);
      ...
      const int aIndex2 = WrapRange(i + (973), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(250u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[8u]), 0x27D4EB2Du, 8u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (6509), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-6413), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-6413 + 19), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 13u + 3U) & 15U), static_cast<std::size_t>(i + 22u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 25u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(13u)) ^ static_cast<std::uint32_t>(181u)) & 0xFFu);
      const int aIndex2 = WrapRange(aReverseIndex + (5639), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(aReverseIndex + (-2646), aStart, aEnd);
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex3]);
      const std::uint32_t aRoute = ((aSourceCtrl >> 2u) ^ aWorkerCtrl ^ aKeyByte ^ aMaskByte ^ static_cast<std::uint32_t>(0u)) % 5U;
      std::uint32_t aValue = a;
      switch (aRoute) {
        case 0U: aValue = a; break;
        case 1U: aValue = b; break;
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB + static_cast<std::uint32_t>(125u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[25u]), 0x27D4EB2Du, 8u);
    }
  }

  TwistCandidate_42021_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_42021_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_42021_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (5610), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-769), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 22u + 811u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    auto storm_block = LoadBlock16Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (801), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock16Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (5378), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackA, chunk + 1310u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackB, chunk + 1310u + 17u);
    ...
    switch (static_cast<unsigned>(storm_control[1] ^ storm_mask_a[2] ^ storm_mask_b[3] ^ pSalt[0]) % 5u) {
      ...
      case 3u: storm.ApplyFastOp(LightningFastOp::kRotateRowRight, static_cast<std::uint8_t>(storm_control[3] ^ storm_mask_a[4] ^ storm_mask_b[5]), static_cast<std::uint8_t>(storm_control[5] + storm_mask_a[6] + storm_mask_b[7])); break;
      case 4u: storm.ApplyFastOp(LightningFastOp::kWeaveRows, static_cast<std::uint8_t>(storm_control[3] ^ storm_mask_a[4] ^ storm_mask_b[5]), static_cast<std::uint8_t>(storm_control[5] + storm_mask_a[6] + storm_mask_b[7])); break;
    }
    }
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kTyphoonBlockBytes) {
    auto storm_block = LoadBlock128Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (613), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock128Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (4273), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackA, chunk + 650u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackB, chunk + 650u + 23u);
    ...
    switch (static_cast<unsigned>(storm_control[3] ^ storm_mask_a[11] ^ storm_mask_b[13] ^ pSalt[0]) % 7u) {
      ...
      case 6u: storm.ApplyFastOp(TyphoonFastOp::kRotateRowRight, static_cast<std::uint8_t>(storm_control[17] ^ storm_mask_a[23] ^ storm_mask_b[29]), static_cast<std::uint8_t>(storm_control[29] + storm_mask_a[31] + storm_mask_b[37])); break;
    }
    }
  ...
  TwistCandidate_42021_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (565), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (-2649), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (-1101), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (-1555), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_42021_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_42021_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 12u + (4837)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 12u + (-2990)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 28u + (-1755)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 12u + static_cast<unsigned int>(2u)) & 31U;
...
static void TwistCandidate_42021_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 14u + (-4791)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 20u + (-6797)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 11u + (-5957)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_42021_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 6u + (6325)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 25u + (2484)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 9u + (-7119)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(171u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(171u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_42021(
  ...
  TwistCandidate_42021_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_42021_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_42021_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_42021_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_42021_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_42021_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_42021_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_42021_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_42021",
    &TwistCandidate_42021,
    &TwistCandidate_42021_KeySeed,
    &TwistCandidate_42021_SaltSeed,
    &TwistCandidate_42021_MaskSeedA,
    &TwistCandidate_42021_MaskSeedB,
    &TwistCandidate_42021_TwistBlock,
    &TwistCandidate_42021_PushKeyRound,
    &TwistCandidate_42021_PushMaskRoundA,
    &TwistCandidate_42021_PushMaskRoundB,
  }
};
