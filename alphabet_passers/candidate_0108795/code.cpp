...
// Candidate 108796: TwistCandidate_108796
// family=ferocious_dual_worker op_budget=9 worker_shapes=1x2/1x1 worker_reverse=(False, True, True, False)/(False, False, True, True) twiddle1=mix/none twiddle2=xor/i_mul twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m0i3v222,aBreakerCD:4m2i3v223,aBreakerEF:4m1i2v202 lane_breaker=true braid_breaker=false jump_breaker=true swap_breaker=true mask_template=1 mask_seed_a=add_wave mask_seed_b=sbox_wave lightning=false/2 typhoon=false hurricane=true final_reverse=(True, True, False, False)
// ferocious[wa=1x2; wb=1x1; tw1=mix/none; tw2=xor/i_mul; tw2src=B; saltbox=128:000ff7ba:5e6d1f4b; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m0i3v222,aBreakerCD:4m2i3v223,aBreakerEF:4m1i2v202; wa_rev=(False, True, True, False); wb_rev=(False, False, True, True); lane=on:2; braid=off:1; jump=on:1; swap=on:3; mask=workerax1; lightning=off:2; typhoon=off; hurricane=on; final=4/(True, True, False, False); key_rot=11; maskA_seed=add_wave/True; maskB_seed=sbox_wave; maskA_bias=172; maskB_bias=157]
static std::uint32_t TwistCandidate_108796_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x75B83612u;
  aState = AdvanceTwiddle32(
      ...
      0x4846C7F9u,
      10u);
  aState ^= RotateLeft32(pValue + pExtra + 0x5BFECD7Au, 12U);
  return static_cast<std::uint32_t>(aState + 0x3DFEF1EBu);
}

static std::uint32_t TwistCandidate_108796_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x8BE37EBEu;
  aState = AdvanceTwiddle32(
      ...
      0xD01DB3C4u,
      11u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x45D5E9D6u, 11U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5B947u);
}

static void TwistCandidate_108796_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_108796_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_108796_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_108796_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_108796_AdvanceTwiddle32B(
...
static void TwistCandidate_108796_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x81U, 0xBCU, 0xEFU, 0xE0U, 0x1EU, 0xBDU, 0x02U, 0x68U, 0xF1U, 0x05U, 0x56U, 0x6FU, 0x7DU, 0xB5U, 0x19U, 0x3DU,
      0xC5U, 0xF5U, 0x74U, 0x38U, 0x4CU, 0x30U, 0x86U, 0xAAU, 0x92U, 0x08U, 0xDEU, 0x69U, 0x88U, 0x87U, 0xC8U, 0xAFU,
      0x01U, 0xB7U, 0xC9U, 0x4AU, 0xF0U, 0xF2U, 0xACU, 0x84U, 0xB8U, 0x23U, 0x25U, 0x58U, 0x2DU, 0xD8U, 0x65U, 0xCBU,
      0x3EU, 0xC1U, 0x71U, 0xB1U, 0x4EU, 0xD3U, 0x98U, 0x3BU, 0x7BU, 0x07U, 0x73U, 0x7AU, 0xB9U, 0x5EU, 0x0CU, 0x09U,
      0x5BU, 0x6AU, 0x40U, 0xC3U, 0x10U, 0xBBU, 0x24U, 0x7CU, 0x49U, 0xD5U, 0xC6U, 0x96U, 0x16U, 0xDFU, 0x8AU, 0xA2U,
      0x99U, 0x75U, 0x76U, 0xD0U, 0x0EU, 0x11U, 0x3CU, 0x14U, 0x61U, 0x48U, 0xEAU, 0x6DU, 0x45U, 0xE2U, 0xD9U, 0x59U,
      0xF9U, 0x2AU, 0x94U, 0x2FU, 0xDCU, 0x78U, 0x66U, 0x39U, 0xEBU, 0xEDU, 0x83U, 0xE1U, 0x04U, 0x52U, 0xB3U, 0x4DU,
      0xCEU, 0xD4U, 0x8CU, 0xD6U, 0x44U, 0x79U, 0xA8U, 0x5CU, 0x03U, 0xB4U, 0x7FU, 0xA7U, 0xB0U, 0x4FU, 0xADU, 0x4BU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 75u ^ 117u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 4u + (6979)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 13u + (963)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 4u + static_cast<unsigned int>(75u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 75u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(117u), 5u, false);
  ...
  TwistCandidate_108796_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(117u), 5u);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 34u + (910)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 23u + (7032)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 34u + pSalt[(aSourceIndex + 165u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(165u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(165u), 4u, false);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_108796_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(165u), static_cast<std::uint32_t>(165u), 4u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(165u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_108796_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 14u + (5432)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 4u + (908)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 75u) ^ ((b << 5u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_108796_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 77u ^ 172u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 9u + (1481)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 10u + (5737)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (9u + 10u) + (7072)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 5u + (-7407)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte + b + static_cast<std::uint32_t>(149u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 9u * 8U + static_cast<std::size_t>(aSourceIndex) * 10u + 77u + static_cast<std::size_t>(aTurbulence * ((5u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(77u), 4u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 149u) * ((5u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 2u, false);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 52u + (1481)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 32u + (5737)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (52u + 32u) + (7072)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aTurbulence & 0xFFu), 2u))) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c + static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 2u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 52u * 8U + static_cast<std::size_t>(aSourceIndex) * 32u * 3U + 144u + static_cast<std::size_t>(aTurbulence * ((5u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 2u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(144u), 2u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 149u) * ((5u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 2u, true);
  }
}

static void TwistCandidate_108796_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 101u ^ 157u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 23u + (-7297)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 28u + (1615)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (23u + 28u) + (-5810)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 31u + (-5595)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>(aMixBox[(aWorkerByte ^ a ^ static_cast<std::uint32_t>(62u)) & 0xFFu]);
    const std::uint32_t aSeedA = static_cast<std::uint32_t>((a ^ aTurbulence) & 0xFFu);
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(62u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 23u * 8U + static_cast<std::size_t>(aSourceIndex) * 28u + 101u + static_cast<std::size_t>(aTurbulence * ((31u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(101u), 4u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 62u) * ((31u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 1u, false);
  }
}

static void TwistCandidate_108796_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x2BU, 0x4FU, 0x77U, 0xECU, 0x14U, 0xFDU, 0x4EU, 0xA7U, 0x8DU, 0xA0U, 0x6EU, 0x39U, 0x6FU, 0x5EU, 0xCFU, 0x62U,
      0x8EU, 0xACU, 0xCAU, 0xB4U, 0x12U, 0x4DU, 0x56U, 0xA3U, 0x06U, 0x71U, 0xC0U, 0xB9U, 0x67U, 0x69U, 0x1AU, 0x8AU,
      0x2CU, 0x28U, 0xD3U, 0xE0U, 0xC5U, 0x91U, 0x5CU, 0x83U, 0x9DU, 0x37U, 0x3FU, 0x79U, 0x66U, 0xEDU, 0x72U, 0x4AU,
      0x50U, 0x45U, 0x18U, 0x61U, 0xF8U, 0x32U, 0x9CU, 0xEAU, 0xB5U, 0x26U, 0x5BU, 0x1DU, 0x5DU, 0xF4U, 0xE9U, 0x90U,
      0xD6U, 0x47U, 0x3BU, 0x04U, 0xF9U, 0xB1U, 0xFEU, 0x05U, 0x0DU, 0xF1U, 0x20U, 0xBDU, 0x4BU, 0x65U, 0x1EU, 0x9BU,
      0x21U, 0x3CU, 0x46U, 0x96U, 0x09U, 0xCCU, 0x10U, 0xA8U, 0x1CU, 0xDEU, 0xD5U, 0xC8U, 0x2AU, 0xE3U, 0xAAU, 0x57U,
      0x7BU, 0xD8U, 0x81U, 0xAEU, 0x89U, 0xFBU, 0x2DU, 0x74U, 0x60U, 0x5FU, 0x3DU, 0x85U, 0x82U, 0x6CU, 0xCDU, 0xB7U,
      0xBCU, 0x76U, 0x02U, 0x2FU, 0x73U, 0xE8U, 0x33U, 0xC4U, 0xF7U, 0x30U, 0xA2U, 0xD0U, 0xFCU, 0x53U, 0x08U, 0x44U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0xF2E2D8A5u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0xE3B7C418u, 6u);
  std::uint32_t aAccB = AdvanceTwiddle32(0xCB2EC766u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x3BC5584Cu, 8u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-4814), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-1686), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (3122), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-6950), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0xE3B7C418u, 6u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x3BC5584Cu, 8u);
...
static void TwistCandidate_108796_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x6EU, 0xE5U, 0x8FU, 0xA2U, 0xCBU, 0x06U, 0x64U, 0xFFU, 0x63U, 0x36U, 0xB3U, 0xDDU, 0x09U, 0x70U, 0xDFU, 0x03U,
      0x71U, 0x48U, 0xE3U, 0x93U, 0x78U, 0x59U, 0xAFU, 0xACU, 0x54U, 0x1BU, 0x86U, 0x0EU, 0x77U, 0x3FU, 0xA4U, 0x3DU,
      0x90U, 0xBEU, 0x43U, 0x1AU, 0xF5U, 0x4CU, 0xD4U, 0x38U, 0x24U, 0x2DU, 0x35U, 0xECU, 0x16U, 0x4BU, 0xF1U, 0x6DU,
      0x4EU, 0x55U, 0x98U, 0x7FU, 0x20U, 0x00U, 0x89U, 0xD1U, 0xEAU, 0xC4U, 0xB4U, 0x15U, 0xF0U, 0x68U, 0x6CU, 0xCAU,
      0xE0U, 0x4DU, 0x9CU, 0x88U, 0x5AU, 0xB1U, 0x53U, 0xD2U, 0xCEU, 0x9FU, 0x51U, 0x22U, 0x82U, 0x72U, 0x45U, 0x80U,
      0xE4U, 0x0AU, 0xF7U, 0xA5U, 0x91U, 0x17U, 0x6FU, 0xF2U, 0xCDU, 0x75U, 0x7CU, 0x97U, 0x3EU, 0xA7U, 0x21U, 0x49U,
      0x32U, 0x65U, 0xA1U, 0xFCU, 0x1FU, 0x28U, 0x39U, 0x2AU, 0x9DU, 0x7BU, 0xF6U, 0x2FU, 0xDEU, 0x3AU, 0x5FU, 0x40U,
      0x29U, 0x57U, 0xDBU, 0x37U, 0xBCU, 0xAAU, 0xF3U, 0xDCU, 0xC1U, 0xFEU, 0xB6U, 0x6BU, 0x7EU, 0xADU, 0x02U, 0x42U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0xF93DB5B5u, 0xAE44FBABu, 7u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (7427), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xAE44FBABu, 7u);
...
static void TwistCandidate_108796_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_108796_AdvanceTwiddle32A(
      0x03770B70u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_108796_AdvanceTwiddle32B(
      0x935D47F0u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_108796_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_108796_AdvanceTwiddle32A(
      0x428B937Au ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[26U]),
      0x803A874Bu ^ static_cast<std::uint32_t>(pSalt[13U]));
  std::uint32_t aPulseB = TwistCandidate_108796_AdvanceTwiddle32B(
      0xA6F3FD8Du + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[13U]),
      0x81273EC4u ^ static_cast<std::uint32_t>(pSalt[26U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    for (std::size_t aLane = 0; aLane < kMatrixBlockBytes; ++aLane) {
      ...
      const unsigned char aMix = FixedMixBoxByte(static_cast<unsigned char>(
          ...
          pSalt[(aLane + 13U + (aPulseByteA & 3U)) & 31U]));
    ...
    aPulseA = TwistCandidate_108796_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_108796_AdvanceTwiddle32B(
...
static void TwistCandidate_108796_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x89BF2012u ^ (pRound * 37u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (5432), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x89BF2012u ^ (pRound * 37u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (908), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (6433), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-2863), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 115u + 30u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-2863 + 30), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 2u + 3U) & 15U), static_cast<std::size_t>(i + 2u)));
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA ^ static_cast<std::uint32_t>(pSalt[4u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(89u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 1u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (4070), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (3837), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 171u + 28u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (3837 + 28), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 23u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(37u)) ^ static_cast<std::uint32_t>(150u)) & 0xFFu);
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(188u)), 0x85EBCA6Bu, 3u);
    }
  }

  TwistCandidate_108796_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_108796_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_108796_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_108796_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (2568), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-7184), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-3707), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 23u + 713u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-7284), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (4221), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 914u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 914u + 29u);
  ...
  TwistCandidate_108796_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (-451), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (-2914), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (1282), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (-2083), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_108796_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_108796_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 8u + (-7116)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 30u + (-4796)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 4u + (-420)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 8u + static_cast<unsigned int>(11u)) & 31U;
...
static void TwistCandidate_108796_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 7u + (4249)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-6006)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 23u + (7366)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_108796_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 17u + (7069)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 26u + (7076)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 24u + (595)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(157u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(157u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_108796(
  ...
  TwistCandidate_108796_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_108796_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_108796_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_108796_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_108796_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_108796_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_108796_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_108796_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_108796",
    &TwistCandidate_108796,
    &TwistCandidate_108796_KeySeed,
    &TwistCandidate_108796_SaltSeed,
    &TwistCandidate_108796_MaskSeedA,
    &TwistCandidate_108796_MaskSeedB,
    &TwistCandidate_108796_TwistBlock,
    &TwistCandidate_108796_PushKeyRound,
    &TwistCandidate_108796_PushMaskRoundA,
    &TwistCandidate_108796_PushMaskRoundB,
  }
};
