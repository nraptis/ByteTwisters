...
// Candidate 18354: TwistCandidate_18354
// family=ferocious_dual_worker op_budget=9 worker_shapes=2x4/1x1 worker_reverse=(False, False, True, True)/(True, True, False, True) twiddle1=mix/none twiddle2=xor/i twiddle2_source=A tsunami=custom breaker_shapes=aBreakerAB:4m2i1v103 lane_breaker=true braid_breaker=true jump_breaker=false swap_breaker=false mask_template=4 mask_seed_a=off mask_seed_b=off lightning=false/3 typhoon=true hurricane=true final_reverse=(False, True, False, True)
// ferocious[wa=2x4; wb=1x1; tw1=mix/none; tw2=xor/i; tw2src=A; saltbox=128:001048d5:297559b2; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m2i1v103; wa_rev=(False, False, True, True); wb_rev=(True, True, False, True); lane=on:1; braid=on:0; jump=off:2; swap=off:1; mask=workerbx4; lightning=off:3; typhoon=on; hurricane=on; final=1/(False, True, False, True); key_rot=6; maskA_seed=off/True; maskB_seed=off; maskA_bias=219; maskB_bias=249]
static std::uint32_t TwistCandidate_18354_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x22DE36BEu;
  aState = AdvanceTwiddle32(
      ...
      0x237BE107u,
      10u);
  aState ^= RotateLeft32(pValue + pExtra + 0x67A5E8AEu, 9U);
  return static_cast<std::uint32_t>(aState + 0x01A5D7B9u);
}

static std::uint32_t TwistCandidate_18354_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xE0DE0B94u;
  aState = AdvanceTwiddle32(
      ...
      0x877BE33Au,
      8u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xAA5A19F8u, 11U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5EA93u);
}

static void TwistCandidate_18354_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_18354_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_18354_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_18354_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_18354_AdvanceTwiddle32B(
...
static void TwistCandidate_18354_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xAEU, 0x66U, 0x5CU, 0x89U, 0x10U, 0x34U, 0xC7U, 0x88U, 0xCDU, 0x7DU, 0x95U, 0x05U, 0xC0U, 0x5DU, 0xA0U, 0xE7U,
      0x86U, 0xDEU, 0xFDU, 0x8CU, 0x38U, 0x01U, 0x71U, 0x46U, 0xAAU, 0xE4U, 0x16U, 0xCFU, 0xF9U, 0x24U, 0x37U, 0xD4U,
      0x3FU, 0x7FU, 0x19U, 0x43U, 0xE2U, 0xD2U, 0xA8U, 0x7EU, 0xE5U, 0xE0U, 0x74U, 0xBDU, 0xF8U, 0x00U, 0x77U, 0x9AU,
      0x29U, 0xBEU, 0x8FU, 0xC1U, 0xB5U, 0x1EU, 0xBFU, 0x0AU, 0xEBU, 0xE8U, 0x61U, 0x6DU, 0x36U, 0x6BU, 0x02U, 0x20U,
      0x6AU, 0xAFU, 0xD6U, 0xB3U, 0xD8U, 0xDDU, 0x13U, 0x44U, 0x69U, 0x70U, 0xCBU, 0xFCU, 0x1FU, 0xFEU, 0xD5U, 0x4FU,
      0xACU, 0xEDU, 0x2EU, 0x23U, 0xFFU, 0x5EU, 0x31U, 0xA2U, 0x8BU, 0x48U, 0xC2U, 0x2BU, 0xB2U, 0x28U, 0xEFU, 0x98U,
      0x83U, 0xBCU, 0xA6U, 0x3BU, 0x1BU, 0xADU, 0xFAU, 0xB1U, 0x04U, 0x21U, 0x03U, 0x6EU, 0xF7U, 0x0EU, 0x1AU, 0xA3U,
      0xC4U, 0xB7U, 0x0BU, 0x52U, 0x1CU, 0x78U, 0x2AU, 0x55U, 0xC8U, 0xE3U, 0x53U, 0xD7U, 0x82U, 0x94U, 0x57U, 0xF2U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 56u ^ 192u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 10u + (-3658)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-4221)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 10u + static_cast<unsigned int>(56u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 56u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(192u), 5u, true);
  ...
  TwistCandidate_18354_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(192u), 5u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(192u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 58u + (1663)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 63u + (-9542)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 58u + pSalt[(aSourceIndex + 81u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(81u), 1u, true);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_18354_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(81u), static_cast<std::uint32_t>(81u), 1u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
...
static void TwistCandidate_18354_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 23u + (-6686)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 5u + (-5256)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 56u) ^ ((b << 5u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_18354_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 144u ^ 219u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 28u + (-6826)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 2u + (-7284)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (28u + 2u) + (4024)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 28u * 8U + static_cast<std::size_t>(aReverseIndex) * 2u + 144u + static_cast<std::size_t>(aTurbulence * ((5u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(144u), 4u, true);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 51u + (-6826)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 52u + (-7284)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (51u + 52u) + (4024)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 51u * 8U + static_cast<std::size_t>(aSourceIndex) * 52u * 3U + 248u + static_cast<std::size_t>(aTurbulence * ((5u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 2u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(248u), 2u, true);
  }
}

static void TwistCandidate_18354_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 226u ^ 249u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 29u + (5741)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 6u + (1217)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (29u + 6u) + (-7475)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const std::uint32_t aWorkerByte = 0U;
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 29u * 8U + static_cast<std::size_t>(aReverseIndex) * 6u + 226u + static_cast<std::size_t>(aTurbulence * ((31u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(226u), 5u, true);
  }
}

static void TwistCandidate_18354_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x02U, 0x18U, 0x50U, 0xD9U, 0xE9U, 0xD5U, 0xD3U, 0x7CU, 0x8BU, 0x9DU, 0x15U, 0xA2U, 0x76U, 0x5CU, 0x7AU, 0x42U,
      0xF0U, 0x26U, 0x4AU, 0xFAU, 0x0EU, 0xDEU, 0x0DU, 0x2AU, 0xB8U, 0xC3U, 0x90U, 0xBCU, 0x51U, 0xAAU, 0x93U, 0x82U,
      0xF5U, 0xB5U, 0x8EU, 0x95U, 0x57U, 0x70U, 0x78U, 0x0FU, 0xC6U, 0xEFU, 0x83U, 0x89U, 0x2DU, 0xF6U, 0x36U, 0x3CU,
      0x22U, 0x7FU, 0x4DU, 0x62U, 0x7DU, 0x5DU, 0x80U, 0x45U, 0xBEU, 0x29U, 0xADU, 0x72U, 0xF7U, 0xC8U, 0x05U, 0xFCU,
      0x32U, 0xCDU, 0xA0U, 0xA3U, 0x6EU, 0x38U, 0x3BU, 0x79U, 0x4EU, 0xDCU, 0x2EU, 0x63U, 0xC4U, 0x0CU, 0xA6U, 0x48U,
      0x7BU, 0xE7U, 0x6AU, 0x66U, 0xC1U, 0x97U, 0xBDU, 0xF4U, 0x73U, 0x60U, 0xABU, 0x39U, 0x8CU, 0x11U, 0x92U, 0x96U,
      0x7EU, 0x6CU, 0x85U, 0x9AU, 0x1DU, 0x74U, 0xACU, 0xF9U, 0x9CU, 0x1FU, 0x44U, 0x24U, 0x8FU, 0xA9U, 0xFEU, 0x81U,
      0xD0U, 0x6DU, 0x46U, 0x67U, 0xEEU, 0xCAU, 0xC0U, 0xCEU, 0x06U, 0x52U, 0x16U, 0x2BU, 0xE0U, 0x07U, 0xB3U, 0x0BU
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x6D43379Fu ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0xF1E4C1D0u, 11u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x2A92EC5Du ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x3A7185EAu, 5u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4075), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (5334), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-7170), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-2506), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltA = static_cast<std::uint32_t>(pSalt[(i + 0u) & 31U]);
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0xF1E4C1D0u, 11u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x3A7185EAu, 5u);
...
static void TwistCandidate_18354_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xE8U, 0x6CU, 0xE3U, 0x1DU, 0x52U, 0xB5U, 0x2CU, 0x53U, 0x6DU, 0xBAU, 0x12U, 0x56U, 0xFFU, 0x07U, 0x94U, 0xC9U,
      0x0DU, 0x99U, 0xF6U, 0xDEU, 0xD5U, 0x59U, 0x4EU, 0x57U, 0x03U, 0xA2U, 0x3AU, 0x25U, 0xB3U, 0x08U, 0x84U, 0x6BU,
      0xA7U, 0xF1U, 0x9BU, 0xD4U, 0x83U, 0x58U, 0xA9U, 0x44U, 0xACU, 0xAAU, 0xCCU, 0x31U, 0xEAU, 0xDFU, 0xB6U, 0x7CU,
      0x68U, 0xBBU, 0x8FU, 0x46U, 0xBDU, 0xB0U, 0xE4U, 0xE0U, 0x78U, 0xEFU, 0xE7U, 0xB9U, 0x82U, 0x29U, 0x0BU, 0xC5U,
      0xFCU, 0x04U, 0x66U, 0x7AU, 0x4FU, 0x05U, 0x16U, 0x37U, 0xAEU, 0x6EU, 0x00U, 0x28U, 0xB1U, 0xB2U, 0xC0U, 0x23U,
      0x36U, 0x7BU, 0x20U, 0xA8U, 0x76U, 0x47U, 0x9EU, 0x24U, 0xF4U, 0x5CU, 0x32U, 0xEEU, 0x2FU, 0x26U, 0xD1U, 0x67U,
      0x7FU, 0xADU, 0x3BU, 0xE9U, 0x60U, 0x27U, 0x33U, 0x15U, 0x85U, 0x2AU, 0xDDU, 0xF2U, 0x1AU, 0x8BU, 0xD6U, 0x01U,
      0x71U, 0xC1U, 0xCEU, 0x3FU, 0xDCU, 0x18U, 0x17U, 0x11U, 0xECU, 0x7EU, 0x40U, 0x9AU, 0x88U, 0xBFU, 0xABU, 0x2DU
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x3DBA7E42u, 0x05D1812Eu, 6u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (3557), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x05D1812Eu, 6u);
...
static void TwistCandidate_18354_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_18354_AdvanceTwiddle32A(
      0xC615C0CEu ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_18354_AdvanceTwiddle32B(
      0x4B1755C2u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_18354_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_18354_AdvanceTwiddle32A(
      0xE2F490AAu ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[10U]),
      0x2046E098u ^ static_cast<std::uint32_t>(pSalt[11U]));
  std::uint32_t aPulseB = TwistCandidate_18354_AdvanceTwiddle32B(
      0x2C97A3EBu + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[11U]),
      0x0B4370FEu ^ static_cast<std::uint32_t>(pSalt[10U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_18354_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_18354_AdvanceTwiddle32B(
...
static void TwistCandidate_18354_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x51691C31u ^ (pRound * 11u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-6686), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x51691C31u ^ (pRound * 11u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-5256), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-2312), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (2474), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 47u + 16u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (2474 + 16), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const int aIndex2 = WrapRange(i + (-713), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA ^ static_cast<std::uint32_t>(pSalt[26u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(166u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 11u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (-3568), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (-446), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 12u + 12u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-446 + 12), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 11u + 3U) & 15U), static_cast<std::size_t>(i + 29u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 12u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>((aLaneIndex ^ static_cast<std::uint32_t>(73u)) & 0xFFu);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(9u)), 0x85EBCA6Bu, 4u);
    }
  }

  TwistCandidate_18354_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_18354_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (6398), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (923), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (4279), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 5u + 546u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kTyphoonBlockBytes) {
    auto storm_block = LoadBlock128Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (4454), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock128Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-1598), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackA, chunk + 336u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackB, chunk + 336u + 23u);
    ...
    switch (static_cast<unsigned>(storm_control[3] ^ storm_mask_a[11] ^ storm_mask_b[13] ^ pSalt[0]) % 4u) {
      ...
      case 3u: storm.ApplyFastOp(TyphoonFastOp::kRotateColumnDown, static_cast<std::uint8_t>(storm_control[17] ^ storm_mask_a[23] ^ storm_mask_b[29]), static_cast<std::uint8_t>(storm_control[29] + storm_mask_a[31] + storm_mask_b[37])); break;
    }
    }
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-4143), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-4055), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 910u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 910u + 29u);
  ...
  TwistCandidate_18354_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (-1164), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (4187), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-7585), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (1208), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_18354_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_18354_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 30u + (3333)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 9u + (3133)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 22u + (6748)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 30u + static_cast<unsigned int>(6u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 192u)));
...
static void TwistCandidate_18354_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 30u + (-7144)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 22u + (-7095)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 2u + (-4698)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_18354_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 5u + (5139)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-1747)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-6793)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
void TwistCandidate_18354(
  ...
  TwistCandidate_18354_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_18354_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_18354_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_18354_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_18354_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_18354_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_18354_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_18354_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_18354",
    &TwistCandidate_18354,
    &TwistCandidate_18354_KeySeed,
    &TwistCandidate_18354_SaltSeed,
    &TwistCandidate_18354_MaskSeedA,
    &TwistCandidate_18354_MaskSeedB,
    &TwistCandidate_18354_TwistBlock,
    &TwistCandidate_18354_PushKeyRound,
    &TwistCandidate_18354_PushMaskRoundA,
    &TwistCandidate_18354_PushMaskRoundB,
  }
};
