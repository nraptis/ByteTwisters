...
// Candidate 84915: TwistCandidate_84915
// family=ferocious_dual_worker op_budget=6 worker_shapes=3x2/1x2 worker_reverse=(False, False, False, False)/(True, False, False, False) twiddle1=xor/i_mul twiddle2=mix/i twiddle2_source=A tsunami=custom breaker_shapes=aBreakerAB:4m3i0v223 lane_breaker=false braid_breaker=false jump_breaker=false swap_breaker=false mask_template=1 mask_seed_a=sbox_wave mask_seed_b=sbox_wave lightning=false/0 typhoon=true hurricane=false final_reverse=(False, True, False, True)
// ferocious[wa=3x2; wb=1x2; tw1=xor/i_mul; tw2=mix/i; tw2src=A; saltbox=128:00110424:6bd32476; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m3i0v223; wa_rev=(False, False, False, False); wb_rev=(True, False, False, False); lane=off:3; braid=off:1; jump=off:1; swap=off:2; mask=workerbx1; lightning=off:0; typhoon=on; hurricane=off; final=1/(False, True, False, True); key_rot=18; maskA_seed=sbox_wave/False; maskB_seed=sbox_wave; maskA_bias=30; maskB_bias=118]
static std::uint32_t TwistCandidate_84915_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x8D3C890Eu;
  aState = AdvanceTwiddle32(
      ...
      0xBC5179D2u,
      7u);
  aState ^= RotateLeft32(pValue + pExtra + 0x576D5003u, 9U);
  return static_cast<std::uint32_t>(aState + 0x316DF0DCu);
}

static std::uint32_t TwistCandidate_84915_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x7FF494CAu;
  aState = AdvanceTwiddle32(
      ...
      0x2899C4C9u,
      8u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xB5D64DD7u, 8U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5ED18u);
}

static void TwistCandidate_84915_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_84915_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_84915_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_84915_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_84915_AdvanceTwiddle32B(
...
static void TwistCandidate_84915_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xFEU, 0x5FU, 0x23U, 0xE9U, 0x8FU, 0xF5U, 0x14U, 0x57U, 0xBDU, 0x9BU, 0xD2U, 0x2AU, 0x0AU, 0x1AU, 0x55U, 0xE2U,
      0x7BU, 0x15U, 0x54U, 0xAFU, 0xD9U, 0xA3U, 0x62U, 0xFDU, 0x43U, 0x31U, 0x07U, 0x6CU, 0xC7U, 0x33U, 0xADU, 0x95U,
      0x99U, 0x50U, 0x16U, 0x09U, 0xF8U, 0x24U, 0xA5U, 0x2DU, 0x1BU, 0x3FU, 0xCBU, 0xD0U, 0x94U, 0x53U, 0x6EU, 0xC3U,
      0x8EU, 0x97U, 0xECU, 0xBAU, 0x28U, 0x32U, 0x8AU, 0x61U, 0x17U, 0x2FU, 0x63U, 0xF0U, 0xB7U, 0xD4U, 0xB3U, 0x80U,
      0xC8U, 0x88U, 0x11U, 0x72U, 0xD3U, 0x2BU, 0x0BU, 0x71U, 0x37U, 0x85U, 0x4DU, 0xAEU, 0xD8U, 0xFAU, 0xE8U, 0xC6U,
      0xA1U, 0xFCU, 0x9CU, 0x4EU, 0xCAU, 0x59U, 0xD5U, 0x5DU, 0xC4U, 0x22U, 0x47U, 0x92U, 0x7FU, 0xACU, 0xC5U, 0x4BU,
      0x83U, 0xB8U, 0x90U, 0xF1U, 0x44U, 0xD6U, 0x5EU, 0xDEU, 0x9DU, 0x9FU, 0x87U, 0x60U, 0x5AU, 0xCDU, 0xC2U, 0x6AU,
      0x9EU, 0xB5U, 0x70U, 0xF6U, 0x45U, 0x46U, 0x93U, 0x81U, 0x21U, 0x75U, 0xE5U, 0xBFU, 0x3AU, 0x03U, 0xD7U, 0x36U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 245u ^ 130u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 19u + (142)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 30u + (3084)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 19u + static_cast<unsigned int>(245u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 245u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(130u), 2u, false);
  ...
  TwistCandidate_84915_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(130u), 2u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(130u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 58u + (3730)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 60u + (-504)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 58u + pSalt[(aSourceIndex + 253u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(253u), 1u, false);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_84915_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(253u), static_cast<std::uint32_t>(253u), 1u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
...
static void TwistCandidate_84915_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 17u + (4394)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 16u + (7178)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 245u) ^ ((b << 2u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_84915_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 34u ^ 30u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 4u + (6389)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 2u + (-7392)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (4u + 2u) + (175)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 63u + (-1091)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>(aMixBox[(aWorkerByte ^ a ^ static_cast<std::uint32_t>(114u)) & 0xFFu]);
    ...
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(114u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 4u * 8U + static_cast<std::size_t>(aSourceIndex) * 2u + 34u + static_cast<std::size_t>(aTurbulence * ((63u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(34u), 1u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 114u) * ((63u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 7u, false);
  }
}

static void TwistCandidate_84915_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 119u ^ 118u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 16u + (4798)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 15u + (-3231)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (16u + 15u) + (7239)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 1u + (4705)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>(aMixBox[(aWorkerByte ^ a ^ static_cast<std::uint32_t>(244u)) & 0xFFu]);
    const std::uint32_t aSeedA = static_cast<std::uint32_t>((a ^ aTurbulence) & 0xFFu);
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(244u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 16u * 8U + static_cast<std::size_t>(aReverseIndex) * 15u + 119u + static_cast<std::size_t>(aTurbulence * ((1u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(119u), 6u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 244u) * ((1u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 6u, false);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 31u + (4798)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 39u + (-3231)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (31u + 39u) + (7239)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(244u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 31u * 8U + static_cast<std::size_t>(aReverseIndex) * 39u * 3U + 29u + static_cast<std::size_t>(aTurbulence * ((1u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 2u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(29u), 2u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 244u) * ((1u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 2u, true);
  }
}

static void TwistCandidate_84915_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xC6U, 0x6AU, 0x44U, 0xC5U, 0x46U, 0x4BU, 0x13U, 0x4FU, 0x24U, 0x37U, 0x86U, 0x0BU, 0x05U, 0x3CU, 0xA6U, 0xF0U,
      0x01U, 0x78U, 0x75U, 0xEBU, 0x63U, 0x11U, 0xBDU, 0xE9U, 0xFBU, 0xFAU, 0x34U, 0xC9U, 0x8CU, 0x26U, 0xCAU, 0xFDU,
      0x50U, 0x7BU, 0x98U, 0x9BU, 0x0CU, 0x5CU, 0x09U, 0x58U, 0x42U, 0xA7U, 0xB4U, 0x0DU, 0x7DU, 0xA8U, 0x6DU, 0xD4U,
      0xE8U, 0xD5U, 0x8DU, 0x8FU, 0x31U, 0xBAU, 0xE5U, 0xDEU, 0xF3U, 0xFCU, 0x1CU, 0x38U, 0x53U, 0x15U, 0xE7U, 0x2BU,
      0xBCU, 0x59U, 0xD1U, 0xD6U, 0xCCU, 0x77U, 0x96U, 0x03U, 0xCBU, 0xD8U, 0x1DU, 0xAFU, 0x3BU, 0x5FU, 0xE0U, 0x90U,
      0x6CU, 0x60U, 0x20U, 0x9CU, 0x48U, 0x2FU, 0x9DU, 0x1AU, 0x39U, 0x40U, 0xC8U, 0x5AU, 0xB3U, 0x16U, 0x62U, 0xC1U,
      0xB0U, 0x47U, 0x30U, 0x3DU, 0xB7U, 0x84U, 0x1FU, 0xDBU, 0xD7U, 0x94U, 0x5EU, 0x3EU, 0x99U, 0xEEU, 0x32U, 0x56U,
      0x92U, 0xF6U, 0x14U, 0xD0U, 0xF2U, 0x61U, 0x04U, 0x41U, 0xDFU, 0xAAU, 0xBFU, 0x83U, 0x12U, 0xCFU, 0x19U, 0xF8U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0xCA01841Eu ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x41F65DD7u, 6u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x45251BD2u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x6A834A62u, 8u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (675), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-2075), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (2867), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-1322), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltA = static_cast<std::uint32_t>(pSalt[(i + 11u) & 31U]);
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x41F65DD7u, 6u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x6A834A62u, 8u);
...
static void TwistCandidate_84915_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xFDU, 0xAFU, 0x76U, 0x32U, 0xD3U, 0x01U, 0xA5U, 0xEDU, 0xCAU, 0x08U, 0xEAU, 0xCBU, 0x6DU, 0xF3U, 0x1CU, 0xF2U,
      0xD6U, 0x6FU, 0xF5U, 0x2AU, 0x88U, 0x10U, 0x8BU, 0x3AU, 0x06U, 0x26U, 0x4FU, 0x9CU, 0x0FU, 0x40U, 0x92U, 0xD0U,
      0x8CU, 0x9FU, 0x6CU, 0x7BU, 0x49U, 0x95U, 0x43U, 0xE4U, 0xF9U, 0x1EU, 0x0CU, 0x5AU, 0xE8U, 0xF8U, 0x00U, 0xA2U,
      0x9EU, 0xBDU, 0x37U, 0xDBU, 0x5CU, 0xDEU, 0x8FU, 0xA9U, 0x65U, 0x58U, 0x30U, 0xF1U, 0xC8U, 0x67U, 0xD5U, 0xDCU,
      0x99U, 0x3DU, 0xEBU, 0x47U, 0x51U, 0xA0U, 0xE2U, 0x48U, 0x5EU, 0xB9U, 0x11U, 0xFAU, 0x07U, 0x59U, 0x33U, 0x19U,
      0xA8U, 0xDFU, 0xFCU, 0x85U, 0x73U, 0x75U, 0x74U, 0x52U, 0xAEU, 0x72U, 0x4EU, 0x2CU, 0xE7U, 0xA3U, 0x38U, 0x1AU,
      0xE9U, 0x36U, 0xC6U, 0x66U, 0xB7U, 0x27U, 0x71U, 0x63U, 0x9DU, 0xEEU, 0x56U, 0x84U, 0x69U, 0x81U, 0xB4U, 0x80U,
      0x64U, 0x1BU, 0x7EU, 0x41U, 0x97U, 0x7DU, 0x45U, 0x77U, 0x1DU, 0x61U, 0xE6U, 0x39U, 0x0DU, 0x93U, 0xCCU, 0xFEU
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x9FA3E073u, 0x2696115Eu, 4u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-2114), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x2696115Eu, 4u);
...
static void TwistCandidate_84915_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_84915_AdvanceTwiddle32A(
      0xF86D7E37u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_84915_AdvanceTwiddle32B(
      0x4A97DF75u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_84915_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_84915_AdvanceTwiddle32A(
      0x3D6B254Cu ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[12U]),
      0xFFDB8572u ^ static_cast<std::uint32_t>(pSalt[0U]));
  std::uint32_t aPulseB = TwistCandidate_84915_AdvanceTwiddle32B(
      0xF8356FE0u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[0U]),
      0xDFE19509u ^ static_cast<std::uint32_t>(pSalt[12U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_84915_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_84915_AdvanceTwiddle32B(
...
static void TwistCandidate_84915_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x8001C341u ^ (pRound * 22u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (4394), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x8001C341u ^ (pRound * 22u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (7178), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (1563), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (4179), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 117u + 13u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (4179 + 13), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 15u + 3U) & 15U), static_cast<std::size_t>(i + 1u)));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(53u)) ^ static_cast<std::uint32_t>(127u)) & 0xFFu);
      const int aIndex2 = WrapRange(i + (6102), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(i + (885), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(223u)), 0x85EBCA6Bu, 6u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (2053), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-1050), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 136u + 6u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-1050 + 6), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA ^ static_cast<std::uint32_t>(pSalt[2u]), static_cast<std::uint32_t>((aValue) ^ RotateLeft32(static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 7U)), static_cast<std::uint32_t>(226u) + static_cast<std::uint32_t>(aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias), 0x165667B1u, 7u);
    }
  }

  TwistCandidate_84915_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_84915_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (4904), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-2016), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (2251), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 23u + 1513u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kTyphoonBlockBytes) {
    auto storm_block = LoadBlock128Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (1540), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock128Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (4983), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackA, chunk + 1058u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackB, chunk + 1058u + 23u);
  ...
  TwistCandidate_84915_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (-6608), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (1314), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-4380), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (-5823), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_84915_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_84915_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 26u + (7315)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 8u + (-6520)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-3041)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 26u + static_cast<unsigned int>(18u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 130u)));
...
static void TwistCandidate_84915_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 21u + (7374)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 4u + (-3356)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 17u + (547)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(30u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(30u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_84915_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 30u + (-4524)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 13u + (504)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 28u + (-6975)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
void TwistCandidate_84915(
  ...
  TwistCandidate_84915_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_84915_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_84915_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_84915_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_84915_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_84915_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_84915_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_84915_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_84915",
    &TwistCandidate_84915,
    &TwistCandidate_84915_KeySeed,
    &TwistCandidate_84915_SaltSeed,
    &TwistCandidate_84915_MaskSeedA,
    &TwistCandidate_84915_MaskSeedB,
    &TwistCandidate_84915_TwistBlock,
    &TwistCandidate_84915_PushKeyRound,
    &TwistCandidate_84915_PushMaskRoundA,
    &TwistCandidate_84915_PushMaskRoundB,
  }
};
