...
// Candidate 200971: TwistCandidate_200971
// family=ferocious_dual_worker op_budget=8 worker_shapes=2x1/2x1 worker_reverse=(False, False, True, False)/(False, False, True, True) twiddle1=add/i_mul twiddle2=add/i twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m2i2v101,aBreakerCD:4m0i1v221 lane_breaker=true braid_breaker=false jump_breaker=false swap_breaker=true mask_template=4 mask_seed_a=sbox_wave mask_seed_b=off lightning=false/0 typhoon=false hurricane=true final_reverse=(True, True, True, False)
// ferocious[wa=2x1; wb=2x1; tw1=add/i_mul; tw2=add/i; tw2src=B; saltbox=128:000fbe3a:e950af62; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m2i2v101,aBreakerCD:4m0i1v221; wa_rev=(False, False, True, False); wb_rev=(False, False, True, True); lane=on:0; braid=off:1; jump=off:1; swap=on:3; mask=workerbx4; lightning=off:0; typhoon=off; hurricane=on; final=5/(True, True, True, False); key_rot=5; maskA_seed=sbox_wave/True; maskB_seed=off; maskA_bias=239; maskB_bias=70]
static std::uint32_t TwistCandidate_200971_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0xC902F236u;
  aState = AdvanceTwiddle32(
      ...
      0xCCB06F16u,
      9u);
  aState ^= RotateLeft32(pValue + pExtra + 0x63B24EB1u, 12U);
  return static_cast<std::uint32_t>(aState + 0x05B29D20u);
}

static std::uint32_t TwistCandidate_200971_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x0F159929u;
  aState = AdvanceTwiddle32(
      ...
      0x6CA7D798u,
      11u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x35AAC9CEu, 10U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5F63Fu);
}

static void TwistCandidate_200971_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_200971_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_200971_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_200971_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_200971_AdvanceTwiddle32B(
...
static void TwistCandidate_200971_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x1AU, 0xD5U, 0xA3U, 0x79U, 0xAEU, 0xDCU, 0x17U, 0x5FU, 0x16U, 0x45U, 0x8AU, 0xB0U, 0x28U, 0xA1U, 0x40U, 0x35U,
      0xFDU, 0x85U, 0x68U, 0x38U, 0x88U, 0xCEU, 0x90U, 0x1EU, 0x4FU, 0xFEU, 0xECU, 0x15U, 0x6AU, 0x7AU, 0xDEU, 0x18U,
      0x36U, 0x93U, 0xE7U, 0x4DU, 0xE0U, 0x99U, 0x51U, 0x1CU, 0x32U, 0x74U, 0x64U, 0x4AU, 0xC3U, 0xD0U, 0x3CU, 0x14U,
      0x02U, 0x4EU, 0x4BU, 0x7EU, 0x9EU, 0x8FU, 0xC6U, 0x13U, 0x7FU, 0x30U, 0x9BU, 0x33U, 0xC2U, 0xAFU, 0x94U, 0x19U,
      0x25U, 0x42U, 0xA7U, 0xC5U, 0x81U, 0x31U, 0xF0U, 0x01U, 0x70U, 0x7CU, 0xF5U, 0xCBU, 0xD6U, 0x52U, 0x60U, 0xD8U,
      0xE8U, 0x57U, 0x29U, 0x5BU, 0xE1U, 0x61U, 0x11U, 0x58U, 0xC1U, 0x50U, 0x73U, 0x27U, 0xFFU, 0xF6U, 0x05U, 0x55U,
      0x82U, 0x56U, 0x63U, 0xEAU, 0xADU, 0x37U, 0x03U, 0x6FU, 0xC9U, 0xB6U, 0x3EU, 0xDAU, 0xB9U, 0xEDU, 0xA2U, 0xBDU,
      0x53U, 0x6DU, 0xB1U, 0xCFU, 0xDBU, 0x5EU, 0x9CU, 0xA8U, 0x1BU, 0x00U, 0x46U, 0xB7U, 0x3DU, 0xCCU, 0x0DU, 0x62U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 227u ^ 244u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 16u + (-6830)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 5u + (-5925)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 16u + static_cast<unsigned int>(227u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 227u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(244u), 4u, false);
  ...
  TwistCandidate_200971_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(244u), 4u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(244u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
...
static void TwistCandidate_200971_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 25u + (4230)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 16u + (6070)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 227u) ^ ((b << 4u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_200971_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 229u ^ 239u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-7625)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 16u + (-6351)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (26u + 16u) + (7239)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 58u + (-2851)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>(aMixBox[(aWorkerByte ^ a ^ static_cast<std::uint32_t>(15u)) & 0xFFu]);
    ...
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(15u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 26u * 8U + static_cast<std::size_t>(aReverseIndex) * 16u + 229u + static_cast<std::size_t>(aTurbulence * ((58u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(229u), 4u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 15u) * ((58u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 5u, true);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 42u + (-7625)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 55u + (-6351)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (42u + 55u) + (7239)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(15u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 42u * 8U + static_cast<std::size_t>(aSourceIndex) * 55u * 3U + 202u + static_cast<std::size_t>(aTurbulence * ((58u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 5u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(202u), 5u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 15u) * ((58u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 5u, true);
  }
}

static void TwistCandidate_200971_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 50u ^ 70u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 17u + (-7454)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 20u + (3290)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (17u + 20u) + (-733)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 17u * 8U + static_cast<std::size_t>(aSourceIndex) * 20u + 50u + static_cast<std::size_t>(aTurbulence * ((17u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(50u), 2u, true);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 19u + (-7454)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 51u + (3290)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (19u + 51u) + (-733)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const std::uint32_t aWorkerByte = 0U;
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 19u * 8U + static_cast<std::size_t>(aReverseIndex) * 51u * 3U + 243u + static_cast<std::size_t>(aTurbulence * ((17u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 4u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(243u), 4u, true);
  }
}

static void TwistCandidate_200971_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x68U, 0x4BU, 0x1BU, 0xB9U, 0x4AU, 0x4CU, 0xC7U, 0x49U, 0x32U, 0x04U, 0x86U, 0x85U, 0x27U, 0x8DU, 0xE5U, 0xBDU,
      0x1AU, 0x78U, 0x71U, 0x70U, 0x5EU, 0x9AU, 0xB5U, 0x9EU, 0xB1U, 0x5DU, 0xA9U, 0xBCU, 0x34U, 0xEFU, 0x03U, 0xCAU,
      0x89U, 0xC8U, 0xDAU, 0x43U, 0x51U, 0x7CU, 0x9CU, 0xF4U, 0x80U, 0x7EU, 0x7BU, 0x02U, 0xCEU, 0xBEU, 0x28U, 0x8CU,

  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0xBFDECCE7u, 0x36C4CB07u, 4u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-593), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x36C4CB07u, 4u);
...
static void TwistCandidate_200971_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_200971_AdvanceTwiddle32A(
      0x4BE3B29Fu ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_200971_AdvanceTwiddle32B(
      0x00D2F33Du ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_200971_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_200971_AdvanceTwiddle32A(
      0xA29CDA64u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[4U]),
      0x6030005Du ^ static_cast<std::uint32_t>(pSalt[18U]));
  std::uint32_t aPulseB = TwistCandidate_200971_AdvanceTwiddle32B(
      0xB5EA13F2u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[18U]),
      0x923F1918u ^ static_cast<std::uint32_t>(pSalt[4U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_200971_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_200971_AdvanceTwiddle32B(
...
static void TwistCandidate_200971_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x5AA4EF42u ^ (pRound * 45u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (4230), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x5AA4EF42u ^ (pRound * 45u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (6070), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-616), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-1193), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 55u + 11u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-1193 + 11), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 3u + 3U) & 15U), static_cast<std::size_t>(i + 7u)));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(11u)) ^ static_cast<std::uint32_t>(89u)) & 0xFFu);
      const int aIndex2 = WrapRange(i + (1487), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(166u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[20u]), 0x27D4EB2Du, 2u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (-2296), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (-837), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 140u + 28u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-837 + 28), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 0u + 3U) & 15U), static_cast<std::size_t>(i + 3u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 16u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>((aLaneIndex ^ static_cast<std::uint32_t>(222u)) & 0xFFu);
      const int aIndex2 = WrapRange(i + (-5222), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB + static_cast<std::uint32_t>(221u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[16u]), 0x27D4EB2Du, 9u);
    }
  }

  TwistCandidate_200971_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_200971_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_200971_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (620), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-703), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (6636), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 21u + 119u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-6260), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (2783), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 405u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 405u + 29u);
  ...
  TwistCandidate_200971_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (5262), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (-3085), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (3463), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (-4047), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_200971_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_200971_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 23u + (5310)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 18u + (-3162)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 14u + (2516)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 23u + static_cast<unsigned int>(5u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 244u)));
...
static void TwistCandidate_200971_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 13u + (-3905)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 20u + (-3025)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 12u + (-3798)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(239u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(239u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_200971_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-1204)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 16u + (1953)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 24u + (-4609)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
void TwistCandidate_200971(
  ...
  TwistCandidate_200971_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_200971_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_200971_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_200971_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_200971_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_200971_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_200971_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_200971_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_200971",
    &TwistCandidate_200971,
    &TwistCandidate_200971_KeySeed,
    &TwistCandidate_200971_SaltSeed,
    &TwistCandidate_200971_MaskSeedA,
    &TwistCandidate_200971_MaskSeedB,
    &TwistCandidate_200971_TwistBlock,
    &TwistCandidate_200971_PushKeyRound,
    &TwistCandidate_200971_PushMaskRoundA,
    &TwistCandidate_200971_PushMaskRoundB,
  }
};
