...
// Candidate 40286: TwistCandidate_40286
// family=ferocious_dual_worker op_budget=9 worker_shapes=2x2/1x2 worker_reverse=(False, False, True, False)/(True, True, True, True) twiddle1=xor/i_mul twiddle2=add/i_mul twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m3i0v323,aBreakerCD:4m0i2v301,aBreakerEF:4m1i0v032 lane_breaker=true braid_breaker=true jump_breaker=true swap_breaker=true mask_template=1 mask_seed_a=add_wave mask_seed_b=sbox_wave lightning=false/2 typhoon=false hurricane=false final_reverse=(True, False, False, False)
// ferocious[wa=2x2; wb=1x2; tw1=xor/i_mul; tw2=add/i_mul; tw2src=B; saltbox=128:0010272e:b33c6620; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m3i0v323,aBreakerCD:4m0i2v301,aBreakerEF:4m1i0v032; wa_rev=(False, False, True, False); wb_rev=(True, True, True, True); lane=on:3; braid=on:1; jump=on:1; swap=on:1; mask=workerbx1; lightning=off:2; typhoon=off; hurricane=off; final=2/(True, False, False, False); key_rot=15; maskA_seed=add_wave/True; maskB_seed=sbox_wave; maskA_bias=178; maskB_bias=176]
static std::uint32_t TwistCandidate_40286_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x437315A9u;
  aState = AdvanceTwiddle32(
      ...
      0x8293B731u,
      6u);
  aState ^= RotateLeft32(pValue + pExtra + 0xA7E10903u, 10U);
  return static_cast<std::uint32_t>(aState + 0xC1E0A298u);
}

static std::uint32_t TwistCandidate_40286_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x41366923u;
  aState = AdvanceTwiddle32(
      ...
      0xE6D76020u,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x2A4A75C9u, 7U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5DE12u);
}

static void TwistCandidate_40286_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_40286_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_40286_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_40286_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_40286_AdvanceTwiddle32B(
...
static void TwistCandidate_40286_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xA2U, 0xFBU, 0xCFU, 0x0DU, 0x6AU, 0x2CU, 0x13U, 0x62U, 0x3BU, 0x77U, 0x1EU, 0xEBU, 0x7AU, 0xADU, 0x1DU, 0xC3U,
      0xAFU, 0x08U, 0x98U, 0xA5U, 0x93U, 0x4EU, 0x97U, 0xDAU, 0x70U, 0x1CU, 0x48U, 0x84U, 0x94U, 0xE1U, 0x54U, 0x83U,
      0x31U, 0xDFU, 0x96U, 0xBAU, 0x12U, 0x22U, 0xD9U, 0xE5U, 0x02U, 0x24U, 0xDEU, 0xA7U, 0x8FU, 0xC9U, 0x2AU, 0x1FU,
      0x5CU, 0xA6U, 0xC7U, 0x18U, 0x8AU, 0x2EU, 0xAAU, 0x6DU, 0x92U, 0x81U, 0x5FU, 0x1BU, 0x8CU, 0x14U, 0xABU, 0x0BU,
      0x68U, 0xC5U, 0xD2U, 0xF4U, 0xE7U, 0x40U, 0xF9U, 0x44U, 0x8EU, 0x67U, 0xE4U, 0x15U, 0x4AU, 0x4CU, 0x41U, 0xF2U,
      0xCDU, 0x95U, 0x25U, 0x9DU, 0xDDU, 0x0AU, 0x03U, 0x49U, 0x80U, 0x01U, 0xBEU, 0xEAU, 0x19U, 0xF5U, 0x75U, 0x88U,
      0xC4U, 0x89U, 0xCBU, 0x61U, 0x26U, 0x36U, 0x66U, 0xE8U, 0xD8U, 0xC2U, 0x30U, 0xCEU, 0x50U, 0x55U, 0x3FU, 0xE2U,
      0xB0U, 0x76U, 0x42U, 0x2BU, 0xA1U, 0xCCU, 0xC6U, 0x05U, 0x9CU, 0x28U, 0xB2U, 0x99U, 0x4FU, 0x73U, 0xECU, 0xC0U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 114u ^ 225u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 22u + (-6701)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 20u + (2429)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 22u + static_cast<unsigned int>(114u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 114u)) & 127U]);
  ...
  TwistCandidate_40286_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(225u), 1u);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 43u + (-7891)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 50u + (3619)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 43u + pSalt[(aSourceIndex + 236u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(236u), 2u, true);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_40286_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(236u), static_cast<std::uint32_t>(236u), 2u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
...
static void TwistCandidate_40286_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 7u + (-2896)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 15u + (-1827)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 114u) ^ ((b << 1u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_40286_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 150u ^ 178u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 30u + (433)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-6844)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (30u + 31u) + (-5337)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 3u + (6395)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte + b + static_cast<std::uint32_t>(135u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 30u * 8U + static_cast<std::size_t>(aSourceIndex) * 31u + 150u + static_cast<std::size_t>(aTurbulence * ((3u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(150u), 4u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 135u) * ((3u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
  ...
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aReverseIndex * 44u + (433)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 22u + (-6844)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (44u + 22u) + (-5337)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aTurbulence & 0xFFu), 2u))) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c + static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 2u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 44u * 8U + static_cast<std::size_t>(aReverseIndex) * 22u * 3U + 89u + static_cast<std::size_t>(aTurbulence * ((3u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 6u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(89u), 6u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 135u) * ((3u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 6u, true);
  }
}

static void TwistCandidate_40286_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 4u ^ 176u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 2u + (-2412)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 29u + (5532)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (2u + 29u) + (5822)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 39u + (4876)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>(aMixBox[(aWorkerByte ^ a ^ static_cast<std::uint32_t>(89u)) & 0xFFu]);
    const std::uint32_t aSeedA = static_cast<std::uint32_t>((a ^ aTurbulence) & 0xFFu);
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(89u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 2u * 8U + static_cast<std::size_t>(aReverseIndex) * 29u + 4u + static_cast<std::size_t>(aTurbulence * ((39u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(4u), 5u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 89u) * ((39u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 4u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 63u + (-2412)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 61u + (5532)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (63u + 61u) + (5822)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 39u + (4876)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence + aWorkerByte) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(aMixBox[(aWorkerByte + c + static_cast<std::uint32_t>(89u)) & 0xFFu])) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 63u * 8U + static_cast<std::size_t>(aReverseIndex) * 61u * 3U + 81u + static_cast<std::size_t>(aTurbulence * ((39u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 1u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(81u), 1u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 89u) * ((39u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 1u, true);
  }
}

static void TwistCandidate_40286_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x9BU, 0xC7U, 0x97U, 0x93U, 0xF3U, 0xD2U, 0x4BU, 0x55U, 0x3DU, 0x28U, 0x44U, 0xB0U, 0xB7U, 0xCAU, 0x7CU, 0x4AU,
      0x14U, 0x83U, 0xEEU, 0xD1U, 0x13U, 0xB1U, 0x8AU, 0x8DU, 0xA9U, 0x47U, 0xB2U, 0x09U, 0x99U, 0x61U, 0xC6U, 0x96U,
      0x8BU, 0x90U, 0x18U, 0x9FU, 0x1DU, 0x7BU, 0xA8U, 0x2FU, 0xA6U, 0xB6U, 0x4CU, 0x6BU, 0x73U, 0x53U, 0x0EU, 0xE4U,
      0x69U, 0x00U, 0xF9U, 0x1FU, 0xDAU, 0xE2U, 0x85U, 0x23U, 0xE0U, 0xE6U, 0xB5U, 0x48U, 0x95U, 0xCEU, 0xEDU, 0xD9U,
      0x2CU, 0xA4U, 0x0FU, 0x21U, 0x46U, 0x45U, 0xE8U, 0x31U, 0xC1U, 0xAFU, 0x87U, 0x0BU, 0xBEU, 0x01U, 0xB8U, 0x6CU,
      0xCDU, 0x0DU, 0x89U, 0xBFU, 0xAEU, 0xC0U, 0xDFU, 0x71U, 0x07U, 0x75U, 0x94U, 0xFDU, 0x50U, 0x52U, 0x36U, 0x15U,
      0x19U, 0x65U, 0xF5U, 0x30U, 0x5CU, 0x1CU, 0xF8U, 0xADU, 0xF4U, 0x51U, 0x8FU, 0x25U, 0x2DU, 0x39U, 0xF1U, 0x24U,
      0x5FU, 0x42U, 0x6EU, 0x10U, 0x0AU, 0xA5U, 0xABU, 0xBBU, 0xEFU, 0xE3U, 0xDCU, 0x76U, 0xD0U, 0x12U, 0x92U, 0xEBU
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x45D46ABAu ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0xC55A5A2Cu, 7u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x0F474F5Au ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x6EB2F959u, 7u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-1742), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (5923), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (6852), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (5667), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0xC55A5A2Cu, 7u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x6EB2F959u, 7u);
...
static void TwistCandidate_40286_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x70U, 0x4FU, 0x39U, 0xCDU, 0x0AU, 0xC1U, 0xA5U, 0xE3U, 0x71U, 0x56U, 0x18U, 0x76U, 0x40U, 0xE7U, 0x97U, 0xB5U,
      0x2FU, 0xB2U, 0x57U, 0x67U, 0x1CU, 0x60U, 0xD1U, 0xCFU, 0x4DU, 0x0BU, 0xF3U, 0xC2U, 0x8CU, 0xBDU, 0xA1U, 0x2EU,
      0x54U, 0xAFU, 0x65U, 0xEDU, 0x13U, 0x6EU, 0xD5U, 0xC4U, 0x6AU, 0x30U, 0xBCU, 0x26U, 0xA2U, 0x81U, 0x06U, 0x8EU,
      0xFCU, 0x4AU, 0x2BU, 0xA0U, 0x23U, 0x5AU, 0xF9U, 0x3CU, 0xE1U, 0x3DU, 0xC5U, 0x35U, 0x27U, 0xABU, 0x36U, 0xDAU,
      0x68U, 0x04U, 0x6FU, 0x3BU, 0x6BU, 0xCAU, 0x15U, 0x4EU, 0x77U, 0xA7U, 0x53U, 0x29U, 0x85U, 0x44U, 0xD3U, 0x4BU,
      0xF1U, 0x50U, 0x88U, 0x2CU, 0xB7U, 0xA4U, 0x8AU, 0xA3U, 0x22U, 0xEAU, 0x9EU, 0x5DU, 0x58U, 0x03U, 0xE9U, 0x92U,
      0xDDU, 0x99U, 0xC9U, 0x7BU, 0xEEU, 0xFAU, 0x98U, 0xAEU, 0x94U, 0x73U, 0x93U, 0x5EU, 0x66U, 0xF6U, 0xE0U, 0x24U,
      0x7DU, 0x3AU, 0x37U, 0xFFU, 0xE8U, 0x72U, 0xE2U, 0x12U, 0xEBU, 0x9CU, 0x21U, 0x91U, 0x38U, 0xF8U, 0x10U, 0xF4U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x641CEF57u, 0xC627A69Eu, 4u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (2540), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltByte = static_cast<std::uint32_t>(pSalt[(i + 27u) & 31U]);
    const std::uint32_t aMix = static_cast<std::uint32_t>(aMixBox[(aDestByte ^ aSourceByte ^ aSaltByte ^ FoldWordToByte(aAcc)) & 127U]);
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xC627A69Eu, 4u);
...
static void TwistCandidate_40286_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_40286_AdvanceTwiddle32A(
      0xBD441782u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_40286_AdvanceTwiddle32B(
      0x0FB6CCEEu ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_40286_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_40286_AdvanceTwiddle32A(
      0x45E5AB7Du ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[29U]),
      0x8758DD42u ^ static_cast<std::uint32_t>(pSalt[16U]));
  std::uint32_t aPulseB = TwistCandidate_40286_AdvanceTwiddle32B(
      0xAD50A550u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[16U]),
      0x8A8456AAu ^ static_cast<std::uint32_t>(pSalt[29U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    for (std::size_t aLane = 0; aLane < kMatrixBlockBytes; ++aLane) {
      ...
      const unsigned char aMix = FixedMixBoxByte(static_cast<unsigned char>(
          ...
          pSalt[(aLane + 16U + (aPulseByteA & 3U)) & 31U]));
    ...
    aPulseA = TwistCandidate_40286_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_40286_AdvanceTwiddle32B(
...
static void TwistCandidate_40286_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x0485D132u ^ (pRound * 46u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-2896), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x0485D132u ^ (pRound * 46u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-1827), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-651), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-22), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 20u + 26u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-22 + 26), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(13u)) ^ static_cast<std::uint32_t>(95u)) & 0xFFu);
      const int aIndex2 = WrapRange(i + (4470), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(182u)), 0x85EBCA6Bu, 7u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (-2045), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (1794), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 57u + 20u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (1794 + 20), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 0u + 3U) & 15U), static_cast<std::size_t>(i + 15u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 12u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(5u)) ^ static_cast<std::uint32_t>(188u)) & 0xFFu);
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB + static_cast<std::uint32_t>(137u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[12u]), 0x27D4EB2Du, 6u);
    }
  }

  TwistCandidate_40286_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_40286_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_40286_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_40286_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (4258), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (3275), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (3745), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 28u + 549u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatOther = WrapRange(static_cast<int>(aMaskFlat) + (2 + 1), 0, static_cast<int>(kMaskStackTotalBytes));
      const std::uint32_t aMaskByteA = static_cast<std::uint32_t>(pMaskStackB[(aMaskFlat / kMaskBytes) % kMaskStackDepth][aMaskFlat % kMaskBytes]);
  ...
  TwistCandidate_40286_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (4853), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (7030), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (6903), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (-608), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_40286_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_40286_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 14u + (5135)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 8u + (-64)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-5112)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 14u + static_cast<unsigned int>(15u)) & 31U;
...
static void TwistCandidate_40286_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 21u + (-7145)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 11u + (6518)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 27u + (2877)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(178u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(178u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_40286_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-7405)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 20u + (5165)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 10u + (-2555)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
void TwistCandidate_40286(
  ...
  TwistCandidate_40286_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_40286_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_40286_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_40286_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_40286_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_40286_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_40286_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_40286_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_40286",
    &TwistCandidate_40286,
    &TwistCandidate_40286_KeySeed,
    &TwistCandidate_40286_SaltSeed,
    &TwistCandidate_40286_MaskSeedA,
    &TwistCandidate_40286_MaskSeedB,
    &TwistCandidate_40286_TwistBlock,
    &TwistCandidate_40286_PushKeyRound,
    &TwistCandidate_40286_PushMaskRoundA,
    &TwistCandidate_40286_PushMaskRoundB,
  }
};
