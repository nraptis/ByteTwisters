...
// Candidate 118280: TwistCandidate_118280
// family=ferocious_dual_worker op_budget=7 worker_shapes=2x2/2x1 worker_reverse=(False, False, False, False)/(True, True, True, False) twiddle1=add/none twiddle2=xor/i twiddle2_source=A tsunami=custom breaker_shapes=aBreakerAB:4m0i3v321 lane_breaker=false braid_breaker=true jump_breaker=false swap_breaker=false mask_template=0 mask_seed_a=off mask_seed_b=xor_wave lightning=false/3 typhoon=false hurricane=true final_reverse=(False, True, True, False)
// ferocious[wa=2x2; wb=2x1; tw1=add/none; tw2=xor/i; tw2src=A; saltbox=128:0010cccf:01400c7a; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m0i3v321; wa_rev=(False, False, False, False); wb_rev=(True, True, True, False); lane=off:1; braid=on:2; jump=off:3; swap=off:0; mask=workerbx0; lightning=off:3; typhoon=off; hurricane=on; final=3/(False, True, True, False); key_rot=28; maskA_seed=off/True; maskB_seed=xor_wave; maskA_bias=95; maskB_bias=191]
static std::uint32_t TwistCandidate_118280_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x80403D14u;
  aState = AdvanceTwiddle32(
      ...
      0xBAF4EE3Cu,
      8u);
  aState ^= RotateLeft32(pValue + pExtra + 0x5CB79B14u, 10U);
  return static_cast<std::uint32_t>(aState + 0x3AB4D328u);
}

static std::uint32_t TwistCandidate_118280_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x79511BD1u;
  aState = AdvanceTwiddle32(
      ...
      0x25E680C5u,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xA626BDD9u, 9U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5F5EDu);
}

static void TwistCandidate_118280_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_118280_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_118280_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_118280_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_118280_AdvanceTwiddle32B(
...
static void TwistCandidate_118280_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x56U, 0x7FU, 0x4BU, 0x45U, 0xDDU, 0xF6U, 0x59U, 0x0AU, 0xD1U, 0x5AU, 0x7CU, 0xB7U, 0xEEU, 0x50U, 0xFDU, 0x37U,
      0x21U, 0xD5U, 0xCAU, 0x04U, 0x97U, 0x9BU, 0x4AU, 0x3DU, 0x9DU, 0xF8U, 0xCFU, 0x53U, 0xA6U, 0x13U, 0xBDU, 0x83U,
      0xA4U, 0xD4U, 0x99U, 0x5FU, 0x96U, 0x64U, 0x98U, 0xB6U, 0x71U, 0x91U, 0x02U, 0xDEU, 0xC1U, 0xAEU, 0x66U, 0x3BU,
      0xB3U, 0xFAU, 0x8CU, 0x8BU, 0x4FU, 0x3EU, 0xFFU, 0xA7U, 0xC6U, 0xC5U, 0xCBU, 0x19U, 0x6EU, 0x0BU, 0x07U, 0xC0U,
      0x6BU, 0x1FU, 0x03U, 0x0FU, 0xC7U, 0x2DU, 0x5CU, 0xCEU, 0xB5U, 0xAAU, 0x57U, 0xBBU, 0xD7U, 0x3FU, 0x1EU, 0x08U,
      0x2CU, 0x5EU, 0xA1U, 0x7AU, 0xEFU, 0x88U, 0xB1U, 0xF3U, 0x8FU, 0x90U, 0x69U, 0xE2U, 0x52U, 0xE4U, 0xE9U, 0xFBU,
      0x94U, 0x5DU, 0xBEU, 0x48U, 0xAFU, 0x27U, 0x4CU, 0x01U, 0x63U, 0xEDU, 0xBCU, 0xCCU, 0x93U, 0xDAU, 0xE6U, 0x51U,
      0x61U, 0xADU, 0x06U, 0xA8U, 0x8AU, 0x38U, 0x7DU, 0x40U, 0xE7U, 0xD9U, 0x78U, 0x00U, 0xF4U, 0x09U, 0x5BU, 0xBAU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 246u ^ 138u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 29u + (875)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 20u + (-6814)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 29u + static_cast<unsigned int>(246u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 246u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(138u), 3u, true);
  ...
  TwistCandidate_118280_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(138u), 3u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(138u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 53u + (-2297)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 47u + (-3642)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 53u + pSalt[(aSourceIndex + 174u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(174u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(174u), 2u, true);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_118280_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(174u), static_cast<std::uint32_t>(174u), 2u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(174u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_118280_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 29u + (-2996)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 12u + (-4091)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 246u) ^ ((b << 3u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_118280_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 66u ^ 95u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 11u + (-7665)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 16u + (1782)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (11u + 16u) + (1178)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 11u * 8U + static_cast<std::size_t>(aSourceIndex) * 16u + 66u + static_cast<std::size_t>(aTurbulence * ((53u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(66u), 4u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 23u + (-7665)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 26u + (1782)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (23u + 26u) + (1178)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 23u * 8U + static_cast<std::size_t>(aSourceIndex) * 26u * 3U + 253u + static_cast<std::size_t>(aTurbulence * ((53u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 6u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(253u), 6u, false);
  }
}

static void TwistCandidate_118280_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 199u ^ 191u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 14u + (-1147)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 4u + (1670)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (14u + 4u) + (7573)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 38u + (-2112)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 6u)) ^ static_cast<std::uint32_t>(68u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 14u * 8U + static_cast<std::size_t>(aSourceIndex) * 4u + 199u + static_cast<std::size_t>(aTurbulence * ((38u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(199u), 5u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 68u) * ((38u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 6u, false);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 25u + (-1147)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aReverseIndex * 24u + (1670)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (25u + 24u) + (7573)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 38u + (-2112)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 6u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 25u * 8U + static_cast<std::size_t>(aReverseIndex) * 24u * 3U + 115u + static_cast<std::size_t>(aTurbulence * ((38u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 1u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(115u), 1u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 68u) * ((38u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 1u, true);
  }
}

static void TwistCandidate_118280_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0xF7U, 0xCFU, 0xC1U, 0x71U, 0x0DU, 0x86U, 0xC0U, 0x24U, 0x73U, 0x91U, 0x6AU, 0x9FU, 0x33U, 0x17U, 0x76U, 0x81U,
      0x69U, 0x2AU, 0x6CU, 0x20U, 0x36U, 0xF0U, 0xADU, 0x83U, 0xFEU, 0xEEU, 0xA5U, 0xC9U, 0x8EU, 0x49U, 0x5FU, 0xCAU,
      0x79U, 0xE1U, 0xF1U, 0xE8U, 0xA6U, 0x75U, 0x43U, 0x64U, 0xC6U, 0x65U, 0xA2U, 0x11U, 0x8DU, 0xACU, 0x77U, 0xD2U,
      0x40U, 0xB7U, 0x07U, 0xC4U, 0x47U, 0x53U, 0xD6U, 0xAAU, 0x8BU, 0x6DU, 0x31U, 0x50U, 0x92U, 0x1BU, 0x52U, 0x93U,
      0xABU, 0x44U, 0xE7U, 0x9EU, 0x2EU, 0xF6U, 0x38U, 0x16U, 0x18U, 0xDCU, 0xAFU, 0x25U, 0x74U, 0xB6U, 0x1DU, 0xD0U,
      0x0FU, 0x7DU, 0x4CU, 0x21U, 0xCCU, 0x7AU, 0x06U, 0x60U, 0x3CU, 0x9AU, 0x41U, 0x6BU, 0x5EU, 0xFCU, 0xD3U, 0xB4U,
      0x7BU, 0x96U, 0x01U, 0x35U, 0x3BU, 0xE4U, 0xBFU, 0x0AU, 0x2CU, 0x58U, 0x97U, 0x30U, 0xC8U, 0xA1U, 0x7EU, 0x56U,
      0x7FU, 0x2DU, 0x8CU, 0x89U, 0x55U, 0xDDU, 0x9CU, 0x05U, 0x68U, 0x1FU, 0x09U, 0xF5U, 0x23U, 0x0CU, 0x26U, 0xA0U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x319357B4u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x583DD641u, 8u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x76A7C8D3u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x42571E05u, 11u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (7058), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-2350), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4109), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (476), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x583DD641u, 8u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x42571E05u, 11u);
...
static void TwistCandidate_118280_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x31U, 0x25U, 0x4CU, 0xC8U, 0x80U, 0xD4U, 0xFBU, 0x78U, 0x86U, 0x83U, 0xAEU, 0x19U, 0x7CU, 0xBEU, 0x8FU, 0x1DU,
      0xB4U, 0x5FU, 0xBCU, 0x48U, 0x0EU, 0xABU, 0x96U, 0x8EU, 0xF3U, 0x67U, 0xC3U, 0xEEU, 0xFCU, 0x16U, 0xF4U, 0x15U,
      0x22U, 0x98U, 0x2DU, 0x4AU, 0xE7U, 0x44U, 0x6FU, 0x35U, 0x46U, 0x06U, 0x57U, 0x8AU, 0x52U, 0x8CU, 0xECU, 0xFAU,
      0x5AU, 0xACU, 0x7FU, 0x4DU, 0x10U, 0x59U, 0x18U, 0x9FU, 0xC2U, 0x23U, 0xB1U, 0x95U, 0xA9U, 0x00U, 0xBFU, 0x40U,
      0xC0U, 0x3BU, 0xAAU, 0x79U, 0x88U, 0xD6U, 0x5DU, 0x26U, 0x49U, 0x7BU, 0x42U, 0x92U, 0xC7U, 0xEDU, 0xCEU, 0xF7U,
      0x5EU, 0x6EU, 0x36U, 0x1CU, 0xC1U, 0xE4U, 0x65U, 0x97U, 0x01U, 0xE0U, 0xDCU, 0xD8U, 0x94U, 0x2FU, 0x64U, 0xF2U,
      0x71U, 0xA1U, 0xE1U, 0x70U, 0x13U, 0x29U, 0x75U, 0x27U, 0xA5U, 0x32U, 0xE5U, 0x8DU, 0x81U, 0xC5U, 0x9BU, 0x17U,
      0x7AU, 0xEAU, 0x76U, 0xF9U, 0x2BU, 0x54U, 0x55U, 0xA2U, 0x0DU, 0xDEU, 0x73U, 0x93U, 0x51U, 0xADU, 0xA7U, 0x20U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x3E3C64D3u, 0x6330934Bu, 5u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-2840), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x6330934Bu, 5u);
...
static void TwistCandidate_118280_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_118280_AdvanceTwiddle32A(
      0xAC269AA4u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_118280_AdvanceTwiddle32B(
      0x7EAA53FCu ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_118280_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_118280_AdvanceTwiddle32A(
      0xA89A5CC4u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[4U]),
      0x6A2A4EFBu ^ static_cast<std::uint32_t>(pSalt[3U]));
  std::uint32_t aPulseB = TwistCandidate_118280_AdvanceTwiddle32B(
      0x922019A3u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[3U]),
      0xB5F50AC1u ^ static_cast<std::uint32_t>(pSalt[4U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_118280_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_118280_AdvanceTwiddle32B(
...
static void TwistCandidate_118280_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x9547AFFCu ^ (pRound * 32u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-2996), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x9547AFFCu ^ (pRound * 32u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-4091), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (4830), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-4612), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 130u + 4u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-4612 + 4), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const int aIndex2 = WrapRange(i + (-7220), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(139u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[1u]), 0x27D4EB2Du, 4u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (-1567), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-3970), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 27u + 1u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-3970 + 1), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 3u + 3U) & 15U), static_cast<std::size_t>(i + 17u)));
      ...
      const int aIndex2 = WrapRange(aReverseIndex + (-6462), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(182u)), 0x85EBCA6Bu, 1u);
    }
  }

  TwistCandidate_118280_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_118280_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-5536), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-3399), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-3902), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 9u + 238u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (3907), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (3643), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    ...
    switch (static_cast<unsigned>(storm_control[13] ^ storm_mask_a[17] ^ storm_mask_b[19] ^ pSalt[2]) % 7u) {
      ...
      case 6u: storm.ApplyFastOp(HurricaneFastOp::kSwapColumns, static_cast<std::uint8_t>(storm_control[29] ^ storm_mask_a[31] ^ storm_mask_b[37]), static_cast<std::uint8_t>(storm_control[43] + storm_mask_a[47] + storm_mask_b[53])); break;
    }
    }
  ...
  TwistCandidate_118280_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (7225), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (1279), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (2502), aStart, aEnd);
      const int aControlIndex = WrapRange(i + (2584), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_118280_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_118280_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 23u + (3015)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 3u + (2832)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 27u + (4291)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 23u + static_cast<unsigned int>(28u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 138u)));
...
static void TwistCandidate_118280_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 25u + (4682)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 12u + (-912)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 16u + (7417)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_118280_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 6u + (-6340)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 18u + (-5811)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 13u + (803)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(191u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(191u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_118280(
  ...
  TwistCandidate_118280_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_118280_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_118280_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_118280_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_118280_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_118280_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_118280_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_118280_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_118280",
    &TwistCandidate_118280,
    &TwistCandidate_118280_KeySeed,
    &TwistCandidate_118280_SaltSeed,
    &TwistCandidate_118280_MaskSeedA,
    &TwistCandidate_118280_MaskSeedB,
    &TwistCandidate_118280_TwistBlock,
    &TwistCandidate_118280_PushKeyRound,
    &TwistCandidate_118280_PushMaskRoundA,
    &TwistCandidate_118280_PushMaskRoundB,
  }
};
