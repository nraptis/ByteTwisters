...
// Candidate 83495: TwistCandidate_83495
// family=ferocious_dual_worker op_budget=9 worker_shapes=3x4/3x1 worker_reverse=(False, False, False, True)/(True, True, False, True) twiddle1=add/i twiddle2=add/none twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m1i1v032,aBreakerCD:4m3i3v230 lane_breaker=true braid_breaker=false jump_breaker=false swap_breaker=true mask_template=2 mask_seed_a=off mask_seed_b=off lightning=true/3 typhoon=false hurricane=true final_reverse=(True, True, True, True)
// ferocious[wa=3x4; wb=3x1; tw1=add/i; tw2=add/none; tw2src=B; saltbox=128:00102635:6dcadf7d; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m1i1v032,aBreakerCD:4m3i3v230; wa_rev=(False, False, False, True); wb_rev=(True, True, False, True); lane=on:2; braid=off:1; jump=off:2; swap=on:1; mask=workerax2; lightning=on:3; typhoon=off; hurricane=on; final=3/(True, True, True, True); key_rot=19; maskA_seed=off/True; maskB_seed=off; maskA_bias=235; maskB_bias=215]
static std::uint32_t TwistCandidate_83495_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x72D3408Cu;
  aState = AdvanceTwiddle32(
      ...
      0xBB3C4E29u,
      8u);
  aState ^= RotateLeft32(pValue + pExtra + 0xAFEEC076u, 10U);
  return static_cast<std::uint32_t>(aState + 0xC9EF0EA5u);
}

static std::uint32_t TwistCandidate_83495_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0x78998D4Bu;
  aState = AdvanceTwiddle32(
      ...
      0xD7774D3Du,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x4A4A8DC9u, 9U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5C362u);
}

static void TwistCandidate_83495_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_83495_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_83495_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_83495_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_83495_AdvanceTwiddle32B(
...
static void TwistCandidate_83495_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x6CU, 0xEBU, 0x81U, 0x8BU, 0x63U, 0xC9U, 0x56U, 0x5EU, 0x5CU, 0xCFU, 0xBFU, 0xC7U, 0x93U, 0xD7U, 0x2DU, 0x61U,
      0x3FU, 0x19U, 0xEAU, 0xFFU, 0xECU, 0xA2U, 0xF8U, 0xA3U, 0xE2U, 0x68U, 0x2EU, 0xEDU, 0x8CU, 0xBBU, 0x0BU, 0xA0U,
      0x71U, 0xA6U, 0x32U, 0x44U, 0xE9U, 0x80U, 0xB2U, 0x59U, 0x87U, 0x72U, 0x17U, 0x1DU, 0xD4U, 0x96U, 0xDCU, 0x02U,
      0xC4U, 0x00U, 0x60U, 0x28U, 0xAEU, 0x6FU, 0x66U, 0x4BU, 0x0FU, 0x33U, 0x5DU, 0x52U, 0xABU, 0x5FU, 0x7AU, 0x90U,
      0x9AU, 0x48U, 0x04U, 0xA9U, 0x97U, 0x54U, 0x14U, 0x2AU, 0x36U, 0xB5U, 0x95U, 0x4EU, 0x8EU, 0x0CU, 0x41U, 0x8DU,
      0xE0U, 0xE3U, 0xF7U, 0x9CU, 0xBDU, 0xB0U, 0xF2U, 0x67U, 0x92U, 0xA5U, 0x79U, 0x3CU, 0xA4U, 0x58U, 0x31U, 0x1FU,
      0x57U, 0xFAU, 0xC3U, 0x2CU, 0x76U, 0xFEU, 0x83U, 0xC2U, 0x22U, 0x64U, 0xC5U, 0xFCU, 0xD8U, 0x25U, 0x6EU, 0x27U,
      0x0EU, 0x4AU, 0xC8U, 0x16U, 0x8FU, 0xF5U, 0xB3U, 0xC1U, 0x37U, 0xD2U, 0xCBU, 0x21U, 0x30U, 0xF4U, 0x84U, 0x9DU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 236u ^ 253u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 13u + (-7313)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 20u + (-2053)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 13u + static_cast<unsigned int>(236u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 236u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(253u), 3u, false);
  ...
  TwistCandidate_83495_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(253u), 3u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(253u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 18u + (-3558)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 18u + (-5808)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 18u + pSalt[(aSourceIndex + 121u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(121u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(121u), 2u, false);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_83495_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(121u), static_cast<std::uint32_t>(121u), 2u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(121u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_83495_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 9u + (-5930)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 9u + (5243)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 236u) ^ ((b << 3u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_83495_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 205u ^ 235u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 2u + (3856)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 25u + (2300)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (2u + 25u) + (4238)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 2u * 8U + static_cast<std::size_t>(aReverseIndex) * 25u + 205u + static_cast<std::size_t>(aTurbulence * ((63u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(205u), 5u, true);
  }
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 36u + (3856)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 38u + (2300)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (36u + 38u) + (4238)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 36u * 8U + static_cast<std::size_t>(aSourceIndex) * 38u * 3U + 189u + static_cast<std::size_t>(aTurbulence * ((63u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 3u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(189u), 3u, true);
  }
}

static void TwistCandidate_83495_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 145u ^ 215u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 13u + (-3145)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 14u + (5625)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (13u + 14u) + (2619)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const std::uint32_t aWorkerByte = 0U;
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 13u * 8U + static_cast<std::size_t>(aReverseIndex) * 14u + 145u + static_cast<std::size_t>(aTurbulence * ((9u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(145u), 7u, false);
  }
}

static void TwistCandidate_83495_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x4EU, 0x34U, 0xABU, 0xA9U, 0x13U, 0x2AU, 0xFAU, 0x33U, 0xC8U, 0x7EU, 0x51U, 0x7AU, 0xB6U, 0xA2U, 0xEAU, 0x9BU,
      0xD6U, 0x46U, 0x9DU, 0xF4U, 0x9EU, 0x74U, 0xACU, 0xD4U, 0x26U, 0xB3U, 0x39U, 0x91U, 0x63U, 0x10U, 0xDDU, 0x00U,
      0x07U, 0x99U, 0x42U, 0x97U, 0xF6U, 0x11U, 0xE2U, 0x70U, 0x9AU, 0x9CU, 0xD0U, 0x35U, 0xB8U, 0xA5U, 0x98U, 0xC1U,
      0x31U, 0x1BU, 0xC7U, 0xBCU, 0xF7U, 0xAEU, 0xE4U, 0xE6U, 0xEFU, 0x72U, 0x0EU, 0xB7U, 0xE0U, 0x0CU, 0xBBU, 0x28U,
      0xD7U, 0xD1U, 0xF8U, 0x79U, 0xC3U, 0xC4U, 0x27U, 0xEEU, 0x18U, 0x38U, 0x20U, 0xECU, 0x67U, 0x29U, 0x6BU, 0xF9U,
      0x04U, 0x56U, 0xF0U, 0xA0U, 0xD9U, 0x23U, 0x86U, 0xF5U, 0xC2U, 0x62U, 0x16U, 0xCCU, 0x50U, 0x2EU, 0xCBU, 0x36U,
      0x54U, 0x53U, 0x82U, 0x6FU, 0xA6U, 0x40U, 0x6CU, 0x7CU, 0xF1U, 0x85U, 0x24U, 0xBAU, 0xE1U, 0x17U, 0x68U, 0x2DU,
      0xE8U, 0x76U, 0x1AU, 0xB9U, 0x55U, 0xBDU, 0x5CU, 0x25U, 0x78U, 0x88U, 0xDEU, 0x75U, 0xFEU, 0xB2U, 0x4CU, 0x3FU
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0xBDF804F3u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0xE482C8E2u, 6u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x88048D56u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0xA7E6D1E4u, 7u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-7149), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-1546), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (6187), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-4348), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0xE482C8E2u, 6u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0xA7E6D1E4u, 7u);
...
static void TwistCandidate_83495_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x8EU, 0xB1U, 0xB3U, 0xC5U, 0xFDU, 0xFEU, 0x6DU, 0x63U, 0xBCU, 0x25U, 0xE8U, 0xF2U, 0x5AU, 0x60U, 0x84U, 0xD4U,
      0xDDU, 0x7DU, 0xF7U, 0x45U, 0x85U, 0xF5U, 0x32U, 0x78U, 0xCFU, 0x50U, 0x2EU, 0xDFU, 0x72U, 0xE1U, 0xC3U, 0x54U,
      0x97U, 0xA9U, 0xF4U, 0x0CU, 0x8BU, 0x99U, 0x7AU, 0xD5U, 0x42U, 0x3CU, 0x08U, 0x59U, 0x02U, 0x8CU, 0xCBU, 0xB8U,
      0x16U, 0x8DU, 0x83U, 0x03U, 0x07U, 0x36U, 0x6AU, 0x1DU, 0x00U, 0x82U, 0x66U, 0xD7U, 0xC1U, 0x3DU, 0x81U, 0x28U,
      0x1EU, 0x18U, 0x79U, 0x01U, 0xC7U, 0x47U, 0xB7U, 0xAFU, 0xADU, 0x06U, 0xDCU, 0x9EU, 0x8FU, 0xBAU, 0x88U, 0x93U,
      0xD2U, 0x39U, 0x6BU, 0xA4U, 0x4FU, 0xABU, 0x6FU, 0x74U, 0x65U, 0x90U, 0x2CU, 0xD8U, 0x76U, 0x71U, 0x56U, 0x80U,
      0x57U, 0x53U, 0x69U, 0xAEU, 0xE6U, 0x04U, 0x46U, 0xC9U, 0x9BU, 0xC0U, 0x5EU, 0x3FU, 0x49U, 0x21U, 0x6EU, 0xFFU,
      0x14U, 0x0EU, 0xBFU, 0x0FU, 0xACU, 0x12U, 0x67U, 0x86U, 0x3BU, 0x35U, 0xFAU, 0x40U, 0x70U, 0x8AU, 0x89U, 0xD1U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x815C1175u, 0x4981A455u, 6u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4171), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x4981A455u, 6u);
...
static void TwistCandidate_83495_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_83495_AdvanceTwiddle32A(
      0x1C341043u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_83495_AdvanceTwiddle32B(
      0x62B126E9u ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_83495_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_83495_AdvanceTwiddle32A(
      0xB4110786u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[6U]),
      0x76A49BBDu ^ static_cast<std::uint32_t>(pSalt[8U]));
  std::uint32_t aPulseB = TwistCandidate_83495_AdvanceTwiddle32B(
      0x03E48B68u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[8U]),
      0x24307859u ^ static_cast<std::uint32_t>(pSalt[6U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_83495_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_83495_AdvanceTwiddle32B(
...
static void TwistCandidate_83495_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0xFE0AC964u ^ (pRound * 47u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-5930), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0xFE0AC964u ^ (pRound * 47u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (5243), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-6660), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (5741), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 27u + 24u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (5741 + 24), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 11u + 3U) & 15U), static_cast<std::size_t>(i + 22u)));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>((aLaneIndex ^ static_cast<std::uint32_t>(214u)) & 0xFFu);
      const int aIndex2 = WrapRange(i + (4761), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(i + (4127), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(78u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[22u]), 0x27D4EB2Du, 2u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (4895), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (4187), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 88u + 9u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (4187 + 9), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 1u + 3U) & 15U), static_cast<std::size_t>(i + 16u)));
      ...
      const int aIndex2 = WrapRange(aReverseIndex + (-311), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(i + (3617), aStart, aEnd);
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex3]);
      const std::uint32_t aRoute = ((aSourceCtrl >> 2u) ^ aWorkerCtrl ^ aKeyByte ^ aMaskByte ^ static_cast<std::uint32_t>(1u)) % 5U;
      std::uint32_t aValue = a;
      switch (aRoute) {
        case 0U: aValue = a; break;
        case 1U: aValue = b; break;
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB + static_cast<std::uint32_t>(36u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[25u]), 0x27D4EB2Du, 4u);
    }
  }

  TwistCandidate_83495_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_83495_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_83495_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (284), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (6159), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-542), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 20u + 586u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    auto storm_block = LoadBlock16Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-4789), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock16Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (1569), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackA, chunk + 678u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kMatrixBlockBytes>(pMaskStackB, chunk + 678u + 17u);
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-5442), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (5192), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 1381u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 1381u + 29u);
    ...
    switch (static_cast<unsigned>(storm_control[13] ^ storm_mask_a[17] ^ storm_mask_b[19] ^ pSalt[2]) % 7u) {
      ...
      case 1u: storm.ApplyFastOp(HurricaneFastOp::kWeaveRows, static_cast<std::uint8_t>(storm_control[29] ^ storm_mask_a[31] ^ storm_mask_b[37]), static_cast<std::uint8_t>(storm_control[43] + storm_mask_a[47] + storm_mask_b[53])); break;
  ...
  TwistCandidate_83495_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (4302), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (5205), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (-218), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (-4527), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_83495_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_83495_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 15u + (-7337)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 17u + (-2701)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 6u + (-5387)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 15u + static_cast<unsigned int>(19u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 253u)));
...
static void TwistCandidate_83495_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 18u + (203)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 21u + (-7350)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 11u + (-4138)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_83495_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 17u + (-2751)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 5u + (2762)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 18u + (7006)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(215u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(215u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_83495(
  ...
  TwistCandidate_83495_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_83495_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_83495_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_83495_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_83495_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_83495_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_83495_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_83495_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_83495",
    &TwistCandidate_83495,
    &TwistCandidate_83495_KeySeed,
    &TwistCandidate_83495_SaltSeed,
    &TwistCandidate_83495_MaskSeedA,
    &TwistCandidate_83495_MaskSeedB,
    &TwistCandidate_83495_TwistBlock,
    &TwistCandidate_83495_PushKeyRound,
    &TwistCandidate_83495_PushMaskRoundA,
    &TwistCandidate_83495_PushMaskRoundB,
  }
};
