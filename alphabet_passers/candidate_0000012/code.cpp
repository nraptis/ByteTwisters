...
// Candidate 13: TwistCandidate_0013
// family=ferocious_dual_worker op_budget=9 worker_shapes=1x4/2x1 worker_reverse=(True, True, True, True)/(False, True, False, False) twiddle1=xor/i twiddle2=xor/i_mul twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m1i1v132,aBreakerCD:4m0i3v212,aBreakerEF:4m3i0v013 lane_breaker=true braid_breaker=true jump_breaker=false swap_breaker=true mask_template=4 mask_seed_a=xor_wave mask_seed_b=xor_wave lightning=false/0 typhoon=true hurricane=false final_reverse=(True, True, False, True)
// ferocious[wa=1x4; wb=2x1; tw1=xor/i; tw2=xor/i_mul; tw2src=B; saltbox=128:0010fb8a:5a43cd0c; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m1i1v132,aBreakerCD:4m0i3v212,aBreakerEF:4m3i0v013; wa_rev=(True, True, True, True); wb_rev=(False, True, False, False); lane=on:0; braid=on:2; jump=off:3; swap=on:3; mask=workerbx4; lightning=off:0; typhoon=on; hurricane=off; final=5/(True, True, False, True); key_rot=26; maskA_seed=xor_wave/False; maskB_seed=xor_wave; maskA_bias=86; maskB_bias=239]
static std::uint32_t TwistCandidate_0013_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0xB1F50FE8u;
  aState = AdvanceTwiddle32(
      ...
      0x38990B19u,
      5u);
  aState ^= RotateLeft32(pValue + pExtra + 0xEF6C115Au, 8U);
  return static_cast<std::uint32_t>(aState + 0x896C04F1u);
}

static std::uint32_t TwistCandidate_0013_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xFB3D3B06u;
  aState = AdvanceTwiddle32(
      ...
      0x14512A5Cu,
      7u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xC6463A44u, 6U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A4301Fu);
}

static void TwistCandidate_0013_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_0013_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_0013_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_0013_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_0013_AdvanceTwiddle32B(
...
static void TwistCandidate_0013_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xB1U, 0xEEU, 0x94U, 0x5CU, 0x8AU, 0x9DU, 0x0AU, 0x4FU, 0xA4U, 0xD4U, 0xA3U, 0x2EU, 0xDEU, 0x70U, 0x41U, 0x1DU,
      0x4DU, 0x2AU, 0xDCU, 0x19U, 0x10U, 0x6BU, 0x99U, 0x1FU, 0x75U, 0x39U, 0x92U, 0xDAU, 0xBBU, 0xDDU, 0xA7U, 0x84U,
      0xFEU, 0x15U, 0x20U, 0x22U, 0x98U, 0xF7U, 0x42U, 0x77U, 0x27U, 0xC3U, 0x6CU, 0xB8U, 0xC8U, 0xC0U, 0xFDU, 0x3CU,
      0xE1U, 0xF0U, 0x50U, 0x87U, 0x89U, 0x4EU, 0x11U, 0x13U, 0x03U, 0x40U, 0x54U, 0xF4U, 0x7FU, 0x2DU, 0x9BU, 0x0FU,
      0x97U, 0xCFU, 0xD3U, 0x34U, 0x7BU, 0xFAU, 0xA0U, 0x01U, 0x04U, 0xC7U, 0xCEU, 0x62U, 0xC2U, 0x82U, 0x83U, 0xEBU,
      0x1CU, 0xD9U, 0x66U, 0x52U, 0xB9U, 0x43U, 0x8BU, 0x37U, 0x0CU, 0x06U, 0xF6U, 0x93U, 0x6FU, 0x23U, 0xCDU, 0x2CU,
      0x95U, 0xA5U, 0xECU, 0xB6U, 0xC9U, 0xA8U, 0x80U, 0x3EU, 0xAFU, 0xEAU, 0x64U, 0xD6U, 0xB7U, 0xA9U, 0xBCU, 0x76U,
      0xADU, 0x61U, 0xEDU, 0x5DU, 0x4AU, 0x8EU, 0x9CU, 0x7AU, 0xD5U, 0xE2U, 0x3FU, 0x72U, 0xB3U, 0x58U, 0x71U, 0x8CU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 222u ^ 249u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 16u + (-6844)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 15u + (4757)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 16u + static_cast<unsigned int>(222u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 222u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(249u), 7u, true);
  ...
  TwistCandidate_0013_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(249u), 7u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(249u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
...
static void TwistCandidate_0013_KeySeed(
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 20u + (-5194)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 9u + (1949)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 222u) ^ ((b << 7u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_0013_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 21u ^ 86u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 22u + (-6721)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 27u + (2416)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (22u + 27u) + (3419)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 8u + (4640)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 5u)) ^ static_cast<std::uint32_t>(209u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 22u * 8U + static_cast<std::size_t>(aReverseIndex) * 27u + 21u + static_cast<std::size_t>(aTurbulence * ((8u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(21u), 5u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 209u) * ((8u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 5u, false);
  }
}

static void TwistCandidate_0013_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 221u ^ 239u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 5u + (2392)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 7u + (7160)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (5u + 7u) + (7262)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 7u + (2645)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 5u)) ^ static_cast<std::uint32_t>(232u)) & 0xFFu);
    const std::uint32_t aSeedA = static_cast<std::uint32_t>((a ^ aTurbulence) & 0xFFu);
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 5u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 5u * 8U + static_cast<std::size_t>(aSourceIndex) * 7u + 221u + static_cast<std::size_t>(aTurbulence * ((7u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(221u), 1u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 232u) * ((7u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 5u, true);
  }
}

static void TwistCandidate_0013_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x88U, 0x3FU, 0x65U, 0x2CU, 0xAEU, 0x0CU, 0xCAU, 0x8EU, 0x41U, 0xD2U, 0x29U, 0x1DU, 0x14U, 0xEDU, 0xD1U, 0x5DU,
      0xA2U, 0xF1U, 0xA9U, 0xAAU, 0x94U, 0x37U, 0xB7U, 0x52U, 0xE3U, 0x9CU, 0x4AU, 0x9AU, 0xB4U, 0x2DU, 0xD5U, 0x07U,
      0x9EU, 0x9DU, 0xF8U, 0xB6U, 0x7AU, 0xDDU, 0x85U, 0x1CU, 0xF6U, 0x91U, 0xEBU, 0xB1U, 0xB3U, 0x56U, 0x2EU, 0x69U,
      0x6DU, 0xCFU, 0x05U, 0xA7U, 0x3CU, 0x6CU, 0x82U, 0x12U, 0xE2U, 0x4BU, 0x50U, 0x39U, 0x0FU, 0xDAU, 0xF3U, 0x28U,
      0xCCU, 0x93U, 0x3BU, 0x99U, 0x16U, 0xFEU, 0x32U, 0x70U, 0x46U, 0x26U, 0x15U, 0xCDU, 0xFFU, 0x2FU, 0xBCU, 0x76U,
      0x0BU, 0xD7U, 0xBEU, 0xB8U, 0x4CU, 0x5BU, 0xA4U, 0x75U, 0x79U, 0x42U, 0x44U, 0x8FU, 0x4EU, 0x01U, 0xA6U, 0x80U,
      0xB9U, 0x6AU, 0x11U, 0x97U, 0x6EU, 0x98U, 0xFDU, 0x92U, 0xF9U, 0x02U, 0x66U, 0x6BU, 0x1AU, 0xA0U, 0x0EU, 0x5CU,
      0xADU, 0x8DU, 0xABU, 0xB2U, 0x95U, 0xF0U, 0xC7U, 0x89U, 0x25U, 0xD3U, 0xEEU, 0x5EU, 0x64U, 0x74U, 0xE0U, 0x96U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x6EF7A091u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x5D477FBCu, 8u);
  std::uint32_t aAccB = AdvanceTwiddle32(0xFFDC41EDu ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x81492262u, 4u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    ...
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (3313), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-3941), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (3184), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x5D477FBCu, 8u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x81492262u, 4u);
...
static void TwistCandidate_0013_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x61U, 0x84U, 0xDFU, 0x25U, 0x34U, 0x2CU, 0x9BU, 0x2BU, 0x0FU, 0x30U, 0x22U, 0x44U, 0x9DU, 0x9EU, 0x88U, 0xE6U,
      0xABU, 0x80U, 0x2DU, 0xA9U, 0x4CU, 0xBAU, 0x65U, 0x5CU, 0x86U, 0xACU, 0x2AU, 0x10U, 0xF0U, 0x7DU, 0xD6U, 0x7BU,
      0x29U, 0xA7U, 0x57U, 0xC0U, 0xADU, 0x02U, 0xD3U, 0x45U, 0x78U, 0x31U, 0xB4U, 0x52U, 0x7AU, 0x14U, 0xB5U, 0x71U,
      0xD0U, 0xEFU, 0x46U, 0xBCU, 0x24U, 0xEEU, 0xC4U, 0x53U, 0xBEU, 0xD5U, 0x0EU, 0xBFU, 0x41U, 0xF2U, 0x40U, 0xC8U,
      0xCBU, 0x18U, 0xE8U, 0x92U, 0x42U, 0x19U, 0xCCU, 0x5AU, 0x6FU, 0x6BU, 0x96U, 0x59U, 0x5EU, 0x28U, 0x73U, 0x20U,
      0x35U, 0x93U, 0xA8U, 0xB3U, 0xF3U, 0xE9U, 0x6EU, 0x91U, 0x66U, 0xD7U, 0x6CU, 0x06U, 0x01U, 0x6DU, 0x1FU, 0x83U,
      0x07U, 0xE7U, 0x95U, 0xFFU, 0x76U, 0xC1U, 0x17U, 0x8AU, 0xECU, 0x8EU, 0x79U, 0x89U, 0xB8U, 0x69U, 0x1EU, 0xCEU,
      0xE5U, 0x63U, 0xAFU, 0x8BU, 0x72U, 0x08U, 0x1AU, 0x4DU, 0xBBU, 0xEBU, 0xE1U, 0xCAU, 0x3CU, 0x8FU, 0xFBU, 0x00U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0xCD2517EAu, 0xB1A198A1u, 1u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-6528), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xB1A198A1u, 1u);
...
static void TwistCandidate_0013_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_0013_AdvanceTwiddle32A(
      0xAB901F09u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_0013_AdvanceTwiddle32B(
      0x04F879CBu ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_0013_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_0013_AdvanceTwiddle32A(
      0x1678CD6Au ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[10U]),
      0xD4CC135Cu ^ static_cast<std::uint32_t>(pSalt[14U]));
  std::uint32_t aPulseB = TwistCandidate_0013_AdvanceTwiddle32B(
      0xF888620Eu + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[14U]),
      0xDF5CC150u ^ static_cast<std::uint32_t>(pSalt[10U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_0013_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_0013_AdvanceTwiddle32B(
...
static void TwistCandidate_0013_TwistBlock(
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0xE58D17BEu ^ (pRound * 21u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-5194), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0xE58D17BEu ^ (pRound * 21u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (1949), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(aReverseIndex + (-2495), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (4323), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 147u + 22u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (4323 + 22), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>((aLaneIndex ^ static_cast<std::uint32_t>(2u)) & 0xFFu);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(3u)), 0x85EBCA6Bu, 5u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (2834), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-5642), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 28u + 31u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-5642 + 31), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 0u + 3U) & 15U), static_cast<std::size_t>(i + 27u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 20u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(13u)) ^ static_cast<std::uint32_t>(15u)) & 0xFFu);
      const int aIndex2 = WrapRange(aReverseIndex + (-5184), aStart, aEnd);
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(61u)), 0x85EBCA6Bu, 5u);
    }
  }

  TwistCandidate_0013_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_0013_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_0013_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_0013_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (5375), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-6913), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (4525), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 11u + 1404u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kTyphoonBlockBytes) {
    auto storm_block = LoadBlock128Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-5122), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock128Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-3018), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackA, chunk + 976u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackB, chunk + 976u + 23u);
  ...
  TwistCandidate_0013_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(aReverseIndex + (-229), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (-645), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-6724), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (-7661), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_0013_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);
...
static void TwistCandidate_0013_PushKeyRound(
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 19u + (2266)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 6u + (-3279)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 5u + (-3782)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 249u)));
...
static void TwistCandidate_0013_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 13u + (-5067)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 19u + (345)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 11u + (-3470)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(86u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(86u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_0013_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 20u + (446)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 23u + (665)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 24u + (-1613)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
void TwistCandidate_0013(
  ...
  TwistCandidate_0013_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_0013_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_0013_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    ...
    TwistCandidate_0013_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_0013_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_0013",
    &TwistCandidate_0013,
    &TwistCandidate_0013_KeySeed,
    &TwistCandidate_0013_SaltSeed,
    &TwistCandidate_0013_MaskSeedA,
    &TwistCandidate_0013_MaskSeedB,
    &TwistCandidate_0013_TwistBlock,
    &TwistCandidate_0013_PushKeyRound,
    &TwistCandidate_0013_PushMaskRoundA,
    &TwistCandidate_0013_PushMaskRoundB,
  }
};
