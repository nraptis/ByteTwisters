...
// Candidate 156973: TwistCandidate_156973
// family=ferocious_dual_worker op_budget=9 worker_shapes=3x4/1x4 worker_reverse=(False, False, True, False)/(False, False, False, True) twiddle1=add/none twiddle2=xor/none twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m2i0v132,aBreakerCD:4m2i2v103,aBreakerEF:4m3i3v322 lane_breaker=true braid_breaker=true jump_breaker=true swap_breaker=false mask_template=2 mask_seed_a=xor_wave mask_seed_b=add_wave lightning=false/3 typhoon=true hurricane=false final_reverse=(False, True, False, True)
// ferocious[wa=3x4; wb=1x4; tw1=add/none; tw2=xor/none; tw2src=B; saltbox=128:00100b6f:15b95b1a; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m2i0v132,aBreakerCD:4m2i2v103,aBreakerEF:4m3i3v322; wa_rev=(False, False, True, False); wb_rev=(False, False, False, True); lane=on:1; braid=on:1; jump=on:1; swap=off:3; mask=workerax2; lightning=off:3; typhoon=on; hurricane=off; final=0/(False, True, False, True); key_rot=24; maskA_seed=xor_wave/False; maskB_seed=add_wave; maskA_bias=106; maskB_bias=109]
static std::uint32_t TwistCandidate_156973_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x88405BF9u;
  aState = AdvanceTwiddle32(
      ...
      0x7F4799B6u,
      5u);
  aState ^= RotateLeft32(pValue + pExtra + 0x91047128u, 10U);
  return static_cast<std::uint32_t>(aState + 0xF707C24Fu);
}

static std::uint32_t TwistCandidate_156973_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xBCE2B75Fu;
  aState = AdvanceTwiddle32(
      ...
      0x2DE6C677u,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xB6272270u, 6U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A52EE9u);
}

static void TwistCandidate_156973_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_156973_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_156973_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_156973_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_156973_AdvanceTwiddle32B(
...
static void TwistCandidate_156973_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x8EU, 0x53U, 0xB3U, 0x97U, 0x47U, 0x19U, 0xD5U, 0xDDU, 0xB2U, 0x33U, 0x1BU, 0xDFU, 0x06U, 0xC9U, 0x8CU, 0x9EU,
      0xB0U, 0xD1U, 0xBDU, 0x39U, 0x72U, 0xA3U, 0x50U, 0xFFU, 0xD3U, 0x98U, 0xC6U, 0x4FU, 0xDAU, 0xC8U, 0x46U, 0x35U,
      0xC4U, 0x0AU, 0xD2U, 0xCAU, 0xD4U, 0x86U, 0x43U, 0xF7U, 0xB7U, 0x81U, 0xBAU, 0x32U, 0x20U, 0xDCU, 0xF9U, 0x28U,
      0xB4U, 0x31U, 0xADU, 0x71U, 0xA6U, 0x1EU, 0x40U, 0xD9U, 0xA4U, 0xDEU, 0x18U, 0x5EU, 0xD8U, 0x8FU, 0x00U, 0x55U,
      0x30U, 0x45U, 0x4DU, 0xEAU, 0x42U, 0x1AU, 0x44U, 0xC1U, 0x64U, 0x52U, 0x54U, 0x04U, 0x56U, 0x2CU, 0x66U, 0x6DU,
      0xD6U, 0xF5U, 0x23U, 0x49U, 0x9FU, 0xFAU, 0x69U, 0x38U, 0x10U, 0xE7U, 0x93U, 0x89U, 0xCFU, 0x7BU, 0xCEU, 0xB8U,
      0x17U, 0x76U, 0xCBU, 0xE8U, 0x58U, 0x4CU, 0x90U, 0xBFU, 0x21U, 0x83U, 0x15U, 0x2DU, 0x84U, 0xE4U, 0x0DU, 0xAAU,
      0x37U, 0xACU, 0xF8U, 0x68U, 0xE1U, 0x96U, 0x0EU, 0x4BU, 0x27U, 0x8BU, 0xC3U, 0xA2U, 0xA8U, 0x60U, 0xF0U, 0x7AU
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 199u ^ 169u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 25u + (1393)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 27u + (1254)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 25u + static_cast<unsigned int>(199u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 199u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(169u), 7u, false);
  ...
  TwistCandidate_156973_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(169u), 7u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(169u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 53u + (-1055)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 51u + (3702)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 53u + pSalt[(aSourceIndex + 83u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(83u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(83u), 2u, false);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_156973_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(83u), static_cast<std::uint32_t>(83u), 2u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(83u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_156973_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 4u + (-3789)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 21u + (-7486)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 199u) ^ ((b << 7u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_156973_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 155u ^ 106u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 30u + (4098)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 24u + (7490)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (30u + 24u) + (-2469)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 24u + (2379)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 2u)) ^ static_cast<std::uint32_t>(42u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 30u * 8U + static_cast<std::size_t>(aSourceIndex) * 24u + 155u + static_cast<std::size_t>(aTurbulence * ((24u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(155u), 7u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 42u) * ((24u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 2u, false);
  }
}

static void TwistCandidate_156973_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 218u ^ 109u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 12u + (6846)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 11u + (-5946)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (12u + 11u) + (-5237)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 16u + (6872)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 12u * 8U + static_cast<std::size_t>(aSourceIndex) * 11u + 218u + static_cast<std::size_t>(aTurbulence * ((16u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(218u), 4u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 149u) * ((16u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 1u, false);
  }
}

static void TwistCandidate_156973_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x46U, 0x38U, 0x98U, 0xA4U, 0xACU, 0x4FU, 0x9AU, 0x0BU, 0x0FU, 0x86U, 0x21U, 0x5AU, 0x92U, 0x14U, 0xE8U, 0x1BU,
      0x28U, 0x57U, 0x9FU, 0x31U, 0x7FU, 0x4BU, 0x75U, 0xBAU, 0x55U, 0x2AU, 0x29U, 0xAEU, 0x83U, 0xD8U, 0xE5U, 0x00U,
      0x04U, 0xCEU, 0xCCU, 0x96U, 0x09U, 0x52U, 0x23U, 0x39U, 0x3EU, 0x4EU, 0xA2U, 0x7BU, 0x64U, 0xA6U, 0xB3U, 0x36U,
      0x8FU, 0x62U, 0x73U, 0xD7U, 0xB7U, 0xEBU, 0xE2U, 0x6DU, 0x01U, 0xC4U, 0x08U, 0xE3U, 0xF1U, 0x25U, 0xD6U, 0xEDU,
      0x99U, 0xC5U, 0xCDU, 0xF4U, 0x42U, 0x1FU, 0xC6U, 0xEFU, 0x77U, 0xE1U, 0x10U, 0x24U, 0x72U, 0x32U, 0x54U, 0x6CU,
      0xC9U, 0x06U, 0x85U, 0x3CU, 0x4CU, 0xABU, 0x87U, 0x65U, 0x6BU, 0x5CU, 0xD0U, 0xF9U, 0xAAU, 0x69U, 0x07U, 0x05U,
      0xC8U, 0x8AU, 0x5BU, 0xDBU, 0x70U, 0xF5U, 0x41U, 0x18U, 0x88U, 0x90U, 0x40U, 0x20U, 0x89U, 0x79U, 0x8CU, 0x2EU,
      0xB2U, 0x9CU, 0x7EU, 0x7CU, 0xC1U, 0x13U, 0x27U, 0xBEU, 0xAFU, 0xCBU, 0xD5U, 0x94U, 0x6FU, 0x9EU, 0x4AU, 0xF2U
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0xB5D50CB5u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0xE27647FFu, 6u);
  std::uint32_t aAccB = AdvanceTwiddle32(0xA6D4F0A1u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0xF4623891u, 4u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-7193), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (6357), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (1279), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4922), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0xE27647FFu, 6u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0xF4623891u, 4u);
...
static void TwistCandidate_156973_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x41U, 0x38U, 0x37U, 0x48U, 0xD9U, 0xC7U, 0xA1U, 0x60U, 0x54U, 0x2CU, 0x72U, 0xEBU, 0x03U, 0xFAU, 0xBCU, 0xEFU,
      0x4BU, 0xB3U, 0x26U, 0x7DU, 0xACU, 0x96U, 0xF7U, 0xC4U, 0xCFU, 0x59U, 0x62U, 0x76U, 0xD7U, 0xCDU, 0x12U, 0x90U,
      0x3EU, 0x9EU, 0xA4U, 0x8BU, 0x4AU, 0x7EU, 0xD1U, 0xB6U, 0x3CU, 0x53U, 0x2FU, 0x86U, 0x67U, 0x6EU, 0x14U, 0x50U,
      0x02U, 0x95U, 0x8FU, 0x45U, 0xE1U, 0xD4U, 0x75U, 0xFDU, 0x0BU, 0xB7U, 0xA5U, 0x8AU, 0x20U, 0x84U, 0x01U, 0x23U,
      0x6FU, 0x17U, 0xD5U, 0x0FU, 0x16U, 0xF5U, 0x70U, 0x08U, 0xF2U, 0xFCU, 0xFEU, 0x34U, 0x2BU, 0x18U, 0x25U, 0xCAU,
      0x8EU, 0x7AU, 0xBAU, 0x49U, 0x1AU, 0x71U, 0x61U, 0x0AU, 0x7CU, 0x7BU, 0x05U, 0x2EU, 0xB8U, 0xE0U, 0x79U, 0xC2U,
      0x11U, 0xEDU, 0xE8U, 0x2DU, 0x42U, 0x5BU, 0xFBU, 0x55U, 0x1CU, 0x74U, 0x28U, 0x6DU, 0x09U, 0xC1U, 0x0EU, 0x94U,
      0x9DU, 0xCEU, 0x78U, 0xD2U, 0x13U, 0xB0U, 0x5FU, 0xC6U, 0x40U, 0x27U, 0xECU, 0xD0U, 0x63U, 0xB1U, 0xAFU, 0x93U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x640FD7D2u, 0xCB42C560u, 5u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (1293), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xCB42C560u, 5u);
...
static void TwistCandidate_156973_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_156973_AdvanceTwiddle32A(
      0x057953E9u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_156973_AdvanceTwiddle32B(
      0xEE25F56Bu ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_156973_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_156973_AdvanceTwiddle32A(
      0x2E64917Bu ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[27U]),
      0xECD6E548u ^ static_cast<std::uint32_t>(pSalt[25U]));
  std::uint32_t aPulseB = TwistCandidate_156973_AdvanceTwiddle32B(
      0xE3883039u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[25U]),
      0xC45D134Bu ^ static_cast<std::uint32_t>(pSalt[27U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    for (std::size_t aLane = 0; aLane < kMatrixBlockBytes; ++aLane) {
      ...
      const unsigned char aMix = FixedMixBoxByte(static_cast<unsigned char>(
          ...
          pSalt[(aLane + 25U + (aPulseByteA & 3U)) & 31U]));
    ...
    aPulseA = TwistCandidate_156973_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_156973_AdvanceTwiddle32B(
...
static void TwistCandidate_156973_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x9C631B66u ^ (pRound * 57u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-3789), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x9C631B66u ^ (pRound * 57u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-7486), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (-6622), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (152), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 79u + 3u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (152 + 3), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const int aIndex2 = WrapRange(i + (1663), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(aReverseIndex + (-7670), aStart, aEnd);
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex3]);
      const std::uint32_t aRoute = ((aSourceCtrl >> 6u) ^ aWorkerCtrl ^ aKeyByte ^ aMaskByte ^ static_cast<std::uint32_t>(4u)) % 5U;
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(28u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[8u]), 0x27D4EB2Du, 7u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (-1543), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (-3161), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 94u + 20u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-3161 + 20), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 6u + 3U) & 15U), static_cast<std::size_t>(i + 25u)));
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(119u)), 0x85EBCA6Bu, 5u);
    }
  }

  TwistCandidate_156973_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_156973_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_156973_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_156973_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (365), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-1854), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-5520), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 25u + 1050u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kTyphoonBlockBytes) {
    auto storm_block = LoadBlock128Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-3727), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock128Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (6999), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackA, chunk + 1076u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackB, chunk + 1076u + 23u);
    ...
    switch (static_cast<unsigned>(storm_control[3] ^ storm_mask_a[11] ^ storm_mask_b[13] ^ pSalt[0]) % 5u) {
      ...
      case 4u: storm.ApplyFastOp(TyphoonFastOp::kRotateRowRight, static_cast<std::uint8_t>(storm_control[17] ^ storm_mask_a[23] ^ storm_mask_b[29]), static_cast<std::uint8_t>(storm_control[29] + storm_mask_a[31] + storm_mask_b[37])); break;
    }
    }
  ...
  TwistCandidate_156973_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (2311), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (6992), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (6757), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (-1940), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
  ...
  TwistCandidate_156973_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_156973_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 30u + (-5617)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 10u + (-7136)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 25u + (-5073)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 30u + static_cast<unsigned int>(24u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 169u)));
...
static void TwistCandidate_156973_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 14u + (-6940)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 1u + (-1509)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 4u + (5664)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(106u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(106u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_156973_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 28u + (3715)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 5u + (-4413)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 25u + (5909)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(109u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(109u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_156973(
  ...
  TwistCandidate_156973_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_156973_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_156973_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_156973_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_156973_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_156973_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_156973_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_156973_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_156973",
    &TwistCandidate_156973,
    &TwistCandidate_156973_KeySeed,
    &TwistCandidate_156973_SaltSeed,
    &TwistCandidate_156973_MaskSeedA,
    &TwistCandidate_156973_MaskSeedB,
    &TwistCandidate_156973_TwistBlock,
    &TwistCandidate_156973_PushKeyRound,
    &TwistCandidate_156973_PushMaskRoundA,
    &TwistCandidate_156973_PushMaskRoundB,
  }
};
