...
// Candidate 158829: TwistCandidate_158829
// family=ferocious_dual_worker op_budget=9 worker_shapes=3x3/2x2 worker_reverse=(True, True, True, False)/(False, True, False, True) twiddle1=xor/i twiddle2=xor/none twiddle2_source=B tsunami=custom breaker_shapes=aBreakerAB:4m3i1v320,aBreakerCD:4m0i2v301,aBreakerEF:4m1i0v033 lane_breaker=true braid_breaker=false jump_breaker=true swap_breaker=true mask_template=2 mask_seed_a=xor_wave mask_seed_b=off lightning=false/1 typhoon=false hurricane=true final_reverse=(False, True, True, True)
// ferocious[wa=3x3; wb=2x2; tw1=xor/i; tw2=xor/none; tw2src=B; saltbox=128:0010412c:dca70240; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m3i1v320,aBreakerCD:4m0i2v301,aBreakerEF:4m1i0v033; wa_rev=(True, True, True, False); wb_rev=(False, True, False, True); lane=on:2; braid=off:0; jump=on:3; swap=on:3; mask=workerbx2; lightning=off:1; typhoon=off; hurricane=on; final=1/(False, True, True, True); key_rot=17; maskA_seed=xor_wave/True; maskB_seed=off; maskA_bias=43; maskB_bias=69]
static std::uint32_t TwistCandidate_158829_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0xF6A28866u;
  aState = AdvanceTwiddle32(
      ...
      0x7D080E98u,
      5u);
  aState ^= RotateLeft32(pValue + pExtra + 0xEDA57878u, 10U);
  return static_cast<std::uint32_t>(aState + 0x8BAA86FEu);
}

static std::uint32_t TwistCandidate_158829_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xBEA23DB1u;
  aState = AdvanceTwiddle32(
      ...
      0x530745C9u,
      9u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0x49A9CE2Fu, 6U);
  return static_cast<std::uint32_t>(aState ^ 0xC3AA3329u);
}

static void TwistCandidate_158829_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_158829_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_158829_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_158829_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_158829_AdvanceTwiddle32B(
...
static void TwistCandidate_158829_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0x08U, 0xA1U, 0x26U, 0xBFU, 0x33U, 0x5BU, 0xE6U, 0x56U, 0xC4U, 0xD3U, 0x5FU, 0xC0U, 0xA4U, 0x34U, 0xB0U, 0xAFU,
      0xAEU, 0x1CU, 0xD5U, 0xA0U, 0x55U, 0xB5U, 0x0BU, 0x75U, 0x4EU, 0x0DU, 0xF8U, 0x00U, 0x87U, 0x73U, 0x66U, 0x30U,
      0x32U, 0x17U, 0xD6U, 0x4AU, 0x62U, 0x9FU, 0x3EU, 0xCAU, 0x15U, 0x2BU, 0x05U, 0xEAU, 0x50U, 0x98U, 0xBCU, 0x77U,
      0xDAU, 0x6FU, 0x09U, 0x4DU, 0xC7U, 0xB9U, 0x6AU, 0x04U, 0x5EU, 0xDEU, 0xA7U, 0x7BU, 0x95U, 0x02U, 0xACU, 0xB8U,
      0x27U, 0x41U, 0x2CU, 0x7AU, 0xFBU, 0x8EU, 0x1DU, 0x78U, 0x23U, 0x7CU, 0xDFU, 0x47U, 0x3DU, 0xA8U, 0xC3U, 0xE2U,
      0x3AU, 0x74U, 0x28U, 0xEBU, 0xF6U, 0xE4U, 0x49U, 0xF5U, 0x4CU, 0xFEU, 0x4FU, 0x3FU, 0xEEU, 0x52U, 0x3BU, 0xC2U,
      0xA2U, 0xDDU, 0x35U, 0x44U, 0x39U, 0xD1U, 0xE1U, 0x22U, 0x48U, 0xCEU, 0xCDU, 0x68U, 0x80U, 0x0AU, 0x57U, 0x31U,
      0x38U, 0xC8U, 0xF1U, 0x01U, 0x90U, 0x5AU, 0xA3U, 0xCBU, 0xDBU, 0xD4U, 0xBEU, 0x70U, 0x37U, 0xADU, 0xC5U, 0x40U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 181u ^ 244u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 3u + (-6074)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 6u + (6850)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 3u + static_cast<unsigned int>(181u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 181u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(244u), 7u, true);
  ...
  TwistCandidate_158829_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(244u), 7u);
...
static void TwistCandidate_158829_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-4329)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 16u + (7117)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 181u) ^ ((b << 7u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_158829_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 232u ^ 43u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 21u + (4236)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 15u + (-3434)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (21u + 15u) + (802)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 26u + (-3762)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 2u)) ^ static_cast<std::uint32_t>(81u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 21u * 8U + static_cast<std::size_t>(aReverseIndex) * 15u + 232u + static_cast<std::size_t>(aTurbulence * ((26u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(232u), 7u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 81u) * ((26u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 2u, true);
  }
}

static void TwistCandidate_158829_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 179u ^ 69u);
  for (int aLoopIndex = static_cast<int>(PASSWORD_EXPANDED_SIZE) - 1; aLoopIndex != -1; aLoopIndex += -1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 9u + (-5012)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 31u + (-3327)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (9u + 31u) + (2467)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 9u * 8U + static_cast<std::size_t>(aReverseIndex) * 31u + 179u + static_cast<std::size_t>(aTurbulence * ((19u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(179u), 1u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 47u + (-5012)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 44u + (-3327)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aReverseIndex * (47u + 44u) + (2467)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = b;
    const std::uint32_t aSeedC = c;
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 47u * 8U + static_cast<std::size_t>(aSourceIndex) * 44u * 3U + 71u + static_cast<std::size_t>(aTurbulence * ((19u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 6u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(71u), 6u, false);
  }
}

static void TwistCandidate_158829_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x27U, 0x29U, 0xB2U, 0x60U, 0xDBU, 0xD7U, 0xCFU, 0x78U, 0x10U, 0x19U, 0x3CU, 0x5AU, 0x91U, 0x58U, 0xA7U, 0x06U,
      0x75U, 0xF4U, 0x18U, 0x41U, 0xA8U, 0xD5U, 0x50U, 0x79U, 0x55U, 0x97U, 0xCCU, 0x14U, 0x52U, 0xE5U, 0x5DU, 0xF8U,
      0xC5U, 0xEAU, 0x0CU, 0x4AU, 0x54U, 0xE9U, 0x09U, 0x69U, 0x7CU, 0xA9U, 0xDCU, 0x7FU, 0x45U, 0xC3U, 0xEBU, 0x1DU,
      0xD1U, 0x86U, 0xFFU, 0x6BU, 0x99U, 0xA2U, 0x01U, 0xE0U, 0x6EU, 0xAEU, 0x56U, 0x46U, 0x35U, 0x6DU, 0x6FU, 0xA5U,
      0xE3U, 0x93U, 0x3DU, 0xE4U, 0x25U, 0xC8U, 0xDDU, 0x0DU, 0xCBU, 0x98U, 0xC6U, 0x42U, 0xD0U, 0xB7U, 0x5EU, 0x2CU,
      0x9BU, 0x82U, 0x61U, 0xB0U, 0x1BU, 0x3FU, 0xBCU, 0xE8U, 0x87U, 0x8EU, 0x74U, 0xC7U, 0xCAU, 0xBBU, 0xB5U, 0x0AU,
      0x22U, 0x5BU, 0x11U, 0xC4U, 0x62U, 0xE7U, 0x0BU, 0x2EU, 0x6AU, 0x8CU, 0x30U, 0x26U, 0x76U, 0x23U, 0x4DU, 0xC1U,
      0xFAU, 0xB3U, 0x8BU, 0x51U, 0xFDU, 0xAFU, 0x2AU, 0xE2U, 0x04U, 0x1EU, 0x70U, 0x2BU, 0x84U, 0xC0U, 0x2FU, 0xEFU
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x8F533B27u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x31CFCD09u, 11u);
  std::uint32_t aAccB = AdvanceTwiddle32(0xA0F50ADAu ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0x586F6AF1u, 6u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (3638), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-6508), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-4654), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-2724), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x31CFCD09u, 11u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0x586F6AF1u, 6u);
...
static void TwistCandidate_158829_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x04U, 0x23U, 0x63U, 0x45U, 0x91U, 0xECU, 0xEEU, 0x6FU, 0x43U, 0x20U, 0x95U, 0xF7U, 0xFFU, 0xA5U, 0xC9U, 0xE9U,
      0xCEU, 0x12U, 0xD6U, 0xE5U, 0xEFU, 0xC3U, 0x1DU, 0x58U, 0x9EU, 0x84U, 0x79U, 0xE7U, 0xA2U, 0xC7U, 0xB9U, 0x3AU,
      0x4BU, 0xF8U, 0xC4U, 0x71U, 0x70U, 0xABU, 0x6AU, 0xDAU, 0x68U, 0xF3U, 0xFCU, 0x24U, 0xADU, 0xC0U, 0xC5U, 0xA7U,
      0x9CU, 0x18U, 0xD2U, 0x21U, 0x62U, 0x3BU, 0xB1U, 0x80U, 0xA9U, 0x0DU, 0xEAU, 0xEBU, 0x1BU, 0xE8U, 0x02U, 0x51U,
      0xF4U, 0x8FU, 0xBCU, 0x69U, 0x0EU, 0x9BU, 0x66U, 0x1EU, 0x74U, 0xBEU, 0x30U, 0xDEU, 0x7BU, 0x48U, 0x6BU, 0x4AU,
      0xB2U, 0x4EU, 0xFEU, 0xD3U, 0x50U, 0x4DU, 0xFBU, 0xA1U, 0xE6U, 0x0CU, 0x87U, 0xD0U, 0x5EU, 0xE1U, 0x6DU, 0x64U,
      0x15U, 0x93U, 0xD1U, 0x47U, 0x32U, 0x36U, 0xE0U, 0x55U, 0x39U, 0xF5U, 0x1CU, 0x38U, 0x77U, 0xACU, 0x4FU, 0xA3U,
      0x3CU, 0x96U, 0x07U, 0xCFU, 0x08U, 0x1AU, 0x75U, 0x5AU, 0x01U, 0x5CU, 0xF9U, 0x8AU, 0x28U, 0x73U, 0xDDU, 0x27U
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0xE11C2373u, 0x6D88E357u, 6u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-2883), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0x6D88E357u, 6u);
...
static void TwistCandidate_158829_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_158829_AdvanceTwiddle32A(
      0x1B2BEEA9u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_158829_AdvanceTwiddle32B(
      0xDBA16D2Bu ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_158829_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_158829_AdvanceTwiddle32A(
      0x18D3DD64u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[4U]),
      0xDA62AD60u ^ static_cast<std::uint32_t>(pSalt[22U]));
  std::uint32_t aPulseB = TwistCandidate_158829_AdvanceTwiddle32B(
      0xC82BABD6u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[22U]),
      0xEFFF792Au ^ static_cast<std::uint32_t>(pSalt[4U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_158829_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_158829_AdvanceTwiddle32B(
...
static void TwistCandidate_158829_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0x377A8556u ^ (pRound * 37u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-4329), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0x377A8556u ^ (pRound * 37u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (7117), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(aReverseIndex + (-101), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-7133), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 116u + 28u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-7133 + 28), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 6u + 3U) & 15U), static_cast<std::size_t>(i + 17u)));
      ...
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>((aLaneIndex ^ static_cast<std::uint32_t>(164u)) & 0xFFu);
      const int aIndex2 = WrapRange(aReverseIndex + (2198), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      const int aIndex3 = WrapRange(aReverseIndex + (6274), aStart, aEnd);
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex3]);
      const std::uint32_t aRoute = ((aSourceCtrl >> 3u) ^ aWorkerCtrl ^ aKeyByte ^ aMaskByte ^ static_cast<std::uint32_t>(3u)) % 5U;
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(214u)), 0x85EBCA6Bu, 3u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(i + (-3316), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (-5761), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 40u + 4u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-5761 + 4), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const int aIndex2 = WrapRange(aReverseIndex + (-5288), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      ...
      aTwiddleB = AdvanceTwiddle32(aTwiddleB, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(126u)), 0x85EBCA6Bu, 3u);
    }
  }

  TwistCandidate_158829_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_158829_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_158829_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_158829_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (-6324), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-7398), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (1349), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 22u + 250u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kHurricaneBlockBytes) {
    auto storm_block = LoadBlock256Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-7073), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock256Wrapped(pSource, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (-7620), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackA, chunk + 1018u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kHurricaneBlockBytes>(pMaskStackB, chunk + 1018u + 29u);
  ...
  TwistCandidate_158829_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (-4329), aStart, aEnd);
      const int aIndex1 = WrapRange(aReverseIndex + (7182), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (2064), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (1581), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_158829_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_158829_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 25u + (-1296)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 6u + (-1631)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 28u + (1935)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 25u + static_cast<unsigned int>(17u)) & 31U;
...
static void TwistCandidate_158829_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 29u + (4883)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 28u + (-3336)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 29u + (7399)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
...
static void TwistCandidate_158829_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 16u + (-3527)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 9u + (-6425)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 24u + (7157)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(69u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(69u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_158829(
  ...
  TwistCandidate_158829_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_158829_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_158829_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_158829_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_158829_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_158829_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_158829_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_158829_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_158829",
    &TwistCandidate_158829,
    &TwistCandidate_158829_KeySeed,
    &TwistCandidate_158829_SaltSeed,
    &TwistCandidate_158829_MaskSeedA,
    &TwistCandidate_158829_MaskSeedB,
    &TwistCandidate_158829_TwistBlock,
    &TwistCandidate_158829_PushKeyRound,
    &TwistCandidate_158829_PushMaskRoundA,
    &TwistCandidate_158829_PushMaskRoundB,
  }
};
