...
// Candidate 106033: TwistCandidate_106033
// family=ferocious_dual_worker op_budget=10 worker_shapes=2x0/2x1 worker_reverse=(False, True, False, True)/(True, False, False, False) twiddle1=add/none twiddle2=xor/i_mul twiddle2_source=A tsunami=custom breaker_shapes=aBreakerAB:4m2i1v100,aBreakerCD:4m0i1v210,aBreakerEF:4m1i1v123 lane_breaker=true braid_breaker=true jump_breaker=true swap_breaker=true mask_template=2 mask_seed_a=xor_wave mask_seed_b=xor_wave lightning=false/1 typhoon=true hurricane=false final_reverse=(False, False, True, True)
// ferocious[wa=2x0; wb=2x1; tw1=add/none; tw2=xor/i_mul; tw2src=A; saltbox=128:000ff61e:b7e03685; tsunami=custom; final_whitening=custom; breaker_shapes=aBreakerAB:4m2i1v100,aBreakerCD:4m0i1v210,aBreakerEF:4m1i1v123; wa_rev=(False, True, False, True); wb_rev=(True, False, False, False); lane=on:0; braid=on:1; jump=on:2; swap=on:1; mask=workerbx2; lightning=off:1; typhoon=on; hurricane=off; final=1/(False, False, True, True); key_rot=18; maskA_seed=xor_wave/False; maskB_seed=xor_wave; maskA_bias=72; maskB_bias=111]
static std::uint32_t TwistCandidate_106033_AdvanceTwiddle32A(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState ^ 0x22E9E922u;
  aState = AdvanceTwiddle32(
      ...
      0x12F592B3u,
      9u);
  aState ^= RotateLeft32(pValue + pExtra + 0x561CD5CCu, 8U);
  return static_cast<std::uint32_t>(aState + 0x301C7B91u);
}

static std::uint32_t TwistCandidate_106033_AdvanceTwiddle32B(
    std::uint32_t pState,
    std::uint32_t pValue,
    std::uint32_t pExtra) {
  std::uint32_t aState = pState + 0xD1504141u;
  aState = AdvanceTwiddle32(
      ...
      0x874C948Du,
      7u);
  aState ^= RotateLeft32(pValue ^ pExtra ^ 0xAA367DAFu, 10U);
  return static_cast<std::uint32_t>(aState ^ 0xC3A5D3F2u);
}

static void TwistCandidate_106033_ApplySaltMixBox(
    unsigned char (&pSalt)[kSaltBytes],
    std::uint32_t pState,
    std::uint32_t pBias,
    unsigned pRotate) {
  std::uint32_t aTwiddleA = TwistCandidate_106033_AdvanceTwiddle32A(
      pState ^ pBias,
      static_cast<std::uint32_t>(pSalt[(pBias + 3U) & 31U]),
      static_cast<std::uint32_t>(pRotate + 17U));
  std::uint32_t aTwiddleB = TwistCandidate_106033_AdvanceTwiddle32B(
  ...
  for (std::size_t aIndex = 0; aIndex < kSaltBytes; ++aIndex) {
    ...
    aTwiddleA = TwistCandidate_106033_AdvanceTwiddle32A(
    ...
    aTwiddleB = TwistCandidate_106033_AdvanceTwiddle32B(
...
static void TwistCandidate_106033_SaltSeed(
  ...
  static constexpr unsigned char aSaltMixBox[128] = {
      0xEFU, 0x06U, 0x5DU, 0x2FU, 0x0CU, 0xE4U, 0x57U, 0x58U, 0x53U, 0xFAU, 0x68U, 0x4AU, 0xE3U, 0xE5U, 0x8CU, 0xB6U,
      0x1DU, 0x29U, 0xF5U, 0x41U, 0xE9U, 0x9BU, 0xCAU, 0xABU, 0x3BU, 0x7BU, 0x50U, 0xB4U, 0x39U, 0xB1U, 0xEEU, 0xF2U,
      0x6AU, 0xF1U, 0x5EU, 0xE8U, 0x9EU, 0xAFU, 0xA7U, 0xC4U, 0x9AU, 0xA2U, 0xD8U, 0x3CU, 0x33U, 0x77U, 0x1AU, 0xCEU,
      0x35U, 0xACU, 0xE0U, 0xF8U, 0xA4U, 0xA0U, 0xBBU, 0xF9U, 0x25U, 0x45U, 0xADU, 0x08U, 0x18U, 0xC5U, 0xC1U, 0x78U,
      0x51U, 0x44U, 0x3FU, 0xE1U, 0x71U, 0xA8U, 0x89U, 0xD3U, 0x61U, 0x5BU, 0xEBU, 0x2DU, 0x38U, 0xD0U, 0x2CU, 0x49U,
      0xE7U, 0x8AU, 0xBCU, 0x60U, 0x94U, 0xCFU, 0x4DU, 0x5CU, 0xB7U, 0x8DU, 0x2BU, 0x64U, 0xB5U, 0x7FU, 0x6BU, 0x22U,
      0xCDU, 0x1BU, 0x9DU, 0x3DU, 0xF6U, 0x96U, 0x8EU, 0x79U, 0x54U, 0x85U, 0x1FU, 0xFBU, 0x30U, 0x1EU, 0x0FU, 0x00U,
      0x76U, 0xA1U, 0xF0U, 0xC2U, 0xBDU, 0x95U, 0x01U, 0xFEU, 0x8FU, 0xDFU, 0x90U, 0x23U, 0x14U, 0x0DU, 0x2EU, 0xA5U
  };
  std::memset(pSalt, 0, kSaltBytes);
  std::uint32_t aSaltAcc = static_cast<std::uint32_t>(0xA5A5A5A5u ^ 51u ^ 114u);
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 9u + (4121)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 6u + (3847)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = (aSourceIndex * 9u + static_cast<unsigned int>(51u)) & 31U;
    ...
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        ...
        static_cast<std::uint32_t>(aSourceIndex + 51u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aSaltWave, aSaltIndex, aA ^ aSaltWave, aB, static_cast<std::uint32_t>(aSourceIndex), static_cast<std::uint32_t>(114u), 4u, false);
  ...
  TwistCandidate_106033_ApplySaltMixBox(pSalt, aSaltAcc, static_cast<std::uint32_t>(114u), 4u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] ^ pSalt[(aSaltLane + 7U) & 31U] ^ static_cast<unsigned char>(114u) ^ static_cast<unsigned char>(aSaltLane)) & 127U]);
  ...
  for (unsigned int aSourceIndex = 0U; aSourceIndex < PASSWORD_EXPANDED_SIZE; ++aSourceIndex) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 19u + (6694)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 22u + (1274)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aSaltIndex = static_cast<unsigned int>((aSourceIndex * 19u + pSalt[(aSourceIndex + 136u) & 31U]) & 31U);
    const std::uint32_t aA = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t aB = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t aCarry = static_cast<std::uint32_t>(pSalt[(aSaltIndex + 11U) & 31U]);
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        aA ^ aB ^ aCarry ^ static_cast<std::uint32_t>(136u)) & 127U]);
    aSaltAcc = AdvanceSaltSeedAccumulator(pSalt, aSaltAcc ^ aCarry ^ aSaltWave, aSaltIndex, aA ^ aCarry, aB, static_cast<std::uint32_t>(aSourceIndex + aCarry), static_cast<std::uint32_t>(136u), 5u, false);
    pSalt[(aSaltIndex + 23U) & 31U] = static_cast<unsigned char>(pSalt[(aSaltIndex + 23U) & 31U] + static_cast<unsigned char>(aSaltWave));
  }
  TwistCandidate_106033_ApplySaltMixBox(pSalt, aSaltAcc ^ static_cast<std::uint32_t>(136u), static_cast<std::uint32_t>(136u), 5u);
  for (unsigned int aSaltLane = 0U; aSaltLane < kSaltBytes; ++aSaltLane) {
    const std::uint32_t aSaltWave = static_cast<std::uint32_t>(aSaltMixBox[(
        pSalt[aSaltLane] + pSalt[(aSaltLane + 19U) & 31U] + static_cast<unsigned char>(136u + 13u)) & 127U]);
    pSalt[aSaltLane] ^= static_cast<unsigned char>(aSaltWave);
  }
}

static void TwistCandidate_106033_KeySeed(
    unsigned char* pSource,
  ...
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 26u + (-6895)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 25u + (-7124)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aPreMix = static_cast<std::uint32_t>((((a + 51u) ^ ((b << 4u) & 0xFFu) ^ aPosMix ^ aCarry) & 0xFFu));
...
static void TwistCandidate_106033_MaskSeedA(
    unsigned char* pSource,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 175u ^ 72u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    ...
    const int aIndex0 = WrapRange(static_cast<int>(aReverseIndex * 21u + (-5279)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 30u + (-3637)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aReverseIndex * (21u + 30u) + (-5208)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aSourceIndex * 38u + (947)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 2u)) ^ static_cast<std::uint32_t>(183u)) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aReverseIndex) * 21u * 8U + static_cast<std::size_t>(aSourceIndex) * 30u + 175u + static_cast<std::size_t>(aTurbulence * ((38u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(175u), 5u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 183u) * ((38u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 2u, false);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 18u + (-5279)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 54u + (-3637)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (18u + 54u) + (-5208)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 2u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 18u * 8U + static_cast<std::size_t>(aSourceIndex) * 54u * 3U + 120u + static_cast<std::size_t>(aTurbulence * ((38u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackA, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 7u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(120u), 7u, false);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 183u) * ((38u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackA, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 7u, true);
  }
}

static void TwistCandidate_106033_MaskSeedB(
    unsigned char* pSource,
    unsigned char* pWorker,
  ...
  std::uint32_t aMaskSeedState = static_cast<std::uint32_t>(0x6D2B79F5u ^ 22u ^ 111u);
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 19u + (-191)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aReverseIndex * 9u + (-6758)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * (19u + 9u) + (-5524)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 18u + (-2154)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t aWorkerByte = (pWorker != nullptr) ? static_cast<std::uint32_t>(pWorker[aWorkerIndex]) : 0U;
    const std::uint32_t aTurbulence = static_cast<std::uint32_t>((aWorkerByte ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>((a + c) & 0xFFu), 4u)) ^ static_cast<std::uint32_t>(72u)) & 0xFFu);
    const std::uint32_t aSeedA = static_cast<std::uint32_t>((a ^ aTurbulence) & 0xFFu);
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    ...
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 19u * 8U + static_cast<std::size_t>(aReverseIndex) * 9u + 22u + static_cast<std::size_t>(aTurbulence * ((18u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aTurbulence, aFlatBit, aSeedA, aSeedB, aSeedC, static_cast<std::uint32_t>(aSourceIndex + aTurbulence), static_cast<std::uint32_t>(22u), 5u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 72u) * ((18u & 15U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, aMaskSeedState ^ aTurbulence ^ aWorkerByte, 4u, true);
  }
  for (int aLoopIndex = 0; aLoopIndex != static_cast<int>(PASSWORD_EXPANDED_SIZE); aLoopIndex += 1) {
    const unsigned int aSourceIndex = static_cast<unsigned int>(aLoopIndex);
    const unsigned int aReverseIndex = static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE - 1U - aSourceIndex);
    const int aIndex3 = WrapRange(static_cast<int>(aSourceIndex * 27u + (-191)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex4 = WrapRange(static_cast<int>(aSourceIndex * 52u + (-6758)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex5 = WrapRange(static_cast<int>(aSourceIndex * (27u + 52u) + (-5524)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex3]);
    const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex4]);
    const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex5]);
    const int aWorkerIndex = WrapRange(static_cast<int>(aReverseIndex * 18u + (-2154)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    ...
    const std::uint32_t aSeedB = static_cast<std::uint32_t>((b + aTurbulence) & 0xFFu);
    const std::uint32_t aSeedC = static_cast<std::uint32_t>((c ^ static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aWorkerByte & 0xFFu), 4u))) & 0xFFu);
    const std::size_t aFlatBit = (static_cast<std::size_t>(aSourceIndex) * 27u * 8U + static_cast<std::size_t>(aSourceIndex) * 52u * 3U + 238u + static_cast<std::size_t>(aTurbulence * ((18u & 7U) + 1U))) % (kMaskStackTotalBytes * 8U);
    const std::size_t aCarryByte = (aFlatBit >> 3U) % kMaskStackTotalBytes;
    ...
    aMaskSeedState = AdvanceMaskSeedBitstream(pMaskStackB, aMaskSeedState ^ aCarry ^ aTurbulence, aFlatBit + static_cast<std::size_t>((aCarry + aWorkerByte) * 13U), aSeedA ^ aCarry, static_cast<std::uint32_t>(RotateLeft8(static_cast<std::uint8_t>(aSeedB & 0xFFu), 4u)), aSeedC, static_cast<std::uint32_t>(aSourceIndex + aCarry + aTurbulence), static_cast<std::uint32_t>(238u), 4u, true);
    const std::size_t aTurbulenceBit = (aFlatBit + static_cast<std::size_t>((aWorkerByte + aTurbulence + 72u) * ((18u & 31U) + 1U))) % (kMaskStackTotalBytes * 8U);
    UpdateMaskSeedBit(pMaskStackB, aTurbulenceBit, RotateLeft32(aMaskSeedState ^ aTurbulence ^ aCarry, 7U), 4u, true);
  }
}

static void TwistCandidate_106033_TsunamiBreaker(
    unsigned char* pSource,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x3CU, 0x68U, 0xFAU, 0x8EU, 0x8DU, 0x20U, 0xC5U, 0x7BU, 0x21U, 0x4EU, 0x95U, 0xCDU, 0x65U, 0x97U, 0xFFU, 0xB6U,
      0x5CU, 0x9CU, 0x13U, 0xEAU, 0x00U, 0x9DU, 0x53U, 0xD4U, 0x04U, 0x3BU, 0xA6U, 0xF6U, 0xF7U, 0xC4U, 0xA7U, 0x55U,
      0xBEU, 0x1AU, 0xCFU, 0xF2U, 0xC2U, 0x64U, 0x56U, 0x1BU, 0xEDU, 0x32U, 0xC7U, 0x44U, 0x30U, 0xEEU, 0x58U, 0x99U,
      0xCEU, 0x31U, 0x4BU, 0x52U, 0x28U, 0x78U, 0x85U, 0x62U, 0x4DU, 0xD0U, 0xBBU, 0xF3U, 0xEBU, 0xFEU, 0xA2U, 0x9FU,
      0xE4U, 0x6DU, 0x0CU, 0xD2U, 0x3FU, 0x6CU, 0xDAU, 0x6FU, 0xB7U, 0x0BU, 0x4AU, 0x86U, 0xE6U, 0x51U, 0xC0U, 0xDCU,
      0x8FU, 0xF9U, 0x33U, 0x09U, 0x48U, 0x5FU, 0x43U, 0x60U, 0x06U, 0xFDU, 0xB3U, 0x63U, 0x5BU, 0x98U, 0xB8U, 0x25U,
      0x9EU, 0x67U, 0xA3U, 0xAAU, 0x19U, 0x80U, 0xC8U, 0x8CU, 0xA4U, 0x4CU, 0x41U, 0x22U, 0x47U, 0xE3U, 0x29U, 0x07U,
      0x42U, 0x23U, 0x73U, 0x11U, 0xACU, 0x70U, 0xF0U, 0xD1U, 0x77U, 0x69U, 0x93U, 0x88U, 0x91U, 0x5DU, 0x08U, 0x8AU
  };
  std::uint32_t aAccA = AdvanceTwiddle32(0x0E2DCE69u ^ static_cast<std::uint32_t>(pRound), pTwiddleA, pTwiddleB, 0x7046B4C5u, 7u);
  std::uint32_t aAccB = AdvanceTwiddle32(0x40766E87u ^ static_cast<std::uint32_t>(pRound), pTwiddleB, pTwiddleA, 0xCCB39C04u, 10u);
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4813), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aSourceIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-3031), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexA = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-4267), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    const std::size_t aWorkerIndexB = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (4619), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltA = static_cast<std::uint32_t>(pSalt[(i + 14u) & 31U]);
    ...
    aAccA = AdvanceTwiddle32(aAccA ^ pTwiddleA, aTapA ^ aBoxA, aSourceB ^ aSaltB, 0x7046B4C5u, 7u);
    aAccB = AdvanceTwiddle32(aAccB ^ pTwiddleB, aTapB ^ aBoxB, aSourceA ^ aSaltA, 0xCCB39C04u, 10u);
...
static void TwistCandidate_106033_FinalWhitening(
    unsigned char* pSource,
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned int pRound,
  ...
  static constexpr unsigned char aMixBox[128] = {
      0x98U, 0x8FU, 0x5CU, 0x35U, 0x75U, 0xE0U, 0x58U, 0xE6U, 0x89U, 0x61U, 0x62U, 0x2DU, 0x16U, 0x78U, 0xB6U, 0xCFU,
      0xF5U, 0xC7U, 0x95U, 0xC6U, 0x40U, 0xB5U, 0x2EU, 0x0CU, 0xE4U, 0x9EU, 0x4BU, 0x01U, 0xEFU, 0x10U, 0x05U, 0x57U,
      0x36U, 0xC0U, 0x50U, 0x9AU, 0x85U, 0x2FU, 0x39U, 0x65U, 0xF4U, 0x4CU, 0xEAU, 0x6DU, 0x68U, 0xC3U, 0xF9U, 0xC8U,
      0xDBU, 0xDAU, 0x28U, 0xF7U, 0xBFU, 0xBCU, 0x64U, 0x27U, 0xD3U, 0x0FU, 0x73U, 0x5DU, 0xABU, 0x94U, 0xA1U, 0x77U,
      0xAEU, 0x1DU, 0x70U, 0xBDU, 0xA7U, 0x42U, 0xBAU, 0x04U, 0x29U, 0xC2U, 0x4AU, 0x4EU, 0x91U, 0xA6U, 0xA9U, 0x8DU,
      0xCAU, 0x74U, 0xDCU, 0xA3U, 0xB3U, 0xCDU, 0xB9U, 0xADU, 0x30U, 0x41U, 0x87U, 0xA4U, 0x72U, 0x9CU, 0xF2U, 0x51U,
      0xFCU, 0x84U, 0x22U, 0xF1U, 0x06U, 0x47U, 0x5EU, 0xC5U, 0xEBU, 0x8EU, 0x7EU, 0xE8U, 0x26U, 0x1AU, 0x12U, 0xAAU,
      0x67U, 0x7DU, 0x7BU, 0x23U, 0x3FU, 0xA0U, 0x18U, 0xD9U, 0x2BU, 0xB2U, 0x6EU, 0x88U, 0x19U, 0x60U, 0x9DU, 0x9FU
  };
  std::uint32_t aAcc = AdvanceTwiddle32(pTwiddleA ^ static_cast<std::uint32_t>(pRound), pTwiddleB, 0x2F3E9B0Au, 0xFAA6C4EAu, 4u);
  // Final whitening
  for (std::size_t i = 0; i < PASSWORD_EXPANDED_SIZE; ++i) {
    const std::size_t aSourceIndex = static_cast<std::size_t>(WrapRange(static_cast<int>(i) + (-5767), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE)));
    ...
    const std::uint32_t aSaltByte = static_cast<std::uint32_t>(pSalt[(i + 17u) & 31U]);
    const std::uint32_t aMix = static_cast<std::uint32_t>(aMixBox[(aDestByte ^ aSourceByte ^ aSaltByte ^ FoldWordToByte(aAcc)) & 127U]);
    aAcc = AdvanceTwiddle32(aAcc, aDestByte ^ aMix, aSourceByte ^ aNeighborByte ^ aSaltByte, 0xFAA6C4EAu, 4u);
...
static void TwistCandidate_106033_ApplyDualWorkerMatrixBreaker(
  ...
  std::uint32_t aSequenceA = TwistCandidate_106033_AdvanceTwiddle32A(
      0x5C364B05u ^ static_cast<std::uint32_t>(pRound),
  ...
  std::uint32_t aSequenceB = TwistCandidate_106033_AdvanceTwiddle32B(
      0x3B7BF6DFu ^ static_cast<std::uint32_t>(pRound),
...
static void TwistCandidate_106033_ApplyStepMixPulse(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    const unsigned char (&pSalt)[kSaltBytes],
  ...
  std::uint32_t aPulseA = TwistCandidate_106033_AdvanceTwiddle32A(
      0x87372948u ^ static_cast<std::uint32_t>(pRound),
      static_cast<std::uint32_t>(pSalt[8U]),
      0x4586977Au ^ static_cast<std::uint32_t>(pSalt[0U]));
  std::uint32_t aPulseB = TwistCandidate_106033_AdvanceTwiddle32B(
      0xE4FEF7E0u + static_cast<std::uint32_t>(pRound * 17U),
      static_cast<std::uint32_t>(pSalt[0U]),
      0xC32A3CE2u ^ static_cast<std::uint32_t>(pSalt[8U]));
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kMatrixBlockBytes) {
    ...
    aPulseA = TwistCandidate_106033_AdvanceTwiddle32A(
        aPulseA ^ aPulseFold,
    ...
    aPulseB = TwistCandidate_106033_AdvanceTwiddle32B(
...
static void TwistCandidate_106033_TwistBlock(
    unsigned char* pSource,
    unsigned char* pWorkerA,
    unsigned char* pWorkerB,
    unsigned char* pDest,
  ...
  std::uint32_t aTwiddleA = static_cast<std::uint32_t>(0xA511E9B3u ^ 0xA7E0AB38u ^ (pRound * 22u));
  aTwiddleA ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-6895), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleA ^= static_cast<std::uint32_t>(pSalt[(pRound + 0u) & 31U]) << 16U;
  std::uint32_t aTwiddleB = static_cast<std::uint32_t>(0xB44B1D93u ^ 0xA7E0AB38u ^ (pRound * 22u));
  aTwiddleB ^= static_cast<std::uint32_t>(pSource[WrapRange(static_cast<int>(pRound) + (-7124), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))]) << 8U;
  aTwiddleB ^= static_cast<std::uint32_t>(pSalt[(pRound + 11u) & 31U]) << 16U;
  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      ...
      const int aIndex1 = WrapRange(i + (7398), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(aReverseIndex + (-5763), aStart, aEnd);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 65u + 0u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-5763 + 0), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const int aIndex2 = WrapRange(aReverseIndex + (1284), aStart, aEnd);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA + static_cast<std::uint32_t>(249u), static_cast<std::uint32_t>((aValue) + (aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias)), static_cast<std::uint32_t>(pSalt[31u]), 0x27D4EB2Du, 3u);
    }
  }

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex1 = WrapRange(aReverseIndex + (617), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex1]);
      const int aControlIndex = WrapRange(i + (-5937), aStart, aEnd);
      const std::uint32_t aSourceCtrl = static_cast<std::uint32_t>(pSource[aControlIndex]);
      ...
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 120u + 4u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatB = WrapRange(static_cast<int>(aMaskFlat) + (-5937 + 4), 0, static_cast<int>(kMaskStackTotalBytes));
      ...
      const std::uint32_t aMaskByte = static_cast<std::uint32_t>((aMaskByteA ^ RotateLeft32(aMaskByteB, 3U) ^ (aMaskByteB << 1U)) & 0xFFu);
      const std::uint32_t aTwiddleKey = static_cast<std::uint32_t>(KeyStackByte(pKeyStack, static_cast<std::size_t>((pRound + 13u + 3U) & 15U), static_cast<std::size_t>(i + 20u)));
      const std::uint32_t aTwiddleSalt = static_cast<std::uint32_t>(pSalt[(static_cast<unsigned int>(i) + 10u) & 31U]);
      const std::uint32_t aLaneIndex = static_cast<std::uint32_t>(static_cast<unsigned int>(i) & 0xFFu);
      const std::uint32_t aPositionBias = static_cast<std::uint32_t>(((aLaneIndex * static_cast<std::uint32_t>(17u)) ^ static_cast<std::uint32_t>(206u)) & 0xFFu);
      const int aIndex2 = WrapRange(i + (-5182), aStart, aEnd);
      const std::uint32_t b = static_cast<std::uint32_t>(pSource[aIndex2]);
      ...
      aTwiddleA = AdvanceTwiddle32(aTwiddleA, static_cast<std::uint32_t>(aValue), static_cast<std::uint32_t>((aSourceCtrl ^ aTwiddleKey ^ aTwiddleSalt ^ aPositionBias) ^ static_cast<std::uint32_t>(41u)), 0x85EBCA6Bu, 3u);
    }
  }

  TwistCandidate_106033_TsunamiBreaker(pSource, pWorkerA, pWorkerB, pSalt, pRound, aTwiddleA, aTwiddleB);

  {
    ...
    TwistCandidate_106033_ApplyDualWorkerMatrixBreaker(aBreakerAB, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_106033_ApplyDualWorkerMatrixBreaker(aBreakerCD, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    TwistCandidate_106033_ApplyDualWorkerMatrixBreaker(aBreakerEF, pSource, pWorkerA, pWorkerB, pSalt, pKeyStack, pMaskStackA, pMaskStackB, pRound, pBreakerTempA, pBreakerTempB);
  }

  {
    ...
    const int aEnd = static_cast<int>(PASSWORD_EXPANDED_SIZE);
    for (int i = aStart; i < aEnd; ++i) {
      const int aIndex0 = WrapRange(i + (4326), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (-2512), aStart, aEnd);
      const int aIndex2 = WrapRange(i + (-3352), aStart, aEnd);
      ...
      const std::uint32_t c = static_cast<std::uint32_t>(pSource[aIndex2]);
      const std::size_t aMaskFlat = (static_cast<std::size_t>(i) * 15u + 1348u + static_cast<std::size_t>(pRound)) % kMaskStackTotalBytes;
      const int aMaskFlatOther = WrapRange(static_cast<int>(aMaskFlat) + (5 + 2), 0, static_cast<int>(kMaskStackTotalBytes));
      const std::uint32_t aMaskByteA = static_cast<std::uint32_t>(pMaskStackB[(aMaskFlat / kMaskBytes) % kMaskStackDepth][aMaskFlat % kMaskBytes]);
  ...
  for (std::size_t chunk = 0; chunk < PASSWORD_EXPANDED_SIZE; chunk += kTyphoonBlockBytes) {
    auto storm_block = LoadBlock128Wrapped(pWorkerB, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (1227), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_control = LoadBlock128Wrapped(pWorkerA, static_cast<std::size_t>(WrapRange(static_cast<int>(chunk) + (3624), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE))));
    const auto storm_mask_a = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackA, chunk + 89u);
    const auto storm_mask_b = LoadMaskStackBlockWrapped<kTyphoonBlockBytes>(pMaskStackB, chunk + 89u + 23u);
  ...
  TwistCandidate_106033_ApplyStepMixPulse(pSource, pWorkerA, pWorkerB, pSalt, pRound, pBreakerTempA, pBreakerTempB);

  {
    ...
    for (int i = aStart; i < aEnd; ++i) {
      const int aReverseIndex = aEnd - 1 - i;
      const int aIndex0 = WrapRange(i + (-958), aStart, aEnd);
      const int aIndex1 = WrapRange(i + (4761), aStart, aEnd);
      const int aIndex2 = WrapRange(aReverseIndex + (6913), aStart, aEnd);
      const int aControlIndex = WrapRange(aReverseIndex + (3429), aStart, aEnd);
      const std::uint32_t a = static_cast<std::uint32_t>(pSource[aIndex0]);
      const std::uint32_t b = static_cast<std::uint32_t>(pWorkerA[aIndex1]);
  ...
  TwistCandidate_106033_FinalWhitening(pSource, pDest, pSalt, pRound, aTwiddleA, aTwiddleB);

}

static void TwistCandidate_106033_PushKeyRound(
    unsigned char* pDest,
    const unsigned char (&pSalt)[kSaltBytes],
    unsigned char (&pKeyStack)[kRoundKeyStackDepth][kRoundKeyBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 19u + (-2962)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 9u + (491)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 4u + (-2533)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aKeyIndex = (aSourceIndex * 19u + static_cast<unsigned int>(18u)) & 31U;
    ...
    const std::uint32_t aKeyByte = static_cast<std::uint32_t>(KeyStackByte(
        ...
        static_cast<std::size_t>(aSourceIndex + 114u)));
...
static void TwistCandidate_106033_PushMaskRoundA(
    unsigned char* pDest,
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 31u + (-417)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 6u + (6491)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 3u + (6664)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(72u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(72u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
...
static void TwistCandidate_106033_PushMaskRoundB(
    unsigned char* pDest,
    unsigned char (&pMaskStackSelf)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pMaskStackOther)[kMaskStackDepth][kMaskBytes],
    unsigned char (&pNextRoundMaskBuffer)[kMaskBytes],
  ...
  unsigned int aSourceIndex = 0U;
  while (aSourceIndex < PASSWORD_EXPANDED_SIZE) {
    const int aIndex0 = WrapRange(static_cast<int>(aSourceIndex * 15u + (6670)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex1 = WrapRange(static_cast<int>(aSourceIndex * 23u + (2000)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const int aIndex2 = WrapRange(static_cast<int>(aSourceIndex * 20u + (752)), 0, static_cast<int>(PASSWORD_EXPANDED_SIZE));
    const unsigned int aMaskIndex = (aSourceIndex + static_cast<unsigned int>(111u)) & 7U;
    const unsigned int aOtherMaskIndex = (aSourceIndex + static_cast<unsigned int>(111u) + 3U) & 7U;
    const std::uint32_t a = static_cast<std::uint32_t>(pDest[aIndex0]);
    const std::uint32_t b = static_cast<std::uint32_t>(pDest[aIndex1]);
    const std::uint32_t c = static_cast<std::uint32_t>(pDest[aIndex2]);
    const std::uint32_t aOtherMask = static_cast<std::uint32_t>(pMaskStackOther[aSourceIndex % kMaskStackDepth][aOtherMaskIndex]);
...
void TwistCandidate_106033(
  ...
  TwistCandidate_106033_SaltSeed(pSource, aSalt, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_106033_KeySeed(pSource, pKeyStack, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_106033_MaskSeedA(pSource, pWorkerA, pMaskStackA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  TwistCandidate_106033_MaskSeedB(pSource, pWorkerB, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  ...
  for (unsigned int offset = 0U; offset < pLength; offset += PASSWORD_EXPANDED_SIZE, ++aRound) {
    ...
    TwistCandidate_106033_TwistBlock(aRoundSource, pWorkerA, pWorkerB, aRoundDest, aBreakerTempA, aBreakerTempB, aRound, aSalt, pKeyStack, pMaskStackA, pMaskStackB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_106033_PushKeyRound(aRoundDest, aSalt, pKeyStack, pNextRoundKeyBuffer, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_106033_PushMaskRoundA(aRoundDest, pMaskStackA, pMaskStackB, pNextRoundMaskBufferA, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
    TwistCandidate_106033_PushMaskRoundB(aRoundDest, pMaskStackB, pMaskStackA, pNextRoundMaskBufferB, static_cast<unsigned int>(PASSWORD_EXPANDED_SIZE));
  }
}
...
const RegisteredCandidate kRegisteredCandidates[] = {
  {
    ...
    "TwistCandidate_106033",
    &TwistCandidate_106033,
    &TwistCandidate_106033_KeySeed,
    &TwistCandidate_106033_SaltSeed,
    &TwistCandidate_106033_MaskSeedA,
    &TwistCandidate_106033_MaskSeedB,
    &TwistCandidate_106033_TwistBlock,
    &TwistCandidate_106033_PushKeyRound,
    &TwistCandidate_106033_PushMaskRoundA,
    &TwistCandidate_106033_PushMaskRoundB,
  }
};
