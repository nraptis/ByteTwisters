...
enum class HurricaneFastOp : std::uint8_t {
  kRotateRowLeft = 0,
  kRotateRowRight = 1,
  kRotateColumnUp = 2,
  kRotateColumnDown = 3,
...
enum class HurricaneSlowOp : std::uint8_t {
  kRotateRight = 0,
  kRotateLeft = 1,
  kFlipHorizontal = 2,
  kFlipVertical = 3,
...
class HurricaneMatrix final {
 public:
  ...
  HurricaneMatrix();
  explicit HurricaneMatrix(const std::uint8_t* bytes);
  ...
  void XorWith(const HurricaneMatrix& other);
  void AddWith(const HurricaneMatrix& other);
  ...
  void ApplyFastOp(HurricaneFastOp op, std::uint8_t arg0, std::uint8_t arg1);
  void ApplySlowOp(HurricaneSlowOp op, std::uint8_t arg0, std::uint8_t arg1);
