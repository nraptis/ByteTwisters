...
SCREEN_LENGTH_FACTOR="${SCREEN_LENGTH_FACTOR:-64}"
SCREEN_STREAM_BYTES="${SCREEN_STREAM_BYTES:-}"
SCREEN_TRIAL_COUNT="${SCREEN_TRIAL_COUNT:-8}"
SCREEN_CYCLE_BLOCK_COUNT="${SCREEN_CYCLE_BLOCK_COUNT:-64}"
SCREEN_SAMPLE_WINDOWS="${SCREEN_SAMPLE_WINDOWS:-16384}"
SCREEN_SIGNATURE_BYTES="${SCREEN_SIGNATURE_BYTES:-1024}"
SCREEN_AVALANCHE_BLOCKS="${SCREEN_AVALANCHE_BLOCKS:-4}"
SCREEN_AVALANCHE_TRIALS="${SCREEN_AVALANCHE_TRIALS:-1}"

VERIFY_LENGTH_FACTOR="${VERIFY_LENGTH_FACTOR:-}"
VERIFY_STREAM_BYTES="${VERIFY_STREAM_BYTES:-}"
VERIFY_TRIAL_COUNT="${VERIFY_TRIAL_COUNT:-12}"
VERIFY_CYCLE_BLOCK_COUNT="${VERIFY_CYCLE_BLOCK_COUNT:-256}"
VERIFY_SAMPLE_WINDOWS="${VERIFY_SAMPLE_WINDOWS:-32768}"
VERIFY_SIGNATURE_BYTES="${VERIFY_SIGNATURE_BYTES:-4096}"
VERIFY_AVALANCHE_BLOCKS="${VERIFY_AVALANCHE_BLOCKS:-8}"
VERIFY_AVALANCHE_TRIALS="${VERIFY_AVALANCHE_TRIALS:-1}"
VERIFY_LONG_REPEAT_BYTES="${VERIFY_LONG_REPEAT_BYTES:-20000000}"
VERIFY_LONG_REPEAT_WINDOW_A="${VERIFY_LONG_REPEAT_WINDOW_A:-64}"
VERIFY_LONG_REPEAT_WINDOW_B="${VERIFY_LONG_REPEAT_WINDOW_B:-128}"
...
for SUITE in stale pseudorandom structured; do
  ...
  HARNESS_ARGS=(
    ...
    --cycle-block-count "${SCREEN_CYCLE_BLOCK_COUNT}"
...
for CANDIDATE_ID in "${SHORTLIST_IDS[@]}"; do
  ...
  for SUITE in stale pseudorandom structured; do
    ...
    HARNESS_ARGS=(
      ...
      --cycle-block-count "${VERIFY_CYCLE_BLOCK_COUNT}"
      ...
      --long-repeat-top 1
      --long-repeat-window-a "${VERIFY_LONG_REPEAT_WINDOW_A}"
      --long-repeat-window-b "${VERIFY_LONG_REPEAT_WINDOW_B}"
      --top-n 1
    )
